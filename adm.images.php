...
$fields = [
	'imageid' =>	[T_BS_INT, O_NO,	P_SYS,		DB_ID,		'isset({form}) && {form} == "update"'],
	'name' =>		[T_BS_STR, O_NO,	null,		NOT_EMPTY,	'isset({add}) || isset({update})'],
	'imagetype' =>	[T_BS_INT, O_OPT, null,		IN('1,2'),	'isset({add}) || isset({update})'],
	// actions
...
if (hasRequest('imageid')) {
	$dbImage = DBfetch(DBselect('SELECT i.imagetype,i.name FROM images i WHERE i.imageid='.bs_dbstr(getRequest('imageid'))));
	if (empty($dbImage)) {
		access_deny();
	}
}
