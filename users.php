...
$fields = [
	// users
	'userid' =>				[T_BS_INT, O_NO,	P_SYS,	DB_ID,		'isset({form}) && {form} == "update"'],
	'group_userid' =>		[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null],
	'filter_usrgrpid' =>	[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null],
	'alias' =>				[T_BS_STR, O_OPT, null,	NOT_EMPTY,	'isset({add}) || isset({update})', _('Alias')],
	'name' =>				[T_BS_STR, O_OPT, null,	null,		null, _x('Name', 'user first name')],
	'surname' =>			[T_BS_STR, O_OPT, null,	null,		null, _('Surname')],
	'password1' =>			[T_BS_STR, O_OPT, null,	null,		'(isset({add}) || isset({update})) && isset({form}) && {form} != "update" && isset({change_password})'],
	'password2' =>			[T_BS_STR, O_OPT, null,	null,		'(isset({add}) || isset({update})) && isset({form}) && {form} != "update" && isset({change_password})'],
	'user_type' =>			[T_BS_INT, O_OPT, null,	IN('1,2,3'),'isset({add}) || isset({update})'],
	'user_groups' =>		[T_BS_STR, O_OPT, null,	NOT_EMPTY,	null],
	'user_groups_to_del' =>	[T_BS_INT, O_OPT, null,	DB_ID,		null],
	'user_medias' =>		[T_BS_STR, O_OPT, null,	NOT_EMPTY,	null],
	'user_medias_to_del' =>	[T_BS_INT, O_OPT, null,	DB_ID,		null],
	'new_groups' =>			[T_BS_STR, O_OPT, null,	null,		null],
	...
	'theme' =>				[T_BS_STR, O_OPT, null,	IN('"'.implode('","', $themes).'"'), 'isset({add}) || isset({update})'],
	...
	'refresh' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, SEC_PER_HOUR), 'isset({add}) || isset({update})', _('Refresh (in seconds)')],
	'rows_per_page' =>		[T_BS_INT, O_OPT, null,	BETWEEN(1, 999999),'isset({add}) || isset({update})', _('Rows per page')],
	// actions
	'action' =>				[T_BS_STR, O_OPT, P_SYS|P_ACT,	IN('"user.massdelete","user.massunblock"'),	null],
	'register' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT,	IN('"add permission","delete permission"'), null],
	...
	'delete_selected' =>	[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,	null],
	'del_user_group' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,	null],
	'del_user_media' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,	null],
	'del_group_user' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,	null],
	'change_password' =>	[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,	null],
	'cancel' =>				[T_BS_STR, O_OPT, P_SYS,			null,	null],
	// form
	'form' =>				[T_BS_STR, O_OPT, P_SYS,			null,	null],
	'form_refresh' =>		[T_BS_INT, O_OPT, null,			null,	null],
	// sort and sortorder
	'sort' =>				[T_BS_STR, O_OPT, P_SYS, IN('"alias","name","surname","type"'),		null],
...
elseif (hasRequest('add') || hasRequest('update')) {
	...
	if ($authType != BS_AUTH_INTERNAL) {
		if (hasRequest('password1')) {
			...
			if (hasRequest('userid')) {
				...
				$_REQUEST['password1'] = 'bsm';
				$_REQUEST['password2'] = 'bsm';
			}
		}
	}
	else {
	...
	elseif (isset($_REQUEST['password1']) && $_REQUEST['alias'] == BS_GUEST_USER && !bs_empty($_REQUEST['password1'])) {
	...
	if ($isValid) {
		...
		$user['usrgrps'] = bs_toObject($usrgrps, 'usrgrpid');
...
if (!empty($_REQUEST['form'])) {
	...
	$dbSessions = DBselect(
		...
		' FROM sessions s'.
		' WHERE '.dbConditionInt('s.userid', bs_objectValues($data['users'], 'userid')).
