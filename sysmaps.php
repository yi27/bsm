...
$fields = [
	'maps' =>					[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	'sysmapid' =>				[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	'name' =>					[T_BS_STR, O_OPT, null,	NOT_EMPTY, 'isset({add}) || isset({update})', _('Name')],
	'width' =>					[T_BS_INT, O_OPT, null,	BETWEEN(0, 65535), 'isset({add}) || isset({update})', _('Width')],
	'height' =>					[T_BS_INT, O_OPT, null,	BETWEEN(0, 65535), 'isset({add}) || isset({update})', _('Height')],
	'backgroundid' =>			[T_BS_INT, O_OPT, null,	DB_ID,			'isset({add}) || isset({update})'],
	'iconmapid' =>				[T_BS_INT, O_OPT, null,	DB_ID,			'isset({add}) || isset({update})'],
	'expandproblem' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 1),	null],
	'markelements' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 1),	null],
	'show_unack' =>				[T_BS_INT, O_OPT, null,	BETWEEN(0, 2),	null],
	'highlight' =>				[T_BS_INT, O_OPT, null,	BETWEEN(0, 1),	null],
	'label_format' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 1),	null],
	'label_type_host' =>		[T_BS_INT, O_OPT, null,	BETWEEN(MAP_LABEL_TYPE_LABEL, MAP_LABEL_TYPE_CUSTOM), 'isset({add}) || isset({update})'],
	'label_type_hostgroup' =>	[T_BS_INT, O_OPT, null,	BETWEEN(MAP_LABEL_TYPE_LABEL, MAP_LABEL_TYPE_CUSTOM), 'isset({add}) || isset({update})'],
	'label_type_trigger' =>		[T_BS_INT, O_OPT, null,	BETWEEN(MAP_LABEL_TYPE_LABEL, MAP_LABEL_TYPE_CUSTOM), 'isset({add}) || isset({update})'],
	'label_type_map' =>			[T_BS_INT, O_OPT, null,	BETWEEN(MAP_LABEL_TYPE_LABEL, MAP_LABEL_TYPE_CUSTOM), 'isset({add}) || isset({update})'],
	'label_type_image' =>		[T_BS_INT, O_OPT, null,	BETWEEN(MAP_LABEL_TYPE_LABEL, MAP_LABEL_TYPE_CUSTOM), 'isset({add}) || isset({update})'],
	'label_string_host' =>		[T_BS_STR, O_OPT, null,	null,			'isset({add}) || isset({update})'],
	'label_string_hostgroup' =>	[T_BS_STR, O_OPT, null,	null,			'isset({add}) || isset({update})'],
	'label_string_trigger' =>	[T_BS_STR, O_OPT, null,	null,			'isset({add}) || isset({update})'],
	'label_string_map' =>		[T_BS_STR, O_OPT, null,	null,			'isset({add}) || isset({update})'],
	'label_string_image' =>		[T_BS_STR, O_OPT, null,	null,			'isset({add}) || isset({update})'],
	'label_type' =>				[T_BS_INT, O_OPT, null,	BETWEEN(MAP_LABEL_TYPE_LABEL,MAP_LABEL_TYPE_CUSTOM), 'isset({add}) || isset({update})'],
	'label_location' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 3),	'isset({add}) || isset({update})'],
	'urls' =>					[T_BS_STR, O_OPT, null,	null,			null],
	'severity_min' =>			[T_BS_INT, O_OPT, null,	IN('0,1,2,3,4,5'), null],
	'userid' =>					[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	'private' =>				[T_BS_INT, O_OPT, null,	BETWEEN(0, 1),	null],
	'users' =>					[T_BS_INT, O_OPT, null,	null,			null],
	'userGroups' =>				[T_BS_INT, O_OPT, null,	null,			null],
	// actions
	'action' =>					[T_BS_STR, O_OPT, P_SYS|P_ACT, IN('"map.export","map.massdelete"'),		null],
	...
	// sort and sortorder
	'sort' =>					[T_BS_STR, O_OPT, P_SYS, IN('"height","name","width"'),				null],
...
if (hasRequest('add') || hasRequest('update')) {
	...
	foreach ($map['urls'] as $unum => $url) {
		if (bs_empty($url['name']) && bs_empty($url['url'])) {
			unset($map['urls'][$unum]);
		}
	}
	...
	if (hasRequest('update')) {
		...
		if (CWebUser::getType() == USER_TYPE_BSM_USER) {
			unset($map['userid']);
		}
		// Map update with inaccessible user.
		elseif (CWebUser::getType() == USER_TYPE_BSM_ADMIN && $map['userid'] === '') {
...
if (hasRequest('form')) {
	...
	if ($user_type != USER_TYPE_SUPER_ADMIN && $user_type != USER_TYPE_BSM_ADMIN) {
