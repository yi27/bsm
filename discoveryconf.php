...
$fields = [
	'druleid' =>		[T_BS_INT, O_OPT, P_SYS,	DB_ID,		'isset({form}) && {form} == "update"'],
	'name' =>			[T_BS_STR, O_OPT, null,	NOT_EMPTY,	'isset({add}) || isset({update})'],
	'proxy_hostid' =>	[T_BS_INT, O_OPT, null,	DB_ID,		'isset({add}) || isset({update})'],
	'iprange' =>		[T_BS_STR, O_OPT, null,	null,		'isset({add}) || isset({update})'],
	'delay' =>			[T_BS_INT, O_OPT, null,	BETWEEN(1, SEC_PER_WEEK), 'isset({add}) || isset({update})'],
	'status' =>			[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'uniqueness_criteria' => [T_BS_STR, O_OPT, null, null,	'isset({add}) || isset({update})', _('Device uniqueness criteria')],
	'g_druleid' =>		[T_BS_INT, O_OPT, null,	DB_ID,		null],
	...
	'output' =>			[T_BS_STR, O_OPT, P_ACT,	null,		null],
	'ajaxaction' =>		[T_BS_STR, O_OPT, P_ACT,	null,		null],
	'ajaxdata' =>		[T_BS_STR, O_OPT, P_ACT,	null,		null],
	// sort and sortorder
...
elseif (hasRequest('action') && str_in_array(getRequest('action'), ['drule.massenable', 'drule.massdisable']) && hasRequest('g_druleid')) {
	...
	foreach (getRequest('g_druleid') as $druleId) {
		$result &= DBexecute('UPDATE drules SET status='.bs_dbstr($status).' WHERE druleid='.bs_dbstr($druleId));
