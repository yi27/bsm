 ...
 */
var MMenu = {
	menus:			{'view': 0, 'cm': 0, 'reports': 0, 'config': 0, 'admin': 0, 'fzjh':0},
	def_label:		null,
	sub_active: 	false,
	timeout_reset:	null,
	timeout_change:	null,
...
function add2favorites(object, objectid) {
	sendAjaxData('bsm.php?action=favourite.create', {
		data: {
			object: object,
			objectid: objectid
		}
...
function rm4favorites(object, objectid) {
	sendAjaxData('bsm.php?action=favourite.delete', {
		data: {
			object: object,
			objectid: objectid
		}
...
function updateUserProfile(idx, value_int, idx2) {
	sendAjaxData('bsm.php?action=profile.update', {
		data: {
			idx: idx,
			value_int: value_int,
			idx2: idx2
