		...
		jQuery.each(data, function(i, item) {
			removeItems[i] = {
				...
				clickCallback: function() {
					...
					sendAjaxData('bsm.php?action=dashboard.favourite&operation=delete', {
						data: {
							object: favouriteObj,
							'objectids[]': [item.id]
						}
...
function getMenuPopupMap(options) {
	...
	if (typeof options.gotos !== 'undefined') {
		...
		if (typeof options.gotos.submap !== 'undefined') {
			var url = new Curl('bsm.php?action=map.view');
		...
		var item = {
			...
			clickCallback: function() {
				...
				if (options.multiplier) {
					...
					sendAjaxData('bsm.php?action=dashboard.widget', {
						data: jQuery.extend({}, params, {
...
function getMenuPopupTrigger(options) {
	...
	if (typeof options.acknowledge !== 'undefined' && objectSize(options.acknowledge) > 0) {
		var url = new Curl('bsm.php?action=acknowledge.edit&eventids[]=' + options.acknowledge.eventid + '&backurl=' + options.acknowledge.backurl);
