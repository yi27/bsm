...
var timeControl = {
		...
		for (var id in this.objectList) {
			if (!empty(this.objectList[id]) && !this.objectList[id].processed && this.objectList[id].loadScroll) {
				...
				obj.time.starttime = (!isset('starttime', obj.time) || is_null(obj.time['starttime']))
					...
					: nowDate.setBSDate(obj.time.starttime) / 1000;

				obj.time.usertime = (!isset('usertime', obj.time))
					? obj.time.endtime
					: nowDate.setBSDate(obj.time.usertime) / 1000;
	...
	refreshImage: function(id) {
		var obj = this.objectList[id],
			...
			stime = new CDate((this.timeline.usertime() - this.timeline.period()) * 1000).getBSDate();
	...
	objectUpdate: function() {
		...
		if (this.refreshPage) {
			...
			url.setArgument('stime', date.getBSDate());
			...
			flickerfreeScreen.refreshAll(period, date.getBSDate(), this.timeline.isNow());
		}
	},
	...
	addListeners: function() {
		...
		if (IE9 || IE10) {
			...
			jQuery(this.grphobj).click(function() {
				BS_SBOX[obj.sbox_id].ieMouseClick();
			});
		}
		else {
...
function sboxGlobalMove() {
	for (var id in BS_SBOX) {
		if (!empty(BS_SBOX[id])) {
			BS_SBOX[id].moveSBoxByObj();
		}
	}
}
...
function sboxGlobalMousemove(e) {
	for (var id in BS_SBOX) {
		if (!empty(BS_SBOX[id]) && BS_SBOX[id].is_active) {
			BS_SBOX[id].mouseMove(e);
		}
	}
}
