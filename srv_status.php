...
$fields = [
	'serviceid' =>	[T_BS_INT, O_OPT, P_SYS|P_NZERO, DB_ID,	null],
	'showgraph' =>	[T_BS_INT, O_OPT, P_SYS,	IN('1'),		'isset({serviceid})'],
	'period' =>		[T_BS_STR, O_OPT, P_SYS,	IN('"'.implode('","', array_keys($periods)).'"'),	null],
...
if (isset($_REQUEST['serviceid']) && isset($_REQUEST['showgraph'])) {
	...
	if ($tree) {
		...
		$srv_wdgt = (new CWidget())
			->setTitle(_('IT services availability report'))
			->setControls($r_form)
			->addItem(BR())
