...
$fields = [
	...
	'hostid' =>				[T_BS_INT, O_OPT, P_SYS,	DB_ID.NOT_ZERO, 'isset({form}) && !isset({applicationid})'],
	'groupid' =>			[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	'applicationid' =>		[T_BS_INT, O_OPT, P_SYS,	DB_ID,			'isset({form}) && {form} == "update"'],
	'appname' =>			[T_BS_STR, O_OPT, null,	NOT_EMPTY,		'isset({add}) || isset({update})', _('Name')],
	// actions
		...
		foreach ($data['applications'] as &$application) {
			if ($application['applicationDiscovery']) {
				if (count($application['applicationDiscovery']) > 1) {
					$ts_delete = bs_objectValues($application['applicationDiscovery'], 'ts_delete');
