...
$fields = [
	'hostid' =>					[T_BS_INT, O_NO,	P_SYS,	DB_ID,		'(isset({form}) && ({form} == "update"))'],
	'parent_discoveryid' =>		[T_BS_INT, O_MAND, P_SYS,	DB_ID, null],
	'host' =>		        	[T_BS_STR, O_OPT, null,		NOT_EMPTY,	'isset({add}) || isset({update})', _('Host name')],
	'name' =>	            	[T_BS_STR, O_OPT, null,		null,		'isset({add}) || isset({update})'],
	'status' =>		        	[T_BS_INT, O_OPT, null,		IN([HOST_STATUS_NOT_MONITORED, HOST_STATUS_MONITORED]), null],
	'inventory_mode' =>			[T_BS_INT, O_OPT, null, IN([HOST_INVENTORY_DISABLED, HOST_INVENTORY_MANUAL, HOST_INVENTORY_AUTOMATIC]), null],
	'templates' =>		    	[T_BS_STR, O_OPT, null, NOT_EMPTY,	null],
	'add_template' =>			[T_BS_STR, O_OPT, null,		null,	null],
	'add_templates' =>		    [T_BS_STR, O_OPT, null, NOT_EMPTY,	null],
	'group_links' =>			[T_BS_STR, O_OPT, null, NOT_EMPTY,	null],
	'group_prototypes' =>		[T_BS_STR, O_OPT, null, NOT_EMPTY,	null],
	'unlink' =>					[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	'group_hostid' =>			[T_BS_INT, O_OPT, null,	DB_ID,		null],
...
if (getRequest('parent_discoveryid')) {
	...
	if (!$discoveryRule || $discoveryRule['hosts'][0]['flags'] == BS_FLAG_DISCOVERY_CREATED) {
		access_deny();
	}
...
elseif (hasRequest('add') || hasRequest('update')) {
	...
	foreach (getRequest('group_prototypes', []) as $groupPrototype) {
		...
		if (!bs_empty($groupPrototype['name'])) {
	...
	if (getRequest('hostid')) {
		...
		if (!$hostPrototype['templateid']) {
			...
			$groupPrototypesByGroupId = bs_toHash($hostPrototype['groupLinks'], 'groupid');
...
if (isset($_REQUEST['form'])) {
	...
	if (getRequest('hostid') && !getRequest('form_refresh')) {
		...
		$data['groups'] = API::HostGroup()->get([
			'output' => API_OUTPUT_EXTEND,
			'groupids' => bs_objectValues($data['host_prototype']['groupLinks'], 'groupid'),
			'editable' => true,
			'preservekeys' => true
		]);
	...
	foreach ($data['hostPrototypes'] as $hostPrototype) {
		$templateIds = array_merge($templateIds, bs_objectValues($hostPrototype['templates'], 'templateid'));
	}
	...
	$data['linkedTemplates'] = bs_toHash($linkedTemplates, 'templateid');

	// fetch source templates and LLD rules
	$hostPrototypeSourceIds = getHostPrototypeSourceParentIds(bs_objectValues($data['hostPrototypes'], 'hostid'));
