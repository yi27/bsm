...
$fields = [
	...
	'new_httpstep'		=> [T_BS_STR, O_OPT, null,	null,				null],
	'sel_step'			=> [T_BS_INT, O_OPT, null,	BETWEEN(0, 65534),	null],
	'group_httptestid'	=> [T_BS_INT, O_OPT, null,	DB_ID,				null],
	// form
	'hostid'          => [T_BS_INT, O_OPT, P_SYS, DB_ID.NOT_ZERO,          'isset({form}) || isset({add}) || isset({update})'],
	'applicationid'   => [T_BS_INT, O_OPT, null,  DB_ID,                   null, _('Application')],
	'httptestid'      => [T_BS_INT, O_NO,  P_SYS, DB_ID,                   'isset({form}) && {form} == "update"'],
	'name'            => [T_BS_STR, O_OPT, null,  NOT_EMPTY,               'isset({add}) || isset({update})', _('Name')],
	'delay'           => [T_BS_INT, O_OPT, null,  BETWEEN(1, SEC_PER_DAY), 'isset({add}) || isset({update})', _('Update interval (in sec)')],
	'retries'         => [T_BS_INT, O_OPT, null,  BETWEEN(1, 10),          'isset({add}) || isset({update})',
		_('Attempts')
	],
	'status'          => [T_BS_STR, O_OPT, null,  null,                    null],
	'agent'           => [T_BS_STR, O_OPT, null, null,                     'isset({add}) || isset({update})'],
	'agent_other'     => [T_BS_STR, O_OPT, null, null,
		'(isset({add}) || isset({update})) && {agent} == '.BS_AGENT_OTHER
	],
	'variables'       => [T_BS_STR, O_OPT, null,  null,                    'isset({add}) || isset({update})'],
	'steps'           => [T_BS_STR, O_OPT, null,  null,                    'isset({add}) || isset({update})', _('Steps')],
	'authentication'  => [T_BS_INT, O_OPT, null,  IN('0,1,2'),             'isset({add}) || isset({update})'],
	'http_user'       => [T_BS_STR, O_OPT, null,  NOT_EMPTY,               '(isset({add}) || isset({update})) && isset({authentication}) && ({authentication} == '.HTTPTEST_AUTH_BASIC.
		' || {authentication} == '.HTTPTEST_AUTH_NTLM.')', _('User')],
	'http_password'		=> [T_BS_STR, O_OPT, P_NO_TRIM,	NOT_EMPTY,		'(isset({add}) || isset({update})) && isset({authentication}) && ({authentication} == '.HTTPTEST_AUTH_BASIC.
		' || {authentication} == '.HTTPTEST_AUTH_NTLM.')', _('Password')],
	'http_proxy'		=> [T_BS_STR, O_OPT, null,	null,				'isset({add}) || isset({update})'],
	'new_application'	=> [T_BS_STR, O_OPT, null,	null,				null],
	'hostname'			=> [T_BS_STR, O_OPT, null,	null,				null],
	'templated'			=> [T_BS_STR, O_OPT, null,	null,				null],
	'verify_host'		=> [T_BS_STR, O_OPT, null,	null,				null],
	'verify_peer'		=> [T_BS_STR, O_OPT, null,	null,				null],
	'headers'			=> [T_BS_STR, O_OPT, null, null,					'isset({add}) || isset({update})'],
	'ssl_cert_file'		=> [T_BS_STR, O_OPT, null, null,					'isset({add}) || isset({update})'],
	'ssl_key_file'		=> [T_BS_STR, O_OPT, null, null,					'isset({add}) || isset({update})'],
	'ssl_key_password'	=> [T_BS_STR, O_OPT, P_NO_TRIM, null,				'isset({add}) || isset({update})'],
	// filter
	...
	// sort and sortorder
	'sort'				=> [T_BS_STR, O_OPT, P_SYS, IN('"hostname","name","status"'),				null],
...
elseif (hasRequest('del_history') && hasRequest('httptestid')) {
	...
	if ($httpTests) {
		...
		$result = ($result && DBexecute('UPDATE httptest SET nextcheck=0 WHERE httptestid='.bs_dbstr($httpTestId)));
...
elseif (hasRequest('add') || hasRequest('update')) {
	...
	try {
		...
		if ($new_application) {
			 ...
			 */
			if ($exApp) {
				if ($exApp[0]['flags'] == BS_FLAG_DISCOVERY_CREATED) {
		...
		if (isset($_REQUEST['httptestid'])) {
			...
			$dbHttpSteps = bs_toHash($dbHttpTest['steps'], 'httpstepid');
...
if (isset($_REQUEST['form'])) {
		...
		while ($httpTestId) {
			$dbTest = DBfetch(DBselect(
				'SELECT h.hostid,h.name,ht.httptestid,ht.templateid'.
					...
					' AND ht.httptestid='.bs_dbstr($httpTestId)
			));
			$httpTestId = null;
	...
	if ((isset($_REQUEST['httptestid']) && !isset($_REQUEST['form_refresh']))) {
		$dbHttpTest = DBfetch(DBselect(
			'SELECT ht.*'.
			' FROM httptest ht'.
			' WHERE ht.httptestid='.bs_dbstr($_REQUEST['httptestid'])
		));
		...
		$data['steps'] = DBfetchArray(DBselect('SELECT h.* FROM httpstep h WHERE h.httptestid='.bs_dbstr($_REQUEST['httptestid']).' ORDER BY h.no'));
	}
	else {
		...
		$data['agent'] = getRequest('agent', BS_DEFAULT_AGENT);
		$data['agent_other'] = getRequest('agent_other');

		if ($data['agent'] == BS_AGENT_OTHER) {
			foreach (userAgents() as $userAgents) {
	...
	if (!empty($data['hostid'])) {
		$dbApps = DBselect(
			...
			' WHERE a.hostid='.bs_dbstr($data['hostid']).
				' AND a.flags='.BS_FLAG_DISCOVERY_NORMAL
		);
	...
	if ($data['pageFilter']->hostsSelected) {
		...
		$dbHttpTests = DBselect(
			'SELECT ht.httptestid,ht.name,ht.delay,ht.status,ht.hostid,ht.templateid,h.name AS hostname,ht.retries,'.
				...
				' WHERE '.dbConditionInt('ht.httptestid', bs_objectValues($httpTests, 'httptestid'))
		);
		$httpTests = [];
		...
		$dbHttpSteps = DBselect(
			'SELECT hs.httptestid,COUNT(*) AS stepscnt'.
				' FROM httpstep hs'.
				' WHERE '.dbConditionInt('hs.httptestid', bs_objectValues($httpTests, 'httptestid')).
