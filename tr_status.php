...
$fields = [
	...
	'btnSelect' =>			[T_BS_STR, O_OPT, null,	null,		null],
	// filter
	'filter_rst' =>			[T_BS_STR, O_OPT, P_SYS,	null,		null],
	'filter_set' =>			[T_BS_STR, O_OPT, P_SYS,	null,		null],
	'show_triggers' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'show_events' =>		[T_BS_INT, O_OPT, P_SYS,	null,		null],
	'ack_status' =>			[T_BS_INT, O_OPT, P_SYS,	null,		null],
	'show_severity' =>		[T_BS_INT, O_OPT, P_SYS,	null,		null],
	'show_details' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'show_maintenance' =>	[T_BS_INT, O_OPT, null,	null,		null],
	'status_change_days' =>	[T_BS_INT, O_OPT, null,	BETWEEN(1, DAY_IN_YEAR * 2), null],
	'status_change' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'txt_select' =>			[T_BS_STR, O_OPT, null,	null,		null],
	'application' =>		[T_BS_STR, O_OPT, null,	null,		null],
	'inventory' =>			[T_BS_STR, O_OPT, null,	null,		null],
	// sort and sortorder
	'sort' =>				[T_BS_STR, O_OPT, P_SYS, IN('"description","lastchange","priority"'),	null],
...
if (hasRequest('filter_set')) {
	...
	if ($config['event_ack_enable'] == EVENT_ACK_ENABLED) {
		CProfile::update('web.tr_status.filter.ack_status', getRequest('ack_status', BS_ACK_STS_ANY), PROFILE_TYPE_INT);
	}
...
$ackStatus = ($config['event_ack_enable'] == EVENT_ACK_DISABLED)
	? BS_ACK_STS_ANY : CProfile::get('web.tr_status.filter.ack_status', BS_ACK_STS_ANY);
...
$sortOrder = getRequest('sortorder', CProfile::get('web.'.$page['file'].'.sortorder', BS_SORT_DOWN));
 ...
 * Form
 */
$triggerForm = (new CForm('get', 'bsm.php'))
	->setName('tr_status')
	->addVar('backurl', $page['file'])
	->addVar('acknowledge_type', BS_ACKNOWLEDGE_PROBLEM);
...
if ($showEvents == EVENTS_OPTION_ALL || $showEvents == EVENTS_OPTION_NOT_ACK) {
	$showHideAllDiv = (new CColHeader(
		(new CDiv())
			->addClass(BS_STYLE_TREEVIEW)
			->setId($switcherName)
			->addItem((new CSpan())->addClass(BS_STYLE_ARROW_RIGHT))
...
if ($filter['inventory']) {
	...
	$options['hostids'] = bs_objectValues($hosts, 'hostid');
}
...
if ($filter['application'] !== '') {
	...
	$options['applicationids'] = bs_objectValues($applications, 'applicationid');
}

if (!bs_empty($txtSelect)) {
...
if ($ackStatus == BS_ACK_STS_WITH_UNACK) {
	$options['withUnacknowledgedEvents'] = 1;
}
if ($ackStatus == BS_ACK_STS_WITH_LAST_UNACK) {
...
foreach ($triggers as &$trigger) {
	if (count($trigger['hosts']) > 1) {
		order_result($trigger['hosts'], 'name', BS_SORT_UP);
	}
}
unset($trigger);
...
if ($config['event_ack_enable']) {
	...
	if (!empty($triggerIdsWithoutUnackEvents)) {
		...
		$allEventCounts = bs_toHash($allEventCounts, 'objectid');
...
if ($showEvents == EVENTS_OPTION_ALL || $showEvents == EVENTS_OPTION_NOT_ACK) {
	...
	$options = [
		...
		'source' => EVENT_SOURCE_TRIGGERS,
		'object' => EVENT_OBJECT_TRIGGER,
		'objectids' => bs_objectValues($triggers, 'triggerid'),
...
foreach ($triggers as $trigger) {
	...
	$description[] = (new CSpan(CMacrosResolverHelper::resolveEventDescription(bs_array_merge($trigger, $event))))
	...
	foreach ($trigger['hosts'] as $host) {
		...
		if ($host['maintenance_status'] == HOST_MAINTENANCE_STATUS_ON) {
			...
			$host_name = (new CSpan([$host_name, $maintenance_icon]))->addClass(BS_STYLE_REL_CONTAINER);
		}
	...
	if ($config['event_ack_enable']) {
		if ($trigger['hasEvents']) {
			...
			if ($trigger['event_count']) {
				$ackColumn = [
					(new CLink(_('No'),
						'bsm.php?action=acknowledge.edit'.
							'&acknowledge_type='.BS_ACKNOWLEDGE_PROBLEM.
			...
			$ack_checkbox = '';
			$ackColumn = (new CCol(_('No events')))->addClass(BS_STYLE_GREY);
		}
	}
	else {
		$ack_checkbox = null;
	...
	if (isset($triggerEditable[$trigger['triggerid']])) {
		$comments = new CLink(bs_empty($trigger['comments']) ? _('Add') : _('Show'), 'tr_comments.php?triggerid='.$trigger['triggerid']);
	}
	else {
		$comments = bs_empty($trigger['comments'])
			? new CSpan('')
	...
	$triggerTable->addRow([
		...
		($trigger['lastchange'] == 0)
			? _('Never')
			: new CLink(bs_date2str(DATE_TIME_FORMAT_SECONDS, $trigger['lastchange']),
				'events.php?filter_set=1&triggerid='.$trigger['triggerid'].'&source='.EVENT_SOURCE_TRIGGERS.
					'&stime='.date(TIMESTAMP_FORMAT, $trigger['lastchange']).'&period='.BS_PERIOD_DEFAULT
			),
		($trigger['lastchange'] == 0) ? '' : bs_date2age($trigger['lastchange']),
	...
	if ($showEvents == EVENTS_OPTION_ALL || $showEvents == EVENTS_OPTION_NOT_ACK) {
		...
		foreach (array_slice($trigger['events'], 0, $config['event_show_max']) as $enum => $event) {
			...
			$clock = new CLink(bs_date2str(DATE_TIME_FORMAT_SECONDS, $event['clock']),
			...
			$triggerTable->addRow(
				(new CRow([
					...
					bs_date2age($next_event_clock, $event['clock']),
...
bs_add_post_js('jqBlink.blink();');
bs_add_post_js('var switcher = new CSwitcher(\''.$switcherName.'\');');
