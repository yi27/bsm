...
if (!$data['alias'] || $data['users']) {
	...
	CArrayHelper::sort($data['alerts'], [
		['field' => 'alertid', 'order' => BS_SORT_DOWN]
	]);

	$data['alerts'] = array_slice($data['alerts'], 0, $config['search_limit'] + 1);

	// paging
	$data['paging'] = getPagingLine($data['alerts'], BS_SORT_DOWN, new CUrl('auditacts.php'));

	// get users
	if (!$data['alias']) {
		$data['users'] = API::User()->get([
			'output' => ['userid', 'alias', 'name', 'surname'],
			'userids' => bs_objectValues($data['alerts'], 'userid'),
			'preservekeys' => true
		]);
	}
}
...
if ($userId) {
	$firstAlert = DBfetch(DBselect(
		...
		' FROM alerts a'.
		' WHERE a.userid='.bs_dbstr($userId)
	));
}
elseif ($data['alias'] === '') {
...
if ($data['alerts']) {
	$data['actions'] = API::Action()->get([
		...
		'actionids' => array_unique(bs_objectValues($data['alerts'], 'actionid')),
		'preservekeys' => true
	]);
}
...
$data['timeline'] = [
	...
	'usertime' => $data['stime'] ? date(TIMESTAMP_FORMAT, bsDateToTime($data['stime']) + $effectivePeriod) : null
];
