...
$fields = [
	...
	'esc_period' =>			[T_BS_INT, O_OPT, null,	BETWEEN(60, 999999), null, _('Default operation step duration')],
	'status' =>				[T_BS_INT, O_OPT, null,	IN([ACTION_STATUS_ENABLED, ACTION_STATUS_DISABLED]), null],
	'def_shortdata' =>		[T_BS_STR, O_OPT, null,	null,		'isset({add}) || isset({update})'],
	'def_longdata' =>		[T_BS_STR, O_OPT, null,	null,		'isset({add}) || isset({update})'],
	'recovery_msg' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'r_shortdata' =>		[T_BS_STR, O_OPT, null,	null,		'isset({recovery_msg}) && (isset({add}) || isset({update}))', _('Recovery subject')],
	'r_longdata' =>			[T_BS_STR, O_OPT, null,	null,		'isset({recovery_msg}) && (isset({add}) || isset({update}))', _('Recovery message')],
	'g_actionid' =>			[T_BS_INT, O_OPT, null,	DB_ID,		null],
	...
	'edit_operationid' =>	[T_BS_STR, O_OPT,	P_ACT,	null,		null],
	...
	'add_condition' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'cancel_new_condition' => [T_BS_STR, O_OPT, P_SYS|P_ACT, null, null],
	'add_operation' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'cancel_new_operation' => [T_BS_STR, O_OPT, P_SYS|P_ACT, null, null],
	'add_opcondition' =>	[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'cancel_new_opcondition' => [T_BS_STR, O_OPT, P_SYS|P_ACT, null, null],
...
elseif (isset($_REQUEST['add_condition']) && isset($_REQUEST['new_condition'])) {
	...
	if ($newCondition) {
		...
		$usedFormulaIds = bs_objectValues($conditions, 'formulaid');

		$validateConditions = $conditions;

		if (isset($newCondition['value'])) {
			$newConditionValues = bs_toArray($newCondition['value']);
...
elseif (hasRequest('action') && str_in_array(getRequest('action'), ['action.massenable', 'action.massdisable']) && hasRequest('g_actionid')) {
	...
	while ($row = DBfetch($dbActions)) {
		$result &= DBexecute(
			'UPDATE actions'.
			' SET status='.bs_dbstr($status).
			' WHERE actionid='.bs_dbstr($row['actionid'])
		);
		if ($result) {
			...
			switch ($operation['operationtype']) {
				...
				case OPERATION_TYPE_GROUP_REMOVE:
					$operation = [
						...
						'groupids' => bs_objectValues($operation['opgroup'], 'groupid')
					];
					break;
				...
				case OPERATION_TYPE_TEMPLATE_REMOVE:
					$operation = [
						...
						'operationid' => $operation['operationid'],
						'operationtype' => $operation['operationtype'],
						'templateids' => bs_objectValues($operation['optemplate'], 'templateid')
					];
					break;
			}
		}
