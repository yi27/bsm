...
$fields = [
	'refresh_unsupported' =>	[T_BS_INT, O_OPT, null, BETWEEN(0, 65535), 'isset({update})',
		_('Refresh unsupported items (in sec)')
	],
	'discovery_groupid' =>		[T_BS_INT, O_OPT, null, DB_ID, 'isset({update})',
		_('Group for discovered hosts')
	],
	'default_inventory_mode' =>	[T_BS_INT, O_OPT, null,
		IN(HOST_INVENTORY_DISABLED.','.HOST_INVENTORY_MANUAL.','.HOST_INVENTORY_AUTOMATIC), 'isset({update})',
		_('Default host inventory mode')
	],
	'alert_usrgrpid' =>			[T_BS_INT, O_OPT, null, DB_ID, 'isset({update})',
		_('User group for database down message')
	],
	'snmptrap_logging' =>		[T_BS_INT, O_OPT, null, IN('1'), null, _('Log unmatched SNMP traps')],
	// actions
