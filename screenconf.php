...
if (hasRequest('action') && getRequest('action') == 'screen.export' && hasRequest('screens')) {
	...
	$page['file'] = 'bs_export_screens.xml';
}
else {
	$isExportData = false;
...
$fields = [
	'screens' =>		[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	'screenid' =>		[T_BS_INT, O_NO,	P_SYS,	DB_ID,			'isset({form}) && {form} == "update"'],
	'templateid' =>		[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	'name' =>			[T_BS_STR, O_OPT, null,	NOT_EMPTY,		'isset({add}) || isset({update})', _('Name')],
	'hsize' =>			[T_BS_INT, O_OPT, null,	BETWEEN(SCREEN_MIN_SIZE, SCREEN_MAX_SIZE),
		'isset({add}) || isset({update})', _('Columns')
	],
	'vsize' =>			[T_BS_INT, O_OPT, null,	BETWEEN(SCREEN_MIN_SIZE, SCREEN_MAX_SIZE),
	...
	// actions
	'action' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT, IN('"screen.export","screen.massdelete"'),		null],
...
if (hasRequest('add') || hasRequest('update')) {
	...
	if (hasRequest('update')) {
		...
		if (hasRequest('templateid')) {
			...
			elseif (CWebUser::getType() == USER_TYPE_BSM_ADMIN && $screen['userid'] === '') {
