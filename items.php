...
$fields = [
	'groupid' =>				[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null],
	'hostid' =>					[T_BS_INT, O_OPT, P_SYS,	DB_ID.NOT_ZERO, 'isset({form}) && !isset({itemid})'],
	'interfaceid' =>			[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null, _('Interface')],
	'copy_type' =>				[T_BS_INT, O_OPT, P_SYS,	IN('0,1,2'), 'isset({copy})'],
	'copy_mode' =>				[T_BS_INT, O_OPT, P_SYS,	IN('0'),	null],
	'itemid' =>					[T_BS_INT, O_NO,	P_SYS,	DB_ID,		'isset({form}) && {form} == "update"'],
	'name' =>					[T_BS_STR, O_OPT, null,	NOT_EMPTY, 'isset({add}) || isset({update})', _('Name')],
	'description' =>			[T_BS_STR, O_OPT, null,	null,		'isset({add}) || isset({update})'],
	'key' =>					[T_BS_STR, O_OPT, null,	NOT_EMPTY, 'isset({add}) || isset({update})', _('Key')],
	'delay' =>					[T_BS_INT, O_OPT, null,	BETWEEN(0, SEC_PER_DAY),
		'(isset({add}) || isset({update})) && isset({type}) && {type}!='.ITEM_TYPE_TRAPPER.' && {type}!='.ITEM_TYPE_SNMPTRAP,
		_('Update interval (in sec)')],
	'delay_flex' =>				[T_BS_STR, O_OPT, null,	null,			null],
	'history' =>				[T_BS_INT, O_OPT, null,	BETWEEN(0, 65535), 'isset({add}) || isset({update})',
		_('History storage period')
	],
	'status' =>					[T_BS_INT, O_OPT, null,	IN([ITEM_STATUS_DISABLED, ITEM_STATUS_ACTIVE]), null],
	'type' =>					[T_BS_INT, O_OPT, null,
		IN([-1, ITEM_TYPE_BSM, ITEM_TYPE_SNMPV1, ITEM_TYPE_TRAPPER, ITEM_TYPE_SIMPLE, ITEM_TYPE_SNMPV2C,
			ITEM_TYPE_INTERNAL, ITEM_TYPE_SNMPV3, ITEM_TYPE_BSM_ACTIVE, ITEM_TYPE_AGGREGATE, ITEM_TYPE_EXTERNAL,
	...
	'trends' =>					[T_BS_INT, O_OPT, null,	BETWEEN(0, 65535), '(isset({add}) || isset({update})) && isset({value_type}) && '.
		IN(ITEM_VALUE_TYPE_FLOAT.','.ITEM_VALUE_TYPE_UINT64, 'value_type'), _('Trend storage period')
	],
	'value_type' =>				[T_BS_INT, O_OPT, null,	IN('0,1,2,3,4'), 'isset({add}) || isset({update})'],
	'data_type' =>				[T_BS_INT, O_OPT, null,
	...
	'valuemapid' =>				[T_BS_INT, O_OPT, null,	DB_ID,		'(isset({add}) || isset({update})) && isset({value_type}) && '.
		IN(ITEM_VALUE_TYPE_FLOAT.','.ITEM_VALUE_TYPE_UINT64, 'value_type')],
	'authtype' =>				[T_BS_INT, O_OPT, null,	IN(ITEM_AUTHTYPE_PASSWORD.','.ITEM_AUTHTYPE_PUBLICKEY),
		'(isset({add}) || isset({update})) && isset({type}) && {type}=='.ITEM_TYPE_SSH],
	'username' =>				[T_BS_STR, O_OPT, null,	NOT_EMPTY,
		'(isset({add}) || isset({update})) && isset({type}) && '.IN(ITEM_TYPE_SSH.','.ITEM_TYPE_TELNET, 'type'), _('User name')],
	'password' =>				[T_BS_STR, O_OPT, null,	null,
		'(isset({add}) || isset({update})) && isset({type}) && '.IN(ITEM_TYPE_SSH.','.ITEM_TYPE_TELNET, 'type')],
	'publickey' =>				[T_BS_STR, O_OPT, null,	NOT_EMPTY,
		'(isset({add}) || isset({update})) && isset({type}) && {type} == '.ITEM_TYPE_SSH.' && {authtype} == '.ITEM_AUTHTYPE_PUBLICKEY],
	'privatekey' =>				[T_BS_STR, O_OPT, null,	NOT_EMPTY,
		'(isset({add}) || isset({update})) && isset({type}) && ({type}) == '.ITEM_TYPE_SSH.' && ({authtype}) == '.ITEM_AUTHTYPE_PUBLICKEY],
	$paramsFieldName =>			[T_BS_STR, O_OPT, null,	NOT_EMPTY,	'(isset({add}) || isset({update})) && isset({type}) && '.
	...
	'inventory_link' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 65535), '(isset({add}) || isset({update})) && {value_type} != '.ITEM_VALUE_TYPE_LOG],
	'snmp_community' =>			[T_BS_STR, O_OPT, null,	NOT_EMPTY,
		'(isset({add}) || isset({update})) && isset({type}) && '.IN(ITEM_TYPE_SNMPV1.','.ITEM_TYPE_SNMPV2C, 'type'), _('SNMP community')],
	'snmp_oid' =>				[T_BS_STR, O_OPT, null,	NOT_EMPTY, '(isset({add}) || isset({update})) && isset({type}) && '.IN(
		ITEM_TYPE_SNMPV1.','.ITEM_TYPE_SNMPV2C.','.ITEM_TYPE_SNMPV3, 'type'), _('SNMP OID')],
	'port' =>					[T_BS_STR, O_OPT, null,	BETWEEN(0, 65535), '(isset({add}) || isset({update})) && isset({type}) && '.IN(
		ITEM_TYPE_SNMPV1.','.ITEM_TYPE_SNMPV2C.','.ITEM_TYPE_SNMPV3, 'type'), _('Port')],
	'snmpv3_securitylevel' =>	[T_BS_INT, O_OPT, null,	IN('0,1,2'),
		'(isset({add}) || isset({update})) && isset({type}) && {type} == '.ITEM_TYPE_SNMPV3],
	'snmpv3_contextname' =>	[T_BS_STR, O_OPT, null,	null,
		'(isset({add}) || isset({update})) && isset({type}) && {type} == '.ITEM_TYPE_SNMPV3],
	'snmpv3_securityname' =>	[T_BS_STR, O_OPT, null,	null,
		'(isset({add}) || isset({update})) && isset({type}) && {type} == '.ITEM_TYPE_SNMPV3],
	'snmpv3_authprotocol' =>	[T_BS_INT, O_OPT, null,	IN(ITEM_AUTHPROTOCOL_MD5.','.ITEM_AUTHPROTOCOL_SHA),
	...
	'snmpv3_authpassphrase' =>	[T_BS_STR, O_OPT, null,	null,
		'(isset({add}) || isset({update})) && isset({type}) && {type} == '.ITEM_TYPE_SNMPV3.' && ({snmpv3_securitylevel} == '.
		ITEM_SNMPV3_SECURITYLEVEL_AUTHPRIV.' || {snmpv3_securitylevel} == '.ITEM_SNMPV3_SECURITYLEVEL_AUTHNOPRIV.')'],
	'snmpv3_privprotocol' =>	[T_BS_INT, O_OPT, null,	IN(ITEM_PRIVPROTOCOL_DES.','.ITEM_PRIVPROTOCOL_AES),
		'(isset({add}) || isset({update})) && isset({type}) && {type} == '.ITEM_TYPE_SNMPV3.' && {snmpv3_securitylevel} == '.ITEM_SNMPV3_SECURITYLEVEL_AUTHPRIV],
	'snmpv3_privpassphrase' =>	[T_BS_STR, O_OPT, null,	null,
		'(isset({add}) || isset({update})) && isset({type}) && {type} == '.ITEM_TYPE_SNMPV3.' && {snmpv3_securitylevel} == '.ITEM_SNMPV3_SECURITYLEVEL_AUTHPRIV],
	'ipmi_sensor' =>			[T_BS_STR, O_OPT, P_NO_TRIM, NOT_EMPTY,
		'(isset({add}) || isset({update})) && isset({type}) && {type} == '.ITEM_TYPE_IPMI, _('IPMI sensor')],
	'trapper_hosts' =>			[T_BS_STR, O_OPT, null,	null,		'(isset({add}) || isset({update})) && isset({type}) && {type} == 2'],
	'units' =>					[T_BS_STR, O_OPT, null,	null,		'(isset({add}) || isset({update})) && isset({value_type}) && '.
		IN('0,3', 'value_type').'isset({data_type}) && {data_type} != '.ITEM_DATA_TYPE_BOOLEAN],
	'multiplier' =>				[T_BS_INT, O_OPT, null,	null,		null],
	'delta' =>					[T_BS_INT, O_OPT, null,	IN('0,1,2'), '(isset({add}) || isset({update})) && isset({value_type}) && '.
		IN('0,3', 'value_type').'isset({data_type}) && {data_type} != '.ITEM_DATA_TYPE_BOOLEAN],
	'formula' =>				[T_BS_DBL_STR, O_OPT, null,
		'({value_type} == 0 && {} != 0) || ({value_type} == 3 && {} > 0)',
		'(isset({add}) || isset({update})) && isset({multiplier}) && {multiplier} == 1', _('Custom multiplier')
	],
	'logtimefmt' =>				[T_BS_STR, O_OPT, null,	null,
		'(isset({add}) || isset({update})) && isset({value_type}) && {value_type} == 2'],
	'group_itemid' =>			[T_BS_INT, O_OPT, null,	DB_ID,		null],
	'copy_targetid' =>		    [T_BS_INT, O_OPT, null,	DB_ID,		null],
	'copy_groupid' =>		    [T_BS_INT, O_OPT, P_SYS,	DB_ID,		'isset({copy}) && (isset({copy_type}) && {copy_type} == 0)'],
	'new_application' =>		[T_BS_STR, O_OPT, null,	null,		'isset({add}) || isset({update})'],
	'visible' =>		[T_BS_STR, O_OPT, null,		null,		null],
	'applications' =>			[T_BS_INT, O_OPT, null,	DB_ID,		null],
	'new_applications' =>		[T_BS_STR, O_OPT, null,	null,		null],
	'del_history' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	// actions
	'action' =>					[T_BS_STR, O_OPT, P_SYS|P_ACT,
	...
	'clone' =>					[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'copy' =>					[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'delete' =>					[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'massupdate' =>				[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'cancel' =>					[T_BS_STR, O_OPT, P_SYS,	null,		null],
	'form' =>					[T_BS_STR, O_OPT, P_SYS,	null,		null],
	'form_refresh' =>			[T_BS_INT, O_OPT, null,	null,		null],
	// filter
	'filter_set' =>				[T_BS_STR, O_OPT, null,	null,		null],
	'filter_rst' =>				[T_BS_STR, O_OPT, null,	null,		null],
	'filter_groupid' =>			[T_BS_INT, O_OPT, null,	DB_ID,		null],
	'filter_hostid' =>			[T_BS_INT, O_OPT, null,	DB_ID,		null],
	'filter_application' =>		[T_BS_STR, O_OPT, null,	null,		null],
	'filter_name' =>			[T_BS_STR, O_OPT, null,	null,		null],
	'filter_type' =>			[T_BS_INT, O_OPT, null,
		IN([-1, ITEM_TYPE_BSM, ITEM_TYPE_SNMPV1, ITEM_TYPE_TRAPPER, ITEM_TYPE_SIMPLE, ITEM_TYPE_SNMPV2C,
		ITEM_TYPE_INTERNAL, ITEM_TYPE_SNMPV3, ITEM_TYPE_BSM_ACTIVE, ITEM_TYPE_AGGREGATE, ITEM_TYPE_EXTERNAL, ITEM_TYPE_DB_MONITOR,
		ITEM_TYPE_IPMI, ITEM_TYPE_SSH, ITEM_TYPE_TELNET, ITEM_TYPE_JMX, ITEM_TYPE_CALCULATED, ITEM_TYPE_SNMPTRAP]), null],
	'filter_key' =>				[T_BS_STR, O_OPT, null,	null,		null],
	'filter_snmp_community' =>	[T_BS_STR, O_OPT, null,	null,		null],
	'filter_snmpv3_securityname' => [T_BS_STR, O_OPT, null, null,		null],
	'filter_snmp_oid' =>		[T_BS_STR, O_OPT, null,	null,		null],
	'filter_port' =>			[T_BS_INT, O_OPT, P_UNSET_EMPTY, BETWEEN(0, 65535), null, _('Port')],
	'filter_value_type' =>		[T_BS_INT, O_OPT, null,	IN('-1,0,1,2,3,4'), null],
	'filter_data_type' =>		[T_BS_INT, O_OPT, null,	BETWEEN(-1, ITEM_DATA_TYPE_BOOLEAN), null],
	'filter_delay' =>			[T_BS_INT, O_OPT, P_UNSET_EMPTY, BETWEEN(0, SEC_PER_DAY), null, _('Update interval')],
	'filter_history' =>			[T_BS_INT, O_OPT, P_UNSET_EMPTY, BETWEEN(0, 65535), null,	_('History')],
	'filter_trends' =>			[T_BS_INT, O_OPT, P_UNSET_EMPTY, BETWEEN(0, 65535), null, _('Trends')],
	'filter_status' =>			[T_BS_INT, O_OPT, null,	IN([-1, ITEM_STATUS_ACTIVE, ITEM_STATUS_DISABLED]), null],
	'filter_state' =>			[T_BS_INT, O_OPT, null,	IN([-1, ITEM_STATE_NORMAL, ITEM_STATE_NOTSUPPORTED]), null],
	'filter_templated_items' => [T_BS_INT, O_OPT, null,	IN('-1,0,1'), null],
	'filter_with_triggers' =>	[T_BS_INT, O_OPT, null,	IN('-1,0,1'), null],
	'filter_ipmi_sensor' =>		[T_BS_STR, O_OPT, null,	null,		null],
	// subfilters
	'subfilter_set' =>			[T_BS_STR, O_OPT, null,	null,		null],
	'subfilter_apps' =>			[T_BS_STR, O_OPT, null,	null,		null],
	'subfilter_types' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'subfilter_value_types' =>	[T_BS_INT, O_OPT, null,	null,		null],
	'subfilter_status' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'subfilter_state' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'subfilter_templated_items' => [T_BS_INT, O_OPT, null, null,		null],
	'subfilter_with_triggers' => [T_BS_INT, O_OPT, null,	null,		null],
	'subfilter_hosts' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'subfilter_interval' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'subfilter_history' =>		[T_BS_INT, O_OPT, null,	null,		null],
	'subfilter_trends' =>		[T_BS_INT, O_OPT, null,	null,		null],
	// sort and sortorder
	'sort' =>					[T_BS_STR, O_OPT, P_SYS,
									IN('"delay","history","key_","name","status","trends","type"'),
									null
								],
	'sortorder' =>				[T_BS_STR, O_OPT, P_SYS, IN('"'.BS_SORT_DOWN.'","'.BS_SORT_UP.'"'),	null]
];
check_fields($fields);
...
elseif (hasRequest('add') || hasRequest('update')) {
	...
	if (!bs_empty($_REQUEST['new_application'])) {
...
elseif (((hasRequest('action') && getRequest('action') == 'item.massupdateform') || hasRequest('massupdate')) && hasRequest('group_itemid')) {
	$data = [
		...
		'itemids' => getRequest('group_itemid', []),
		'description' => getRequest('description', ''),
		'delay' => getRequest('delay', BS_ITEM_DELAY_DEFAULT),
	...
	if ($hostCount > 1) {
		...
		if ($hostCount == 1 && $data['displayInterfaces']) {
			...
			$items = API::Item()->get([
				'itemids' => bs_objectValues($data['hosts']['items'], 'itemid'),
				'output' => ['itemid', 'type']
			]);
			$usedInterfacesTypes = [];
			foreach ($items as $item) {
...
// list of items
	...
	$sortOrder = getRequest('sortorder', CProfile::get('web.'.$page['file'].'.sortorder', BS_SORT_UP));
	...
	if (isset($_REQUEST['filter_application']) && !bs_empty($_REQUEST['filter_application'])) {
		$options['application'] = $_REQUEST['filter_application'];
	}
	if (isset($_REQUEST['filter_name']) && !bs_empty($_REQUEST['filter_name'])) {
		$options['search']['name'] = $_REQUEST['filter_name'];
	}
	if (isset($_REQUEST['filter_type']) && !bs_empty($_REQUEST['filter_type']) && $_REQUEST['filter_type'] != -1) {
		$options['filter']['type'] = $_REQUEST['filter_type'];
	}
	if (isset($_REQUEST['filter_key']) && !bs_empty($_REQUEST['filter_key'])) {
		$options['search']['key_'] = $_REQUEST['filter_key'];
	}
	if (isset($_REQUEST['filter_snmp_community']) && !bs_empty($_REQUEST['filter_snmp_community'])) {
		$options['filter']['snmp_community'] = $_REQUEST['filter_snmp_community'];
	}
	if (isset($_REQUEST['filter_snmpv3_securityname']) && !bs_empty($_REQUEST['filter_snmpv3_securityname'])) {
		$options['filter']['snmpv3_securityname'] = $_REQUEST['filter_snmpv3_securityname'];
	}
	if (isset($_REQUEST['filter_snmp_oid']) && !bs_empty($_REQUEST['filter_snmp_oid'])) {
		$options['filter']['snmp_oid'] = $_REQUEST['filter_snmp_oid'];
	}
	if (isset($_REQUEST['filter_port']) && !bs_empty($_REQUEST['filter_port'])) {
		$options['filter']['port'] = $_REQUEST['filter_port'];
	}
	if (isset($_REQUEST['filter_value_type']) && !bs_empty($_REQUEST['filter_value_type'])
			&& $_REQUEST['filter_value_type'] != -1) {
		$options['filter']['value_type'] = $_REQUEST['filter_value_type'];
	}
	if (isset($_REQUEST['filter_data_type']) && !bs_empty($_REQUEST['filter_data_type'])
	...
	if (hasRequest('filter_delay')) {
		...
		if ($filter_delay !== '') {
			if ($filter_type == -1 && $filter_delay == 0) {
				$options['filter']['type'] = [ITEM_TYPE_BSM, ITEM_TYPE_SNMPV1, ITEM_TYPE_SIMPLE,
					ITEM_TYPE_SNMPV2C, ITEM_TYPE_INTERNAL, ITEM_TYPE_SNMPV3, ITEM_TYPE_BSM_ACTIVE,
	...
	if (isset($_REQUEST['filter_history']) && !bs_empty($_REQUEST['filter_history'])) {
	...
	if (isset($_REQUEST['filter_status']) && !bs_empty($_REQUEST['filter_status']) && $_REQUEST['filter_status'] != -1) {
		$options['filter']['status'] = $_REQUEST['filter_status'];
	}
	if (isset($_REQUEST['filter_state']) && !bs_empty($_REQUEST['filter_state']) && $_REQUEST['filter_state'] != -1) {
		$options['filter']['status'] = ITEM_STATUS_ACTIVE;
		$options['filter']['state'] = $_REQUEST['filter_state'];
	}
	if (isset($_REQUEST['filter_templated_items']) && !bs_empty($_REQUEST['filter_templated_items'])
			&& $_REQUEST['filter_templated_items'] != -1) {
		$options['inherited'] = $_REQUEST['filter_templated_items'];
	}
	if (isset($_REQUEST['filter_with_triggers']) && !bs_empty($_REQUEST['filter_with_triggers'])
			&& $_REQUEST['filter_with_triggers'] != -1) {
		$options['with_triggers'] = $_REQUEST['filter_with_triggers'];
	}
	if (isset($_REQUEST['filter_ipmi_sensor']) && !bs_empty($_REQUEST['filter_ipmi_sensor'])) {
	...
	if ($data['items']) {
		...
		$dbHostItems = DBselect(
			...
			' WHERE i.hostid=h.hostid'.
				' AND '.dbConditionInt('i.itemid', bs_objectValues($data['items'], 'templateid'))
		);
		...
		foreach ($data['items'] as &$item) {
			$item['hostids'] = bs_objectValues($item['hosts'], 'hostid');
	...
	$itemTriggerIds = [];
	foreach ($data['items'] as $item) {
		$itemTriggerIds = array_merge($itemTriggerIds, bs_objectValues($item['triggers'], 'triggerid'));
	}
