...
$fields = [
	'action' =>			[T_BS_INT, O_OPT, P_SYS,	BETWEEN(-1, 6), null],
	'resourcetype' =>	[T_BS_INT, O_OPT, P_SYS,	BETWEEN(-1, 31), null],
	'filter_rst' =>		[T_BS_STR, O_OPT, P_SYS,	null,	null],
	'filter_set' =>		[T_BS_STR, O_OPT, P_SYS,	null,	null],
	'alias' =>			[T_BS_STR, O_OPT, P_SYS,	null,	null],
...
$sqlWhere = [];
if (!empty($data['alias'])) {
	$sqlWhere['alias'] = ' AND u.alias='.bs_dbstr($data['alias']);
}
if ($data['action'] > -1) {
	$sqlWhere['action'] = ' AND a.action='.bs_dbstr($data['action']);
}
if ($data['resourcetype'] > -1) {
	$sqlWhere['resourcetype'] = ' AND a.resourcetype='.bs_dbstr($data['resourcetype']);
}
$sqlWhere['from'] = ' AND a.clock>'.bs_dbstr($from);
$sqlWhere['till'] = ' AND a.clock<'.bs_dbstr($till);
...
while ($audit = DBfetch($dbAudit)) {
	...
	if (empty($audit['details'])) {
		$audit['details'] = DBfetchArray(DBselect(
			...
			' WHERE ad.auditid='.bs_dbstr($audit['auditid'])
		));
	}
	$data['actions'][$audit['auditid']] = $audit;
}
if (!empty($data['actions'])) {
	order_result($data['actions'], 'clock', BS_SORT_DOWN);
}

// get paging
$data['paging'] = getPagingLine($data['actions'], BS_SORT_DOWN, new CUrl('auditlogs.php'));
...
$data['timeline'] = [
	...
	'usertime' => isset($_REQUEST['stime']) ? date(TIMESTAMP_FORMAT, bsDateToTime($data['stime']) + $effectivePeriod) : null
];
