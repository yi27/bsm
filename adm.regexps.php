...
$fields = [
	'regexpids' =>		[T_BS_INT, O_OPT, P_SYS,		DB_ID,	null],
	'regexpid' =>		[T_BS_INT, O_OPT, P_SYS,		DB_ID,	'isset({form}) && {form} == "update"'],
	'name' =>			[T_BS_STR, O_OPT, null,		NOT_EMPTY, 'isset({add}) || isset({update})', _('Name')],
	'test_string' =>	[T_BS_STR, O_OPT, P_NO_TRIM,	null,	'isset({add}) || isset({update})', _('Test string')],
	'expressions' =>	[T_BS_STR, O_OPT, P_NO_TRIM,	null,	'isset({add}) || isset({update})'],
	// actions
	'action' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT, IN('"regexp.massdelete"'),	null],
	...
	'ajaxdata' =>		[T_BS_STR, O_OPT, P_ACT|P_NO_TRIM,		null,	null]
];
check_fields($fields);
...
if (isset($_REQUEST['regexpid'])) {
	$regExp = DBfetch(DBSelect('SELECT re.regexpid FROM regexps re WHERE re.regexpid='.bs_dbstr(getRequest('regexpid'))));
	if (empty($regExp)) {
		access_deny();
	}
}
...
if (isset($_REQUEST['form'])) {
	...
	if (isset($_REQUEST['regexpid']) && !isset($_REQUEST['form_refresh'])) {
		$regExp = DBfetch(DBSelect(
			...
			' FROM regexps re'.
			' WHERE re.regexpid='.bs_dbstr($_REQUEST['regexpid'])
		));
		...
		$data['expressions'] = DBfetchArray(DBselect(
			...
			' FROM expressions e'.
			' WHERE e.regexpid='.bs_dbstr($_REQUEST['regexpid']).
