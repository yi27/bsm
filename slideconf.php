...
$fields = [
	'shows' =>			[T_BS_INT, O_OPT,	P_SYS,		DB_ID,	null],
	'slideshowid' =>	[T_BS_INT, O_NO,	P_SYS,		DB_ID,	'isset({form}) && {form} == "update"'],
	'name' => [T_BS_STR, O_OPT, null, NOT_EMPTY, 'isset({add}) || isset({update})', _('Name')],
	'delay' => [T_BS_INT, O_OPT, null, BETWEEN(1, SEC_PER_DAY), 'isset({add}) || isset({update})',_('Default delay (in seconds)')],
	...
	// actions
	'action' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT, IN('"slideshow.massdelete"'),	null],
	...
	// sort and sortorder
	'sort' =>			[T_BS_STR, O_OPT, P_SYS, IN('"cnt","delay","name"'),					null],
...
elseif (hasRequest('add') || hasRequest('update')) {
	...
	if (hasRequest('update')) {
		...
		elseif (CWebUser::getType() == USER_TYPE_BSM_ADMIN && $data['userid'] === '') {
...
if (hasRequest('form')) {
	...
	if (array_key_exists('slideshowid', $db_slideshow) && !isset($_REQUEST['form_refresh'])) {
		...
		$data['slideshow']['slides'] = DBfetchArray(DBselect(
				...
				' FROM slides s'.
				' WHERE s.slideshowid='.bs_dbstr($db_slideshow['slideshowid']).
				' ORDER BY s.step'
		));
	}
	else {
		...
		if (hasRequest('form_refresh')) {
			if (CWebUser::getType() == USER_TYPE_SUPER_ADMIN || CWebUser::getType() == USER_TYPE_BSM_ADMIN) {
	...
	if ($data['filter']['name'] !== '') {
		...
		$sql_where = ' WHERE UPPER(s.name) LIKE '.bs_dbstr('%'.mb_strtoupper($pattern).'%')." ESCAPE '!'";
	}
	else {
		$sql_where = '';
	}
