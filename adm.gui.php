...
$fields = [
	'default_theme'				=> [T_BS_STR, O_OPT, null, IN('"'.implode('","', $themes).'"'), 'isset({update})',
		_('Default theme')
	],
	'dropdown_first_entry'		=> [T_BS_INT, O_OPT, null, IN('0,1,2'), 'isset({update})',
		_('Dropdown first entry')
	],
	'dropdown_first_remember'	=> [T_BS_INT, O_OPT, null, IN('1'), null, _('remember selected')],
	'search_limit'				=> [T_BS_INT, O_OPT, null, BETWEEN(1, 999999), 'isset({update})',
		_('Search/Filter elements limit')
	],
	'max_in_table'				=> [T_BS_INT, O_OPT, null, BETWEEN(1, 99999), 'isset({update})',
		_('Max count of elements to show inside table cell')
	],
	'event_ack_enable'			=> [T_BS_INT, O_OPT, null, IN('1'), null, _('Enable event acknowledges')],
	'event_expire'				=> [T_BS_INT, O_OPT, null, BETWEEN(1, 99999), 'isset({update})',
		_('Show events not older than (in days)')
	],
	'event_show_max'			=> [T_BS_INT, O_OPT, null, BETWEEN(1, 99999), 'isset({update})',
		_('Max count of events per trigger to show')
	],
	'server_check_interval'		=> [T_BS_INT, O_OPT, null, IN(SERVER_CHECK_INTERVAL), null,
		_('Show warning if Bsm server is down')
	],
	// actions
