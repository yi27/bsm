...
$fields = [
	'config' => [T_BS_INT, O_OPT, P_SYS, IN($queueModes), null]
];
...
// fetch data
$bsmServer = new CBsmServer($BS_SERVER, $BS_SERVER_PORT, BS_SOCKET_TIMEOUT, BS_SOCKET_BYTES_LIMIT);
$queueRequests = [
	QUEUE_OVERVIEW => CBsmServer::QUEUE_OVERVIEW,
	QUEUE_OVERVIEW_BY_PROXY => CBsmServer::QUEUE_OVERVIEW_BY_PROXY,
	QUEUE_DETAILS => CBsmServer::QUEUE_DETAILS
];
$queueData = $bsmServer->getQueue($queueRequests[$config], get_cookie('bs_sessionid'));

// check for errors error
if ($bsmServer->getError()) {
	error($bsmServer->getError());
...
if ($config == QUEUE_OVERVIEW) {
	...
	$queueData = bs_toHash($queueData, 'itemtype');
...
elseif ($config == QUEUE_OVERVIEW_BY_PROXY) {
	...
	$queueData = bs_toHash($queueData, 'proxyid');
...
elseif ($config == QUEUE_DETAILS) {
	$queueData = bs_toHash($queueData, 'itemid');
	...
	foreach ($queueData as $itemData) {
		...
		$table->addRow([
			bs_date2str(DATE_TIME_FORMAT_SECONDS, $itemData['nextcheck']),
			bs_date2age($itemData['nextcheck']),
