...
$fields = [
	...
	'severities'=>		[T_BS_INT,	O_OPT,	P_SYS,			null,	null],
	'filter_from' =>	[T_BS_STR,	O_OPT,	P_UNSET_EMPTY,	null,	null],
	'filter_till' =>	[T_BS_STR,	O_OPT,	P_UNSET_EMPTY,	null,	null],
...
$timeFrom = hasRequest('filter_from') ? bsDateToTime(getRequest('filter_from')) : $today;
$timeTill = hasRequest('filter_till') ? bsDateToTime(getRequest('filter_till')) : $tomorrow;
...
$sql = 'SELECT e.objectid,count(distinct e.eventid) AS cnt_event'.
		...
		' WHERE t.triggerid=e.objectid'.
			...
			' AND e.clock>='.bs_dbstr($data['filter']['filter_from']).
			' AND e.clock<='.bs_dbstr($data['filter']['filter_till']).
...
$sql .= ' AND '.dbConditionInt('t.flags', [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_CREATED]).
		' GROUP BY e.objectid'.
...
CArrayHelper::sort($data['triggers'], [
	['field' => 'cnt_event', 'order' => BS_SORT_DOWN],
