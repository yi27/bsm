...
$fields = [
	'description' =>	[T_BS_STR, O_OPT, null,		NOT_EMPTY,			'isset({add}) || isset({update})',
		_('Name')
	],
	'itemid' =>			[T_BS_INT, O_OPT, P_SYS,		DB_ID,				'isset({add}) || isset({update})'],
	'sform' =>			[T_BS_INT, O_OPT, null,		IN('0,1'),			null],
	'sitems' =>			[T_BS_INT, O_OPT, null,		IN('0,1'),			null],
	...
	'expressions' =>	[T_BS_STR, O_OPT, null,		NOT_EMPTY,			'isset({add}) || isset({update})'],
	'expr_type' =>		[T_BS_INT, O_OPT, null,		IN('0,1'),			null],
	'comments' =>		[T_BS_STR, O_OPT, null,		null,				null],
	'url' =>			[T_BS_STR, O_OPT, null,		null,				null],
	'status' =>			[T_BS_INT, O_OPT, null,		IN('0,1'),			null],
	...
	'keys' => 			[T_BS_STR, O_OPT, null,		null,				null],
];
check_fields($fields);
...
if (hasRequest('add') || hasRequest('update')) {
	...
	if ($exprs && ($expression = $constructor->getExpressionFromParts($host['host'], $item['key_'], $exprs))) {
		...
		if ($result) {
			...
			bs_add_post_js('closeForm("items.php");');
...
if (hasRequest('sform')) {
	...
	if (hasRequest('triggerid') && !hasRequest('form_refresh')) {
		$result = DBselect(
			...
			' FROM triggers t'.
			' WHERE t.triggerid='.bs_dbstr(getRequest('triggerid')).
				' AND EXISTS ('.
					'SELECT NULL'.
	...
	$form_list->addRow(_('Name'), (new CTextBox('description', $description))->setWidth(BS_TEXTAREA_STANDARD_WIDTH));
	...
	bs_add_post_js('logexpr_count='.($maxId + 1).';');
	bs_add_post_js('processExpressionList();');
	...
	bs_add_post_js('key_count='.($maxId + 1).';');
	...
	$form_list->addRow(_('Comments'), (new CTextArea('comments', $comments))->setWidth(BS_TEXTAREA_STANDARD_WIDTH));
	$form_list->addRow(_('URL'), (new CTextBox('url', $url))->setWidth(BS_TEXTAREA_STANDARD_WIDTH));
