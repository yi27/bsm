...
switch ($srctbl) {
	case 'hosts':
		$page['title'] = _('Hosts');
		$min_user_type = USER_TYPE_BSM_USER;
		break;
	case 'templates':
		$page['title'] = _('Templates');
		$min_user_type = USER_TYPE_BSM_ADMIN;
		break;
	case 'host_templates':
		$page['title'] = _('Hosts');
		$min_user_type = USER_TYPE_BSM_ADMIN;
...
$fields = [
	'dstfrm' =>						[T_BS_STR, O_OPT, P_SYS,	NOT_EMPTY,	'!isset({multiselect})'],
	'dstfld1' =>					[T_BS_STR, O_OPT, P_SYS,	NOT_EMPTY,	'!isset({multiselect})'],
	'srctbl' =>						[T_BS_STR, O_MAND, P_SYS,	NOT_EMPTY,	null],
	'srcfld1' =>					[T_BS_STR, O_MAND, P_SYS,	IN($allowedSrcFields[$_REQUEST['srctbl']]), null],
	'groupid' =>					[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null],
	'group' =>						[T_BS_STR, O_OPT, null,	null,		null],
	'hostid' =>						[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null],
	'host' =>						[T_BS_STR, O_OPT, null,	null,		null],
	'parent_discoveryid' =>			[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null],
	'screenid' =>					[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null],
	'templates' =>					[T_BS_STR, O_OPT, null,	NOT_EMPTY,	null],
	'host_templates' =>				[T_BS_STR, O_OPT, null,	NOT_EMPTY,	null],
	'multiselect' =>				[T_BS_INT, O_OPT, null,	null,		null],
	'submit' =>						[T_BS_STR, O_OPT, null,	null,		null],
	'excludeids' =>					[T_BS_STR, O_OPT, null,	null,		null],
	'only_hostid' =>				[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null],
	'monitored_hosts' =>			[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'templated_hosts' =>			[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'real_hosts' =>					[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'normal_only' =>				[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'with_applications' =>			[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'with_graphs' =>				[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'with_items' =>					[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'with_simple_graph_items' =>	[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'with_triggers' =>				[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'with_monitored_triggers' =>	[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'itemtype' =>					[T_BS_INT, O_OPT, null,	null,		null],
	'value_types' =>				[T_BS_INT, O_OPT, null,	BETWEEN(0, 15), null],
	'numeric' =>					[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'reference' =>					[T_BS_STR, O_OPT, null,	null,		null],
	'writeonly' =>					[T_BS_STR, O_OPT, null,	null,		null],
	'noempty' =>					[T_BS_STR, O_OPT, null,	null,		null],
	'select' =>						[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'submitParent' =>				[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'templateid' =>					[T_BS_INT, O_OPT, P_SYS,	DB_ID,		null]
];

// unset disabled item types
$allowedItemTypes = [ITEM_TYPE_BSM, ITEM_TYPE_BSM_ACTIVE, ITEM_TYPE_SIMPLE, ITEM_TYPE_INTERNAL,
...
for ($i = 2; $i <= $dstfldCount; $i++) {
	$fields['dstfld'.$i] = [T_BS_STR, O_OPT, P_SYS, null, null];
}
$srcfldCount = countRequest('srcfld');
for ($i = 2; $i <= $srcfldCount; $i++) {
	$fields['srcfld'.$i] = [T_BS_STR, O_OPT, P_SYS, IN($allowedSrcFields[$_REQUEST['srctbl']]), null];
}
check_fields($fields);
...
$excludeids = bs_toHash(getRequest('excludeids', []));
 ...
 */
if (!empty($group)) {
	$dbGroup = DBfetch(DBselect('SELECT g.groupid FROM groups g WHERE g.name='.bs_dbstr($group)));
...
if (!empty($host)) {
	$dbHost = DBfetch(DBselect('SELECT h.hostid FROM hosts h WHERE h.name='.bs_dbstr($host)));
...
if ($srctbl == 'usrgrp') {
	...
	foreach ($userGroups as $userGroup) {
		...
		if ($multiselect) {
			$js_action = "javascript: addValue(".bs_jsvalue($reference).', '.bs_jsvalue($userGroup['usrgrpid']).');';
		}
		else {
			$values = [
			...
			$js_action = 'javascript: addValues('.bs_jsvalue($dstfrm).', '.bs_jsvalue($values).'); close_window(); return false;';
		}
		...
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('usrgrps', ".bs_jsvalue($reference).');')
			)
		);

		insert_js('var popupReference = '.bs_jsvalue($userGroups, true).';');
	}
...
elseif ($srctbl === 'users') {
	...
	$data = [];
	$parentid = $dstfld1 ? bs_jsvalue($dstfld1) : 'null';
	...
	foreach ($users as &$user) {
		...
		$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($user['userid']).', '.
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('users', ".bs_jsvalue($reference).', '.$parentid.');')
			)
		);
	}

	insert_js('var popupReference = '.bs_jsvalue($data, true).';');
...
elseif ($srctbl == 'templates') {
	...
	foreach ($templates as &$template) {
		...
		if (isset($excludeids[$template['templateid']])) {
			...
			$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($template['templateid']).', '.
				$parentId.');';
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('templates', ".bs_jsvalue($reference).', '.$parentId.');')
			)
		);
	}
...
elseif ($srctbl == 'hosts') {
	...
	foreach ($hosts as &$host) {
		...
		if (isset($excludeids[$host['hostid']])) {
			...
			$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($host['hostid']).', '.
				$parentId.');';
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('hosts', ".bs_jsvalue($reference).', '.$parentId.');')
			)
		);
	}
...
elseif ($srctbl == 'host_groups') {
	...
	foreach ($hostgroups as &$hostgroup) {
		...
		if (isset($excludeids[$hostgroup['groupid']])) {
			...
			$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($hostgroup['groupid']).', '.
				$parentId.');';
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('hostGroups', ".bs_jsvalue($reference).', '.$parentId.');')
			)
		);
	}
...
elseif ($srctbl === 'triggers' || $srctbl === 'trigger_prototypes') {
	...
	if ($srctbl === 'trigger_prototypes') {
		...
		if ($normalOnly) {
			$options['filter']['flags'] = BS_FLAG_DISCOVERY_NORMAL;
		}
	...
	foreach ($triggers as $trigger) {
		...
		if ($multiselect) {
			$js_action = 'addValue('.bs_jsvalue($reference).', '.bs_jsvalue($trigger['triggerid']).', '.$parentId.');';
		}
		else {
			$values = [
			...
			$js_action = 'addValues('.bs_jsvalue($dstfrm).', '.bs_jsvalue($values).'); return false;';
		}
		...
		if ($trigger['dependencies']) {
			...
			$dependencies = CMacrosResolverHelper::resolveTriggerNames(
				bs_toHash($trigger['dependencies'], 'triggerid')
			);
		...
		$table->addRow([
			$multiselect
				? new CCheckBox('triggers['.bs_jsValue($trigger[$srcfld1]).']', $trigger['triggerid'])
				: null,
			$description,
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("addSelectedValues('triggers', ".bs_jsvalue($reference).', '.$parentId.');')
			)
		);

		insert_js('var popupReference = '.bs_jsValue($jsTriggers, true).';');
	}
...
elseif ($srctbl === 'items' || $srctbl === 'item_prototypes') {
	...
	foreach ($items as $item) {
		...
		if ($multiselect) {
			$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($item['itemid']).');';
		}
		else {
			$values = [];
			for ($i = 1; $i <= $dstfldCount; $i++) {
			...
			$js_action = 'javascript: addValues('.bs_jsvalue($dstfrm).', '.bs_jsvalue($values).', '.
		...
		$table->addRow([
			...
			$multiselect
				? new CCheckBox('items['.bs_jsValue($item[$srcfld1]).']', $item['itemid'])
				: null,
			$description,
			$item['key_'],
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('items', ".bs_jsvalue($reference).');')
			)
		);

		insert_js('var popupReference = '.bs_jsvalue($jsItems, true).';');
	}
...
elseif ($srctbl === 'graphs' || $srctbl === 'graph_prototypes') {
	...
	foreach ($graphs as $graph) {
		...
		if ($multiselect) {
			$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($graph['graphid']).');';
		}
		else {
			$values = [
		...
		$table->addRow([
			$multiselect
				? new CCheckBox('graphs['.bs_jsValue($graph[$srcfld1]).']', $graph['graphid'])
				: null,
			$description,
			$graphtype
		]);
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('graphs', ".bs_jsvalue($reference).');')
			)
		);

		insert_js('var popupReference = '.bs_jsvalue($graphs, true).';');
	}
...
elseif ($srctbl == 'sysmaps') {
	...
	foreach ($sysmaps as $sysmap) {
		if (isset($excludeids[$sysmap['sysmapid']])) {
			...
			if ($multiselect) {
				$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($sysmap['sysmapid']).');';
			}
			else {
				$values = [
		...
		$table->addRow([
			$multiselect ? new CCheckBox('sysmaps['.bs_jsValue($sysmap[$srcfld1]).']', $sysmap['sysmapid']) : null,
			$description
		]);
		unset($description);
	}
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('sysmaps', ".bs_jsvalue($reference).');')
			)
		);

		insert_js('var popupReference = '.bs_jsvalue($sysmaps, true).';');
	}
...
elseif ($srctbl == 'slides') {
	...
	foreach ($dbSlideshows as $dbSlideshow) {
		...
		if ($multiselect) {
			$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($dbSlideshow['slideshowid']).');';
		}
		else {
			$values = [
		...
		$table->addRow([
			$multiselect
				? new CCheckBox('slides['.bs_jsValue($dbSlideshow[$srcfld1]).']', $dbSlideshow['slideshowid'])
				: null,
			$name
		]);
	}
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('slides', ".bs_jsvalue($reference).');')
			)
		);

		insert_js('var popupReference = '.bs_jsvalue($slideshows, true).';');
	}
...
elseif ($srctbl == 'screens') {
	...
	foreach ($screens as $screen) {
		...
		if ($multiselect) {
			$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($screen['screenid']).');';
		}
		else {
			$values = [
		...
		$table->addRow([
			$multiselect ? new CCheckBox('screens['.bs_jsValue($screen[$srcfld1]).']', $screen['screenid']) : null,
			$name
		]);
	}
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('screens', ".bs_jsvalue($reference).');')
			)
		);

		insert_js('var popupReference = '.bs_jsvalue($screens, true).';');
	}
...
elseif ($srctbl == 'scripts') {
	...
	foreach ($scripts as $script) {
		...
		if ($multiselect) {
			$js_action = 'javascript: addValue('.bs_jsvalue($reference).', '.bs_jsvalue($script['scriptid']).');';
		}
		else {
			$values = [
		...
		$table->addRow([
			$multiselect ? new CCheckBox('scripts['.bs_jsValue($script[$srcfld1]).']', $script['scriptid']) : null,
			$description,
			$scriptExecuteOn,
	...
	if ($multiselect) {
		$table->setFooter(
			new CCol(
				(new CButton('select', _('Select')))
					->onClick("javascript: addSelectedValues('scripts', ".bs_jsvalue($reference).');')
			)
		);
		insert_js('var popupReference = '.bs_jsvalue($scripts, true).';');
	}
