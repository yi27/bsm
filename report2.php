...
$fields = [
	'mode' =>				[T_BS_INT,	O_OPT,	P_SYS,			IN('0,1'),	null],
	'hostgroupid' =>		[T_BS_INT,	O_OPT,	P_SYS,			DB_ID,		null],
	'tpl_triggerid' =>		[T_BS_INT,	O_OPT,	P_SYS,			DB_ID,		null],
	'triggerid' =>			[T_BS_INT,	O_OPT,	P_SYS|P_NZERO,	DB_ID,		null],
	// filter
	'filter_groupid'=>		[T_BS_INT,	O_OPT,	P_SYS,			DB_ID,		null],
	'filter_hostid' =>		[T_BS_INT,	O_OPT,	P_SYS,			DB_ID,		null],
	'filter_rst'=>			[T_BS_STR,	O_OPT,	P_SYS,			null,		null],
	'filter_set' =>			[T_BS_STR,	O_OPT,	P_SYS,			null,		null],
	'filter_timesince' =>	[T_BS_STR,	O_OPT,	P_UNSET_EMPTY,	null,		null],
	'filter_timetill' =>	[T_BS_STR,	O_OPT,	P_UNSET_EMPTY,	null,		null]
];
check_fields($fields);
...
if ($_REQUEST['filter_timetill'] > 0 && $_REQUEST['filter_timesince'] > $_REQUEST['filter_timetill']) {
	bs_swap($_REQUEST['filter_timesince'], $_REQUEST['filter_timetill']);
}

$_REQUEST['filter_timesince'] = bsDateToTime($_REQUEST['filter_timesince']
	? $_REQUEST['filter_timesince'] : date(TIMESTAMP_FORMAT_ZERO_TIME, time() - SEC_PER_DAY));
$_REQUEST['filter_timetill'] = bsDateToTime($_REQUEST['filter_timetill']
...
elseif (isset($_REQUEST['filter_hostid'])) {
	...
	if ($availabilityReportMode == AVAILABILITY_REPORT_BY_TEMPLATE) {
		...
		if (!empty($_REQUEST['filter_hostid'])) {
			...
			$triggerOptions['hostids'] = bs_objectValues($hosts, 'hostid');
		}
		...
		$sqlCondition = empty($_REQUEST['filter_hostid'])
			...
			: ' AND h.hostid='.bs_dbstr($_REQUEST['filter_hostid']);
	...
	$paging = getPagingLine($triggers, BS_SORT_UP, new CUrl('report2.php'));
	...
	foreach ($triggers as $trigger) {
		...
		$triggerTable->addRow([
			...
			$availability['true'] == 0 ? '' : (new CSpan(sprintf('%.4f%%', $availability['true'])))->addClass(BS_STYLE_RED),
			$availability['false'] == 0 ? '' : (new CSpan(sprintf('%.4f%%', $availability['false'])))->addClass(BS_STYLE_GREEN),
