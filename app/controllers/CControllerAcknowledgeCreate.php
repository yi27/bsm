...
class CControllerAcknowledgeCreate extends CController {
	...
	protected function checkInput() {
		$fields = [
			...
			'acknowledge_type' =>	'in '.BS_ACKNOWLEDGE_SELECTED.','.BS_ACKNOWLEDGE_PROBLEM.','.BS_ACKNOWLEDGE_ALL,
			'backurl' =>			'string'
		];
		...
		if (!$ret) {
			switch ($this->GetValidationError()) {
				case self::VALIDATION_ERROR:
					$response = new CControllerResponseRedirect('bsm.php?action=acknowledge.edit');
	...
	protected function doAction() {
		...
		if ($acknowledge_type == BS_ACKNOWLEDGE_PROBLEM || $acknowledge_type == BS_ACKNOWLEDGE_ALL) {
			$events = API::Event()->get([
				'output' => ['objectid'],
				'source' => EVENT_SOURCE_TRIGGERS,
				'object' => EVENT_OBJECT_TRIGGER,
			...
			if ($acknowledge_type == BS_ACKNOWLEDGE_PROBLEM) {
