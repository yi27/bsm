...
class CControllerScriptEdit extends CController {
	...
	protected function doAction() {
		...
		$data = [
			...
			'scriptid' => 0,
			'name' => '',
			'type' => BS_SCRIPT_TYPE_CUSTOM_SCRIPT,
			'execute_on' => BS_SCRIPT_EXECUTE_ON_AGENT,
			'command' => '',
			'commandipmi' => '',
			'description' => '',
			'usrgrpid' => 0,
		...
		if ($this->hasInput('scriptid')) {
			...
			$data['command'] = ($script['type'] == BS_SCRIPT_TYPE_CUSTOM_SCRIPT) ? $script['command'] : '';
			$data['commandipmi'] = ($script['type'] == BS_SCRIPT_TYPE_IPMI) ? $script['command'] : '';
