...
class CControllerProxyUpdate extends CController {
		...
		if (!$ret) {
			switch ($this->GetValidationError()) {
				case self::VALIDATION_ERROR:
					$response = new CControllerResponseRedirect('bsm.php?action=proxy.edit');
	...
	protected function doAction() {
		...
		if ($result) {
			$response = new CControllerResponseRedirect('bsm.php?action=proxy.list&uncheck=1');
			$response->setMessageOk(_('Proxy updated'));
		}
		else {
			$response = new CControllerResponseRedirect(
				'bsm.php?action=proxy.edit&proxyid='.$this->getInput('proxyid')
			);
			$response->setFormData($this->getInputAll());
			$response->setMessageError(_('Cannot update proxy'));
		}
