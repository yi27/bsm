...
class CControllerScriptUpdate extends CController {
	...
	protected function checkInput() {
		$fields = [
			...
			'type' =>					'               db scripts.type        |in '.BS_SCRIPT_TYPE_CUSTOM_SCRIPT.','.BS_SCRIPT_TYPE_IPMI,
			'execute_on' =>				'               db scripts.execute_on  |in '.BS_SCRIPT_EXECUTE_ON_AGENT.','.BS_SCRIPT_EXECUTE_ON_SERVER,
		...
		if (!$ret) {
			switch ($this->GetValidationError()) {
				case self::VALIDATION_ERROR:
					$response = new CControllerResponseRedirect('bsm.php?action=script.edit');
	...
	protected function doAction() {
		...
		if ($this->getInput('type', BS_SCRIPT_TYPE_CUSTOM_SCRIPT) == BS_SCRIPT_TYPE_IPMI
		...
		if ($result) {
			$response = new CControllerResponseRedirect('bsm.php?action=script.list&uncheck=1');
			$response->setMessageOk(_('Script updated'));
		}
		else {
			$response = new CControllerResponseRedirect('bsm.php?action=script.edit&scriptid='.$this->getInput('scriptid'));
			$response->setFormData($this->getInputAll());
			$response->setMessageError(_('Cannot update script'));
		}
		$this->setResponse($response);
