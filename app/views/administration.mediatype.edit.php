...
$mediaTypeFormList
	...
	->addRow(_('SMTP server'),
		(new CTextBox('smtp_server', $data['smtp_server']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	)
	->addRow(_('SMTP server port'),
		(new CNumericBox('smtp_port', $data['smtp_port'], 5, false, false, false))->setWidth(BS_TEXTAREA_TINY_WIDTH)
	)
	->addRow(_('SMTP helo'), (new CTextBox('smtp_helo', $data['smtp_helo']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH))
	->addRow(_('SMTP email'), (new CTextBox('smtp_email', $data['smtp_email']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH))
	...
	->addRow(_('Username'), (new CTextBox('smtp_username', $data['smtp_username']))->setWidth(BS_TEXTAREA_SMALL_WIDTH))
	->addRow(_('Script name'), (new CTextBox('exec_path', $data['exec_path']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH));
...
$mediaTypeFormList->addRow(_('GSM modem'),
	(new CTextBox('gsm_modem', $data['gsm_modem']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
);
...
if ($data['passwd'] != '') {
	...
	$passwdField = (new CPassBox('passwd'))->setWidth(BS_TEXTAREA_SMALL_WIDTH);
}

// append password field to form list
$mediaTypeFormList
	->addRow(_('Jabber identifier'),
		(new CTextBox('jabber_username', $data['jabber_username']))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
	)
	->addRow(_('Username'),
		(new CTextBox('eztext_username', $data['eztext_username']))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
	)
...
$cancelButton = (new CRedirectButton(_('Cancel'), 'bsm.php?action=mediatype.list'))->setId('cancel');
...
if ($data['mediatypeid'] == 0) {
	...
	$deleteButton = (new CRedirectButton(_('Delete'),
		'bsm.php?action=mediatype.delete&sid='.$data['sid'].'&mediatypeids[]='.$data['mediatypeid'],
		_('Delete media type?')
	))
		->setId('delete');
