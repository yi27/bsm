...
if (array_key_exists('event', $data)) {
	...
	foreach ($data['event']['acknowledges'] as $acknowledge) {
		$acknowledgesTable->addRow([
			(new CCol(bs_date2str(DATE_TIME_FORMAT_SECONDS, $acknowledge['clock'])))->addClass(BS_STYLE_NOWRAP),
			(new CCol(getUserFullname($acknowledge)))->addClass(BS_STYLE_NOWRAP),
			bs_nl2br($acknowledge['message'])
		]);
	}
	...
	(new CDiv(
		(new CRadioButtonList('acknowledge_type', (int) $data['acknowledge_type']))
			...
			->addValue([
				...
				$selected_events > 1 ? (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN) : null,
				$selected_events > 1 ? new CSup(_n('%1$s event', '%1$s events', $selected_events)) : null
			], BS_ACKNOWLEDGE_SELECTED)
			->addValue([
