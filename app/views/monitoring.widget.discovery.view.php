...
foreach ($drules as $drule) {
	$table->addRow([
		new CLink($drule['name'], 'bsm.php?action=discovery.view&druleid='.$drule['druleid']),
		(new CSpan($drule['up']))->addClass(BS_STYLE_GREEN),
		(new CSpan($drule['down']))->addClass(($drule['down'] != 0) ? BS_STYLE_RED : BS_STYLE_GREEN)
	]);
}
