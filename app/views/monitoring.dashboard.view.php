...
$widgets = [
	...
	WIDGET_FAVOURITE_MAPS => [
		...
		'links' => [
			['name' => _('Maps'), 'url' => 'bsm.php?action=map.view']
		],
...
foreach ($widgets as $widgetid => $widget) {
	$icon = (new CButton(null))
		->addClass(BS_STYLE_BTN_WIDGET_ACTION)
		->setTitle(_('Action'))
		->setId($widget['id'])
	...
	$dashboardGrid[$col][$row] = (new CCollapsibleUiWidget($widgetid, $widget['data']))
		...
		->setHeader($widget['header'], [$icon], true, 'bsm.php?action=dashboard.widget')
		->setFooter($footer);
}
...
if ($data['show_status_widget']) {
	$widgets[WIDGET_BSM_STATUS] = [
		'action' => 'widget.status.view',
		'header' => _('Status of Bsm'),
...
foreach ($widgets as $widgetid => $widget) {
	...
	$widgetRefreshParams[$widgetid] = [
		'frequency' => $rate,
		'url' => 'bsm.php?action='.$widget['action'],
		'counter' => 0,
		'darken' => 0,
...
<script type="text/javascript">
	...
	function addPopupValues(list) {
		...
		if (isset(list.object, favourites)) {
			...
			sendAjaxData('bsm.php?action=dashboard.favourite&operation=create', {
				data: {
					object: list.object,
