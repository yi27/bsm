...
$scriptFormList->addRow(_('Type'),
	new CComboBox('type', $data['type'], null, [
		...
		BS_SCRIPT_TYPE_CUSTOM_SCRIPT => _('Script')
	])
);
...
$scriptFormList->addRow(_('Command'),
	(new CTextBox('commandipmi', $data['commandipmi']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
);
...
]))->setWidth(BS_TEXTAREA_STANDARD_WIDTH), 'hostGroupSelection');
...
$scriptFormList->addRow($confirmationLabel, [
	(new CTextBox('confirmation', $data['confirmation']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	SPACE,
	(new CButton('testConfirmation', _('Test confirmation')))->addClass(BS_STYLE_BTN_GREY)
]);

$scriptView = (new CTabView())->addTab('scripts', _('Script'), $scriptFormList);

// footer
$cancelButton = (new CRedirectButton(_('Cancel'), 'bsm.php?action=script.list'))->setId('cancel');
...
if ($data['scriptid'] == 0) {
	...
	$deleteButton = (new CRedirectButton(_('Delete'),
		'bsm.php?action=script.delete&sid='.$data['sid'].'&scriptids[]='.$data['scriptid'],
		_('Delete script?')
	))
		->setId('delete');
