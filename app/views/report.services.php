...
order_result($data['sla']['sla'], 'from', BS_SORT_DOWN);

foreach ($data['sla']['sla'] as $sla) {
	switch ($data['period']) {
		case 'yearly':
			$from = bs_date2str(_x('Y', DATE_FORMAT_CONTEXT), $sla['from']);
			$to = null;
			break;

		case 'monthly':
			$from =  bs_date2str(_x('F', DATE_FORMAT_CONTEXT), $sla['from']);
			$to = null;
			break;

		case 'daily':
			$from = bs_date2str(DATE_FORMAT, $sla['from']);
			$to = null;
			break;

		case 'weekly':
			$from = bs_date2str(DATE_TIME_FORMAT, $sla['from']);
			$to = bs_date2str(DATE_TIME_FORMAT, $sla['to']);
			break;
	}
	...
	$percentage = $data['service']['showsla']
		? (new CSpan(sprintf('%2.4f', $sla['sla'])))
			->addClass($sla['sla'] >= $data['service']['goodsla'] ? BS_STYLE_GREEN : BS_STYLE_RED)
		: '';
