...
$httpTestData = Manager::HttpTest()->getLastData(bs_objectValues($result, 'httptestid'));
...
foreach ($groups as $group) {
	if (array_key_exists($group['groupid'], $table_data)) {
		$table->addRow([
			new CLink($group['name'], 'bsm.php?action=web.view&groupid='.$group['groupid'].'&hostid=0'),
			(new CSpan($table_data[$group['groupid']]['ok']))->addClass(BS_STYLE_GREEN),
			(new CSpan($table_data[$group['groupid']]['failed']))
				->addClass($table_data[$group['groupid']]['failed'] == 0 ? BS_STYLE_GREEN : BS_STYLE_RED),
			(new CSpan($table_data[$group['groupid']]['unknown']))->addClass(BS_STYLE_GREY)
		]);
	}
}
...
$output = [
	...
	'footer' => (new CListItem(_s('Updated: %s', bs_date2str(TIME_FORMAT_SECONDS))))->toString()
];
