...
$interfaceTable = (new CTable())
	...
	->addRow([
		(new CTextBox('ip', $data['ip'], false, 64))->setWidth(BS_TEXTAREA_INTERFACE_IP_WIDTH),
		(new CTextBox('dns', $data['dns'], false, 64))->setWidth(BS_TEXTAREA_INTERFACE_DNS_WIDTH),
		(new CRadioButtonList('useip', (int) $data['useip']))
			->addValue(_('IP'), INTERFACE_USE_IP)
			->addValue(_('DNS'), INTERFACE_USE_DNS)
			->setModern(true),
		(new CTextBox('port', $data['port'], false, 64))->setWidth(BS_TEXTAREA_INTERFACE_PORT_WIDTH)
	]);
...
foreach ($data['all_hosts'] as $host) {
	...
	if ($host['flags'] == BS_FLAG_DISCOVERY_NORMAL
		...
		$hosts_tween_box->addItem(
			...
			null,
			$host['proxy_hostid'] == 0
				|| (bccomp($host['proxy_hostid'], $data['proxyid']) == 0 && $host['flags'] == BS_FLAG_DISCOVERY_NORMAL)
		);
	}
}
...
$proxy_form_list = (new CFormList('proxyFormList'))
	...
	->addRow(_('Interface'), (new CDiv($interfaceTable))->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR))
...
$encryption_form_list = (new CFormList('encryption'))
	...
	->addRow(_('PSK identity'),
		(new CTextBox('tls_psk_identity', $data['tls_psk_identity'], false, 128))->setWidth(BS_TEXTAREA_BIG_WIDTH)
	)
	->addRow(_('PSK'),
		(new CTextBox('tls_psk', $data['tls_psk'], false, 512))->setWidth(BS_TEXTAREA_BIG_WIDTH)
	)
	->addRow(_('Issuer'),
		(new CTextBox('tls_issuer', $data['tls_issuer'], false, 1024))->setWidth(BS_TEXTAREA_BIG_WIDTH)
	)
	->addRow(_x('Subject', 'encryption certificate'),
		(new CTextBox('tls_subject', $data['tls_subject'], false, 1024))->setWidth(BS_TEXTAREA_BIG_WIDTH)
	);
...
$cancelButton = new CRedirectButton(_('Cancel'), 'bsm.php?action=proxy.list');
	...
	$tabs->setFooter(makeFormFooter(
		...
		[
			...
			new CRedirectButton(_('Delete'),
				'bsm.php?action=proxy.delete&sid='.$data['sid'].'&proxyids[]='.$data['proxyid'],
				_('Delete proxy?')
			),
			$cancelButton
		]
