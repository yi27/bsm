...
$fields = [
	'serviceid' => [T_BS_INT, O_MAND, P_SYS, DB_ID, null]
];
...
$d = bs_date2str(_x('Y', DATE_FORMAT_CONTEXT));
...
for ($i = 0, $period_start = $start; $i <= $sizeX; $i += $sizeX / 52) {
	...
	imageText($im, 6, 90, $i + $shiftX + 4, $sizeY + $shiftYup + 35, $textcolor, bs_date2str(_('d.M'), $period_start));
...
imagestringup($im, 1, imagesx($im) - 10, imagesy($im) - 50, BSM_HOMEPAGE, $grey);
