...
$fields = [
	...
	'triggerid' =>			[T_BS_INT, O_OPT, P_SYS,	DB_ID,		'(isset({form}) && ({form} == "update"))'],
	'copy_type' =>			[T_BS_INT, O_OPT, P_SYS,	IN([COPY_TYPE_TO_HOST, COPY_TYPE_TO_TEMPLATE, COPY_TYPE_TO_HOST_GROUP]), 'isset({copy})'],
	'copy_mode' =>			[T_BS_INT, O_OPT, P_SYS,	IN('0'),	null],
	'type' =>				[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'description' =>		[T_BS_STR, O_OPT, null,	NOT_EMPTY,	'isset({add}) || isset({update})', _('Name')],
	'expression' =>			[T_BS_STR, O_OPT, null,	NOT_EMPTY,	'isset({add}) || isset({update})', _('Expression')],
	'priority' =>			[T_BS_INT, O_OPT, null,	IN('0,1,2,3,4,5'), 'isset({add}) || isset({update})'],
	'comments' =>			[T_BS_STR, O_OPT, null,	null,		'isset({add}) || isset({update})'],
	'url' =>				[T_BS_STR, O_OPT, null,	null,		'isset({add}) || isset({update})'],
	'status' =>				[T_BS_STR, O_OPT, null,	null,		null],
	'input_method' =>		[T_BS_INT, O_OPT, null,	NOT_EMPTY,	'isset({toggle_input_method})'],
	'expr_temp' =>			[T_BS_STR, O_OPT, null,	NOT_EMPTY,	'(isset({add_expression}) || isset({and_expression}) || isset({or_expression}) || isset({replace_expression}))', _('Expression')],
	'expr_target_single' => [T_BS_STR, O_OPT, null,	NOT_EMPTY,	'(isset({and_expression}) || isset({or_expression}) || isset({replace_expression}))'],
	'dependencies' =>		[T_BS_INT, O_OPT, null,	DB_ID,		null],
	'new_dependency' =>		[T_BS_INT, O_OPT, null,	DB_ID.'{}>0', 'isset({add_dependency})'],
	'g_triggerid' =>		[T_BS_INT, O_OPT, null,	DB_ID,		null],
	...
	'filter_priority' =>	[T_BS_INT, O_OPT, null,
		IN([
	...
	'filter_state' =>		[T_BS_INT, O_OPT, null,	IN([-1, TRIGGER_STATE_NORMAL, TRIGGER_STATE_UNKNOWN]), null],
	'filter_status' =>		[T_BS_INT, O_OPT, null,
	...
	'toggle_input_method' =>[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'add_expression' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'and_expression' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'or_expression' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'replace_expression' =>	[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'remove_expression' =>	[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'test_expression' =>	[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'add_dependency' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'group_enable' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'group_disable' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	'group_delete' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT, null,	null],
	...
	// sort and sortorder
	'sort' =>				[T_BS_STR, O_OPT, P_SYS, IN('"description","priority","status"'),		null],
...
$triggerIds = bs_toArray($triggerIds);
...
elseif (hasRequest('add') || hasRequest('update')) {
	$trigger = [
		...
		'dependencies' => bs_toObject(getRequest('dependencies', []), 'triggerid')
	];
	...
	if (hasRequest('update')) {
		...
		$old_trigger['dependencies'] = bs_toHash(bs_objectValues($old_trigger['dependencies'], 'triggerid'));
	...
	if ($visible) {
		...
		foreach (getRequest('g_triggerid') as $triggerid) {
			...
			if (isset($visible['dependencies'])) {
				$trigger['dependencies'] = bs_toObject(getRequest('dependencies', []), 'triggerid');
			}
...
elseif (hasRequest('action') && getRequest('action') == 'trigger.masscopyto' && hasRequest('g_triggerid')) {
	...
	$data['triggers'] = API::Trigger()->get([
		...
		'triggerids' => bs_objectValues($data['triggers'], 'triggerid')
	]);
	...
	if ($depTriggerIds) {
		...
		foreach ($data['triggers'] as &$trigger) {
			order_result($trigger['dependencies'], 'description', BS_SORT_UP);
		}
		unset($trigger);

		foreach ($dependencyTriggers as &$dependencyTrigger) {
			order_result($dependencyTrigger['hosts'], 'name', BS_SORT_UP);
		}
