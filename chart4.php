...
$fields = [
	'triggerid' => [T_BS_INT, O_MAND, P_SYS, DB_ID, null]
];
...
$str = _s('%1$s (year %2$s)', $dbTrigger['description'], bs_date2str(_x('Y', DATE_FORMAT_CONTEXT)));
...
for ($i = 0, $periodStart = $start; $i <= $sizeX; $i += $sizeX / 52) {
	...
	imageText($im, 6, 90, $i + $shiftX + 4, $sizeY + $shiftYup + 30, $black, bs_date2str(_('d.M'), $periodStart));
...
imagestringup($im, 1, imagesx($im) - 10, imagesy($im) - 50, BSM_HOMEPAGE, $gray);
