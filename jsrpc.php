...
switch ($data['method']) {
				...
				if (!isset($usedTriggers[$event['objectid']])) {
					...
					$result[$number] = [
						...
						'body' => [
							...
							'[url='.$url_tr_events.']'.
								bs_date2str(DATE_TIME_FORMAT_SECONDS, $event['clock']).'[/url]',
						],
	...
	case 'bsm.status':
		...
		if (!CSession::keyExists('serverCheckResult')
			...
			CSession::setValue('serverCheckResult', $bsmServer->isRunning());
		...
		switch ($data['objectName']) {
			...
			case 'applications':
				$applications = API::Application()->get([
					'hostids' => bs_toArray($data['hostid']),
			...
			case 'triggers':
				...
				if ($triggers) {
					CArrayHelper::sort($triggers, [
						['field' => 'description', 'order' => BS_SORT_UP]
					]);
