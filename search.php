...
$fields = [
	...
	'search' =>	[T_BS_STR, O_OPT, P_SYS, null, null],
	// Ajax
	'widget' =>	[T_BS_STR, O_OPT, P_ACT,
		IN('"'.WIDGET_SEARCH_HOSTS.'","'.WIDGET_SEARCH_HOSTGROUP.'","'.WIDGET_SEARCH_TEMPLATES.'"'),
		null
	],
	'state'=>	[T_BS_INT, O_OPT, P_ACT, IN('0,1'), null]
];
check_fields($fields);
...
$rw_hosts = bs_toHash($rw_hosts, 'hostid');
...
foreach ($hosts as $hnum => $host) {
	...
	$table->addRow([
		...
		new CLink(_('Web'), 'bsm.php?action=web.view&'.$link),
		$applications_link,
		$items_link,
		$triggers_link,
		$graphs_link,
...
$groupids = bs_objectValues($hostGroups, 'groupid');
...
$rw_hostGroups = bs_toHash($rw_hostGroups, 'groupid');
...
if ($admin) {
	...
	$rw_templates = bs_toHash($rw_templates, 'templateid');
