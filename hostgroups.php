...
$fields = [
	...
	// group
	'groupid' =>		[T_BS_INT, O_OPT, P_SYS,	DB_ID,		'isset({form}) && {form} == "update"'],
	'name' =>			[T_BS_STR, O_OPT, null,	NOT_EMPTY,	'isset({add}) || isset({update})', _('Group name')],
...
if (hasRequest('form')) {
	...
	elseif (hasRequest('add') || hasRequest('update')) {
		...
		if ($groupId) {
			...
			if ($oldGroup['flags'] != BS_FLAG_DISCOVERY_CREATED) {
			...
			if ($result) {
				...
				$oldHostIds = bs_objectValues($oldGroup['hosts'], 'hostid');
				$newHostIds = array_keys($hosts);
				$oldTemplateIds = bs_objectValues($oldGroup['templates'], 'templateid');
				...
				if ($hostIdsToAdd || $templateIdsToAdd) {
					...
					if ($hostIdsToAdd) {
						$massAdd['hosts'] = bs_toObject($hostIdsToAdd, 'hostid');
					}

					if ($templateIdsToAdd) {
						$massAdd['templates'] = bs_toObject($templateIdsToAdd, 'templateid');
					}
...
if (hasRequest('form')) {
	...
	if ($data['groupid'] != 0) {
		...
		if (!hasRequest('form_refresh')) {
			...
			$data['hosts'] = bs_toHash(bs_objectValues($data['hosts'], 'hostid'), 'hostid');
		}
	}
