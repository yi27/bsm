...
$fields = [
	'config' =>			[T_BS_INT, O_OPT, null, IN(BS_AUTH_INTERNAL.','.BS_AUTH_LDAP.','.BS_AUTH_HTTP), null],
	'form_refresh' =>	[T_BS_INT, O_OPT, null,			null, null],
	// actions
	'update' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT,	null, null],
	'test' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT,	null, null],
	// LDAP
	'ldap_host' =>		[T_BS_STR, O_OPT, null,			NOT_EMPTY,
		'isset({config}) && {config} == '.BS_AUTH_LDAP.' && (isset({update}) || isset({test}))',	_('LDAP host')],
	'ldap_port' =>		[T_BS_INT, O_OPT, null,			BETWEEN(0, 65535),
		'isset({config}) && {config} == '.BS_AUTH_LDAP.' && (isset({update}) || isset({test}))',	_('Port')],
	'ldap_base_dn' =>	[T_BS_STR, O_OPT, null,			NOT_EMPTY,
		'isset({config}) && {config} == '.BS_AUTH_LDAP.' && (isset({update}) || isset({test}))',	_('Base DN')],
	'ldap_bind_dn' =>	[T_BS_STR, O_OPT, null,			null,
		'isset({config}) && {config} == '.BS_AUTH_LDAP.' && (isset({update}) || isset({test}))'],
	'ldap_bind_password' => [T_BS_STR, O_OPT, null,		null, null,				_('Bind password')],
	'ldap_search_attribute' => [T_BS_STR, O_OPT, null,	NOT_EMPTY,
		'isset({config}) && {config} == '.BS_AUTH_LDAP.' && (isset({update}) || isset({test}))',	_('Search attribute')],
	'user' =>			[T_BS_STR, O_OPT, null,			NOT_EMPTY,
		'isset({config}) && {config} == '.BS_AUTH_LDAP.' && (isset({update}) || isset({test}))'],
	'user_password' =>	[T_BS_STR, O_OPT, null,			NOT_EMPTY,
		'isset({config}) && {config} == '.BS_AUTH_LDAP.' && (isset({update}) || isset({test}))',	_('User password')],
	'change_bind_password' => [T_BS_STR, O_OPT, null, null,	null]
];
check_fields($fields);
 ...
 * Actions
 */
if ($config['authentication_type'] == BS_AUTH_INTERNAL) {
	if (hasRequest('update')) {
		$messageSuccess = _('Authentication method changed to Bsm internal');
		$messageFailed = _('Cannot change authentication method to Bsm internal');
		...
		if ($result) {
			...
			if ($isAuthenticationTypeChanged) {
				$result &= DBexecute(
					'UPDATE sessions SET status='.BS_SESSION_PASSIVE.
					' WHERE sessionid<>'.bs_dbstr(CWebUser::$data['sessionid'])
				);
			}

			$isAuthenticationTypeChanged = false;

			add_audit(AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_BSM_CONFIG, $messageSuccess);
		}
...
elseif ($config['authentication_type'] == BS_AUTH_LDAP) {
...
elseif ($config['authentication_type'] == BS_AUTH_HTTP) {
	if (hasRequest('update')) {
