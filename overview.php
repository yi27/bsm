...
$fields = [
	...
	'view_style'  => [T_BS_INT, O_OPT, P_SYS, IN('0,1'), null],
	'type'        => [T_BS_INT, O_OPT, P_SYS, IN('0,1'), null],
...
if (hasRequest('filter_set')) {
	...
	if ($config['event_ack_enable'] == EVENT_ACK_ENABLED) {
		CProfile::update('web.overview.filter.ack_status', getRequest('ack_status', BS_ACK_STS_ANY), PROFILE_TYPE_INT);
	}
...
if ($type == SHOW_TRIGGERS) {
	...
	$options = [
		...
		'withUnacknowledgedEvents' => ($filter['ackStatus'] == BS_ACK_STS_WITH_UNACK) ? true : null,
		'withLastEventUnacknowledged' => ($filter['ackStatus'] == BS_ACK_STS_WITH_LAST_UNACK) ? true : null,
