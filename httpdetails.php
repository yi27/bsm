...
$fields = [
	...
	'httptestid' =>	[T_BS_INT, O_MAND, P_SYS,	DB_ID,		null],
...
$itemids = bs_objectValues($itemids, 'itemid');
...
bs_add_post_js('timeControl.addObject("graph_in", '.bs_jsvalue($graph_in->timeline).', '.
	bs_jsvalue($time_control_data).');'
);
...
bs_add_post_js('timeControl.addObject("graph_time", '.bs_jsvalue($graph_in->timeline).', '.
	bs_jsvalue($time_control_data).');'
);
...
(new CWidget())
	...
	->addItem((new CDiv($graphs))->addClass(BS_STYLE_TABLE_FORMS_CONTAINER))
	->show();
