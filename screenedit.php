...
$fields = [
	'screenid' =>		[T_BS_INT, O_MAND, P_SYS,	DB_ID,			null],
	'screenitemid' =>	[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	'resourcetype' =>	[T_BS_INT, O_OPT, null,	IN($knownResourceTypes), 'isset({add}) || isset({update})'],
	'caption' =>		[T_BS_STR, O_OPT, null,	null,			null],
	'resourceid' =>		[T_BS_INT, O_OPT, null,	DB_ID,			null,
		hasRequest('add') || hasRequest('update') ? getResourceNameByType(getRequest('resourcetype')) : null],
	'templateid' =>		[T_BS_INT, O_OPT, null,	DB_ID,			null],
	'width' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 65535), null, _('Width')],
	'height' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 65535), null, _('Height')],
	'max_columns' =>	[T_BS_INT, O_OPT, null,
		BETWEEN(SCREEN_SURROGATE_MAX_COLUMNS_MIN, SCREEN_SURROGATE_MAX_COLUMNS_MAX), null, _('Max columns')
	],
	'colspan' =>		[T_BS_INT, O_OPT, null,	BETWEEN(1, 100), null, _('Column span')],
	'rowspan' =>		[T_BS_INT, O_OPT, null,	BETWEEN(1, 100), null, _('Row span')],
	'elements' =>		[T_BS_INT, O_OPT, null,	BETWEEN(1, 100), null, _('Show lines')],
	'sort_triggers' =>	[T_BS_INT, O_OPT, null,	BETWEEN(SCREEN_SORT_TRIGGERS_DATE_DESC, SCREEN_SORT_TRIGGERS_RECIPIENT_DESC), null],
	'valign' =>			[T_BS_INT, O_OPT, null,	BETWEEN(VALIGN_MIDDLE, VALIGN_BOTTOM), null],
	'halign' =>			[T_BS_INT, O_OPT, null,	BETWEEN(HALIGN_CENTER, HALIGN_RIGHT), null],
	'style' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 2),	'isset({add}) || isset({update})'],
	'url' =>			[T_BS_STR, O_OPT, null,	null,			'isset({add}) || isset({update})'],
	'dynamic' =>		[T_BS_INT, O_OPT, null,	null,			null],
	'x' =>				[T_BS_INT, O_OPT, null,	BETWEEN(1, 100), '(isset({add}) || isset({update})) && isset({form}) && {form} != "update"'],
	'y' =>				[T_BS_INT, O_OPT, null,	BETWEEN(1, 100), '(isset({add}) || isset({update})) && isset({form}) && {form} != "update"'],
	'screen_type' =>	[T_BS_INT, O_OPT, null,	null,			null],
	'tr_groupid' =>		[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	'tr_hostid' =>		[T_BS_INT, O_OPT, P_SYS,	DB_ID,			null],
	...
	'add_row' =>		[T_BS_INT, O_OPT, P_SYS|P_ACT,	BETWEEN(0, 100), null],
	'add_col' =>		[T_BS_INT, O_OPT, P_SYS|P_ACT,	BETWEEN(0, 100), null],
	'rmv_row' =>		[T_BS_INT, O_OPT, P_SYS|P_ACT,	BETWEEN(0, 100), null],
	'rmv_col' =>		[T_BS_INT, O_OPT, P_SYS|P_ACT,	BETWEEN(0, 100), null],
	'sw_pos' =>			[T_BS_INT, O_OPT, null,	BETWEEN(0, 100), null],
	'ajaxAction' =>		[T_BS_STR, O_OPT, P_ACT,	null,			null]
];
check_fields($fields);
...
if (!empty($_REQUEST['ajaxAction']) && $_REQUEST['ajaxAction'] == 'sw_pos') {
	...
	if (count($sw_pos) > 3) {
		...
		$fitem = DBfetch(DBselect(
			...
			' WHERE s.y='.bs_dbstr($sw_pos[0]).
				' AND s.x='.bs_dbstr($sw_pos[1]).
				' AND s.screenid='.bs_dbstr($screen['screenid'])
		));

		$sitem = DBfetch(DBselect(
			'SELECT s.screenitemid,s.colspan,s.rowspan'.
			' FROM screens_items s'.
			' WHERE s.y='.bs_dbstr($sw_pos[2]).
				' AND s.x='.bs_dbstr($sw_pos[3]).
				' AND s.screenid='.bs_dbstr($screen['screenid'])
		));

		if ($fitem) {
			DBexecute('UPDATE screens_items'.
						' SET y='.bs_dbstr($sw_pos[2]).',x='.bs_dbstr($sw_pos[3]).
						',colspan='.(isset($sitem['colspan']) ? bs_dbstr($sitem['colspan']) : 1).
						',rowspan='.(isset($sitem['rowspan']) ? bs_dbstr($sitem['rowspan']) : 1).
						' WHERE y='.bs_dbstr($sw_pos[0]).
							' AND x='.bs_dbstr($sw_pos[1]).
							' AND screenid='.bs_dbstr($screen['screenid']).
							' AND screenitemid='.bs_dbstr($fitem['screenitemid'])
			);
		}
		if ($sitem) {
			DBexecute('UPDATE screens_items '.
						' SET y='.bs_dbstr($sw_pos[0]).',x='.bs_dbstr($sw_pos[1]).
						',colspan='.(isset($fitem['colspan']) ? bs_dbstr($fitem['colspan']) : 1).
						',rowspan='.(isset($fitem['rowspan']) ? bs_dbstr($fitem['rowspan']) : 1).
						' WHERE y='.bs_dbstr($sw_pos[2]).
							' AND x='.bs_dbstr($sw_pos[3]).
							' AND screenid='.bs_dbstr($screen['screenid']).
							' AND screenitemid='.bs_dbstr($sitem['screenitemid'])
			);
		}
