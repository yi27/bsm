...
$fields = [
	...
	'select' =>				[T_BS_STR, O_OPT, null,	null,		null],
	'show_without_data' =>	[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	'show_details' =>		[T_BS_INT, O_OPT, null,	IN('0,1'),	null],
	...
	// sort and sortorder
	'sort' =>				[T_BS_STR, O_OPT, P_SYS, IN('"host","lastclock","name"'),				null],
...
if ($hosts) {
	...
	$items = API::Item()->get([
		...
		'applicationids' => ($applications !== null) ? bs_objectValues($applications, 'applicationid') : null,
		'webitems' => true,
		'filter' => [
	...
	if ($applications === null) {
		$applications = API::Application()->get([
			'output' => API_OUTPUT_EXTEND,
			'hostids' => array_keys(array_flip(bs_objectValues($items, 'hostid'))),
			'search' => ['name' => $filter['application']],
			'preservekeys' => true
		]);
	}
...
if ($items) {
	...
	if ($items) {
		// get history
		$history = Manager::History()->getLast($items, 2, BS_HISTORY_PERIOD);
...
$filterColumn1->addRow(
	...
	(new CMultiSelect(
		[
			...
			'popup' => [
				'parameters' => 'srctbl=host_groups&dstfrm=bs_filter&dstfld1=groupids_'.
					'&srcfld1=groupid&multiselect=1'
			]
	]))->setWidth(BS_TEXTAREA_FILTER_STANDARD_WIDTH)
);
$filterColumn1->addRow(
		_('Hosts'),
		(new CMultiSelect(
			[
				...
				'popup' => [
					'parameters' => 'srctbl=hosts&dstfrm=bs_filter&dstfld1=hostids_&srcfld1=hostid'.
...
$filterColumn1->addRow(
	_('Application'),
	[
		(new CTextBox('application', $filter['application']))->setWidth(BS_TEXTAREA_FILTER_STANDARD_WIDTH),
		...
		(new CButton('application_name', _('Select')))
			...
			->onClick('return PopUp("popup.php?srctbl=applications&srcfld1=name&real_hosts=1&dstfld1=application'.
				'&with_applications=1&dstfrm=bs_filter");'
			)
	]
);
...
$filterColumn2->addRow(
	_('Name'),
	(new CTextBox('select', $filter['select']))->setWidth(BS_TEXTAREA_FILTER_STANDARD_WIDTH)
);
$filterColumn2->addRow(
...
// table
$table = (new CTableInfo())->addClass(BS_STYLE_OVERFLOW_ELLIPSIS);
if (!$filterSet) {
...
foreach ($items as $key => $item){
	...
	if ($lastHistory) {
		$lastClock = bs_date2str(DATE_TIME_FORMAT_SECONDS, $lastHistory['clock']);
	...
	$state_css = ($item['state'] == ITEM_STATE_NOTSUPPORTED) ? BS_STYLE_GREY : null;

	if ($filter['showDetails']) {
		// item key
		$itemKey = ($item['type'] == ITEM_TYPE_HTTPTEST || $item['flags'] == BS_FLAG_DISCOVERY_CREATED)
			? (new CSpan($item['key_expanded']))->addClass(BS_STYLE_GREEN)
...
foreach ($applications as $appid => $dbApp) {
	...
	if (!$singleHostSelected) {
		...
		if ($host['status'] == HOST_STATUS_NOT_MONITORED) {
			$hostName->addClass(BS_STYLE_RED);
		}
	}
