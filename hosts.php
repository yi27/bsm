...
$fields = [
	'hosts' =>			[T_BS_INT, O_OPT, P_SYS,			DB_ID,		null],
	'groups' =>			[T_BS_INT, O_OPT, P_SYS,			DB_ID,		null],
	'new_groups' =>		[T_BS_STR, O_OPT, P_SYS,			null,		null],
	'hostids' =>		[T_BS_INT, O_OPT, P_SYS,			DB_ID,		null],
	'groupids' =>		[T_BS_INT, O_OPT, P_SYS,			DB_ID,		null],
	'applications' =>	[T_BS_INT, O_OPT, P_SYS,			DB_ID,		null],
	'groupid' =>		[T_BS_INT, O_OPT, P_SYS,			DB_ID,		null],
	'hostid' =>			[T_BS_INT, O_OPT, P_SYS,			DB_ID,		'isset({form}) && {form} == "update"'],
	'clone_hostid' =>	[T_BS_INT, O_OPT, P_SYS,			DB_ID,		'isset({form}) && {form} == "full_clone"'],
	'host' =>			[T_BS_STR, O_OPT, null,			NOT_EMPTY,	'isset({add}) || isset({update})', _('Host name')],
	'visiblename' =>	[T_BS_STR, O_OPT, null,			null,		'isset({add}) || isset({update})'],
	'description' =>	[T_BS_STR, O_OPT, null,			null,		null],
	'proxy_hostid' =>	[T_BS_INT, O_OPT, P_SYS,		    DB_ID,		null],
	'status' =>			[T_BS_INT, O_OPT, null,			IN([HOST_STATUS_MONITORED, HOST_STATUS_NOT_MONITORED]), null],
	'newgroup' =>		[T_BS_STR, O_OPT, null,			null,		null],
	'interfaces' =>		[T_BS_STR, O_OPT, null,			NOT_EMPTY,	'isset({add}) || isset({update})', _('Agent or SNMP or JMX or IPMI interface')],
	'mainInterfaces' =>	[T_BS_INT, O_OPT, null,			DB_ID,		null],
	'templates' =>		[T_BS_INT, O_OPT, null,			DB_ID,		null],
	'add_template' =>	[T_BS_STR, O_OPT, null,			null,		null],
	'add_templates' => [T_BS_INT, O_OPT, null,			DB_ID,		null],
	'templates_rem' =>	[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	'clear_templates' => [T_BS_INT, O_OPT, null,			DB_ID,		null],
	'ipmi_authtype' =>	[T_BS_INT, O_OPT, null,			BETWEEN(-1, 6), null],
	'ipmi_privilege' =>	[T_BS_INT, O_OPT, null,			BETWEEN(0, 5), null],
	'ipmi_username' =>	[T_BS_STR, O_OPT, null,			null,		null],
	'ipmi_password' =>	[T_BS_STR, O_OPT, null,			null,		null],
	'tls_connect' =>	[T_BS_INT, O_OPT, null,
		IN([HOST_ENCRYPTION_NONE, HOST_ENCRYPTION_PSK, HOST_ENCRYPTION_CERTIFICATE]), null],
	'tls_accept' =>		[T_BS_INT, O_OPT, null,
		BETWEEN(0, (HOST_ENCRYPTION_NONE | HOST_ENCRYPTION_PSK | HOST_ENCRYPTION_CERTIFICATE)), null],
	'tls_subject' =>	[T_BS_STR, O_OPT, null,		null,		null],
	'tls_issuer' =>		[T_BS_STR, O_OPT, null,		null,		null],
	'tls_psk_identity' =>	[T_BS_STR, O_OPT, null,		null,		null],
	'tls_psk' =>		[T_BS_STR, O_OPT, null,			null,		null],
	'flags' =>			[T_BS_INT, O_OPT, null,
		IN([BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_CREATED]), null],
	'mass_replace_tpls' => [T_BS_STR, O_OPT, null,		null,		null],
	'mass_clear_tpls' => [T_BS_STR, O_OPT, null,			null,		null],
	'inventory_mode' => [T_BS_INT, O_OPT, null,
		IN(HOST_INVENTORY_DISABLED.','.HOST_INVENTORY_MANUAL.','.HOST_INVENTORY_AUTOMATIC), null],
	'host_inventory' =>	[T_BS_STR, O_OPT, P_UNSET_EMPTY,	null,		null],
	'macros' =>			[T_BS_STR, O_OPT, P_SYS,			null,		null],
	'visible' =>		[T_BS_STR, O_OPT, null,			null,		null],
	'show_inherited_macros' => [T_BS_INT, O_OPT, null, IN([0,1]), null],
	// actions
	...
	'add_to_group' =>	[T_BS_INT, O_OPT, P_SYS|P_ACT,	DB_ID,		null],
	'delete_from_group' => [T_BS_INT, O_OPT, P_SYS|P_ACT,	DB_ID,		null],
	'unlink' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	'unlink_and_clear' => [T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	'add' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	'update' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	'masssave' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	'clone' =>			[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	'full_clone' =>		[T_BS_STR, O_OPT, P_SYS|P_ACT,	null,		null],
	...
	// filter
	'filter_set' =>		[T_BS_STR, O_OPT, P_SYS,		null,			null],
	'filter_rst' =>		[T_BS_STR, O_OPT, P_SYS,		null,			null],
	'filter_host' =>	[T_BS_STR, O_OPT, null,		null,			null],
	'filter_ip' =>		[T_BS_STR, O_OPT, null,		null,			null],
	'filter_dns' =>		[T_BS_STR, O_OPT, null,		null,			null],
	'filter_port' =>	[T_BS_STR, O_OPT, null,		null,			null],
	// sort and sortorder
	'sort' =>			[T_BS_STR, O_OPT, P_SYS, IN('"name","status"'),						null],
...
elseif (hasRequest('action') && getRequest('action') == 'host.massupdate' && hasRequest('masssave')) {
	...
	try {
		...
		$hosts = API::Host()->get([
			'output' => ['hostid'],
			'hostids' => $hostIds,
			'filter' => ['flags' => [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_CREATED]]
		]);
		$hosts = ['hosts' => $hosts];
		...
		if (isset($_REQUEST['mass_replace_tpls'])) {
			if (isset($_REQUEST['mass_clear_tpls'])) {
				...
				$hostTemplateIds = bs_objectValues($hostTemplates, 'templateid');
				$templatesToDelete = array_diff($hostTemplateIds, $templateIds);

				$hosts['templates_clear'] = bs_toObject($templatesToDelete, 'templateid');
			}
		...
		if ($newHostGroupIds && (!isset($visible['groups']) || !isset($replaceHostGroups))) {
			$add['groups'] = bs_toObject($newHostGroupIds, 'groupid');
		}
...
elseif (hasRequest('add') || hasRequest('update')) {
	try {
		...
		// host data
		if (!$create && $dbHost['flags'] == BS_FLAG_DISCOVERY_CREATED) {
			$host = [
				'hostid' => $hostId,
			...
			foreach ($interfaces as $key => $interface) {
				if (bs_empty($interface['ip']) && bs_empty($interface['dns'])) {
					unset($interface[$key]);
					continue;
				}
			...
			$groups = bs_toObject($groups, 'groupid');
			...
			if (!$create) {
				$host['templates_clear'] = bs_toObject(getRequest('clear_templates', []), 'templateid');
			}
		}
		...
		if (getRequest('form', '') === 'full_clone' && getRequest('clone_hostid', 0) != 0) {
			...
			if ($dbTriggers) {
				if (!copyTriggersToHosts(bs_objectValues($dbTriggers, 'triggerid'), $hostId, $srcHostId)) {
					throw new Exception();
				}
			}
			...
			if ($dbDiscoveryRules) {
				$copyDiscoveryRules = API::DiscoveryRule()->copy([
					'discoveryids' => bs_objectValues($dbDiscoveryRules, 'itemid'),
					'hostids' => [$hostId]
				]);
...
elseif (hasRequest('action') && str_in_array(getRequest('action'), ['host.massenable', 'host.massdisable']) && hasRequest('hosts')) {
	...
	$actHosts = bs_objectValues($actHosts, 'hostid');
...
if (hasRequest('action') && getRequest('action') === 'host.massupdateform' && hasRequest('hosts')) {
	$data = [
		...
		'inventories' => bs_toHash(getHostInventories(), 'db_field'),
		'tls_connect' => getRequest('tls_connect', HOST_ENCRYPTION_NONE),
		'tls_accept' => getRequest('tls_accept', HOST_ENCRYPTION_NONE),
		'tls_issuer' => getRequest('tls_issuer', ''),
		'tls_subject' => getRequest('tls_subject', ''),
...
elseif (hasRequest('form')) {
	$data = [
		...
		'flags' => getRequest('flags', BS_FLAG_DISCOVERY_NORMAL),
	...
	if (!hasRequest('form_refresh')) {
		if ($data['hostid'] != 0) {
			...
			if ($data['flags'] == BS_FLAG_DISCOVERY_CREATED) {
			...
			$data['groups'] = bs_objectValues($dbHost['groups'], 'groupid');
			...
			// Templates
			$data['templates'] = bs_objectValues($dbHost['parentTemplates'], 'templateid');
		...
		if ($data['hostid'] != 0) {
			...
			$templateids = bs_objectValues($dbHost['parentTemplates'], 'templateid');
	...
	if ($data['hostid'] != 0) {
		...
		$data['inventory_items'] = bs_toHash($data['inventory_items'], 'inventory_link');
	...
	if (!$data['macros'] && $data['flags'] != BS_FLAG_DISCOVERY_CREATED) {
		$macro = ['macro' => '', 'value' => ''];
		if ($data['show_inherited_macros']) {
	...
	$hosts = API::Host()->get([
		'hostids' => bs_objectValues($hosts, 'hostid'),
		'output' => API_OUTPUT_EXTEND,
	...
	foreach ($hosts as $host) {
		$templateIds = array_merge($templateIds, bs_objectValues($host['parentTemplates'], 'templateid'));
	}
