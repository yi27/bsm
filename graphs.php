...
$fields = [
	...
	'copy_type' => [T_BS_INT, O_OPT, P_SYS, IN([COPY_TYPE_TO_HOST, COPY_TYPE_TO_HOST_GROUP, COPY_TYPE_TO_TEMPLATE]), 'isset({copy})'],
	'copy_mode' =>			[T_BS_INT, O_OPT, P_SYS,		IN('0'),		null],
	'graphid' =>			[T_BS_INT, O_OPT, P_SYS,		DB_ID,			'isset({form}) && {form} == "update"'],
	'name' =>				[T_BS_STR, O_OPT, null,		NOT_EMPTY,		'isset({add}) || isset({update})', _('Name')],
	'width' =>				[T_BS_INT, O_OPT, null,		BETWEEN(20, 65535), 'isset({add}) || isset({update})', _('Width')],
	'height' =>				[T_BS_INT, O_OPT, null,		BETWEEN(20, 65535), 'isset({add}) || isset({update})', _('Height')],
	'graphtype' =>			[T_BS_INT, O_OPT, null,		IN('0,1,2,3'),	'isset({add}) || isset({update})'],
	'show_3d' =>			[T_BS_INT, O_OPT, P_NZERO,	IN('0,1'),		null],
	'show_legend' =>		[T_BS_INT, O_OPT, P_NZERO,	IN('0,1'),		null],
	'ymin_type' =>			[T_BS_INT, O_OPT, null,		IN('0,1,2'),	null],
	'ymax_type' =>			[T_BS_INT, O_OPT, null,		IN('0,1,2'),	null],
	'yaxismin' =>			[T_BS_DBL, O_OPT, null,		null,			'(isset({add}) || isset({update})) && isset({graphtype}) && ({graphtype} == '.GRAPH_TYPE_NORMAL.' || {graphtype} == '.GRAPH_TYPE_STACKED.')'],
	'yaxismax' =>			[T_BS_DBL, O_OPT, null,		null,			'(isset({add}) || isset({update})) && isset({graphtype}) && ({graphtype} == '.GRAPH_TYPE_NORMAL.' || {graphtype} == '.GRAPH_TYPE_STACKED.')'],
	'ymin_itemid' =>		[T_BS_INT, O_OPT, null,		DB_ID,			'(isset({add}) || isset({update})) && isset({ymin_type}) && {ymin_type} == '.GRAPH_YAXIS_TYPE_ITEM_VALUE],
	'ymax_itemid' =>		[T_BS_INT, O_OPT, null,		DB_ID,			'(isset({add}) || isset({update})) && isset({ymax_type}) && {ymax_type} == '.GRAPH_YAXIS_TYPE_ITEM_VALUE],
	'percent_left' =>		[T_BS_DBL, O_OPT, null,		BETWEEN(0, 100), null, _('Percentile line (left)')],
	'percent_right' =>		[T_BS_DBL, O_OPT, null,		BETWEEN(0, 100), null, _('Percentile line (right)')],
	'visible' =>			[T_BS_INT, O_OPT, null,		BETWEEN(0, 1),	null],
	'items' =>				[T_BS_STR, O_OPT, null,		null,			null],
	'show_work_period' =>	[T_BS_INT, O_OPT, null,		IN('1'),		null],
	'show_triggers' =>		[T_BS_INT, O_OPT, null,		IN('1'),		null],
	'group_graphid' =>		[T_BS_INT, O_OPT, null,		DB_ID,			null],
	...
	// actions
	'action' =>				[T_BS_STR, O_OPT, P_SYS|P_ACT, IN('"graph.masscopyto","graph.massdelete"'),	null],
	...
	// sort and sortorder
	'sort' =>				[T_BS_STR, O_OPT, P_SYS, IN('"graphtype","name"'),					null],
...
if (isset($_REQUEST['yaxismin']) && bs_empty($_REQUEST['yaxismin'])) {
	unset($_REQUEST['yaxismin']);
}
if (isset($_REQUEST['yaxismax']) && bs_empty($_REQUEST['yaxismax'])) {
...
elseif (hasRequest('add') || hasRequest('update')) {
	...
	if (hasRequest('parent_discoveryid')) {
		$graph['flags'] = BS_FLAG_DISCOVERY_PROTOTYPE;
...
elseif (hasRequest('action') && getRequest('action') == 'graph.massdelete' && hasRequest('group_graphid')) {
	...
	if (getRequest('copy_targetid') != 0 && hasRequest('copy_type')) {
		...
		// host groups
			...
			bs_value2array(getRequest('copy_targetid'));
			...
			$dbGroups = bs_toHash($dbGroups, 'groupid');
...
elseif (isset($_REQUEST['form'])) {
	...
	if ($data['items']) {
		$items = API::Item()->get([
			...
			'itemids' => bs_objectValues($data['items'], 'itemid'),
			'filter' => [
				'flags' => [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_PROTOTYPE, BS_FLAG_DISCOVERY_CREATED]
			],
			'webitems' => true,
			'preservekeys' => true
		]);
	...
	$options = [
		'graphids' => bs_objectValues($data['graphs'], 'graphid'),
