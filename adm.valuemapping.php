...
if (hasRequest('action') && getRequest('action') === 'valuemap.export' && hasRequest('valuemapids')) {
	$page['file'] = 'bs_export_valuemaps.xml';
...
$fields = [
	'valuemapids' =>	[T_BS_INT, O_OPT,	P_SYS,			DB_ID,		null],
	'valuemapid' =>		[T_BS_INT, O_NO,	P_SYS,			DB_ID,		'isset({form}) && {form} == "update"'],
	'name' =>			[T_BS_STR, O_OPT,	null,			NOT_EMPTY,	'isset({add}) || isset({update})'],
	'mappings' =>		[T_BS_STR, O_OPT,	null,			null,		null],
	// actions
	...
	'action' =>			[T_BS_STR, O_OPT,	P_SYS|P_ACT,	IN('"valuemap.export","valuemap.delete"'), null],
	// sort and sortorder
...
if (hasRequest('form')) {
	...
	if ($data['valuemapid'] != 0 && !hasRequest('form_refresh')) {
		...
		$data = bs_array_merge($data, $valuemap);
