...
$fields = [
	'filterEnable' =>	[T_BS_INT, O_OPT, P_SYS,			null,			null],
	'grpswitch' =>		[T_BS_INT, O_OPT, P_SYS,			BETWEEN(0, 1),	null],
	'groupids' =>		[T_BS_INT, O_OPT, P_SYS,			null,			null],
	'hidegroupids' =>	[T_BS_INT, O_OPT, P_SYS,			null,			null],
	'trgSeverity' =>	[T_BS_INT, O_OPT, P_SYS,			null,			null],
	'trigger_name' =>	[T_BS_STR, O_OPT, P_SYS,			null,			null],
	'maintenance' =>	[T_BS_INT, O_OPT, P_SYS,			BETWEEN(0, 1),	null],
	'extAck' =>			[T_BS_INT, O_OPT, P_SYS,			null,			null],
	'form_refresh' =>	[T_BS_INT, O_OPT, P_SYS,			null,			null],
...
if (hasRequest('form_refresh')) {
	...
	$data['groupIds'] = bs_toHash($data['groupIds']);
	$data['hideGroupIds'] = getRequest('hidegroupids', []);
	$data['hideGroupIds'] = bs_toHash($data['hideGroupIds']);
}
else {
	...
	$data['severity'] = bs_empty($data['severity']) ? [] : explode(';', $data['severity']);
	...
	$data['groupIds'] = bs_objectValues($data['groupIds'], 'value');
	$data['groupIds'] = bs_toHash($data['groupIds']);
	$data['hideGroupIds'] = CFavorite::get('web.dashconf.groups.hide.groupids');
	$data['hideGroupIds'] = bs_objectValues($data['hideGroupIds'], 'value');
	$data['hideGroupIds'] = bs_toHash($data['hideGroupIds']);
}

$data['severity'] = bs_toHash($data['severity']);
$data['severities'] = [
