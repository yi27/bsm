...
if (hasRequest('csv_export')) {
	...
	$page['file'] = 'bs_events_export.csv';
...
if ($csvExport) {
	$period = getRequest('period', BS_PERIOD_DEFAULT);
	...
	if (hasRequest('period')) {
		$_REQUEST['period'] = getRequest('period', BS_PERIOD_DEFAULT);
...
if ($csvExport) {
	...
	if ($source == EVENT_SOURCE_TRIGGERS) {
		...
		if ($triggerId != 0 && $pageFilter->hostid != 0) {
			...
			$old_trigger['hosts'] = bs_toHash($old_trigger['hosts'], 'hostid');
			...
			if (!array_key_exists($pageFilter->hostid, $old_trigger['hosts'])) {
				...
				foreach ($new_triggers as $new_trigger) {
					$new_trigger['hosts'] = bs_toHash($new_trigger['hosts'], 'hostid');
	...
	if ($source == EVENT_SOURCE_TRIGGERS) {
		...
		$filterColumn->addRow(
			_('Trigger'),
			[
				(new CTextBox('trigger', $trigger, true))->setWidth(BS_TEXTAREA_FILTER_BIG_WIDTH),
...
if (!$firstEvent) {
	...
	if (!$csvExport) {
		$events = [];
		$paging = getPagingLine($events, BS_SORT_UP, $url);
	}
}
else {
	...
	if ($source == EVENT_SOURCE_DISCOVERY) {
		...
		$paging = getPagingLine($dsc_events, BS_SORT_DOWN, $url);
		...
		foreach ($dsc_events as $event_data) {
			...
			if ($csvExport) {
				$csvRows[] = [
					bs_date2str(DATE_TIME_FORMAT_SECONDS, $event_data['clock']),
				...
				$table->addRow([
					bs_date2str(DATE_TIME_FORMAT_SECONDS, $event_data['clock']),
					$event_data['object_data']['ip'],
					bs_empty($event_data['object_data']['dns']) ? SPACE : $event_data['object_data']['dns'],
					$event_data['description'],
	...
	// source not discovery i.e. trigger
		...
		if ($csvExport || $pageFilter->hostsSelected || $triggerId != 0) {
			...
			while (true) {
				...
				$triggerIdsFromSlice = array_keys(array_flip(bs_objectValues($allEventsSlice, 'objectid')));
			...
			$paging = getPagingLine($events, BS_SORT_DOWN, $url);

			// query event with extend data
			$events = API::Event()->get([
				'source' => EVENT_SOURCE_TRIGGERS,
				'object' => EVENT_OBJECT_TRIGGER,
				'eventids' => bs_objectValues($events, 'eventid'),
				'output' => API_OUTPUT_EXTEND,
			...
			$triggers = API::Trigger()->get([
				...
				'triggerids' => bs_objectValues($events, 'objectid'),
				'preservekeys' => true
			]);
			...
			foreach ($events as $event) {
				...
				$event['duration'] = ($nextEvent = get_next_event($event, $events))
					? bs_date2age($event['clock'], $nextEvent['clock'])
				...
				if ($csvExport) {
					...
					$table->addRow([
						(new CLink(bs_date2str(DATE_TIME_FORMAT_SECONDS, $event['clock']),
...
if ($csvExport) {
	...
	$objData = [
		...
		'sliderMaximumTimePeriod' => BS_MAX_PERIOD
	];

	bs_add_post_js('jqBlink.blink();');
	bs_add_post_js('timeControl.addObject("scroll_events_id", '.bs_jsvalue($timeline).', '.bs_jsvalue($objData).');');
	bs_add_post_js('timeControl.processObjects();');
