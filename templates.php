...
if (hasRequest('action') && getRequest('action') == 'template.export' && hasRequest('templates')) {
	...
	$page['file'] = 'bs_export_templates.xml';
}
else {
	$exportData = false;
...
$fields = [
	...
	'clear_templates'	=> [T_BS_INT, O_OPT, P_SYS,		DB_ID,	null],
	...
	'templateid'		=> [T_BS_INT, O_OPT, P_SYS,		DB_ID,	'isset({form}) && {form} == "update"'],
	'template_name'		=> [T_BS_STR, O_OPT, null,		NOT_EMPTY, 'isset({add}) || isset({update})', _('Template name')],
...
elseif (hasRequest('add') || hasRequest('update')) {
	...
	try {
		...
		$templatesClear = bs_toObject($templatesClear, 'templateid');
		...
		if ($templateId && $cloneTemplateId && getRequest('form') === 'full_clone') {
			...
			if ($dbTriggers) {
				$result &= copyTriggersToHosts(bs_objectValues($dbTriggers, 'triggerid'),
			...
			if ($dbTemplateScreens) {
				$result &= API::TemplateScreen()->copy([
					'screenIds' => bs_objectValues($dbTemplateScreens, 'screenid'),
...
elseif (isset($_REQUEST['delete']) && isset($_REQUEST['templateid'])) {
	...
	$result = API::Template()->massUpdate([
		'templates' => bs_toObject($_REQUEST['templateid'], 'templateid'),
		'hosts' => []
	]);
	if ($result) {
...
elseif (hasRequest('action') && str_in_array(getRequest('action'), ['template.massdelete', 'template.massdeleteclear']) && hasRequest('templates')) {
	...
	if (getRequest('action') === 'template.massdelete') {
		$result = API::Template()->massUpdate([
			'templates' => bs_toObject($templates, 'templateid'),
			'hosts' => []
		]);
	}
...
if (hasRequest('form')) {
	...
	if ($templateId != 0 && !hasRequest('form_refresh')) {
		$data['groupIds'] = bs_objectValues($data['dbTemplate']['groups'], 'groupid');
	...
	foreach ($templates as $template) {
		...
		foreach ($linkedToObjects as $linkedToObject) {
			...
			$link = (new CLink($linkedToObject['name'], $url))->addClass(BS_STYLE_LINK_ALT);

			if ($linkedToObject['status'] == HOST_STATUS_TEMPLATE) {
				$link->addClass(BS_STYLE_GREY);
			}
			else {
				$link->addClass($linkedToObject['status'] == HOST_STATUS_MONITORED ? BS_STYLE_GREEN : BS_STYLE_RED);
			}
		...
		$table->addRow([
			...
			(new CCol($templatesOutput))->addClass(BS_STYLE_NOWRAP),
			[
