...
$fields = [
	'year' =>		[T_BS_INT, O_OPT, P_SYS|P_NZERO,	null,	null],
	'period' =>		[T_BS_STR, O_OPT, P_SYS|P_NZERO,	IN('"daily","weekly","monthly","yearly"'), null],
	'media_type' =>	[T_BS_INT, O_OPT, P_SYS,			DB_ID,	null]
];
check_fields($fields);
...
if (bs_empty($media_types)) {
	...
	foreach ($intervals as $from => $till) {
		// interval start
		$row = [bs_date2str($dateFormat, $from)];

		// interval end, displayed only for week intervals
		if ($period == 'weekly') {
			$row[] = bs_date2str($dateFormat, min($till, time()));
		}
	...
	if ($media_type == 0) {
		...
		foreach ($media_types as $id => $description) {
			$links[] = new CLink($description, 'bsm.php?action=mediatype.edit&mediatypeid='.$id);
			$links[] = SPACE.'/'.SPACE;
		}
		array_pop($links);
