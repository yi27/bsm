...
if ($_REQUEST['fullscreen'] === '1') {
	...
	define('BS_PAGE_FULLSCREEN', 1);
}

require_once dirname(__FILE__).'/menu.inc.php';

if (!defined('BS_PAGE_NO_THEME')) {
	define('BS_PAGE_NO_THEME', false);
}
...
switch ($page['type']) {
	...
	default:
		...
		if (isset($BS_SERVER_NAME) && $BS_SERVER_NAME !== '') {
			$pageTitle = $BS_SERVER_NAME.NAME_DELIMITER;
		}

		$pageTitle .= isset($page['title']) ? $page['title'] : _('Bsm');

		if ((defined('BS_PAGE_DO_REFRESH') || defined('BS_PAGE_DO_JS_REFRESH')) && CWebUser::$data['refresh']) {
...
$denied_page_requested = bs_construct_menu($main_menu, $sub_menus, $page);
...
if ($page['type'] == PAGE_TYPE_HTML) {
	...
	if (!BS_PAGE_NO_THEME) {
		...
		if (!empty($DB['DB'])) {
			...
			if ((!defined('BS_PAGE_NO_MENU') || defined('BS_PAGE_FULLSCREEN')) && $config['server_check_interval']
					&& !empty($BS_SERVER) && !empty($BS_SERVER_PORT)) {
...
if ($pageTitle == "Bsm"){
	...
	if ($pageTitle == "Bsm"){
		$pageHeader->addCssFile('styles/'.CHtml::encode('style').'.css');
	}
	...
	$pageHeader->addJsFile('js/bootstrap.js');
	...
	$path = 'jsLoader.php?ver='.BSM_VERSION.'&amp;lang='.CWebUser::$data['lang'].'&showGuiMessaging='.$showGuiMessaging;
...
?>
<body>
<div class="<?= BS_STYLE_MSG_BAD_GLOBAL ?>" id="msg-bad-global"></div>
<?php
}

define('PAGE_HEADER_LOADED', 1);

if (defined('BS_PAGE_NO_HEADER')) {
	return null;
}
...
if ($failedAttempts = CProfile::get('web.login.attempt.failed', 0)) {
	...
	$error_msg = _n('%4$s failed login attempt logged. Last failed attempt was from %1$s on %2$s at %3$s.',
		...
		$attempip,
		bs_date2str(DATE_FORMAT, $attempdate),
		bs_date2str(TIME_FORMAT, $attempdate),
		$failedAttempts
	);
	error($error_msg);
