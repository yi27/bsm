 ...
 * @return bool
 */
function bs_is_callable(array $names) {
...
function bs_setcookie($name, $value, $time = null) {
	setcookie($name, $value, isset($time) ? $time : 0, null, null, HTTPS);
	$_COOKIE[$name] = $value;
}

function bs_unsetcookie($name) {
	bs_setcookie($name, null, -99999);
	unset($_COOKIE[$name]);
}
...
function dowHrMinToSec($dow, $hr, $min) {
	if (bs_empty($dow) || bs_empty($hr) || bs_empty($min) || !bs_ctype_digit($dow) || !bs_ctype_digit($hr) || !bs_ctype_digit($min)) {
		return false;
	}
...
function bs_date2str($format, $value = null) {
...
function bs_date2age($startDate, $endDate = 0, $utime = false) {
	if (!$utime) {
 ...
 * @return int
 */
function bsAddSecondsToUnixtime($sec, $unixtime) {
	return strtotime('+'.$sec.' seconds', bsDateToTime($unixtime));
}
...
function bs_num2bitstr($num, $rev = false) {
	if (!is_numeric($num)) {
		return 0;
	}
...
function convertUnitsS($value, $ignore_millisec = false) {
	$secs = round($value * 1000, BS_UNITS_ROUNDOFF_UPPER_LIMIT) / 1000;
	if ($secs < 0) {
		$secs = -$secs;
		$str = '-';
	}
	...
	if ($n == 12) {
		...
		if ($ignore_millisec) {
			$n = round($secs, BS_UNITS_ROUNDOFF_UPPER_LIMIT);
			if ($n_unit < 1 && $n != 0) {
				$values['s'] += $n;
			}
		}
		else {
			$n = round($secs * 1000, BS_UNITS_ROUNDOFF_UPPER_LIMIT);
			if ($n_unit < 1 && $n != 0) {
				$values['ms'] = $n;
			}
		}
...
function convert_units($options = []) {
	...
	if ($options['units'] == 'unixtime') {
		return bs_date2str(DATE_TIME_FORMAT_SECONDS, $options['value']);
	}
	...
	if (in_array($options['units'], $blackList) || (bs_empty($options['units'])
			&& ($options['convert'] == ITEM_CONVERT_WITH_UNITS))) {
		if (abs($options['value']) >= BS_UNITS_ROUNDOFF_THRESHOLD) {
			$options['value'] = round($options['value'], BS_UNITS_ROUNDOFF_UPPER_LIMIT);
		}
		$options['value'] = sprintf('%.'.BS_UNITS_ROUNDOFF_LOWER_LIMIT.'f', $options['value']);
	...
	if (bccomp($abs, 1) == -1) {
		$options['value'] = round($options['value'], BS_UNITS_ROUNDOFF_MIDDLE_LIMIT);
	...
	if (round($valUnit['value'], BS_UNITS_ROUNDOFF_MIDDLE_LIMIT) > 0) {
 ...
 */
function bs_swap(&$a, &$b) {
	$tmp = $a;
	$a = $b;
	$b = $tmp;
}

function bs_avg($values) {
	bs_value2array($values);
	$sum = 0;
 ...
 * @return array
 */
function bs_array_diff(array $primary, array $secondary, $field) {
	$fields1 = bs_objectValues($primary, $field);
	$fields2 = bs_objectValues($secondary, $field);

	$first = array_diff($fields1, $fields2);
	$first = bs_toHash($first);

	$second = array_diff($fields2, $fields1);
	$second = bs_toHash($second);
...
function bs_array_push(&$array, $add) {
 ...
 * @return array
 */
function bs_arrayFindDuplicates(array $array) {
...
function bs_rksort(&$array, $flags = null) {
	if (is_array($array)) {
 ...
 * @see order_macros()
 */
function order_result(&$data, $sortfield = null, $sortorder = BS_SORT_UP) {
	if (empty($data)) {
		return false;
	}
 ...
 * @return array
 */
function order_macros(array $macros, $sortfield, $order = BS_SORT_UP) {
	$temp = [];
...
function bs_value2array(&$values) {
...
function createParentToChildRelation(&$chain, $link, $parentField, $childField) {
	...
	if (isset($chain[$link[$childField]])) {
		$chain[$link[$parentField]] = bs_array_merge($chain[$link[$parentField]], $chain[$link[$childField]]);
	}
}

// object or array of objects to hash
function bs_toHash($value, $field = null) {
	if (is_null($value)) {
		return $value;
	}
	$result = [];
 ...
 *
 * E.g:
 * bs_toObject(array(1, 2), 'hostid')  // returns array(array('hostid' => 1), array('hostid' => 2))
 * bs_toObject(3, 'hostid')            // returns array(array('hostid' => 3))
 *
 * @param $value
 * @param $field
 *
 * @return array
 */
function bs_toObject($value, $field) {
	if (is_null($value)) {
		return $value;
	}
	$result = [];
...
function bs_objectValues($value, $field) {
	if (is_null($value)) {
		return $value;
	}
	$result = [];
...
function bs_array_mintersect($keys, $array) {
	$result = [];
...
function bs_subarray_push(&$mainArray, $sIndex, $element = null, $key = null) {
...
function make_sorting_header($obj, $tabfield, $sortField, $sortOrder, $link = null) {
	$sortorder = ($sortField == $tabfield && $sortOrder == BS_SORT_UP) ? BS_SORT_DOWN : BS_SORT_UP;
	...
	if ($tabfield == $sortField) {
		if ($sortorder == BS_SORT_UP) {
			$arrow = (new CSpan())->addClass(BS_STYLE_ARROW_DOWN);
		}
		else {
			$arrow = (new CSpan())->addClass(BS_STYLE_ARROW_UP);
		}
	}
...
function getPagingLine(&$items, $sortorder, CUrl $url) {
	...
	if ($pagesCount > 1) {
		...
		if ($currentPage > 1) {
			...
			$tags[] = new CLink(
				(new CSpan())->addClass(BS_STYLE_ARROW_LEFT), $url->getUrl()
			);
		}
		...
		for ($p = $startPage; $p <= $endPage; $p++) {
			...
			if ($p == $currentPage) {
				$link->addClass(BS_STYLE_PAGING_SELECTED);
			}
		...
		if ($currentPage < $pagesCount) {
			$url->setArgument('page', $currentPage + 1);
			$tags[] = new CLink((new CSpan())->addClass(BS_STYLE_ARROW_RIGHT), $url->getUrl());
		}
	...
	return (new CDiv())
		->addClass(BS_STYLE_TABLE_PAGING)
		->addItem(
			(new CDiv())
				->addClass(BS_STYLE_PAGING_BTN_CONTAINER)
				->addItem($tags)
				->addItem(
					(new CDiv())
						->addClass(BS_STYLE_TABLE_STATS)
						->addItem($table_stats)
				)
		);
}
	...
	// deny access to a page
		...
		if (CWebUser::isLoggedIn()) {
			...
			$data['buttons'][] = (new CButton('back', _('Go to dashboard')))
				->onClick('javascript: document.location = "bsm.php?action=dashboard.view"');
		}
...
{
	$class = $good ? BS_STYLE_MSG_GOOD : BS_STYLE_MSG_BAD;
	$msg_box = (new CDiv($title))->addClass($class);

	if ($messages) {
		$msg_details = (new CDiv())->addClass(BS_STYLE_MSG_DETAILS);

		if ($title !== null) {
			$link = (new CSpan(_('Details')))
				->addClass(BS_STYLE_LINK_ACTION)
				->onClick('javascript: showHide($(this).next(\'.'.BS_STYLE_MSG_DETAILS_BORDER.'\'));');
			$msg_details->addItem($link);
		}

		$list = new CList();
		if ($title !== null) {
			$list->addClass(BS_STYLE_MSG_DETAILS_BORDER);
	...
	if ($show_close_box) {
		$msg_box->addItem((new CSpan())
			->addClass(BS_STYLE_OVERLAY_CLOSE_BTN)
...
function getMessages()
{
	...
	$message_box = isset($BS_MESSAGES) && $BS_MESSAGES ? makeMessageBox(false, $BS_MESSAGES) : null;
...
function show_messages($good = false, $okmsg = null, $errmsg = null) {
	global $page, $BS_MESSAGES;

	if (!defined('PAGE_HEADER_LOADED')) {
//		return null;
	}
	if (defined('BS_API_REQUEST')) {
		return null;
	}
	if (!isset($page['type'])) {
	...
	$messages = isset($BS_MESSAGES) ? $BS_MESSAGES : [];
...
function info($msgs) {
	...
	foreach ($msgs as $msg) {
		$BS_MESSAGES[] = ['type' => 'info', 'message' => $msg];
	}
}
...
function error($msgs) {
	...
	$msgs = bs_toArray($msgs);

	foreach ($msgs as $msg) {
		$BS_MESSAGES[] = ['type' => 'error', 'message' => $msg];
	}
}
...
function clear_messages($count = null) {
	...
	if ($count != null) {
		...
		while ($count-- > 0) {
			array_unshift($result, array_pop($BS_MESSAGES));
		}
	}
	else {
		$result = $BS_MESSAGES;
...
function get_status() {
	global $BS_SERVER, $BS_SERVER_PORT;
	...
	// server
	$bsmServer = new CBsmServer($BS_SERVER, $BS_SERVER_PORT, BS_SOCKET_TIMEOUT, 0);
	$status['bsm_server'] = $bsmServer->isRunning() ? _('Yes') : _('No');
	...
	$dbTriggers = DBselect(
		'SELECT COUNT(DISTINCT t.triggerid) AS cnt,t.status,t.value'.
			...
			' WHERE NOT EXISTS ('.
				...
				')'.
			' AND t.flags IN ('.BS_FLAG_DISCOVERY_NORMAL.','.BS_FLAG_DISCOVERY_CREATED.')'.
	...
	$dbItems = DBselect(
		'SELECT COUNT(i.itemid) AS cnt,i.status,i.state'.
				...
				' WHERE h.status='.HOST_STATUS_MONITORED.
					' AND i.flags IN ('.BS_FLAG_DISCOVERY_NORMAL.','.BS_FLAG_DISCOVERY_CREATED.')'.
	...
	$dbHosts = DBselect(
		...
		' WHERE '.dbConditionInt('h.status', [
			...
			' AND '.dbConditionInt('h.flags', [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_CREATED]).
		' GROUP BY h.status');
	...
	$db_sessions = DBselect(
		...
		' FROM sessions s'.
		' WHERE s.status='.BS_SESSION_ACTIVE.
		' GROUP BY s.userid,s.status'
	);
	while ($session = DBfetch($db_sessions)) {
		if (($session['lastaccess'] + BS_USER_ONLINE_TIME) >= time()) {
			$status['users_online']++;
		}
	}
	...
	$row = DBfetch(DBselect(
		...
		' AND i.delay<>0'.
		' AND i.flags<>'.BS_FLAG_DISCOVERY_PROTOTYPE
	));
...
function hasErrorMesssages() {
	...
	if (isset($BS_MESSAGES)) {
		foreach ($BS_MESSAGES as $message) {
 ...
 */
function bs_err_handler($errno, $errstr, $errfile, $errline) {
