...
function authentication2str($type) {
	$authentications = [
		BS_AUTH_INTERNAL => _('Bsm internal authentication'),
		BS_AUTH_LDAP => _('LDAP authentication'),
		BS_AUTH_HTTP => _('HTTP authentication')
	];
...
function getUserGuiAccess($userId, $maxGuiAccess = null) {
	...
	$guiAccess = DBfetch(DBselect(
		...
		' WHERE ug.userid='.bs_dbstr($userId).
			' AND g.usrgrpid=ug.usrgrpid'.
			(($maxGuiAccess === null) ? '' : ' AND g.gui_access<='.bs_dbstr($maxGuiAccess))
	));
...
function getUserAuthenticationType($userId, $maxGuiAccess = null) {
	...
	switch (getUserGuiAccess($userId, $maxGuiAccess)) {
		...
		case GROUP_GUI_ACCESS_INTERNAL:
			return ($config['authentication_type'] == BS_AUTH_HTTP) ? BS_AUTH_HTTP : BS_AUTH_INTERNAL;
...
function get_accessible_hosts_by_rights(&$rights, $user_type, $perm) {
	...
	array_push($where, dbConditionInt('h.flags', [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_CREATED]));
	...
	CArrayHelper::sort($result, [
		['field' => 'host_name', 'order' => BS_SORT_UP]
	]);
...
function getUserGroupsByUserId($userId) {
	...
	if (!isset($userGroups[$userId])) {
		...
		$result = DBselect('SELECT usrgrpid FROM users_groups WHERE userid='.bs_dbstr($userId));
