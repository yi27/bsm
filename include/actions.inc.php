 ...
 *															Possible values: BS_SCRIPT_TYPE_IPMI, BS_SCRIPT_TYPE_SSH,
 *															BS_SCRIPT_TYPE_TELNET, BS_SCRIPT_TYPE_CUSTOM_SCRIPT
 *															and BS_SCRIPT_TYPE_GLOBAL_SCRIPT
	...
	foreach ($operations as $key => $operation) {
		...
		switch ($operation['operationtype']) {
			case OPERATION_TYPE_MESSAGE:
				...
				$result[$key][] = [bold($subject), BR(), BR(), bs_nl2br($message)];
				break;
				...
				switch ($operation['opcommand']['type']) {
					case BS_SCRIPT_TYPE_IPMI:
						$result[$key][] = [bold(_('Run IPMI command').': '), BR(),
							italic(bs_nl2br($operation['opcommand']['command']))
						];
						break;
					...
					case BS_SCRIPT_TYPE_CUSTOM_SCRIPT:
						if ($operation['opcommand']['execute_on'] == BS_SCRIPT_EXECUTE_ON_AGENT) {
							$result[$key][] = [bold(_('Run custom commands on Bsm agent').': '), BR(),
								italic(bs_nl2br($operation['opcommand']['command']))
							];
						}
						else {
							$result[$key][] = [bold(_('Run custom commands on Bsm server').': '), BR(),
								italic(bs_nl2br($operation['opcommand']['command']))
							];
						}
						break;
...
function getActionMessages(array $alerts) {
	$dbUsers = API::User()->get([
		'output' => ['userid', 'alias', 'name', 'surname'],
		'userids' => bs_objectValues($alerts, 'userid'),
		'preservekeys' => true
	]);
	...
	foreach ($alerts as $alert) {
		...
		if ($alert['status'] == ALERT_STATUS_SENT) {
			$status = (new CSpan(_('Sent')))->addClass(BS_STYLE_GREEN);
			$retries = '';
		}
		elseif ($alert['status'] == ALERT_STATUS_NOT_SENT) {
			$status = (new CSpan(_('In progress')))->addClass(BS_STYLE_YELLOW);
			$retries = (new CSpan(ALERT_MAX_RETRIES - $alert['retries']))->addClass(BS_STYLE_YELLOW);
		}
		else {
			$status = (new CSpan(_('Not sent')))->addClass(BS_STYLE_RED);
			$retries = (new CSpan('0'))->addClass(BS_STYLE_RED);
		}
		...
		$table->addRow([
			$alert['esc_step'],
			bs_date2str(DATE_TIME_FORMAT_SECONDS, $alert['clock']),
			isset($mediaType['description']) ? $mediaType['description'] : '',
			$status,
			$retries,
			$recipient,
			[bold($alert['subject']), BR(), BR(), bs_nl2br($alert['message'])],
	...
	foreach ($alerts as $alert) {
		...
		switch ($alert['status']) {
			case ALERT_STATUS_SENT:
				$status = (new CSpan(_('Executed')))->addClass(BS_STYLE_GREEN);
				break;
		...
		$table->addRow([
			...
			bs_date2str(DATE_TIME_FORMAT_SECONDS, $alert['clock']),
			$status,
			bs_nl2br($alert['message']),
			$alert['error'] ? (new CSpan($alert['error']))->addClass(BS_STYLE_RED) : ''
		]);
	}
...
function makeActionHints($alerts, $mediatypes, $users, $status) {
	...
	foreach ($alerts as $alert) {
		switch ($status) {
			case ALERT_STATUS_NOT_SENT:
				$status_str = (new CSpan(_('In progress')))->addClass(BS_STYLE_YELLOW);
				break;
			...
			default:
				$status_str = (new CSpan(_('Not sent')))->addClass(BS_STYLE_RED);
		}
		...
		switch ($alert['alerttype']) {
			...
			case ALERT_TYPE_COMMAND:
				$user = '';
				$message = [bold(_('Command').NAME_DELIMITER), BR(), bs_nl2br($alert['message'])];
				break;
			default:
				$user = '';
				$message = '';
	...
	foreach ($events as $eventid => &$event) {
		$event = (new CList([
			...
			$event[ALERT_STATUS_FAILED]
				...
				: ''
		]))->addClass(BS_STYLE_LIST_HOR_MIN_WIDTH);
	}
	unset($event);
