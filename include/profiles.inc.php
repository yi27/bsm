...
function setHostGroupInternal($groupid, $internal) {
	return DBexecute(
		'UPDATE groups'.
		' SET internal='.bs_dbstr($internal).
...
function update_config($config) {
	...
	if (array_key_exists('alert_usrgrpid', $config) && $config['alert_usrgrpid'] != 0) {
		$userGroup = DBfetch(DBselect(
			'SELECT u.name'.
			' FROM usrgrp u'.
			' WHERE u.usrgrpid='.bs_dbstr($config['alert_usrgrpid'])
		));
		if (!$userGroup) {
	...
	if ($result) {
		...
		if (array_key_exists('server_check_interval', $config)) {
			$msg[] = _s('Bsm server is running check interval "%1$s".', $config['server_check_interval']);
		}
		...
		if (array_key_exists('discovery_groupid', $config)) {
			...
			if (bccomp($config['discovery_groupid'], $configOrig['discovery_groupid']) != 0) {
				setHostGroupInternal($configOrig['discovery_groupid'], BS_NOT_INTERNAL_GROUP);
				setHostGroupInternal($config['discovery_groupid'], BS_INTERNAL_GROUP);
			}
		}
		...
		if ($msg) {
			add_audit(AUDIT_ACTION_UPDATE, AUDIT_RESOURCE_BSM_CONFIG, implode('; ', $msg));
		}
	}
