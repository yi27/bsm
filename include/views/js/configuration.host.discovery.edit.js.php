...
<script type="text/x-jquery-tmpl" id="condition-row">
	<tr class="form_row">
		...
		<td>
			<input class="<?= BS_STYLE_UPPERCASE ?> macro" type="text" id="conditions_#{rowNum}_macro" name="conditions[#{rowNum}][macro]" style="width: <?= BS_TEXTAREA_MACRO_WIDTH ?>px" maxlength="64" placeholder="{#MACRO}" data-formulaid="#{formulaId}">
		</td>
		<td>
			<span><?= _('matches') ?></span>
		</td>
		<td>
			<input type="text" id="conditions_#{rowNum}_value" name="conditions[#{rowNum}][value]" style="width: <?= BS_TEXTAREA_MACRO_VALUE_WIDTH ?>px" maxlength="255" placeholder="<?= _('regular expression') ?>">
		</td>
		<td class="<?= BS_STYLE_NOWRAP ?>">
			<button class="<?= BS_STYLE_BTN_LINK ?> element-table-remove" type="button" id="conditions_#{rowNum}_remove" name="conditions_#{rowNum}_remove"><?= _('Remove') ?></button>
		</td>
	</tr>
</script>
