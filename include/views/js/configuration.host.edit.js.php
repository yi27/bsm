...
<tr class="interfaceRow" id="hostInterfaceRow_#{iface.interfaceid}" data-interfaceid="#{iface.interfaceid}">
	<td class="interface-drag-control <?= BS_STYLE_TD_DRAG_ICON ?>">
		<div class="<?= BS_STYLE_DRAG_ICON ?>"></div>
	...
	<td class="interface-ip">
		...
		<input name="interfaces[#{iface.interfaceid}][ip]" type="text" style="width: <?= BS_TEXTAREA_INTERFACE_IP_WIDTH ?>px" maxlength="64" value="#{iface.ip}">
	...
	<td class="interface-dns">
		<input name="interfaces[#{iface.interfaceid}][dns]" type="text" style="width: <?= BS_TEXTAREA_INTERFACE_DNS_WIDTH ?>px" maxlength="64" value="#{iface.dns}">
	</td>
	<?= (new CCol(
	...
	<td class="interface-port">
		<input name="interfaces[#{iface.interfaceid}][port]" type="text" style="width: <?= BS_TEXTAREA_INTERFACE_PORT_WIDTH ?>px" maxlength="64" value="#{iface.port}">
	</td>
	...
	<td class="<?= BS_STYLE_NOWRAP ?> interface-control">
		<button class="<?= BS_STYLE_BTN_LINK ?> remove" type="button" id="removeInterface_#{iface.interfaceid}" data-interfaceid="#{iface.interfaceid}" #{attrs.disabled}><?= _('Remove') ?></button>
	</td>
</tr>;
</script>
...
<script type="text/javascript">
	var hostInterfacesManager = (function() {
		...
		function addNotDraggableIcon(domElement) {
			jQuery('td.<?= BS_STYLE_TD_DRAG_ICON ?> div.<?= BS_STYLE_DRAG_ICON ?>', domElement)
				.addClass('<?= BS_STYLE_DISABLED ?>')
				.hover(
					function (event) {
		...
		jQuery('#agentInterfaces, #SNMPInterfaces, #JMXInterfaces, #IPMIInterfaces').parent().droppable({
			...
			activate: function(event, ui) {
				if (!jQuery(this).find(ui.draggable).length) {
					jQuery(this).addClass('<?= BS_STYLE_DRAG_DROP_AREA ?>');
				}
			},
			deactivate: function(event, ui) {
				jQuery(this).removeClass('<?= BS_STYLE_DRAG_DROP_AREA ?>');
			}
		});
