	...
	jQuery(function($) {
		...
		var initialize = function() {
			...
			$('.app-list-toggle').each(function() {
				...
				$('span', this)
					.addClass(open_state == '0' ? '<?= BS_STYLE_ARROW_RIGHT ?>' : '<?= BS_STYLE_ARROW_DOWN ?>');
			...
			$('.app-list-toggle-all span')
				.addClass(open_state_all == '0' ? '<?= BS_STYLE_ARROW_RIGHT ?>' : '<?= BS_STYLE_ARROW_DOWN ?>');
		};
		...
		$('.app-list-toggle-all').click(function() {
			...
			var open_state = ($(this).data('open-state') == '0') ? '1' : '0',
				del_class = (open_state == '0') ? '<?= BS_STYLE_ARROW_DOWN ?>' : '<?= BS_STYLE_ARROW_RIGHT ?>',
				add_class = (open_state == '0') ? '<?= BS_STYLE_ARROW_RIGHT ?>' : '<?= BS_STYLE_ARROW_DOWN ?>',
				applicationids = [],
				hostids = [];
