...
<tr id="items_#{number}" class="sortable">
	...
	<?php if (!$this->data['templates']): ?>
		<td class="<?= BS_STYLE_NOWRAP ?>">
			<button type="button" class="<?= BS_STYLE_BTN_LINK ?>" id="items_#{number}_remove" data-remove="#{number}" onclick="removeItem(this);"><?= _('Remove') ?></button>
		</td>
	<?php endif ?>
</tr>
</script>
	...
	function rewriteNameLinks() {
		...
		for (var i = 0; i < size; i++) {
			var nameLink = 'PopUp("popup.php?writeonly=1&numeric=1&dstfrm=graphForm'
				...
				+ (jQuery('#items_' + i + '_flags').val() == <?= BS_FLAG_DISCOVERY_PROTOTYPE ?>
