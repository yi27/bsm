...
<tr class="sortable" id="iconmapidRow_#{iconmappingid}">
	<td class="<?= BS_STYLE_TD_DRAG_ICON ?>">
	...
	<td>
		<input id="iconmap_mappings_#{iconmappingid}_expression" name="iconmap[mappings][#{iconmappingid}][expression]" value="" style="width: <?= BS_TEXTAREA_SMALL_WIDTH ?>px" maxlength="64" type="text" />
	</td>
	<td>
	...
	<td class="<?= BS_STYLE_NOWRAP ?>">
		<button class="<?= BS_STYLE_BTN_LINK ?> removeMapping" type="button" id="remove" name="remove">Remove</button>
	</td>
</tr>
</script>
		...
		iconMapTable.find('tbody')
			...
			.delegate('select.mappingIcon, select#iconmap_default_iconid', 'change', function() {
				$(this).closest('tr').find('.preview')
					.attr('src', 'imgstore.php?&width=<?= BS_ICON_PREVIEW_WIDTH ?>&height=<?= BS_ICON_PREVIEW_HEIGHT ?>&iconid=' + $(this).val())
