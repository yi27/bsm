<script type="text/x-jquery-tmpl" id="mapElementFormTpl">
	<?= (new CDiv(new CTag('h4', true, _('Map element'))))
			->addClass(BS_STYLE_DASHBRD_WIDGET_HEAD)
			->addClass(BS_STYLE_CURSOR_MOVE)
			...
			->addItem(
				(new CFormList())
					...
					->addRow(new CLabel(_('Coordinates'), 'x'), [
						_('X'),
						(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
						(new CTextBox('x'))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH),
						(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
						_('Y'),
						(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
						(new CTextBox('y'))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
					], 'areaSizeRow')
					->addRow(_('URLs'),
						(new CDiv([
							(new CTable())
					...
					->addItem([
						...
						(new CDiv([
							...
							->addClass(BS_STYLE_TFOOT_BUTTONS)
					])
			)
			->toString()
	?>
...
<script; type="text/x-jquery-tmpl"; id="selementFormUrls">
	<?= (new CRow([
			(new CTextBox('url_#{selementurlid}_name', '#{name}'))->setWidth(BS_TEXTAREA_SMALL_WIDTH),
			(new CTextBox('url_#{selementurlid}_url', '#{url}'))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
			(new CCol(
				(new CButton(null, _('Remove')))
					->addClass(BS_STYLE_BTN_LINK)
...
<script; type="text/javascript">
	...
	function addPopupValues(data) {
		if (data.object === 'name') {
			...
			BSM.apps.map.object.linkForm.addNewTriggers(data.values);
		}
	};
</script>
