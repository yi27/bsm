<script type="text/x-jquery-tmpl" id="groupPrototypeRow">
	<tr class="form_row">
		<td>
			<input name="group_prototypes[#{i}][name]" type="text" value="#{name}" style="width: <?= BS_TEXTAREA_STANDARD_WIDTH ?>px" placeholder="{#MACRO}">
		</td>
		<td class="<?= BS_STYLE_NOWRAP ?>">
			<button class="<?= BS_STYLE_BTN_LINK ?> group-prototype-remove" type="button" name="remove"><?= _('Remove') ?></button>
