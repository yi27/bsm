<script type="text/javascript">
	...
	function addPopupValues(list) {
		...
		for (var i = 0; i < list.values.length; i++) {
			...
			create_var('bs_filter', 'itemids[' + list.values[i].itemid + ']', list.values[i].itemid, false);
		}

		jQuery('form[name=bs_filter]').submit();
	}
		...
		$('#remove_log').click(function() {
			...
			jQuery('option:selected', obj).each(function(){
				...
				if ($('option', obj).length > 1) {
					$('form[name=bs_filter] #itemids_' + self.val()).remove();
					self.remove();
				}
				else {
