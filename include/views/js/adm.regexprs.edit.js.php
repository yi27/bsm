<script type="text/x-jquery-tmpl" id="row_expr">
	<?= (new CRow([
			...
			(new CTextBox('expressions[#{rowNum}][expression]', '', false, 255))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
...
<script>
			...
			showTestResults: function(response) {
				...
				$('#tbl_expr .form_row').each(function() {
					...
					$('#testResultTable').append(obj.testTableRowTpl.evaluate({
						...
						result: result,
						resultClass: expr_result ? '<?= BS_STYLE_GREEN ?>' : '<?= BS_STYLE_RED ?>'
					}));
				});

				if (hasErrors) {
					tplData = {
						resultClass: '<?= BS_STYLE_RED ?>',
						result: <?= CJs::encodeJson(_('UNKNOWN')) ?>
					};
				}
				else {
					tplData = {
						resultClass: response.data.final ? '<?= BS_STYLE_GREEN ?>' : '<?= BS_STYLE_RED ?>',
	...
	jQuery(function($) {
		$('#testExpression, #tab_test').click(function() {
			bsmRegExp.testExpressions($('#test_string').val());
		});
