...
foreach ($data['expressions'] as $i => $expression) {
	...
	$row[] = (new CCol($button_cell))->addClass(BS_STYLE_NOWRAP);
...
$testTab = (new CFormList())
	->addRow(_('Test string'),
		(new CTextArea('test_string', $data['test_string']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	)
	->addRow('', (new CButton('testExpression', _('Test expressions')))->addClass(BS_STYLE_BTN_ALT))
	->addRow(_('Result'),
		(new CDiv(
			(new CTable())
