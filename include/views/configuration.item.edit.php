...
if ($this->data['limited']) {
	...
	$itemFormList->addRow(_('Type'),
		(new CTextBox('typename', item_type2str($this->data['type']), true))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
	);
}
else {
...
$key_controls = [
	(new CTextBox('key', $this->data['key'], $this->data['limited']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
];
if (!$this->data['limited']) {
	$key_controls[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
...
if (!empty($this->data['interfaces'])) {
	...
	$interfaceGroups = [];
	foreach (bs_objectValues($this->data['interfaces'], 'type') as $interfaceType) {
...
$itemFormList->addRow(_('SNMP OID'),
	(new CTextBox('snmp_oid', $this->data['snmp_oid'], $this->data['limited']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'row_snmp_oid'
);
$itemFormList->addRow(_('Context name'),
	(new CTextBox('snmpv3_contextname', $this->data['snmpv3_contextname']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'row_snmpv3_contextname'
);
$itemFormList->addRow(_('SNMP community'),
	(new CTextBox('snmp_community', $this->data['snmp_community'], false, 64))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'row_snmp_community'
);
...
$itemFormList->addRow(_('Port'),
	(new CTextBox('port', $this->data['port'], false, 64))->setWidth(BS_TEXTAREA_SMALL_WIDTH),
	'row_port'
);
...
$itemFormList->addRow(_('Public key file'),
	(new CTextBox('publickey', $this->data['publickey'], false, 64))->setWidth(BS_TEXTAREA_SMALL_WIDTH),
	'row_publickey'
);
$itemFormList->addRow(_('Private key file'),
	(new CTextBox('privatekey', $this->data['privatekey'], false, 64))->setWidth(BS_TEXTAREA_SMALL_WIDTH),
	'row_privatekey'
);
$itemFormList->addRow(_('Password'),
	(new CTextBox('password', $this->data['password'], false, 64))->setWidth(BS_TEXTAREA_SMALL_WIDTH),
	'row_password'
);
$itemFormList->addRow(_('Executed script'),
	(new CTextArea('params_es', $this->data['params']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'label_executed_script'
);
$itemFormList->addRow(_('SQL query'),
	(new CTextArea('params_ap', $this->data['params']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'label_params'
);
$itemFormList->addRow(_('Formula'),
	(new CTextArea('params_f', $this->data['params']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'label_formula'
);
...
$itemFormList->addRow(_('Units'),
	(new CTextBox('units', $this->data['units'], $this->data['limited']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'row_units'
);
...
$itemFormList->addRow(_('Update interval (in sec)'),
	(new CNumericBox('delay', $this->data['delay'], 5))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH),
	'row_delay'
);
...
foreach ($data['delay_flex'] as $i => $delay_flex) {
	...
	if ($delay_flex['type'] == ITEM_DELAY_FLEX_TYPE_FLEXIBLE) {
		...
		$period_input = (new CTextBox('delay_flex['.$i.'][period]', $delay_flex['period'], false, 255))
			->setAttribute('placeholder', BS_DEFAULT_INTERVAL);
...
$keepHistory = [];
$keepHistory[] = (new CNumericBox('history', $this->data['history'], 8))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH);
...
$keepTrend = [];
$keepTrend[] = (new CNumericBox('trends', $this->data['trends'], 8))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH);
...
if ($this->data['limited']) {
	...
	$deltaComboBox = (new CTextBox('delta_name', $deltaOptions[$this->data['delta']], true))->setWidth(BS_TEXTAREA_SMALL_WIDTH);
}
else {
...
$itemFormList->addRow(_('Allowed hosts'),
	(new CTextBox('trapper_hosts', $this->data['trapper_hosts']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'row_trapper_hosts');

$itemFormList->addRow(new CLabel(_('New application'), 'new_application'),
	(new CSpan(
		(new CTextBox('new_application', $this->data['new_application']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	))->addClass(BS_STYLE_FORM_NEW_GROUP)
);
...
$itemFormList->addRow(_('Description'),
	(new CTextArea('description', $this->data['description']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
);
