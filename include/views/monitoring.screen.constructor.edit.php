...
if (hasRequest('screenitemid')) {
	...
	$screenItems = bs_toHash($this->data['screen']['screenitems'], 'screenitemid');
}
else {
	$form
...
if ($resourceType == SCREEN_RESOURCE_GRAPH) {
	...
	$screenFormList->addRow(_('Graph'), [
		(new CTextBox('caption', $caption, true))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
...
elseif ($resourceType == SCREEN_RESOURCE_PLAIN_TEXT) {
	...
	$screenFormList
		...
		->addRow(_('Show lines'),
			(new CNumericBox('elements', $elements, 3))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
		)
...
elseif (in_array($resourceType, [SCREEN_RESOURCE_TRIGGERS_OVERVIEW, SCREEN_RESOURCE_DATA_OVERVIEW])) {
	...
	$screenFormList->addRow(_('Application'),
		(new CTextBox('application', $application, false, 255))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
}
...
elseif ($resourceType == SCREEN_RESOURCE_SCREEN) {
	...
	if ($resourceId > 0) {
		$db_screens = DBselect('SELECT s.screenid,s.name FROM screens s WHERE s.screenid='.bs_dbstr($resourceId));
...
elseif ($resourceType == SCREEN_RESOURCE_CLOCK) {
	...
	if (bs_empty($caption) && TIME_TYPE_HOST == $style && $resourceId > 0) {
		$items = API::Item()->get([
			'output' => ['itemid', 'hostid', 'key_', 'name'],
			'selectHosts' => ['name'],
			'itemids' => $resourceId,
...
if (in_array($resourceType, [SCREEN_RESOURCE_URL])) {
	$screenFormList->addRow(_('URL'), (new CTextBox('url', $url))->setWidth(BS_TEXTAREA_STANDARD_WIDTH));
}
else {
	$form->addVar('url', '');
}
...
if (in_array($resourceType, $resourcesWithWidthAndHeight)) {
	$screenFormList->addRow(_('Width'),
		(new CNumericBox('width', $width, 5))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
	);
	$screenFormList->addRow(_('Height'),
		(new CNumericBox('height', $height, 5))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
	);
}
else {
	$form
...
$screenFormList->addRow(_('Column span'),
	(new CNumericBox('colspan', $colspan, 3))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
);
$screenFormList->addRow(_('Row span'),
	(new CNumericBox('rowspan', $rowspan, 3))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
);
