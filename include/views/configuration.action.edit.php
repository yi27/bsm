...
$actionFormList = (new CFormList())
	->addRow(_('Name'),
		(new CTextBox('name', $this->data['action']['name']))
			->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
			->setAttribute('autofocus', 'autofocus')
	)
	->addRow(_('Default subject'),
		(new CTextBox('def_shortdata', $this->data['action']['def_shortdata']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	)
	->addRow(_('Default message'),
		(new CTextArea('def_longdata', $this->data['action']['def_longdata']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
...
if ($this->data['eventsource'] == EVENT_SOURCE_TRIGGERS || $this->data['eventsource'] == EVENT_SOURCE_INTERNAL) {
	...
	$actionFormList->addRow(_('Recovery subject'),
		(new CTextBox('r_shortdata', $this->data['action']['r_shortdata']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
	$actionFormList->addRow(_('Recovery message'),
		(new CTextArea('r_longdata', $this->data['action']['r_longdata']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
}
...
$conditionFormList->addRow(_('Conditions'),
	(new CDiv($conditionTable))
		->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
		->setAttribute('style', 'min-width: '.BS_TEXTAREA_BIG_WIDTH.'px;')
);
...
switch ($this->data['new_condition']['conditiontype']) {
	...
	case CONDITION_TYPE_TRIGGER_NAME:
		$condition = (new CTextBox('new_condition[value]', ''))->setWidth(BS_TEXTAREA_STANDARD_WIDTH);
		break;
	...
	case CONDITION_TYPE_TIME_PERIOD:
		$condition = (new CTextBox('new_condition[value]', BS_DEFAULT_INTERVAL))
			->setWidth(BS_TEXTAREA_STANDARD_WIDTH);
		break;
	...
	case CONDITION_TYPE_DRULE:
		...
		$condition = [
			(new CTextBox('drule', '', true))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
			(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
	...
	case CONDITION_TYPE_DCHECK:
		$conditionFormList->addItem(new CVar('new_condition[value]', '0'));
		$condition = [
			(new CTextBox('dcheck', '', true))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
			(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
			(new CButton('btn1', _('Select')))
				->addClass(BS_STYLE_BTN_GREY)
	...
	case CONDITION_TYPE_PROXY:
		$conditionFormList->addItem(new CVar('new_condition[value]', '0'));
		$condition = [
			(new CTextBox('proxy', '', true))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
			(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
			(new CButton('btn1', _('Select')))
				->addClass(BS_STYLE_BTN_GREY)
	...
	case CONDITION_TYPE_DSERVICE_PORT:
		$condition = (new CTextBox('new_condition[value]', '0-1023,1024-49151'))->setWidth(BS_TEXTAREA_STANDARD_WIDTH);
		break;
	...
	case CONDITION_TYPE_DUPTIME:
		$condition = (new CNumericBox('new_condition[value]', 600, 15))->setWidth(BS_TEXTAREA_NUMERIC_BIG_WIDTH);
		break;
...
$conditionFormList->addRow(_('New condition'),
	(new CDiv(
		(new CTable())
			...
			->addRow((new CSubmit('add_condition', _('Add')))->addClass(BS_STYLE_BTN_LINK))
	))
		->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
		->setAttribute('style', 'min-width: '.BS_TEXTAREA_BIG_WIDTH.'px;')
);
	...
	foreach ($this->data['action']['operations'] as $operationid => $operation) {
		...
		if ($this->data['eventsource'] == EVENT_SOURCE_TRIGGERS
			...
			$operationRow = [
				...
				(new CCol(
					new CHorList([
						(new CSubmit('edit_operationid['.$operationid.']', _('Edit')))->addClass(BS_STYLE_BTN_LINK),
						[
							(new CButton('remove', _('Remove')))
		...
		$operation['opmessage_grp'] = isset($operation['opmessage_grp'])
			? bs_toHash($operation['opmessage_grp'], 'usrgrpid')
			: null;
		$operation['opmessage_usr'] = isset($operation['opmessage_usr'])
			? bs_toHash($operation['opmessage_usr'], 'userid')
			: null;
		$operation['opcommand_grp'] = isset($operation['opcommand_grp'])
			? bs_toHash($operation['opcommand_grp'], 'groupid')
			: null;
		$operation['opcommand_hst'] = isset($operation['opcommand_hst'])
			? bs_toHash($operation['opcommand_hst'], 'hostid')
			: null;
	}
}
...
if (empty($this->data['new_operation'])) {
	$footer = (new CSubmit('new_operation', _('New')))->addClass(BS_STYLE_BTN_LINK);
}
...
if (!empty($this->data['new_operation'])) {
	...
	switch ($this->data['new_operation']['operationtype']) {
		case OPERATION_TYPE_MESSAGE:
			...
			$usrgrpids = isset($this->data['new_operation']['opmessage_grp'])
				? bs_objectValues($this->data['new_operation']['opmessage_grp'], 'usrgrpid')
				: [];

			$userids = isset($this->data['new_operation']['opmessage_usr'])
				? bs_objectValues($this->data['new_operation']['opmessage_usr'], 'userid')
				: [];
			...
			$jsInsert = 'addPopupValues('.bs_jsvalue(['object' => 'usrgrpid', 'values' => $usrgrps]).');';
			$jsInsert .= 'addPopupValues('.bs_jsvalue(['object' => 'userid', 'values' => $users]).');';
			bs_add_post_js($jsInsert);

			$new_operation_formlist
				->addRow(_('Send to User groups'),
					(new CDiv($usrgrpList))
						->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
						->setAttribute('style', 'min-width: '.BS_TEXTAREA_STANDARD_WIDTH.'px;')
				)
		...
		case OPERATION_TYPE_COMMAND:
			...
			$this->data['new_operation']['opcommand']['type'] = isset($this->data['new_operation']['opcommand']['type'])
				? $this->data['new_operation']['opcommand']['type'] : BS_SCRIPT_TYPE_CUSTOM_SCRIPT;
			...
			$this->data['new_operation']['opcommand']['execute_on'] = isset($this->data['new_operation']['opcommand']['execute_on'])
				? $this->data['new_operation']['opcommand']['execute_on'] : BS_SCRIPT_EXECUTE_ON_AGENT;
			...
			if (!bs_empty($this->data['new_operation']['opcommand']['scriptid'])) {
			...
			$hosts = API::Host()->get([
				'hostids' => bs_objectValues($this->data['new_operation']['opcommand_hst'], 'hostid'),
				'output' => ['hostid', 'name'],
				'preservekeys' => true,
				'editable' => true
			]);
			...
			$groups = API::HostGroup()->get([
				'groupids' => bs_objectValues($this->data['new_operation']['opcommand_grp'], 'groupid'),
				'output' => ['groupid', 'name'],
				'preservekeys' => true,
				'editable' => true
			]);
			...
			// js add commands
			$jsInsert = 'addPopupValues('.bs_jsvalue(['object' => 'hostid', 'values' => $this->data['new_operation']['opcommand_hst']]).');';
			$jsInsert .= 'addPopupValues('.bs_jsvalue(['object' => 'groupid', 'values' => $this->data['new_operation']['opcommand_grp']]).');';
			bs_add_post_js($jsInsert);
			...
			$typeComboBox = new CComboBox('new_operation[opcommand][type]',
				...
				'showOpTypeForm()',
				[
					BS_SCRIPT_TYPE_IPMI => _('IPMI'),
					BS_SCRIPT_TYPE_CUSTOM_SCRIPT => _('Custom script'),
					BS_SCRIPT_TYPE_SSH => _('SSH'),
					BS_SCRIPT_TYPE_TELNET => _('Telnet'),
					BS_SCRIPT_TYPE_GLOBAL_SCRIPT => _('Global script')
				]
			);
			...
			$userScript = [
				...
				(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
				(new CButton('select_opcommand_script', _('Select')))->addClass(BS_STYLE_BTN_GREY)
			];
			...
			$new_operation_formlist->addRow(_('Execute on'),
				(new CRadioButtonList('new_operation[opcommand][execute_on]',
					...
					->addValue(_('Bsm agent'), BS_SCRIPT_EXECUTE_ON_AGENT)
					->addValue(_('Bsm server'), BS_SCRIPT_EXECUTE_ON_SERVER)
					->setModern(true)
			);
	...
	if ($this->data['eventsource'] == 0) {
		if (!isset($this->data['new_operation']['opconditions'])) {
			...
			bs_rksort($this->data['new_operation']['opconditions']);
		}
		...
		if (!isset($_REQUEST['new_opcondition'])) {
			$operationConditionsTable->addRow((new CCol(
				(new CSubmit('new_opcondition', _('New')))->addClass(BS_STYLE_BTN_LINK)
			))->setColspan(3));
		}
	...
	if (isset($_REQUEST['new_opcondition'])) {
		...
		if ($new_opcondition['conditiontype'] == CONDITION_TYPE_EVENT_ACKNOWLEDGED) {
			$rowCondition[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
		...
		$new_operation_formlist->addRow(_('Operation condition'),
			(new CDiv([
				...
				new CHorList([
					(new CSubmit('add_opcondition', _('Add')))->addClass(BS_STYLE_BTN_LINK),
					(new CSubmit('cancel_new_opcondition', _('Cancel')))->addClass(BS_STYLE_BTN_LINK)
				])
			]))
				->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
				->setAttribute('style', 'min-width: '.BS_TEXTAREA_STANDARD_WIDTH.'px;')
