...
$overviewFormList->addRow(_('Host name'), (new CDiv($host_name))->setWidth(BS_TEXTAREA_BIG_WIDTH));

if ($data['host']['host'] !== $data['host']['name']) {
	$overviewFormList->addRow(_('Visible name'), (new CDiv($data['host']['name']))->setWidth(BS_TEXTAREA_BIG_WIDTH));
}
		...
		foreach ($interfaces[$type] as $interface) {
			...
			$ifTab->addRow([
				(new CDiv($interface['main'] ? bold($interface['ip']) : $interface['ip']))
					->setWidth(BS_TEXTAREA_INTERFACE_IP_WIDTH),
				(new CDiv($interface['main'] ? bold($interface['dns']) : $interface['dns']))
					->setWidth(BS_TEXTAREA_INTERFACE_DNS_WIDTH),
				(new CDiv($interface['main'] ? bold($connect_to) : $connect_to))
					->setWidth(BS_TEXTAREA_INTERFACE_USEIP_WIDTH),
				(new CDiv($interface['main'] ? bold($interface['port']) : $interface['port']))
					->setWidth(BS_TEXTAREA_INTERFACE_PORT_WIDTH)
			]);
		}

		$overviewFormList->addRow($interface_names[$type],
			(new CDiv($ifTab))->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
		);
	}
}
	...
	if (array_key_exists($key, $data['host']['inventory'])) {
		if ($data['host']['inventory'][$key] !== '') {
			$overviewFormList->addRow($data['tableTitles'][$key]['title'],
				(new CDiv(bs_str2links($data['host']['inventory'][$key])))->setWidth(BS_TEXTAREA_BIG_WIDTH)
			);
		}
	}
}
...
if ($data['host']['description'] !== '') {
	$overviewFormList->addRow(_('Description'),
		(new CDiv(bs_str2links($data['host']['description'])))->setWidth(BS_TEXTAREA_BIG_WIDTH)
	);
}
...
$overviewFormList->addRow(_('Monitoring'),
	new CHorList([
		new CLink(_('Web'), 'bsm.php?action=web.view&hostid='.$data['host']['hostid'].url_param('groupid')),
...
foreach ($data['host']['inventory'] as $key => $value) {
	if ($value !== '') {
		$detailsFormList->addRow($data['tableTitles'][$key]['title'],
			(new CDiv(bs_str2links($value)))->setWidth(BS_TEXTAREA_BIG_WIDTH)
		);
