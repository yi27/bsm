...
foreach ($this->data['triggers'] as $tnum => $trigger) {
	...
	if ($trigger['templateid'] > 0) {
		if (!isset($this->data['realHosts'][$triggerid])) {
			$description[] = (new CSpan(_('Host')))->addClass(BS_STYLE_GREY);
	...
	if ($trigger['dependencies']) {
		...
		foreach ($trigger['dependencies'] as $dependency) {
			...
			$depTriggerDescription = CHtml::encode(
				implode(', ', bs_objectValues($depTrigger['hosts'], 'name')).NAME_DELIMITER.$depTrigger['description']
			);

			if ($depTrigger['flags'] == BS_FLAG_DISCOVERY_NORMAL) {
