...
$item = [
	(new CTextBox('description', $this->data['description'], true))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	...
	(new CButton('select', _('Select')))
		...
		->onClick('return PopUp(\'popup.php?writeonly=1&dstfrm='.$expressionForm->getName().
			...
			'&srctbl=items&srcfld1=itemid&srcfld2=name\', 0, 0, \'bs_popup_item\');')
];

if (!empty($this->data['parent_discoveryid'])) {
	$item[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
	$item[] = (new CButton('select', _('Select prototype')))
		...
		->onClick('return PopUp(\'popup.php?dstfrm='.$expressionForm->getName().
			...
			'&srctbl=item_prototypes&srcfld1=itemid&srcfld2=name\', 0, 0, \'bs_popup_item\');');
}
	...
	foreach ($this->data['functions'][$this->data['selectedFunction']]['params'] as $paramId => $paramFunction) {
		...
		if ($paramFunction['T'] == T_BS_INT) {
			...
			if ($paramId == 1
					...
					&& substr($this->data['expr_type'], 0, 7) != 'iregexp') {
				$paramTypeElement = _('Time');
				$paramField = (new CTextBox('params['.$paramId.']', $paramValue))->setWidth(BS_TEXTAREA_SMALL_WIDTH);
			}
			else {
				$paramField = ($this->data['paramtype'] == PARAM_TYPE_COUNTS)
					...
					: (new CTextBox('params['.$paramId.']', $paramValue))->setWidth(BS_TEXTAREA_SMALL_WIDTH);
			}
...
$expressionFormList->addRow('N', (new CTextBox('value', $this->data['value']))->setWidth(BS_TEXTAREA_SMALL_WIDTH));
