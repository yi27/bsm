...
if (!$data['is_profile']) {
	...
	$userFormList->addRow(_x('Name', 'user first name'),
		(new CTextBox('name', $this->data['name']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
	$userFormList->addRow(_('Surname'),
		(new CTextBox('surname', $this->data['surname']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
}
	...
	$userFormList->addRow(_('Groups'),
		[
			...
			(count($this->data['user_groups']) > 0)
				? (new CSubmit('del_user_group', _('Delete selected')))->addClass(BS_STYLE_BTN_GREY)
				: null
		]
	);
}
...
if ($data['auth_type'] == BS_AUTH_INTERNAL) {
	if ($data['userid'] == 0 || isset($this->data['change_password'])) {
		$userFormList->addRow(
			_('Password'),
			(new CPassBox('password1', $this->data['password1']))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		);
		$userFormList->addRow(
			_('Password (once again)'),
			(new CPassBox('password2', $this->data['password2']))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		);
		...
		$passwdButton = (new CSubmit('change_password', _('Change password')))->addClass(BS_STYLE_BTN_GREY);
		if ($this->data['alias'] == BS_GUEST_USER) {
...
foreach (getLocales() as $localeId => $locale) {
	if ($locale['display']) {
		...
		$localeExists = (setlocale(LC_MONETARY , bs_locale_variants($localeId)) || $localeId == 'en_GB');
...
setlocale(LC_MONETARY, bs_locale_variants(CWebUser::$data['lang']));
...
$userFormList
	->addRow(_('Refresh (in seconds)'),
		(new CNumericBox('refresh', $this->data['refresh'], 4))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
	)
	...
	->addRow(_('URL (after login)'),
		(new CTextBox('url', $this->data['url']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
...
$userFormList->addRow(
	...
	$languageError
		...
		: $languageComboBox->addClass( 'yincang')
);
// append themes to form list
$themes = array_merge([THEME_DEFAULT => _('System default')], Z::getThemes());
$userFormList->addRow(_(' '), (new CComboBox('theme', $this->data['theme'], null, $themes))->addClass( 'yincang'));

/*
 * Media tab
 */
if (uint_in_array(CWebUser::$data['type'], [USER_TYPE_BSM_ADMIN, USER_TYPE_SUPER_ADMIN])) {
	...
	foreach ($this->data['user_medias'] as $id => $media) {
		...
		for ($severity = TRIGGER_SEVERITY_NOT_CLASSIFIED; $severity < TRIGGER_SEVERITY_COUNT; $severity++) {
			...
			$mediaSeverity[$severity] = (new CSpan(mb_substr($severityName, 0, 1)))
				...
				->addClass($mediaActive ? BS_STYLE_GREEN : BS_STYLE_GREY);
		}
...
if ($this->data['is_profile']) {
	...
	foreach ($bsSounds as $filename => $file) {
	...
	for ($severity = TRIGGER_SEVERITY_NOT_CLASSIFIED; $severity < TRIGGER_SEVERITY_COUNT; $severity++) {
		...
		bs_subarray_push($msgVisibility, 1, 'messages[triggers.severities]['.$severity.']');
		bs_subarray_push($msgVisibility, 1, 'messages[sounds.'.$severity.']');
	}
...
if (!$this->data['is_profile']) {
	...
	$userTypeComboBox = new CComboBox('user_type', $this->data['user_type'], 'submit();', [
		USER_TYPE_BSM_USER => user_type2str(USER_TYPE_BSM_USER),
		USER_TYPE_BSM_ADMIN => user_type2str(USER_TYPE_BSM_ADMIN),
