...
$multiselect_data = [
	...
	'selectedLimit' => 1,
	'objectName' => 'users',
	'disabled' => ($user_type != USER_TYPE_SUPER_ADMIN && $user_type != USER_TYPE_BSM_ADMIN),
	'popup' => [
...
if (!$map_ownerid || $map_ownerid && array_key_exists($map_ownerid, $data['users'])) {
	...
	$map_tab->addRow(_('Owner'),
		(new CMultiSelect($multiselect_data))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
}
else {
	$multiselect_userid = (new CMultiSelect($multiselect_data))->setWidth(BS_TEXTAREA_STANDARD_WIDTH);

	// Administrators can change map owner, but cannot see users from other groups.
	if ($user_type == USER_TYPE_BSM_ADMIN) {
...
foreach ($data['sysmap']['urls'] as $url) {
	$url_table->addRow(
		(new CRow([
			(new CTextBox('urls['.$i.'][name]', $url['name']))->setWidth(BS_TEXTAREA_SMALL_WIDTH),
			(new CTextBox('urls['.$i.'][url]', $url['url']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
...
$template_url_row = (new CRow([
	...
	$template_url_link,
	$template_url_etype,
	(new CCol($template_remove_button))->addClass(BS_STYLE_NOWRAP)
]))
	->addStyle('display: none')
	->setId('urlEntryTpl');
...
$js_insert = 'addPopupValues('.bs_jsvalue(['object' => 'usrgrpid', 'values' => $user_groups]).');';
...
$js_insert .= 'addPopupValues('.bs_jsvalue(['object' => 'userid', 'values' => $users]).');';

bs_add_post_js($js_insert);
