...
$description = (new CLink(_('root'), '#'))
	->onClick('javascript:
		jQuery(\'#parent_name\', window.opener.document).val('.bs_jsvalue(_('root')).');
		jQuery(\'#parentname\', window.opener.document).val('.bs_jsvalue(_('root')).');
		jQuery(\'#parentid\', window.opener.document).val('.bs_jsvalue(0).');
		self.close();
		return false;'
	);
...
foreach ($this->data['db_pservices'] as $db_service) {
	$description = (new CSpan($db_service['name']))
		->addClass('link')
		->onClick('javascript:
			jQuery(\'#parent_name\', window.opener.document).val('.bs_jsvalue($db_service['name']).');
			jQuery(\'#parentname\', window.opener.document).val('.bs_jsvalue($db_service['name']).');
			jQuery(\'#parentid\', window.opener.document).val('.bs_jsvalue($db_service['serviceid']).');
			self.close();
			return false;'
		);
