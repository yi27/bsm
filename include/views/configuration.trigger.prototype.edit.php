...
$expressionRow = [$expressionTextBox, (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN), $addExpressionButton];
...
if ($this->data['input_method'] == IM_TREE) {
	...
	$expressionRow[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
	$expressionRow[] = $insertMacroButton;

	array_push($expressionRow, BR());
	if (empty($this->data['outline'])) {
		// add button
		$addExpressionButton = (new CSubmit('add_expression', _('Add')))->addClass(BS_STYLE_BTN_GREY);
		if ($this->data['limited']) {
			$addExpressionButton->setAttribute('disabled', 'disabled');
		}
		...
		// add button
		$addExpressionButton = (new CSubmit('and_expression', _('And')))->addClass(BS_STYLE_BTN_GREY);
		if ($this->data['limited']) {
			$addExpressionButton->setAttribute('disabled', 'disabled');
		}
		$expressionRow[] = $addExpressionButton;
		...
		// or button
		$orExpressionButton = (new CSubmit('or_expression', _('Or')))->addClass(BS_STYLE_BTN_GREY);
		if ($this->data['limited']) {
		...
		// replace button
		$replaceExpressionButton = (new CSubmit('replace_expression', _('Replace')))->addClass(BS_STYLE_BTN_GREY);
		if ($this->data['limited']) {
			...
			if ($this->data['limited']) {
				...
				foreach ($e['list'] as &$obj) {
					if (gettype($obj) == 'object' && get_class($obj) == 'CSpan'
							&& $obj->getAttribute('class') == BS_STYLE_LINK_ACTION) {
...
$triggersFormList
	...
	->addRow(_('Description'),
		(new CTextArea('comments', $this->data['comments']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	)
	->addRow(_('URL'), (new CTextBox('url', $this->data['url']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH))
...
foreach ($this->data['db_dependencies'] as $dependency) {
	...
	$depTriggerDescription = CHtml::encode(
		implode(', ', bs_objectValues($dependency['hosts'], 'name')).NAME_DELIMITER.$dependency['description']
	);

	if ($dependency['flags'] == BS_FLAG_DISCOVERY_PROTOTYPE) {
	...
	elseif ($dependency['flags'] == BS_FLAG_DISCOVERY_NORMAL) {
		$description = (new CLink($depTriggerDescription,
