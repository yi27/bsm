...
if ($config['event_ack_enable']) {
	$column1->addRow(_('Acknowledge status'),
		new CComboBox('ack_status', $filter['ackStatus'], null, [
			BS_ACK_STS_ANY => _('Any'),
			BS_ACK_STS_WITH_UNACK => _('With unacknowledged events'),
			BS_ACK_STS_WITH_LAST_UNACK => _('With last event unacknowledged')
		])
	);
}
...
$column1->addRow(_('Filter by name'),
	(new CTextBox('txt_select', $filter['txtSelect']))->setWidth(BS_TEXTAREA_FILTER_STANDARD_WIDTH)
);

$application_name_url =
	'popup.php?srctbl=applications&srcfld1=name&real_hosts=1&dstfld1=application&with_applications=1&dstfrm=bs_filter';
...
foreach ($inventoryFilters as $field) {
	$inventoryFilterTable->addRow([
		...
		(new CTextBox('inventory['.$i.'][value]', $field['value']))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH),
		(new CCol(
