...
$hostGroupTable = (new CTableInfo())
	->setHeader([
		...
		_('Templates'),
		_('Members'),
		(new CColHeader(_('Info')))->addClass(BS_STYLE_CELL_WIDTH)
	]);
...
foreach ($this->data['groups'] as $group) {
	...
	foreach ($group['hosts'] as $host) {
		...
		$hostsOutput[] = (new CLink($host['name'], $url))
			->addClass(BS_STYLE_LINK_ALT)
			->addClass($host['status'] == HOST_STATUS_MONITORED ? BS_STYLE_GREEN : BS_STYLE_RED);
	}
	...
	if ($group['flags'] == BS_FLAG_DISCOVERY_CREATED && $group['groupDiscovery']['ts_delete'] != 0) {
