...
if (isset($_REQUEST['active_since'])) {
	...
	$fromDate = bsDateToTime($this->data['active_since']);
...
if (isset($_REQUEST['active_till'])) {
	...
	$toDate = bsDateToTime($this->data['active_till']);
...
foreach ($this->data['timeperiods'] as $id => $timeperiod) {
	$maintenancePeriodTable->addRow([
		(new CCol(timeperiod_type2str($timeperiod['timeperiod_type'])))->addClass(BS_STYLE_NOWRAP),
		shedule2str($timeperiod),
		(new CCol(bs_date2age(0, $timeperiod['period'])))->addClass(BS_STYLE_NOWRAP),
		(new CCol(
			new CHorList([
				(new CSubmit('edit_timeperiodid['.$id.']', _('Edit')))->addClass(BS_STYLE_BTN_LINK),
...
if (!isset($_REQUEST['new_timeperiod'])) {
	$periodsDiv->addItem((new CSubmit('new_timeperiod', _('New')))->addClass(BS_STYLE_BTN_LINK));
}
...
if (isset($_REQUEST['new_timeperiod'])) {
	...
	$maintenancePeriodFormList->addRow(_('Maintenance period'),
		(new CDiv([
			get_timeperiod_form(),
			new CHorList([
				(new CSubmit('add_timeperiod', $saveLabel))->addClass(BS_STYLE_BTN_LINK),
