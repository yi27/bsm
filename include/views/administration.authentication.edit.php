...
$authenticationFormList->addRow(_('Default authentication'),
	(new CRadioButtonList('config', (int) $this->data['config']['authentication_type']))
		->addValue(_x('Internal', 'authentication'), BS_AUTH_INTERNAL, null, 'submit()')
		->addValue(_('LDAP'), BS_AUTH_LDAP, null, 'submit()')
		->addValue(_('HTTP'), BS_AUTH_HTTP, null, 'submit()')
		->setModern(true)
);

// append LDAP fields to form list
if ($this->data['config']['authentication_type'] == BS_AUTH_LDAP) {
	if ($this->data['user_list']) {
		...
		$userComboBox = (new CTextBox('user', $this->data['user'], true))->setWidth(BS_TEXTAREA_STANDARD_WIDTH);
	}

	$authenticationFormList->addRow(
		_('LDAP host'),
		(new CTextBox('ldap_host', $this->data['config']['ldap_host']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
	$authenticationFormList->addRow(
		_('Port'),
	...
	$authenticationFormList->addRow(
		_('Base DN'),
		(new CTextBox('ldap_base_dn', $this->data['config']['ldap_base_dn']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);
	$authenticationFormList->addRow(
		_('Search attribute'),
		(new CTextBox(
			'ldap_search_attribute',
			(bs_empty($this->data['config']['ldap_search_attribute']) && $this->data['form_refresh'] == 0)
				? 'uid'
	...
	$authenticationFormList->addRow(
		_('Bind DN'),
		(new CTextBox('ldap_bind_dn', $this->data['config']['ldap_bind_dn']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	);

	// bind password
	if (isset($this->data['change_bind_password']) || bs_empty($this->data['config']['ldap_bind_password'])) {
		$authenticationForm->addVar('change_bind_password', 1);
		$authenticationFormList->addRow(
			_('Bind password'),
			(new CPassBox('ldap_bind_password'))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		);
	}
	else {
		$authenticationFormList->addRow(
			_('Bind password'),
			(new CSubmit('change_bind_password', _('Change password')))->addClass(BS_STYLE_BTN_GREY)
		);
	}
	...
	$authenticationFormList->addRow(_('User password'), (new CPassBox('user_password'))->setWidth(BS_TEXTAREA_SMALL_WIDTH));
}
...
elseif ($this->data['config']['authentication_type'] != BS_AUTH_LDAP) {
