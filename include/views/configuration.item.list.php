...
foreach ($this->data['items'] as $item) {
	...
	if ($data['showInfoColumn']) {
		...
		if ($item['status'] == ITEM_STATUS_ACTIVE && !bs_empty($item['error'])) {
			$infoIcons[] = makeErrorIcon($item['error']);
		}

		// discovered item lifetime indicator
		if ($item['flags'] == BS_FLAG_DISCOVERY_CREATED && $item['itemDiscovery']['ts_delete'] != 0) {
			if ($infoIcons) {
				$infoIcons[] = SPACE;
			}
	...
	foreach ($item['triggers'] as $num => &$trigger) {
		...
		if ($trigger['templateid'] > 0) {
			if (!isset($this->data['triggerRealHosts'][$trigger['triggerid']])) {
				$triggerDescription[] = (new CSpan('HOST'))->addClass(BS_STYLE_GREY);
				$triggerDescription[] = ':';
			}
			else {
		...
		$trigger['hosts'] = bs_toHash($trigger['hosts'], 'hostid');

		if ($trigger['flags'] == BS_FLAG_DISCOVERY_CREATED) {
		...
		$trigger['functions'] = bs_toHash($trigger['functions'], 'functionid');
...
bs_add_post_js('cookie.prefix = "'.$this->data['hostid'].'";');
