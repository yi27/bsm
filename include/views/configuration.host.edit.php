...
$hostList->addRow(_('Host name'),
	(new CTextBox('host', $data['host'], ($data['flags'] == BS_FLAG_DISCOVERY_CREATED), 128))
		->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
		->setAttribute('autofocus', 'autofocus')
);

$hostList->addRow(_('Visible name'),
	(new CTextBox('visiblename', $data['visiblename'], ($data['flags'] == BS_FLAG_DISCOVERY_CREATED), 128))
		->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
);
...
if ($data['flags'] != BS_FLAG_DISCOVERY_CREATED) {
	...
	$hostList->addRow(new CLabel($new_group_label, 'newgroup'),
		(new CSpan($new_group))->addClass(BS_STYLE_FORM_NEW_GROUP)
	);
}
else {
...
if ($data['flags'] != BS_FLAG_DISCOVERY_CREATED) {
	...
	$ifTab = (new CTable())
		...
		->addRow((new CRow([
			(new CCol(
				(new CButton('addAgentInterface', _('Add')))->addClass(BS_STYLE_BTN_LINK)
			))->setColSpan(7)
	...
	$hostList->addRow(_('Agent interfaces'),
		(new CDiv($ifTab))
			...
			->setWidth(BS_HOST_INTERFACE_WIDTH)
	);
	...
	$ifTab = (new CTable())
		...
		->addRow((new CRow([
			(new CCol(
				(new CButton('addSNMPInterface', _('Add')))->addClass(BS_STYLE_BTN_LINK)
			))->setColSpan(7)
	...
	$ifTab = (new CTable())
		...
		->addRow((new CRow([
			(new CCol(
				(new CButton('addJMXInterface', _('Add')))->addClass(BS_STYLE_BTN_LINK)
			))->setColSpan(7)
	...
	$ifTab = (new CTable())
		...
		->addRow((new CRow([
			(new CCol(
				(new CButton('addIPMIInterface', _('Add')))->addClass(BS_STYLE_BTN_LINK)
			))->setColSpan(7)
...
// interfaces for discovered hosts
	...
	bs_add_post_js('hostInterfacesManager.add('.CJs::encodeJson($data['interfaces']).');');
	bs_add_post_js('hostInterfacesManager.disable();');
...
$hostList->addRow(_('Description'),
	(new CTextArea('description', $data['description']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
);

// Proxy
if ($data['flags'] != BS_FLAG_DISCOVERY_CREATED) {
	$proxy = new CComboBox('proxy_hostid', $data['proxy_hostid'], null, [0 => _('(no proxy)')] + $data['proxies']);
	$proxy->setEnabled($data['flags'] != BS_FLAG_DISCOVERY_CREATED);
}
else {
...
if ($data['flags'] != BS_FLAG_DISCOVERY_CREATED) {
	...
	foreach ($data['linked_templates'] as $template) {
		...
		$linkedTemplateTable->addRow([
			...
			(new CCol(
				new CHorList([
					(new CSubmit('unlink['.$template['templateid'].']', _('Unlink')))->addClass(BS_STYLE_BTN_LINK),
	...
	$newTemplateTable = (new CTable())
		...
		->addRow([(new CSubmit('add_template', _('Add')))->addClass(BS_STYLE_BTN_LINK)]);
...
$divTabs->addTab('ipmiTab', _('IPMI'),
	(new CFormList())
		...
		->addRow(_('Username'),
			(new CTextBox('ipmi_username', $data['ipmi_username'], ($data['flags'] == BS_FLAG_DISCOVERY_CREATED)))
				->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		)
		->addRow(_('Password'),
			(new CTextBox('ipmi_password', $data['ipmi_password'], ($data['flags'] == BS_FLAG_DISCOVERY_CREATED)))
				->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		)
);
...
$macrosView = new CView('hostmacros', [
	...
	'is_template' => false,
	'readonly' => ($data['flags'] == BS_FLAG_DISCOVERY_CREATED)
]);
...
$inventoryFormList->addRow(null,
	(new CRadioButtonList('inventory_mode', (int) $data['inventory_mode']))
		...
		->setEnabled($data['flags'] != BS_FLAG_DISCOVERY_CREATED)
		->setModern(true)
);
if ($data['flags'] == BS_FLAG_DISCOVERY_CREATED) {
...
foreach ($hostInventoryFields as $inventoryNo => $inventoryInfo) {
	...
	if ($hostInventoryTable['fields'][$field_name]['type'] == DB::FIELD_TYPE_TEXT) {
		$input = (new CTextArea('host_inventory['.$field_name.']', $data['host_inventory'][$field_name]))
			->setWidth(BS_TEXTAREA_BIG_WIDTH);
	}
	else {
		$field_length = $hostInventoryTable['fields'][$field_name]['length'];

		if ($field_length < 39) {
			$width = BS_TEXTAREA_SMALL_WIDTH;
		}
		elseif ($field_length < 64) {
			$width = BS_TEXTAREA_STANDARD_WIDTH;
		}
		else {
			$width = BS_TEXTAREA_BIG_WIDTH;
		}
...
$encryption_form_list = (new CFormList('encryption'))
	...
	->addRow(_('Connections from host'), [
		new CLabel([(new CCheckBox('tls_in_none'))->setEnabled($data['flags'] != BS_FLAG_DISCOVERY_CREATED),
			_('No encryption')
		]),
		BR(),
		new CLabel([(new CCheckBox('tls_in_psk'))->setEnabled($data['flags'] != BS_FLAG_DISCOVERY_CREATED), _('PSK')]),
		BR(),
		new CLabel([(new CCheckBox('tls_in_cert'))->setEnabled($data['flags'] != BS_FLAG_DISCOVERY_CREATED),
			_('Certificate')
		])
	])
	->addRow(_('PSK identity'),
		(new CTextBox('tls_psk_identity', $data['tls_psk_identity'], $data['flags'] == BS_FLAG_DISCOVERY_CREATED, 128))
			->setWidth(BS_TEXTAREA_BIG_WIDTH)
	)
	->addRow(_('PSK'),
		(new CTextBox('tls_psk', $data['tls_psk'], $data['flags'] == BS_FLAG_DISCOVERY_CREATED, 512))
			->setWidth(BS_TEXTAREA_BIG_WIDTH)
	)
	->addRow(_('Issuer'),
		(new CTextBox('tls_issuer', $data['tls_issuer'], $data['flags'] == BS_FLAG_DISCOVERY_CREATED, 1024))
			->setWidth(BS_TEXTAREA_BIG_WIDTH)
	)
	->addRow(_x('Subject', 'encryption certificate'),
		(new CTextBox('tls_subject', $data['tls_subject'], $data['flags'] == BS_FLAG_DISCOVERY_CREATED, 1024))
			->setWidth(BS_TEXTAREA_BIG_WIDTH)
	);
