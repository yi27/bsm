...
$discoveryFormList
	...
	->addRow(_('Delay (in sec)'),
		(new CNumericBox('delay', $this->data['drule']['delay'], 6))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
	);
	...
	(new CDiv(
		(new CTable())
			...
			->setFooter(
				(new CRow(
					(new CCol(
						(new CButton('newCheck', _('New')))->addClass(BS_STYLE_BTN_LINK)
					))->setColSpan(2)
...
$discoveryFormList->addRow(_('Device uniqueness criteria'),
	(new CDiv(
		(new CRadioButtonList('uniqueness_criteria', (int) $this->data['drule']['uniqueness_criteria']))
			->makeVertical()
			->addValue(_('IP address'), -1, bs_formatDomId('uniqueness_criteria_ip'))
	))
		->setAttribute('style', 'min-width: '.BS_TEXTAREA_STANDARD_WIDTH.'px;')
		->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
);
