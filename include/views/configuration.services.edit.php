...
$servicesFormList->addRow(_('Parent service'), [
	(new CTextBox('parent_name', $this->data['parentname'], true, 128))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	...
	(new CButton('select_parent', _('Change')))
		...
		->onClick(
			"javascript: openWinCentered('services.php?pservices=1".url_param('serviceid')."', ".
				"'BS_Services_List', 740, 420, 'scrollbars=1, toolbar=0, menubar=0, resizable=1, dialog=0');")
]);
...
$goodslaTextBox = (new CTextBox('goodsla', $this->data['goodsla'], false, 8))->setWidth(BS_TEXTAREA_TINY_WIDTH);
...
$servicesFormList->addRow(_('Calculate SLA, acceptable SLA (in %)'), [
	$showslaCheckbox, (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN), $goodslaTextBox
]);

// append trigger to form list
$servicesFormList->addRow(_('Trigger'), [
	(new CTextBox('trigger', $this->data['trigger'], true))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
...
$servicesFormList->addRow(_('Sort order (0->999)'), (new CTextBox('sortorder', $this->data['sortorder'], false, 3))
	->setWidth(BS_TEXTAREA_TINY_WIDTH));
...
$servicesDependenciesFormList->addRow(
	...
	(new CDiv([
		...
		(new CButton('add_child_service', _('Add')))
			->onClick("javascript: openWinCentered('services.php?cservices=1".url_param('serviceid')."', 'BS_Services_List', 640, 520, 'scrollbars=1, toolbar=0, menubar=0, resizable=0');")
			->addClass(BS_STYLE_BTN_LINK)
	]))
...
foreach ($this->data['times'] as $serviceTime) {
	switch ($serviceTime['type']) {
		...
		case SERVICE_TIME_TYPE_ONETIME_DOWNTIME:
			...
			$from = bs_date2str(DATE_TIME_FORMAT, $serviceTime['ts_from']);
			$to = bs_date2str(DATE_TIME_FORMAT, $serviceTime['ts_to']);
			break;
	}
	$row = new CRow([
		[
...
if ($this->data['new_service_time']['type'] == SERVICE_TIME_TYPE_ONETIME_DOWNTIME) {
	...
	if (isset($_REQUEST['new_service_time']['from'])) {
		...
		$fromDate = bsDateToTime($downtimeSince);
	...
	if (isset($_REQUEST['new_service_time']['to'])) {
		...
		$toDate = bsDateToTime($downtimeTill);
...
$servicesTimeFormList->addRow(_('New service time'),
	(new CDiv([
		$serviceTimeTable,
		(new CButton('add_service_time', _('Add')))->addClass(BS_STYLE_BTN_LINK)
	]))
		->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
		->setAttribute('style', 'min-width: '.BS_TEXTAREA_BIG_WIDTH.'px;')
);
