...
$filterColumn->addRow(
		...
		[
			...
			(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
			(new CTextBox('filter_field_value', $this->data['filterFieldValue']))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		]
);
...
foreach ($this->data['hosts'] as $host) {
	...
	$row = [
		(new CLink($host['name'], '?hostid='.$host['hostid'].url_param('groupid')))
			->addClass($host['status'] == HOST_STATUS_NOT_MONITORED ? BS_STYLE_RED : null),
		$hostGroups,
		bs_str2links($host['inventory']['name']),
		bs_str2links($host['inventory']['type']),
		bs_str2links($host['inventory']['os']),
		bs_str2links($host['inventory']['serialno_a']),
		bs_str2links($host['inventory']['tag']),
		bs_str2links($host['inventory']['macaddress_a'])
	];
