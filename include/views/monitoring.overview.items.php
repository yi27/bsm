...
// hint table
$help_hint = (new CList())
	->addClass(BS_STYLE_NOTIF_BODY)
	->addStyle('min-width: '.BS_OVERVIEW_HELP_MIN_WIDTH.'px');
for ($severity = TRIGGER_SEVERITY_NOT_CLASSIFIED; $severity < TRIGGER_SEVERITY_COUNT; $severity++) {
	$help_hint->addItem([
		(new CDiv())
			->addClass(BS_STYLE_NOTIF_INDIC)
			->addClass(getSeverityStyle($severity)),
		new CTag('h4', true, getSeverityName($severity, $data['config'])),
		(new CTag('p', true, _('PROBLEM')))->addClass(BS_STYLE_GREY)
	]);
}
...
$column->addRow(_('Filter by application'), [
	(new CTextBox('application', $this->data['filter']['application']))->setWidth(BS_TEXTAREA_FILTER_STANDARD_WIDTH),
