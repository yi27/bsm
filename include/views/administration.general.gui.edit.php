...
$guiTab = (new CFormList())
	...
	->addRow(_('Dropdown first entry'), [
		new CComboBox('dropdown_first_entry', $data['dropdown_first_entry'], null, [
			BS_DROPDOWN_FIRST_NONE => _('None'),
			BS_DROPDOWN_FIRST_ALL => _('All')
		]),
	...
	->addRow(_('Search/Filter elements limit'),
		(new CNumericBox('search_limit', $data['search_limit'], 6))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
	)
	->addRow(_('Max count of elements to show inside table cell'),
		(new CNumericBox('max_in_table', $data['max_in_table'], 5))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
	)
	...
	->addRow(_('Show events not older than (in days)'),
		(new CTextBox('event_expire', $data['event_expire']))->setWidth(BS_TEXTAREA_TINY_WIDTH)
	)
	->addRow(_('Max count of events per trigger to show'),
		(new CTextBox('event_show_max', $data['event_show_max']))->setWidth(BS_TEXTAREA_TINY_WIDTH)
	)
	->addRow(_('Show warning if Bsm server is down'),
