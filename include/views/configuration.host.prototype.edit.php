...
if ($parentHost['status'] != HOST_STATUS_TEMPLATE) {
	...
	bs_add_post_js('hostInterfacesManager.add('.CJs::encodeJson(array_values($parentHost['interfaces'])).');');
...
$buttonColumn = (new CCol(
	(new CButton('group_prototype_add', _('Add')))->addClass(BS_STYLE_BTN_LINK)
...
$groupList->addRow(_('Group prototypes'), (new CDiv($customGroupTable))->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR));
...
if ($parentHost['status'] != HOST_STATUS_TEMPLATE) {
	...
	$ipmiList->addRow(_('Username'),
		(new CTextBox('ipmi_username', $parentHost['ipmi_username'], true))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
	);
	$ipmiList->addRow(_('Password'),
		(new CTextBox('ipmi_password', $parentHost['ipmi_password'], true))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
	);
	...
	if ($data['show_inherited_macros']) {
		$macros = mergeInheritedMacros($macros,
			getInheritedMacros(bs_objectValues($hostPrototype['templates'], 'templateid'))
		);
	}
