...
foreach ($this->data['triggers'] as $trigger) {
	...
	if ($trigger['templateid'] > 0) {
		if (!isset($this->data['realHosts'][$triggerid])) {
			$description[] = (new CSpan(_('Template')))->addClass(BS_STYLE_GREY);
	...
	if ($trigger['dependencies']) {
		...
		foreach ($trigger['dependencies'] as $dependency) {
			...
			if ($depTrigger['flags'] == BS_FLAG_DISCOVERY_PROTOTYPE) {
			...
			elseif ($depTrigger['flags'] == BS_FLAG_DISCOVERY_NORMAL) {
				$triggerDependencies[] = (new CLink(
					$depTriggerDescription,
...
bs_add_post_js('cookie.prefix = "'.$this->data['parent_discoveryid'].'";');
