...
if ($this->data['graphtype'] == GRAPH_TYPE_NORMAL || $this->data['graphtype'] == GRAPH_TYPE_STACKED) {
	...
	if ($this->data['ymin_type'] == GRAPH_YAXIS_TYPE_FIXED) {
		$yaxisMinData[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
	...
	elseif ($this->data['ymin_type'] == GRAPH_YAXIS_TYPE_ITEM_VALUE) {
		...
		$yaxisMinData[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
		$yaxisMinData[] = (new CTextBox('ymin_name', $ymin_name, $is_templated))->setWidth(BS_TEXTAREA_STANDARD_WIDTH);
		$yaxisMinData[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
		$yaxisMinData[] = (new CButton('yaxis_min', _('Select')))
			...
			->onClick('javascript: '.
				'return PopUp("popup.php?dstfrm='.$graphForm->getName().
					...
					'&srcfld2=name'.
					'&numeric=1'.
					'&writeonly=1" + getOnlyHostParam(), 0, 0, "bs_popup_item");')
			->setEnabled(!$is_templated);
	...
	if ($this->data['ymax_type'] == GRAPH_YAXIS_TYPE_FIXED) {
		$yaxisMaxData[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
	...
	elseif ($this->data['ymax_type'] == GRAPH_YAXIS_TYPE_ITEM_VALUE) {
		...
		$yaxisMaxData[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
		$yaxisMaxData[] = (new CTextBox('ymax_name', $ymax_name, $is_templated))->setWidth(BS_TEXTAREA_STANDARD_WIDTH);
		$yaxisMaxData[] = (new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN);
...
foreach ($this->data['items'] as $n => $item) {
	...
	if (bs_empty($item['drawtype'])) {
		$item['drawtype'] = 0;
	}

	if (bs_empty($item['yaxisside'])) {
		$item['yaxisside'] = 0;
	}
...
$graphFormList->addRow(_('Items'), (new CDiv($itemsTable))->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR));
