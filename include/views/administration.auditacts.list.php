...
$filterColumn->addRow(_('Recipient'), [
	(new CTextBox('alias', $this->data['alias']))->setWidth(BS_TEXTAREA_FILTER_STANDARD_WIDTH),
	(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
	(new CButton('btn1', _('Select')))
		->addClass(BS_STYLE_BTN_GREY)
		->onClick('return PopUp("popup.php?dstfrm=bs_filter&dstfld1=alias&srctbl=users&srcfld1=alias");')
]);
...
foreach ($this->data['alerts'] as $alert) {
	...
	if ($alert['status'] == ALERT_STATUS_SENT) {
		$status = ($alert['alerttype'] == ALERT_TYPE_MESSAGE)
			? (new CSpan(_('Sent')))->addClass(BS_STYLE_GREEN)
			: (new CSpan(_('Executed')))->addClass(BS_STYLE_GREEN);
	}
...
bs_add_post_js('timeControl.addObject("events", '.bs_jsvalue($data['timeline']).', '.bs_jsvalue($objData).');');
