...
foreach ($this->data['maintenances'] as $maintenance) {
	...
	switch ($maintenance['status']) {
		case MAINTENANCE_STATUS_EXPIRED:
			$maintenanceStatus = (new CSpan(_x('Expired', 'maintenance status')))->addClass(BS_STYLE_RED);
			break;
		case MAINTENANCE_STATUS_APPROACH:
			$maintenanceStatus = (new CSpan(_x('Approaching', 'maintenance status')))->addClass(BS_STYLE_ORANGE);
			break;
		case MAINTENANCE_STATUS_ACTIVE:
			$maintenanceStatus = (new CSpan(_x('Active', 'maintenance status')))->addClass(BS_STYLE_GREEN);
			break;
	}
	...
	$maintenanceTable->addRow([
		...
		bs_date2str(DATE_TIME_FORMAT, $maintenance['active_since']),
		bs_date2str(DATE_TIME_FORMAT, $maintenance['active_till']),
		$maintenanceStatus,
