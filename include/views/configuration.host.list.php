...
$filter = (new CFilter('web.hosts.filter.state'))
	->addColumn((new CFormList())->addRow(_('Name like'),
		(new CTextBox('filter_host', $data['filter']['host']))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH)
	))
	->addColumn((new CFormList())->addRow(_('DNS like'),
		(new CTextBox('filter_dns', $data['filter']['dns']))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH)
	))
	->addColumn((new CFormList())->addRow(_('IP like'),
		(new CTextBox('filter_ip', $data['filter']['ip']))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH)
	))
	->addColumn((new CFormList())->addRow(_('Port'),
		(new CTextBox('filter_port', $data['filter']['port']))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH)
	));
...
foreach ($data['hosts'] as $host) {
	...
	if ($host['status'] == HOST_STATUS_MONITORED) {
		if ($host['maintenance_status'] == HOST_MAINTENANCE_STATUS_ON) {
			...
			$statusClass = BS_STYLE_ORANGE;
		}
		else {
			$statusCaption = _('Enabled');
			$statusClass = BS_STYLE_GREEN;
		}
	...
	if ($host['flags'] == BS_FLAG_DISCOVERY_CREATED && $host['hostDiscovery']['ts_delete'] != 0) {
	...
	if ($host['tls_connect'] == HOST_ENCRYPTION_NONE
		...
		$encryption = (new CDiv((new CSpan(_('None')))->addClass(BS_STYLE_STATUS_GREEN)))
			->addClass(BS_STYLE_STATUS_CONTAINER);
	}
	else {
		// Incoming encryption.
		if ($host['tls_connect'] == HOST_ENCRYPTION_NONE) {
		...
		if (($host['tls_accept'] & HOST_ENCRYPTION_NONE) == HOST_ENCRYPTION_NONE) {
			$out_encryption[] = (new CSpan(_('None')))->addClass(BS_STYLE_STATUS_GREEN);
		}
		else {
			$out_encryption[] = (new CSpan(_('None')))->addClass(BS_STYLE_STATUS_GREY);
		}

		if (($host['tls_accept'] & HOST_ENCRYPTION_PSK) == HOST_ENCRYPTION_PSK) {
			$out_encryption[] = (new CSpan(_('PSK')))->addClass(BS_STYLE_STATUS_GREEN);
		}
		else {
			$out_encryption[] = (new CSpan(_('PSK')))->addClass(BS_STYLE_STATUS_GREY);
		}

		if (($host['tls_accept'] & HOST_ENCRYPTION_CERTIFICATE) == HOST_ENCRYPTION_CERTIFICATE) {
			$out_encryption[] = (new CSpan(_('CERT')))->addClass(BS_STYLE_STATUS_GREEN);
		}
		else {
			$out_encryption[] = (new CSpan(_('CERT')))->addClass(BS_STYLE_STATUS_GREY);
		}

		$encryption = (new CDiv([$in_encryption, ' ', $out_encryption]))->addClass(BS_STYLE_STATUS_CONTAINER);
	}

	$table->addRow([
		new CCheckBox('hosts['.$host['hostid'].']', $host['hostid']),
		(new CCol($description))->addClass(BS_STYLE_NOWRAP),
		[
