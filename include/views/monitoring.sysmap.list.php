	...
	->addItem(
		(new CFilter('web.sysmapconf.filter.state'))
			->addColumn((new CFormList())->addRow(_('Name like'),
				(new CTextBox('filter_name', $data['filter']['name']))->setWidth(BS_TEXTAREA_FILTER_STANDARD_WIDTH)
			))
	);
...
foreach ($this->data['maps'] as $map) {
	...
	if ($user_type == USER_TYPE_SUPER_ADMIN || $user_type == USER_TYPE_BSM_ADMIN || $map['editable']) {
	...
	$sysmapTable->addRow([
		$checkbox,
		new CLink($map['name'], 'bsm.php?action=map.view&sysmapid='.$map['sysmapid']),
		$map['width'],
		$map['height'],
