...
if ($this->data['application_list']) {
	$applications = bs_array_merge([''], $this->data['application_list']);
...
$httpFormList
	...
	->addRow(_('Attempts'),
		(new CNumericBox('retries', $this->data['retries'], 2))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
	);

$agentComboBox = new CComboBox('agent', $this->data['agent']);

$userAgentsAll = userAgents();
$userAgentsAll[_('Others')][BS_AGENT_OTHER] = _('other').' ...';
...
$httpFormList->addRow(_('User agent string'),
	(new CTextBox('agent_other', $this->data['agent_other']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH),
	'row_agent_other'
);
...
$httpFormList
	...
	->addRow(_('Variables'),
		(new CTextArea('variables', $this->data['variables']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	)
	->addRow(_('Headers'),
		(new CTextArea('headers', $this->data['headers']))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	)
...
$httpAuthenticationFormList
	...
	->addRow(_('SSL certificate file'),
		(new CTextBox('ssl_cert_file', $this->data['ssl_cert_file'], false, 255))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	)
	->addRow(_('SSL key file'),
		(new CTextBox('ssl_key_file', $this->data['ssl_key_file'], false, 255))->setWidth(BS_TEXTAREA_STANDARD_WIDTH)
	)
...
foreach ($this->data['steps'] as $stepid => $step) {
	...
	if ($this->data['templated']) {
		...
		$dragHandler = (new CCol(
			(new CDiv())->addClass(BS_STYLE_DRAG_ICON)
		))->addClass(BS_STYLE_TD_DRAG_ICON);
	...
	$stepsTable->addRow(
		(new CRow([
			...
			$step['status_codes'],
			(new CCol($removeButton))->addClass(BS_STYLE_NOWRAP)
		]))
			->addClass('sortable')
			->setId('steps_'.$stepid)
	);
...
if (!$this->data['templated']) {
	$stepsTable->addRow(
		(new CCol(
			(new CButton('add_step', _('Add')))->addClass(BS_STYLE_BTN_LINK)
		))->setColSpan(8)
	);
}

$httpStepFormList->addRow(_('Steps'),
	(new CDiv($stepsTable))->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
);
...
bs_subarray_push($this->data['agentVisibility'], BS_AGENT_OTHER, 'agent_other');
bs_subarray_push($this->data['agentVisibility'], BS_AGENT_OTHER, 'row_agent_other');
