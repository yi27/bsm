...
function get_item_logtype_style($logtype) {
	switch ($logtype) {
		...
		case ITEM_LOGTYPE_SUCCESS_AUDIT:
		case ITEM_LOGTYPE_VERBOSE:
			return BS_STYLE_LOG_INFO_BG;

		case ITEM_LOGTYPE_WARNING:
			return BS_STYLE_LOG_WARNING_BG;

		case ITEM_LOGTYPE_ERROR:
		case ITEM_LOGTYPE_FAILURE_AUDIT:
			return BS_STYLE_LOG_HIGH_BG;

		case ITEM_LOGTYPE_CRITICAL:
			return BS_STYLE_LOG_DISASTER_BG;

		default:
			return BS_STYLE_LOG_NA_BG;
	}
}
...
function item_type2str($type = null) {
	$types = [
		ITEM_TYPE_BSM => _('Bsm agent'),
		ITEM_TYPE_BSM_ACTIVE => _('Bsm agent (active)'),
		...
		ITEM_TYPE_INTERNAL => _('Bsm internal'),
		ITEM_TYPE_TRAPPER => _('Bsm trapper'),
		ITEM_TYPE_AGGREGATE => _('Bsm aggregate'),
 ...
 */
function orderItemsByStatus(array &$items, $sortorder = BS_SORT_UP) {
	$sort = [];
...
function itemTypeInterface($type = null) {
	$types = [
		...
		ITEM_TYPE_BSM => INTERFACE_TYPE_AGENT,
...
function update_item_status($itemids, $status) {
	...
	while ($item = DBfetch($db_items)) {
		...
		if ($status != $old_status) {
			$result &= DBexecute(
				'UPDATE items SET status='.bs_dbstr($status).' WHERE itemid='.bs_dbstr($item['itemid'])
			);
			if ($result) {
...
function copyItemsToHosts($srcItemIds, $dstHostIds) {
	...
	foreach ($dstHosts as $dstHost) {
		...
		foreach ($srcItems as &$srcItem) {
			...
			$srcItem['applications'] = get_same_applications_for_host(bs_objectValues($srcItem['applications'], 'applicationid'), $dstHost['hostid']);
		}
		unset($srcItem);
...
function copyItems($srcHostId, $dstHostId) {
	...
	foreach ($srcItems as &$srcItem) {
		...
		$srcItem['applications'] = get_same_applications_for_host(bs_objectValues($srcItem['applications'], 'applicationid'), $dstHostId);
	}
	unset($srcItem);
...
function get_item_by_itemid($itemid) {
	$db_items = DBfetch(DBselect('SELECT i.* FROM items i WHERE i.itemid='.bs_dbstr($itemid)));
	if ($db_items) {
		return $db_items;
	}
...
function get_item_by_itemid_limited($itemid) {
	$row = DBfetch(DBselect(
		...
		' FROM items i'.
		' WHERE i.itemid='.bs_dbstr($itemid)));
	if ($row) {
		return $row;
	}
	error(_s('No item with itemid "%1$s".', $itemid));
...
function get_same_item_for_host($item, $dest_hostids) {
	...
	if (isset($itemid)) {
		$db_items = DBselect(
			...
			' WHERE dest.itemid='.bs_dbstr($itemid).
...
function fillItemsWithChildTemplates(&$items) {
	...
	$dbItems = DBselect('SELECT i.itemid,i.templateid FROM items i WHERE '.dbConditionInt('i.itemid', bs_objectValues($items, 'templateid')));
...
function getItemsDataOverview($hostIds, array $applicationIds = null, $viewMode) {
	...
	CArrayHelper::sort($dbItems, [
		['field' => 'name_expanded', 'order' => BS_SORT_UP],
		['field' => 'itemid', 'order' => BS_SORT_UP]
	]);

	// fetch latest values
	$history = Manager::History()->getLast(bs_toHash($dbItems, 'itemid'), 1, BS_HISTORY_PERIOD);
	...
	if ($viewMode == STYLE_TOP) {
		...
		foreach ($hostNames as $hostId => $hostName) {
			...
			$tableRow = [(new CCol($name))->addClass(BS_STYLE_NOWRAP)];
		...
		if ($item['tr_value'] == TRIGGER_VALUE_TRUE) {
			...
			if ($config['event_ack_enable']) {
				...
				$ack = ($ack['acknowledged'] == 1)
					? [SPACE, (new CSpan())->addClass(BS_STYLE_ICON_ACKN)]
					: null;
			}
		}
...
function getItemFunctionalValue($item, $function, $parameter) {
	...
	if (!isset($historyTables[$item['value_type']])) {
		...
		$result = DBselect(
			...
			' AND itemid='.bs_dbstr($item['itemid']).
...
function item_get_history($db_item, $clock, $ns) {
	...
	$sql = 'SELECT value'.
			' FROM '.$table.
			' WHERE itemid='.bs_dbstr($db_item['itemid']).
				' AND clock='.bs_dbstr($clock).
				' AND ns='.bs_dbstr($ns);
