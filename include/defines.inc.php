...
define('BSM_VERSION',		'3.0.4');
define('BSM_API_VERSION',	'3.0.4');
define('BSM_EXPORT_VERSION',	'3.0');
define('BSM_DB_VERSION',		3000000);

define('BSM_COPYRIGHT_FROM',	'2001');
define('BSM_COPYRIGHT_TO',	'2016');

define('BS_LOGIN_ATTEMPTS',	5);
define('BS_LOGIN_BLOCK',		30); // sec

define('BS_MIN_PERIOD',		60); // 1 minute
define('BS_MAX_PERIOD',		63072000); // the maximum period for the time bar control, ~2 years (2 * 365 * 86400)
define('BS_MAX_DATE',			2147483647); // 19 Jan 2038 05:14:07
define('BS_PERIOD_DEFAULT',	3600); // 1 hour
...
define('BS_HISTORY_PERIOD', 86400);

define('BS_WIDGET_ROWS', 20);

define('BS_FONTPATH',				realpath('fonts')); // where to search for font (GD > 2.0.18)
define('BS_GRAPH_FONT_NAME',		'DejaVuSans'); // font file name
define('BS_GRAPH_LEGEND_HEIGHT',	120); // when graph height is less then this value, some legend will not show up

define('BS_SCRIPT_TIMEOUT',		60); // in seconds

define('GRAPH_YAXIS_SIDE_DEFAULT', 0); // 0 - LEFT SIDE, 1 - RIGHT SIDE

define('BS_MAX_IMAGE_SIZE', 1048576); // 1024 * 1024

define('BS_UNITS_ROUNDOFF_THRESHOLD',		0.01);
define('BS_UNITS_ROUNDOFF_UPPER_LIMIT',	2);
define('BS_UNITS_ROUNDOFF_MIDDLE_LIMIT',	4);
define('BS_UNITS_ROUNDOFF_LOWER_LIMIT',	6);

define('BS_PRECISION_10',	10);

define('BS_DEFAULT_INTERVAL', '1-7,00:00-24:00');

define('BS_SCRIPT_TYPE_CUSTOM_SCRIPT',	0);
define('BS_SCRIPT_TYPE_IPMI',			1);
define('BS_SCRIPT_TYPE_SSH',			2);
define('BS_SCRIPT_TYPE_TELNET',		3);
define('BS_SCRIPT_TYPE_GLOBAL_SCRIPT',	4);

define('BS_SCRIPT_EXECUTE_ON_AGENT',	0);
define('BS_SCRIPT_EXECUTE_ON_SERVER',	1);

define('BS_FLAG_DISCOVERY_NORMAL',		0x0);
define('BS_FLAG_DISCOVERY_RULE',		0x1);
define('BS_FLAG_DISCOVERY_PROTOTYPE',	0x2);
define('BS_FLAG_DISCOVERY_CREATED',	0x4);
...
define('BS_ACK_STS_ANY',				1);
define('BS_ACK_STS_WITH_UNACK',		2);
define('BS_ACK_STS_WITH_LAST_UNACK',	3);

define('EVENTS_OPTION_NOEVENT', 1);
define('EVENTS_OPTION_ALL',		2);
define('EVENTS_OPTION_NOT_ACK', 3);

define('BS_FONT_NAME', 'DejaVuSans');

define('BS_AUTH_INTERNAL',	0);
define('BS_AUTH_LDAP',		1);
define('BS_AUTH_HTTP',		2);

define('BS_DB_DB2',		'IBM_DB2');
define('BS_DB_MYSQL',		'MYSQL');
define('BS_DB_ORACLE',		'ORACLE');
define('BS_DB_POSTGRESQL',	'POSTGRESQL');
define('BS_DB_SQLITE3',	'SQLITE3');

define('BS_DB_MAX_ID', '9223372036854775807');

// maximum number of records for create() or update() API calls
define('BS_DB_MAX_INSERTS', 10000);
...
define('BS_SESSION_ACTIVE',	0);
define('BS_SESSION_PASSIVE',	1);

define('BS_DROPDOWN_FIRST_NONE',	0);
define('BS_DROPDOWN_FIRST_ALL',	1);

define('T_BS_STR',			0);
define('T_BS_INT',			1);
define('T_BS_DBL',			2);
define('T_BS_CLR',			5);
define('T_BS_DBL_BIG',		9);
define('T_BS_DBL_STR',		10);
define('T_BS_TP',			11);
...
define('BS_SORT_UP',	'ASC');
define('BS_SORT_DOWN',	'DESC');
...
define('AUDIT_RESOURCE_BSM_CONFIG',	2);
...
define('BS_ITEM_DELAY_DEFAULT', 30);

define('ITEM_TYPE_BSM',			0);
...
define('BS_DEFAULT_KEY_DB_MONITOR',			'db.odbc.select[<unique short description>,<dsn>]');
define('BS_DEFAULT_KEY_DB_MONITOR_DISCOVERY',	'db.odbc.discovery[<unique short description>,<dsn>]');
define('BS_DEFAULT_KEY_SSH',					'ssh.run[<unique short description>,<ip>,<port>,<encoding>]');
define('BS_DEFAULT_KEY_TELNET',				'telnet.run[<unique short description>,<ip>,<port>,<encoding>]');
define('BS_DEFAULT_KEY_JMX',					'jmx[<object name>,<attribute name>]');

define('SYSMAP_ELEMENT_USE_ICONMAP_ON',		1);
define('SYSMAP_ELEMENT_USE_ICONMAP_OFF',	0);

define('BS_ICON_PREVIEW_HEIGHT',	24);
define('BS_ICON_PREVIEW_WIDTH',	24);
...
define('USER_TYPE_BSM_USER',		1);
define('USER_TYPE_BSM_ADMIN',	2);
define('USER_TYPE_SUPER_ADMIN',		3);

define('BS_NOT_INTERNAL_GROUP',	0);
define('BS_INTERNAL_GROUP',		1);
...
define('BS_DEFAULT_AGENT', 'Bsm');
define('BS_AGENT_OTHER', -1);
...
define('BS_ACKNOWLEDGE_SELECTED',	0);
define('BS_ACKNOWLEDGE_PROBLEM',	1);
define('BS_ACKNOWLEDGE_ALL',		2);
...
define('BS_MAX_TREND_DIFF', 3600);

define('BS_GRAPH_MAX_SKIP_CELL',	16);
define('BS_GRAPH_MAX_SKIP_DELAY',	4);
...
// suffixes
define('BS_BYTE_SUFFIXES', 'KMGT');
define('BS_TIME_SUFFIXES', 'smhdw');

// preg
define('BS_PREG_PRINT', '^\x00-\x1F');
define('BS_PREG_MACRO_NAME', '([A-Z0-9\._]+)');
define('BS_PREG_MACRO_NAME_LLD', '([A-Z0-9\._]+)');
define('BS_PREG_INTERNAL_NAMES', '([0-9a-zA-Z_\. \-]+)'); // !!! Don't forget sync code with C !!!
define('BS_PREG_NUMBER', '([\-+]?[0-9]+[.]?[0-9]*['.BS_BYTE_SUFFIXES.BS_TIME_SUFFIXES.']?)');
define('BS_PREG_DEF_FONT_STRING', '/^[0-9\.:% ]+$/');
define('BS_PREG_DNS_FORMAT', '([0-9a-zA-Z_\.\-$]|\{\$?'.BS_PREG_MACRO_NAME.'\})*');
define('BS_PREG_HOST_FORMAT', BS_PREG_INTERNAL_NAMES);
define('BS_PREG_MACRO_NAME_FORMAT', '(\{[A-Z\.]+\})');
define('BS_PREG_EXPRESSION_LLD_MACROS', '(\{\#'.BS_PREG_MACRO_NAME_LLD.'\})');

// !!! should be used with "x" modifier
define('BS_PREG_ITEM_KEY_PARAMETER_FORMAT', '(
...
define('BS_PREG_ITEM_KEY_FORMAT', '([0-9a-zA-Z_\. \-]+? # match key
(?P<param>( # name parameter group used in recursion
	\[ # match opening bracket
		(
			\s*?'.BS_PREG_ITEM_KEY_PARAMETER_FORMAT .' # match spaces and parameter
			(
				\s*?,\s*? # match spaces, comma and spaces
				'.BS_PREG_ITEM_KEY_PARAMETER_FORMAT .' # match parameter
...
define('BS_USER_ONLINE_TIME', 600); // 10min
define('BS_GUEST_USER','guest');
...
define('BS_HAVE_IPV6', 1);
define('BS_DISCOVERER_IPRANGE_LIMIT', 65536);

define('BS_SOCKET_TIMEOUT',        3);         // socket timeout limit
define('BS_SOCKET_BYTES_LIMIT',    1048576);   // socket response size limit, 1048576 is 1MB in bytes
...
define('BS_DEFAULT_IMPORT_HOST_GROUP', 'Imported hosts');

// XML import flags
// See BS-8151. Old version of libxml suffered from setting DTDLOAD and NOENT flags by default, which allowed
...
// API errors
define('BS_API_ERROR_INTERNAL',	111);
define('BS_API_ERROR_PARAMETERS',	100);
define('BS_API_ERROR_PERMISSIONS',	120);
define('BS_API_ERROR_NO_AUTH',		200);
define('BS_API_ERROR_NO_METHOD',	300);
...
define('BS_JAN_2038', 2145916800);

define('DAY_IN_YEAR', 365);

define('BS_MIN_PORT_NUMBER', 0);
define('BS_MAX_PORT_NUMBER', 65535);

// input fields
define('BS_TEXTAREA_MACRO_WIDTH',				200);
define('BS_TEXTAREA_MACRO_VALUE_WIDTH',		250);
define('BS_TEXTAREA_COLOR_WIDTH',				96);
define('BS_TEXTAREA_FILTER_SMALL_WIDTH',		150);
define('BS_TEXTAREA_FILTER_STANDARD_WIDTH',	300);
define('BS_TEXTAREA_FILTER_BIG_WIDTH',			524);
define('BS_TEXTAREA_TINY_WIDTH',				75);
define('BS_TEXTAREA_SMALL_WIDTH',				150);
define('BS_TEXTAREA_STANDARD_WIDTH',			300);
define('BS_TEXTAREA_BIG_WIDTH',				524);
define('BS_TEXTAREA_NUMERIC_STANDARD_WIDTH',	75);
define('BS_TEXTAREA_NUMERIC_BIG_WIDTH',		150);
define('BS_TEXTAREA_2DIGITS_WIDTH',			35);	// please use for date selector only
define('BS_TEXTAREA_4DIGITS_WIDTH',			50);	// please use for date selector only
define('BS_TEXTAREA_INTERFACE_IP_WIDTH',		225);
define('BS_TEXTAREA_INTERFACE_DNS_WIDTH',		175);
define('BS_TEXTAREA_INTERFACE_USEIP_WIDTH',	100);
define('BS_TEXTAREA_INTERFACE_PORT_WIDTH',		100);
define('BS_TEXTAREA_STANDARD_ROWS',			7);

// decoration borders
define('BS_HOST_INTERFACE_WIDTH',				750);

// overviews help
define('BS_OVERVIEW_HELP_MIN_WIDTH',			125);
...
define('WIDGET_BSM_STATUS',			'stsbs');
...
define('BS_VALID_OK',		0);
define('BS_VALID_ERROR',	1);
define('BS_VALID_WARNING',	2);

// user default theme
define('THEME_DEFAULT', 'default');

// the default theme
define('BS_DEFAULT_THEME', 'blue-theme');

define('BSM_HOMEPAGE', '');
define('BS_DEFAULT_URL', 'bsm.php?action=dashboard.view');
...
// CSS styles
// CSS styles
define('BS_STYLE_ACTION_BUTTONS', 'action-buttons');
define('BS_STYLE_ACTIVE_INDIC', 'active-indic');
define('BS_STYLE_ACTIVE_BG', 'active-bg');
define('BS_STYLE_ADM_IMG', 'adm-img');
define('BS_STYLE_ARTICLE', 'article');
define('BS_STYLE_CONTAINER', 'container');
define('BS_STYLE_CONTENT_1', 'content_1');
define('BS_STYLE_AVERAGE_BG', 'average-bg');
define('BS_STYLE_ARROW_DOWN', 'arrow-down');
define('BS_STYLE_ARROW_LEFT', 'arrow-left');
define('BS_STYLE_ARROW_RIGHT', 'arrow-right');
define('BS_STYLE_ARROW_UP', 'arrow-up');
define('BS_STYLE_BLUE', 'blue');
define('BS_STYLE_BTN_ADD_FAV', 'btn-add-fav');
define('BS_STYLE_BTN_ALT', 'btn-alt');
define('BS_STYLE_BTN_CONF', 'btn-conf');
define('BS_STYLE_BTN_DEBUG', 'btn-debug');
define('BS_STYLE_BTN_GREY', 'btn-grey');
define('BS_STYLE_BTN_INFO', 'btn-info');
define('BS_STYLE_BTN_LINK', 'btn-link');
define('BS_STYLE_BTN_MAX', 'btn-max');
define('BS_STYLE_BTN_MIN', 'btn-min');
define('BS_STYLE_BTN_REMOVE_FAV', 'btn-remove-fav');
define('BS_STYLE_BTN_RESET', 'btn-reset');
define('BS_STYLE_BTN_SEARCH', 'btn-search');
define('BS_STYLE_BTN_WIDGET_ACTION', 'btn-widget-action');
define('BS_STYLE_BTN_WIDGET_COLLAPSE', 'btn-widget-collapse');
define('BS_STYLE_BTN_WIDGET_EXPAND', 'btn-widget-expand');
define('BS_STYLE_BOTTOM', 'bottom');
define('BS_STYLE_BROWSER_LOGO_CHROME', 'browser-logo-chrome');
define('BS_STYLE_BROWSER_LOGO_FF', 'browser-logo-ff');
define('BS_STYLE_BROWSER_LOGO_IE', 'browser-logo-ie');
define('BS_STYLE_BROWSER_LOGO_OPERA', 'browser-logo-opera');
define('BS_STYLE_BROWSER_LOGO_SAFARI', 'browser-logo-safari');
define('BS_STYLE_BROWSER_WARNING_CONTAINER', 'browser-warning-container');
define('BS_STYLE_BROWSER_WARNING_FOOTER', 'browser-warning-footer');
define('BS_STYLE_CELL', 'cell');
define('BS_STYLE_CELL_WIDTH', 'cell-width');
define('BS_STYLE_CENTER', 'center');
define('BS_STYLE_CLOCK', 'clock');
define('BS_STYLE_CLOCK_FACE', 'clock-face');
define('BS_STYLE_CLOCK_HAND', 'clock-hand');
define('BS_STYLE_CLOCK_HAND_SEC', 'clock-hand-sec');
define('BS_STYLE_CLOCK_LINES', 'clock-lines');
define('BS_STYLE_COLOR_PICKER', 'color-picker');
define('BS_STYLE_CURSOR_MOVE', 'cursor-move');
define('BS_STYLE_CURSOR_POINTER', 'cursor-pointer');
define('BS_STYLE_DASHBRD_WIDGET_HEAD', 'dashbrd-widget-head');
define('BS_STYLE_DASHBRD_WIDGET_FOOT', 'dashbrd-widget-foot');
define('BS_STYLE_DASHED_BORDER', 'dashed-border');
define('BS_STYLE_DEBUG_OUTPUT', 'debug-output');
define('BS_STYLE_DISABLED', 'disabled');
define('BS_STYLE_DISASTER_BG', 'disaster-bg');
define('BS_STYLE_DRAG_ICON', 'drag-icon');
define('BS_STYLE_DRAG_DROP_AREA', 'drag-drop-area');
define('BS_STYLE_TABLE_FORMS_SEPARATOR', 'table-forms-separator');
define('BS_STYLE_FILTER_CONTAINER', 'filter-container');
define('BS_STYLE_FILTER_BTN_CONTAINER', 'filter-btn-container');
define('BS_STYLE_FILTER_FORMS', 'filter-forms');
define('BS_STYLE_FILTER_TRIGGER', 'filter-trigger');
define('BS_STYLE_FILTER_ACTIVE', 'filter-active');
define('BS_STYLE_FLOAT_LEFT', 'float-left');
define('BS_STYLE_FORM_INPUT_MARGIN', 'form-input-margin');
define('BS_STYLE_FORM_NEW_GROUP', 'form-new-group');
define('BS_STYLE_FOOTER', 'footer');
define('BS_STYLE_GREEN', 'green');
define('BS_STYLE_GREEN_BG', 'green-bg');
define('BS_STYLE_GREY', 'grey');
define('BS_STYLE_HEADER_TITLE', 'header-title');
define('BS_STYLE_HEADER_TITLE_H1', 'header-title-h1');
define('BS_STYLE_HIDDEN', 'hidden');
define('BS_STYLE_HIGH_BG', 'high-bg');
define('BS_STYLE_HOR_LIST', 'hor-list');
define('BS_STYLE_ICON_ACKN', 'icon-ackn');
define('BS_STYLE_ICON_CAL', 'icon-cal');
define('BS_STYLE_ICON_DEPEND_DOWN', 'icon-depend-down');
define('BS_STYLE_ICON_DEPEND_UP', 'icon-depend-up');
define('BS_STYLE_ICON_MAINT', 'icon-maint');
define('BS_STYLE_ICON_WZRD_ACTION', 'icon-wzrd-action');
define('BS_STYLE_INACTIVE_BG', 'inactive-bg');
define('BS_STYLE_INFO_BG', 'info-bg');
define('BS_STYLE_INPUT_COLOR_PICKER', 'input-color-picker');
define('BS_STYLE_LEFT', 'left');
define('BS_STYLE_LINK_ACTION', 'link-action');
define('BS_STYLE_LINK_ALT', 'link-alt');
define('BS_STYLE_LIST_HOR_CHECK_RADIO', 'list-hor-check-radio');
define('BS_STYLE_LIST_HOR_MIN_WIDTH', 'list-hor-min-width');
define('BS_STYLE_LIST_CHECK_RADIO', 'list-check-radio');
define('BS_STYLE_LIST_TABLE', 'list-table');
define('BS_STYLE_LOCAL_CLOCK', 'local-clock');
define('BS_STYLE_LOG_NA_BG', 'log-na-bg');
define('BS_STYLE_LOG_INFO_BG', 'log-info-bg');
define('BS_STYLE_LOG_WARNING_BG', 'log-warning-bg');
define('BS_STYLE_LOG_HIGH_BG', 'log-high-bg');
define('BS_STYLE_LOG_DISASTER_BG', 'log-disaster-bg');
define('BS_STYLE_LOGO', 'logo');
define('BS_STYLE_MAP_AREA', 'map-area');
define('BS_STYLE_MIDDLE', 'middle');
define('BS_STYLE_MSG_GOOD', 'msg-good');
define('BS_STYLE_MSG_BAD', 'msg-bad');
define('BS_STYLE_MSG_BAD_GLOBAL', 'msg-bad-global');
define('BS_STYLE_MSG_DETAILS', 'msg-details');
define('BS_STYLE_MSG_DETAILS_BORDER', 'msg-details-border');
define('BS_STYLE_NA_BG', 'na-bg');
define('BS_STYLE_NAV', 'nav');
define('BS_STYLE_NORMAL_BG', 'normal-bg');
define('BS_STYLE_NOTIF_BODY', 'notif-body');
define('BS_STYLE_NOTIF_INDIC', 'notif-indic');
define('BS_STYLE_NOTIF_INDIC_CONTAINER', 'notif-indic-container');
define('BS_STYLE_NOTHING_TO_SHOW', 'nothing-to-show');
define('BS_STYLE_NOWRAP', 'nowrap');
define('BS_STYLE_ORANGE', 'orange');
define('BS_STYLE_OVERLAY_CLOSE_BTN', 'overlay-close-btn');
define('BS_STYLE_OVERLAY_DESCR', 'overlay-descr');
define('BS_STYLE_OVERLAY_DESCR_URL', 'overlay-descr-url');
define('BS_STYLE_OVERFLOW_ELLIPSIS', 'overflow-ellipsis');
define('BS_STYLE_OBJECT_GROUP', 'object-group');
define('BS_STYLE_PAGING_BTN_CONTAINER', 'paging-btn-container');
define('BS_STYLE_PAGING_SELECTED', 'paging-selected');
define('BS_STYLE_PROGRESS_BAR_BG', 'progress-bar-bg');
define('BS_STYLE_PROGRESS_BAR_CONTAINER', 'progress-bar-container');
define('BS_STYLE_PROGRESS_BAR_LABEL', 'progress-bar-label');
define('BS_STYLE_RADIO_SEGMENTED', 'radio-segmented');
define('BS_STYLE_RED', 'red');
define('BS_STYLE_RED_BG', 'red-bg');
define('BS_STYLE_REL_CONTAINER', 'rel-container');
define('BS_STYLE_RIGHT', 'right');
define('BS_STYLE_ROW', 'row');
define('BS_STYLE_SCREEN_TABLE', 'screen-table');
define('BS_STYLE_SEARCH', 'search');
define('BS_STYLE_SELECTED', 'selected');
define('BS_STYLE_SELECTED_ITEM_COUNT', 'selected-item-count');
define('BS_STYLE_SERVER_NAME', 'server-name');
define('BS_STYLE_SETUP_CONTAINER', 'setup-container');
define('BS_STYLE_SETUP_FOOTER', 'setup-footer');
define('BS_STYLE_SETUP_LEFT', 'setup-left');
define('BS_STYLE_SETUP_LEFT_CURRENT', 'setup-left-current');
define('BS_STYLE_SETUP_RIGHT', 'setup-right');
define('BS_STYLE_SETUP_RIGBS_BODY', 'setup-right-body');
define('BS_STYLE_SETUP_TITLE', 'setup-title');
define('BS_STYLE_SIGNIN_CONTAINER', 'signin-container');
define('BS_STYLE_SIGNIN_INNER_CONTAINER', 'signin-inner-container');
define('BS_STYLE_SIGNIN_BLACKGROUND', 'signin-blackground');
define('BS_STYLE_SIGNIN_OVER', 'signin-over');
define('BS_STYLE_SIGNIN_OVER_H1', 'signin-over-h1');
define('BS_STYLE_COL_MD_1', 'col-md-1');
define('BS_STYLE_COL_MD_2', 'col-md-2');
define('BS_STYLE_COL_MD_3', 'col-md-3');
define('BS_STYLE_COL_MD_4', 'col-md-4');
define('BS_STYLE_COL_MD_5', 'col-md-5');
define('BS_STYLE_COL_MD_6', 'col-md-6');
define('BS_STYLE_COL_MD_7', 'col-md-7');
define('BS_STYLE_COL_MD_8', 'col-md-8');
define('BS_STYLE_COL_MD_9', 'col-md-9');
define('BS_STYLE_COL_MD_10', 'col-md-10');
define('BS_STYLE_COL_MD_11', 'col-md-11');
define('BS_STYLE_COL_MD_12', 'col-md-12');
define('BS_STYLE_FORM_CONTROL', 'form-control');

define('BS_STYLE_SIGNIN_DOWN', 'signin-down');
define('BS_STYLE_SIGNIN_LEFT', 'signin-left');
define('BS_STYLE_SIGNIN_RIGHT', 'signin-right');
define('BS_STYLE_SIGNIN_LOGO', 'signin-logo');
define('BS_STYLE_SIGN_IN_TXT', 'sign-in-txt');
define('BS_STYLE_STATUS_CONTAINER', 'status-container');
define('BS_STYLE_STATUS_GREEN', 'status-green');
define('BS_STYLE_STATUS_GREY', 'status-grey');
define('BS_STYLE_STATUS_RED', 'status-red');
define('BS_STYLE_STATUS_YELLOW', 'status-yellow');
define('BS_STYLE_SUBFILTER_ENABLED', 'subfilter-enabled');
define('BS_STYLE_TABLE', 'table');
define('BS_STYLE_TABLE_FORMS', 'table-forms');
define('BS_STYLE_TABLE_FORMS_CONTAINER', 'table-forms-container');
define('BS_STYLE_TABLE_FORMS_TD_LEFT', 'table-forms-td-left');
define('BS_STYLE_TABLE_FORMS_TD_RIGHT', 'table-forms-td-right');
define('BS_STYLE_TABLE_PAGING', 'table-paging');
define('BS_STYLE_TABLE_STATS', 'table-stats');
define('BS_STYLE_TABS_NAV', 'tabs-nav');
define('BS_STYLE_TFOOT_BUTTONS', 'tfoot-buttons');
define('BS_STYLE_TD_DRAG_ICON', 'td-drag-icon');
define('BS_STYLE_TIME_ZONE', 'time-zone');
define('BS_STYLE_TOP', 'top');
define('BS_STYLE_TOP_NAV', 'top-nav');
define('BS_STYLE_TOP_NAV_CONTAINER', 'top-nav-container');
define('BS_STYLE_TOP_NAV_HELP', 'top-nav-help');
define('BS_STYLE_TOP_NAV_ICONS', 'top-nav-icons');
define('BS_STYLE_TOP_NAV_PROFILE', 'top-nav-profile');
define('BS_STYLE_TOP_NAV_SIGNOUT', 'top-nav-signout');
define('BS_STYLE_TOP_NAV_ZBBSHARE', 'top-nav-zbbshare');
define('BS_STYLE_TOP_SUBNAV', 'top-subnav');
define('BS_STYLE_TOP_SUBNAV_CONTAINER', 'top-subnav-container');
define('BS_STYLE_TREEVIEW', 'treeview');
define('BS_STYLE_TREEVIEW_PLUS', 'treeview-plus');
define('BS_STYLE_UPPERCASE', 'uppercase');
define('BS_STYLE_WARNING_BG', 'warning-bg');
define('BS_STYLE_YELLOW', 'yellow');
...
if (get_magic_quotes_gpc()) {
	function bs_stripslashes($value) {
		$value = is_array($value) ? array_map('bs_stripslashes', $value) : stripslashes($value);
		return $value;
	}
	$_GET = bs_stripslashes($_GET);
	$_POST = bs_stripslashes($_POST);
	$_COOKIE = bs_stripslashes($_COOKIE);
}
