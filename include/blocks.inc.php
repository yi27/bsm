...
function getFavouriteMaps() {
	...
	if ($data) {
		foreach ($data as $map) {
			$favourites->addRow([new CLink($map['label'], 'bsm.php?action=map.view&sysmapid='.$map['id'])]);
		}
	}
	...
	foreach ($triggers as $trigger) {
		...
		foreach ($trigger['groups'] as $group) {
			...
			if (in_array($filter['extAck'], [EXTACK_OPTION_ALL, EXTACK_OPTION_BOTH])) {
				if ($groups[$group['groupid']]['tab_priority'][$trigger['priority']]['count'] < BS_WIDGET_ROWS) {
			...
			if (in_array($filter['extAck'], [EXTACK_OPTION_UNACK, EXTACK_OPTION_BOTH])
				...
				if ($groups[$group['groupid']]['tab_priority'][$trigger['priority']]['count_unack'] < BS_WIDGET_ROWS) {
...
function make_status_of_bs() {
	...
	$table->addRow([
		_('Bsm server is running'),
		(new CSpan($status['bsm_server']))->addClass($status['bsm_server'] == _('Yes') ? BS_STYLE_GREEN : BS_STYLE_RED),
		isset($BS_SERVER, $BS_SERVER_PORT) ? $BS_SERVER.':'.$BS_SERVER_PORT : _('Bsm server IP or port is not set!')
	]);
	...
	$table->addRow([_('Number of hosts (enabled/disabled/templates)'), $status['hosts_count'],
		[
			(new CSpan($status['hosts_count_monitored']))->addClass(BS_STYLE_GREEN), ' / ',
			(new CSpan($status['hosts_count_not_monitored']))->addClass(BS_STYLE_RED), ' / ',
			(new CSpan($status['hosts_count_template']))->addClass(BS_STYLE_GREY)
		]
	]);
	...
	$table->addRow([$title, $status['items_count'],
		[
			(new CSpan($status['items_count_monitored']))->addClass(BS_STYLE_GREEN), ' / ',
			(new CSpan($status['items_count_disabled']))->addClass(BS_STYLE_RED), ' / ',
			(new CSpan($status['items_count_not_supported']))->addClass(BS_STYLE_GREY)
		]
	]);
	...
	$table->addRow([$title, $status['triggers_count'],
		[
			...
			(new CSpan($status['triggers_count_on']))->addClass(BS_STYLE_RED), ' / ',
			(new CSpan($status['triggers_count_off']))->addClass(BS_STYLE_GREEN), ']'
		]
	]);
	$table->addRow([_('Number of users (online)'), $status['users_count'], (new CSpan($status['users_online']))->addClass(BS_STYLE_GREEN)]);
	...
	if (CWebUser::$data['type'] == USER_TYPE_SUPER_ADMIN) {
		foreach ((new CFrontendSetup())->checkRequirements() as $req) {
			if ($req['result'] != CFrontendSetup::CHECK_OK) {
				$class = ($req['result'] == CFrontendSetup::CHECK_WARNING) ? BS_STYLE_ORANGE : BS_STYLE_RED;
				$table->addRow(
...
function make_latest_issues(array $filter = [], $backurl) {
	...
	if (isset($filter['sortfield']) && $filter['sortfield'] !== 'lastchange') {
		...
		$sort_order = [$filter['sortorder'], BS_SORT_DOWN];
	}
	else {
		$sort_field = ['lastchange'];
		$sort_order = [BS_SORT_DOWN];
	}
	...
	if ($show_sort_indicator) {
		$sort_div = (new CDiv())
			->addClass(($filter['sortorder'] === BS_SORT_DOWN) ? BS_STYLE_ARROW_DOWN : BS_STYLE_ARROW_UP);
	}
	...
	foreach ($triggers as $trigger) {
		...
		if ($trigger['lastEvent']) {
			// description
			$description = CMacrosResolverHelper::resolveEventDescription(bs_array_merge($trigger, [
		...
		if ($config['event_ack_enable']) {
			if ($trigger['lastEvent']) {
				...
				$ack = (new CSpan(_('No events')))->addClass(BS_STYLE_GREY);
		}
		else {
			$ack = null;
		}
		...
		// clock
		$clock = new CLink(bs_date2str(DATE_TIME_FORMAT_SECONDS, $trigger['lastchange']),
			'events.php?filter_set=1&triggerid='.$trigger['triggerid'].'&period='.BS_PERIOD_DEFAULT.
		...
		$table->addRow([
			...
			$description,
			$clock,
			bs_date2age($trigger['lastchange']),
			$unknown,
			$ack,
			(new CCol($action_hint))->addClass(BS_STYLE_NOWRAP)
		]);
	}
...
function makeTriggersPopup(array $triggers, $backurl, array $actions, array $config) {
	...
	CArrayHelper::sort($triggers, [['field' => 'lastchange', 'order' => BS_SORT_DOWN]]);

	foreach ($triggers as $trigger) {
		$description = CMacrosResolverHelper::resolveEventDescription(bs_array_merge($trigger, array(
		...
		if ($config['event_ack_enable']) {
			$ack = isset($trigger['event']['eventid'])
				...
				: (new CSpan(_('No events')))->addClass(BS_STYLE_GREY);
		}
		else {
			$ack = null;
		}
		...
		$popupTable->addRow([
			...
			$unknown,
			$ack,
			(new CCol($action))->addClass(BS_STYLE_NOWRAP)
		]);
	}
