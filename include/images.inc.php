...
function get_image_by_imageid($imageid) {
	...
	if (!isset($images[$imageid])) {
		$row = DBfetch(DBselect('SELECT i.* FROM images i WHERE i.imageid='.bs_dbstr($imageid)));
		$row['image'] = bs_unescape_image($row['image']);
		$images[$imageid] = $row;
	}
	return $images[$imageid];
}

function bs_unescape_image($image) {
	global $DB;

	$result = $image ? $image : 0;
	if ($DB['TYPE'] == BS_DB_POSTGRESQL) {
		$result = pg_unescape_bytea($image);
	}
	elseif ($DB['TYPE'] == BS_DB_SQLITE3) {
		$result = pack('H*', $image);
	}
	return $result;
}
