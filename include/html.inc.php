	...
	switch ($type) {
		...
		case 'dashconf':
			$icon = (new CRedirectButton(SPACE, 'dashconf.php'))
				->addClass(BS_STYLE_BTN_CONF)
				->setTitle(_('Configure'));

			if ($params['enabled']) {
				$icon = [$icon, (new CDiv())->addClass(BS_STYLE_ACTIVE_INDIC)];
			}
		...
		case 'overviewhelp':
			return (new CRedirectButton(SPACE, null))
				->addClass(BS_STYLE_BTN_INFO);

		case 'reset':
			return (new CRedirectButton(SPACE, null))
				->addClass(BS_STYLE_BTN_RESET)
				->setTitle(_('Reset'))
...
function get_header_host_table($current_element, $hostid, $lld_ruleid = 0) {
	 ...
	 */
	$list = (new CList())->addClass(BS_STYLE_OBJECT_GROUP);
	...
	if ($is_template) {
		...
		if ($current_element === '') {
			$template->addClass(BS_STYLE_SELECTED);
		}
		...
		switch ($db_host['status']) {
			case HOST_STATUS_MONITORED:
				if ($db_host['maintenance_status'] == HOST_MAINTENANCE_STATUS_ON) {
					$status = (new CSpan(_('In maintenance')))->addClass(BS_STYLE_ORANGE);
				}
				else {
					$status = (new CSpan(_('Enabled')))->addClass(BS_STYLE_GREEN);
				}
				break;
			case HOST_STATUS_NOT_MONITORED:
				$status = (new CSpan(_('Disabled')))->addClass(BS_STYLE_RED);
				break;
			default:
				$status = _('Unknown');
				break;
		...
		if ($current_element === '') {
			$host->addClass(BS_STYLE_SELECTED);
		}
		...
		if ($db_host['flags'] == BS_FLAG_DISCOVERY_CREATED && $db_host['hostDiscovery']['ts_delete'] != 0) {
			$lifetime_indicator = getHostLifetimeIndicator(time(), $db_host['hostDiscovery']['ts_delete']);
			$list->addItem((new CDiv($lifetime_indicator))->addClass(BS_STYLE_STATUS_CONTAINER));
		}
	}
	...
	if ($lld_ruleid == 0) {
		...
		if ($current_element == 'applications') {
			$applications->addClass(BS_STYLE_SELECTED);
		}
		...
		if ($current_element == 'items') {
			$items->addClass(BS_STYLE_SELECTED);
		}
		$list->addItem($items);
		...
		if ($current_element == 'triggers') {
			$triggers->addClass(BS_STYLE_SELECTED);
		}
		...
		if ($current_element == 'graphs') {
			$graphs->addClass(BS_STYLE_SELECTED);
		}
		$list->addItem($graphs);
		...
		if ($is_template) {
			...
			if ($current_element == 'screens') {
				$screens->addClass(BS_STYLE_SELECTED);
			}
		...
		if ($current_element == 'discoveries') {
			$lld_rules->addClass(BS_STYLE_SELECTED);
		}
		...
		if ($current_element == 'web') {
			$http_tests->addClass(BS_STYLE_SELECTED);
		}
		...
		if ($current_element == 'discoveries') {
			$discovery_rule->addClass(BS_STYLE_SELECTED);
		}
		...
		if ($current_element == 'items') {
			$item_prototypes->addClass(BS_STYLE_SELECTED);
		}
		...
		if ($current_element == 'triggers') {
			$trigger_prototypes->addClass(BS_STYLE_SELECTED);
		}
		...
		if ($current_element == 'graphs') {
			$graph_prototypes->addClass(BS_STYLE_SELECTED);
		}
		$list->addItem($graph_prototypes);

		// host prototypes
		if ($db_host['flags'] == BS_FLAG_DISCOVERY_NORMAL) {
			...
			if ($current_element == 'hosts') {
				$host_prototypes->addClass(BS_STYLE_SELECTED);
			}
...
function get_header_sysmap_table($sysmapid, $name, $fullscreen, $severity_min) {
	$list = (new CList())
		...
		->addItem([
			...
			'/',
			(new CSpan())
				->addClass(BS_STYLE_SELECTED)
				->addItem(
					new CLink($name, 'bsm.php?action=map.view&sysmapid='.$sysmapid.'&fullscreen='.$fullscreen.
	...
	if ($parent_sysmaps) {
		...
		foreach ($parent_sysmaps as $parent_sysmap) {
			$hor_list->addItem(
				new CLink($parent_sysmap['name'], 'bsm.php?action=map.view'.
...
function makeFormFooter(CButtonInterface $main_button = null, array $other_buttons = []) {
	foreach ($other_buttons as $other_button) {
		$other_button->addClass(BS_STYLE_BTN_ALT);
	}
	...
	return (new CList())
		...
		->addItem([
			(new CDiv())->addClass(BS_STYLE_TABLE_FORMS_TD_LEFT),
			(new CDiv($other_buttons))
				->addClass(BS_STYLE_TABLE_FORMS_TD_RIGHT)
				->addClass('tfoot-buttons')
		]);
}

/**
 * Returns bs, snmp, jmx, ipmi availability status icons and the discovered host lifetime indicator.
 *
...
function getHostAvailabilityTable($host) {
	$container = (new CDiv())->addClass(BS_STYLE_STATUS_CONTAINER);

	foreach (['bs' => '', 'snmp' => 'snmp_', 'jmx' => 'jmx_', 'ipmi' => 'ipmi_'] as $type => $prefix) {
		switch ($host[$prefix.'available']) {
			case HOST_AVAILABLE_TRUE:
				$ai = (new CSpan($type))->addClass(BS_STYLE_STATUS_GREEN);
				break;
			case HOST_AVAILABLE_FALSE:
				$ai = (new CSpan($type))
					->addClass(BS_STYLE_STATUS_RED)
					->addClass(BS_STYLE_CURSOR_POINTER)
					->setHint($host[$prefix.'error'], BS_STYLE_RED);
				break;
			case HOST_AVAILABLE_UNKNOWN:
				$ai = (new CSpan($type))->addClass(BS_STYLE_STATUS_GREY);
				break;
		}
...
function getHostGroupLifetimeIndicator($current_time, $ts_delete) {
	...
	if ($current_time > $ts_delete) {
		...
		$warning = _s(
			...
			bs_date2age($ts_delete),
			bs_date2str(DATE_FORMAT, $ts_delete),
			bs_date2str(TIME_FORMAT, $ts_delete)
		);
	}
...
function createDateSelector($name, $date, $relatedCalendar = null) {
	...
	$fields = [
		...
		(new CButton())
			->addClass(BS_STYLE_ICON_CAL)
			->onClick($onClick)
	];

	bs_add_post_js('create_calendar(null,'.
...
{
	return (new CDiv([
		$with_version ? '业务网管 ' : null,


	]))->addClass(BS_STYLE_FOOTER);
}
...
{
	return (new CSpan(bold('!')))
		->addClass(BS_STYLE_STATUS_YELLOW)
		->addClass(BS_STYLE_CURSOR_POINTER)
		->setHint($error);
}
		...
		(new CLink(_('Debug'), '#debug'))
			->onClick("javascript: if (!isset('state', this)) { this.state = 'none'; }".
				...
				"jQuery(this)".
					...
					".blur();".
				"showHideByName('bs_debug_info', this.state);"
			)
	))->addClass(BS_STYLE_BTN_DEBUG);
}
...
{
	$severities = [
		BS_STYLE_NA_BG => $config['severity_color_0'],
		BS_STYLE_INFO_BG => $config['severity_color_1'],
		BS_STYLE_WARNING_BG => $config['severity_color_2'],
		BS_STYLE_AVERAGE_BG => $config['severity_color_3'],
		BS_STYLE_HIGH_BG => $config['severity_color_4'],
		BS_STYLE_DISASTER_BG => $config['severity_color_5']
	];
