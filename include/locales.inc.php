...
function getLocales() {
	return [
		'en_GB' => ['name' => _('English (en_GB)'),	'display' => false],
		...
		'it_IT' => ['name' => _('Italian (it_IT)'),	'display' => false],
 ...
 */
function bs_locale_variants($language) {
	if ((stristr($_SERVER['SERVER_SOFTWARE'], 'win32') !== false) || (stristr($_SERVER['SERVER_SOFTWARE'], 'win64') !== false)) {
		return bs_locale_variants_win($language);
	}
	else {
		return bs_locale_variants_unix($language);
	}
}

function bs_locale_variants_unix($language) {
	$postfixes = [
		'',
		'.utf8',
		'.UTF-8',
...
function bs_locale_variants_win($language) {
