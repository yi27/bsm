...
function get_maintenance_by_maintenanceid($maintenanceid) {
	return DBfetch(DBselect(
		'SELECT m.* FROM maintenances m WHERE m.maintenanceid='.bs_dbstr($maintenanceid)
	));
}
...
function shedule2str($timeperiod) {
	...
	if ($timeperiod['timeperiod_type'] == TIMEPERIOD_TYPE_ONETIME) {
		$str = bs_date2str(DATE_TIME_FORMAT, $timeperiod['start_date']);
	}
	...
	elseif ($timeperiod['timeperiod_type'] == TIMEPERIOD_TYPE_WEEKLY) {
		$days = '';
		$dayofweek = bs_num2bitstr($timeperiod['dayofweek'], true);
	...
	elseif ($timeperiod['timeperiod_type'] == TIMEPERIOD_TYPE_MONTHLY) {
		$months = '';
		$month = bs_num2bitstr($timeperiod['month'], true);
		$length = strlen($month);
		for ($i = 0; $i < $length; $i++) {
			if ($month[$i] == 1) {
				if (!bs_empty($months)) {
