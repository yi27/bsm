...
function getHostInventories($orderedByTitle = false) {
	 ...
	 * C analog is located in function DBget_inventory_field() in src/libs/bsdbhigh/db.c
	 */
	$inventoryFields = [
		1 => [
			'nr' => 1,
...
function get_hostgroup_by_groupid($groupid) {
	$groups = DBfetch(DBselect('SELECT g.* FROM groups g WHERE g.groupid='.bs_dbstr($groupid)));
...
function get_host_by_hostid($hostid, $no_error_message = 0) {
	$row = DBfetch(DBselect('SELECT h.* FROM hosts h WHERE h.hostid='.bs_dbstr($hostid)));
...
function updateHostStatus($hostids, $status) {
	...
	$db_hosts = DBselect(
		...
		' WHERE '.dbConditionInt('h.hostid', $hostids).
			' AND h.status='.bs_dbstr($oldStatus)
	);
...
function get_application_by_applicationid($applicationid, $no_error_message = 0) {
	$row = DBfetch(DBselect('SELECT a.* FROM applications a WHERE a.applicationid='.bs_dbstr($applicationid)));
...
function getDeletableHostGroupIds(array $groupIds) {
	...
	$dbResult = DBselect(
		'SELECT g.groupid'.
		' FROM groups g'.
		' WHERE g.internal='.BS_NOT_INTERNAL_GROUP.
...
function isTemplate($hostId) {
	$dbHost = DBfetch(DBselect('SELECT h.status FROM hosts h WHERE h.hostid='.bs_dbstr($hostId)));
	...
	do {
		...
		foreach ($db_templates as $hostid => $db_template) {
			$hosts[$hostid] = [
				...
				'templateids' => bs_objectValues($db_template['parentTemplates'], 'templateid'),
				'macros' => []
			];
