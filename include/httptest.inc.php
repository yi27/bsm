...
function httptest_status2style($status) {
	$statuses = [
		HTTPTEST_STATUS_ACTIVE => BS_STYLE_GREEN,
		HTTPTEST_STATUS_DISABLED => BS_STYLE_RED
	];
...
function get_httptest_by_httptestid($httptestid) {
	return DBfetch(DBselect('SELECT ht.* FROM httptest ht WHERE ht.httptestid='.bs_dbstr($httptestid)));
}

function get_httpstep_by_no($httptestid, $no) {
	return DBfetch(DBselect('SELECT hs.* FROM httpstep hs WHERE hs.httptestid='.bs_dbstr($httptestid).' AND hs.no='.bs_dbstr($no)));
}
