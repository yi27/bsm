...
function check_screen_recursion($mother_screenid, $child_screenid) {
	...
	$db_scr_items = DBselect(
		...
		' WHERE si.screenid='.bs_dbstr($child_screenid).
...
function getSlideshowScreens($slideshowId, $step) {
	$dbSlides = DBfetch(DBselect(
		...
		' FROM slides s'.
		' WHERE s.slideshowid='.bs_dbstr($slideshowId)
	));
	...
	return DBfetch(DBselect(
		...
		' WHERE ss.slideshowid='.bs_dbstr($slideshowId).
			' AND sl.slideshowid=ss.slideshowid'.
			' AND sl.step='.bs_dbstr($currentStep)
	));
}
...
function slideshow_accessible($slideshowid, $perm) {
	...
	if (get_slideshow_by_slideshowid($slideshowid, PERM_READ)) {
		...
		$screens = bs_toHash($screens, 'screenid');
...
function get_slideshow_by_slideshowid($slideshowid, $permission) {
	...
	return DBfetch(DBselect(
		'SELECT s.* FROM slideshows s WHERE s.slideshowid='.bs_dbstr($slideshowid).$condition
	));
}
...
function add_slideshow($data) {
	...
	// Validate screens.
	$screenids = bs_objectValues($data['slides'], 'screenid');
	...
	$db_slideshow = DBfetch(DBselect(
		'SELECT s.slideshowid FROM slideshows s WHERE s.name='.bs_dbstr($data['name'])
	));
	...
	$result = DBexecute(
		...
		' VALUES ('.bs_dbstr($slideshowid).','.bs_dbstr($data['name']).','.bs_dbstr($data['delay']).','.
			bs_dbstr($data['userid']).','.bs_dbstr($data['private']).')'
	);
	...
	foreach ($data['slides'] as $slide) {
		...
		$result = DBexecute(
			...
			' VALUES ('.bs_dbstr($slideid).','.bs_dbstr($slideshowid).','.bs_dbstr($slide['screenid']).','.($i++).','.bs_dbstr($slide['delay']).')'
		);
...
function update_slideshow($data) {
	...
	$db_slideshow = DBfetch(DBselect(
		...
		' FROM slideshows s'.
		' WHERE s.name='.bs_dbstr($data['name']).
			' AND s.slideshowid<>'.bs_dbstr($data['slideshowid'])
	));
	...
	$db_slideshow['users'] = DBfetchArray(DBselect(
		...
		' WHERE s.slideshowid='.bs_dbstr(getRequest('slideshowid'))
	));
	...
	$user_shares_diff = bs_array_diff($data['users'], $db_slideshow['users'], 'userid');
	...
	$shared_userids_to_delete = bs_objectValues($user_shares_diff['second'], 'slideshowuserid');
	...
	$user_group_shares_diff = bs_array_diff($data['userGroups'], $db_slideshow['userGroups'], 'usrgrpid');
	...
	$shared_user_groupids_to_delete = bs_objectValues($user_group_shares_diff['second'], 'slideshowusrgrpid');
	...
	// get slides
	$db_slides = DBfetchArrayAssoc(DBselect('SELECT s.* FROM slides s WHERE s.slideshowid='.bs_dbstr($data['slideshowid'])), 'slideid');

	$slidesToDel = bs_objectValues($db_slides, 'slideid');
	$slidesToDel = bs_toHash($slidesToDel);
	$step = 0;
	foreach ($data['slides'] as $slide) {
		...
		if (isset($db_slides[$slide['slideid']])) {
			...
			if ($db_slides[$slide['slideid']]['delay'] != $slide['delay'] || $db_slides[$slide['slideid']]['step'] != $step) {
				$result = DBexecute('UPDATE slides SET step='.bs_dbstr($step).', delay='.bs_dbstr($slide['delay']).' WHERE slideid='.bs_dbstr($slide['slideid']));
			}
		...
		// insert slide
			...
			$result = DBexecute(
				'INSERT INTO slides (slideid,slideshowid,screenid,step,delay)'.
				' VALUES ('.bs_dbstr($slideid).','.bs_dbstr($data['slideshowid']).','.bs_dbstr($slide['screenid']).','.bs_dbstr($step).','.bs_dbstr($slide['delay']).')'
			);
		}
		$step ++;
		if (!$result) {
...
function delete_slideshow($slideshowid) {
	...
	if (get_slideshow_by_slideshowid($slideshowid, PERM_READ_WRITE)) {
		$result = DBexecute('DELETE FROM slideshows where slideshowid='.bs_dbstr($slideshowid));
		$result &= DBexecute('DELETE FROM slides where slideshowid='.bs_dbstr($slideshowid));
		$result &= DBexecute('DELETE FROM profiles WHERE idx=\'web.favorite.screenids\' AND source=\'slideshowid\' AND value_id='.bs_dbstr($slideshowid));
	}
...
function check_dynamic_items($elid, $config = 0) {
	if ($config == 0) {
		$sql = 'SELECT si.screenitemid'.
				' FROM screens_items si'.
				' WHERE si.screenid='.bs_dbstr($elid).
					' AND si.dynamic='.SCREEN_DYNAMIC_ITEM;
	}
	else {
		$sql = 'SELECT si.screenitemid'.
				' FROM slides s,screens_items si'.
				' WHERE s.slideshowid='.bs_dbstr($elid).
