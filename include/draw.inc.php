 ...
 */
function bs_colormix($image, $bgColor, $fgColor, $alpha) {
 ...
 */
function bs_imageline($image, $x1, $y1, $x2, $y2, $color) {
 ...
 */
function bs_imagealine($image, $x1, $y1, $x2, $y2, $color, $style = LINE_TYPE_NORMAL) {
	$x1 = round($x1);
	$y1 = round($y1);
	$x2 = round($x2);
	$y2 = round($y2);
	...
	if (abs($dx) > abs($dy)) {
		...
		for ($x = $x1, $y = $y1; $x <= $x2; $x++, $y = $y1 + ($x - $x1) * $dy / $dx) {
			...
			if (LINE_TYPE_BOLD == $style) {
				...
				imagesetpixel($image, $x, $yint - 1, bs_colormix($image, $lc, $bc, $yfrac));

				$bc = imagecolorsforindex($image, imagecolorat($image, $x, $yint + 1));
				$bc = [$bc['red'], $bc['green'], $bc['blue']];
				imagesetpixel($image, $x, $yint + 1, bs_colormix($image, $lc, $bc, 1 - $yfrac));
				...
				$bc = [$bc['red'], $bc['green'], $bc['blue']];
				imagesetpixel($image, $x, $yint, bs_colormix($image, $lc, $bc, $yfrac));
		...
		for ($y = $y1, $x = $x1; $y <= $y2; $y++, $x = $x1 + ($y - $y1) * $dx / $dy)
		{
			...
			if (LINE_TYPE_BOLD == $style) {
				...
				$bc = [$bc['red'], $bc['green'], $bc['blue']];
				imagesetpixel($image, $xint - 1, $y, bs_colormix($image, $lc, $bc, $xfrac));

				$bc = imagecolorsforindex($image, imagecolorat($image, $xint + 1, $y));
				$bc = [$bc['red'], $bc['green'], $bc['blue']];
				imagesetpixel($image, $xint + 1, $y, bs_colormix($image, $lc, $bc, 1 - $xfrac));
				...
				$bc = [$bc['red'], $bc['green'], $bc['blue']];
				imagesetpixel($image, $xint, $y, bs_colormix($image, $lc, $bc, $xfrac));
