...
function getGraphDims($graphid = null) {
	...
	$dbGraphs = DBselect(
		...
		' WHERE g.graphid='.bs_dbstr($graphid).
...
function get_hosts_by_graphid($graphid) {
	return DBselect(
		...
		' WHERE h.hostid=i.hostid'.
			...
			' AND gi.graphid='.bs_dbstr($graphid)
	);
}
...
function get_min_itemclock_by_graphid($graphid) {
	...
	$dbItems = DBselect(
		...
		' WHERE gi.graphid='.bs_dbstr($graphid)
	);
...
function get_min_itemclock_by_itemid($itemIds) {
	...
	if (!empty($itemTypes[ITEM_VALUE_TYPE_FLOAT]) || !empty($itemTypes[ITEM_VALUE_TYPE_UINT64])) {
		$itemIdsNumeric = bs_array_merge($itemTypes[ITEM_VALUE_TYPE_FLOAT], $itemTypes[ITEM_VALUE_TYPE_UINT64]);
	...
	foreach ($itemTypes as $type => $items) {
		...
		foreach ($itemIds as $itemId) {
			$sqlUnions[] = 'SELECT MIN(ht.clock) AS c FROM '.$sqlFrom.' ht WHERE ht.itemid='.bs_dbstr($itemId);
		}
...
function getGraphByGraphId($graphId) {
	$dbGraph = DBfetch(DBselect('SELECT g.* FROM graphs g WHERE g.graphid='.bs_dbstr($graphId)));
	...
	foreach ($gitems as $gitem) {
		$dbItem = DBfetch(DBselect(
			...
			' WHERE dest.key_=src.key_'.
				' AND dest.hostid='.bs_dbstr($destinationHostId).
				' AND src.itemid='.bs_dbstr($gitem['itemid']).
				$flagsSql
		));
...
function copyGraphToHost($graphId, $hostId) {
	...
	$graph['gitems'] = getSameGraphItemsForHost(
		...
		$hostId,
		true,
		[BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_CREATED]
	);
...
function navigation_bar_calc($idx = null, $idx2 = 0, $update = false) {
	if (!empty($idx)) {
		if ($update) {
			if (!empty($_REQUEST['period']) && $_REQUEST['period'] >= BS_MIN_PERIOD) {
		...
		$_REQUEST['period'] = getRequest('period', CProfile::get($idx.'.period', BS_PERIOD_DEFAULT, $idx2));
	...
	if ($_REQUEST['period'] < BS_MIN_PERIOD) {
		show_error_message(_n('Minimum time period to display is %1$s minute.',
			'Minimum time period to display is %1$s minutes.',
			(int) BS_MIN_PERIOD / SEC_PER_MIN
		));
		$_REQUEST['period'] = BS_MIN_PERIOD;
	}
	elseif ($_REQUEST['period'] > BS_MAX_PERIOD) {
		show_error_message(_n('Maximum time period to display is %1$s day.',
			'Maximum time period to display is %1$s days.',
			(int) BS_MAX_PERIOD / SEC_PER_DAY
		));
		$_REQUEST['period'] = BS_MAX_PERIOD;
	}

	if (!empty($_REQUEST['stime'])) {
		$time = bsDateToTime($_REQUEST['stime']);
...
function imageText($image, $fontsize, $angle, $x, $y, $color, $string) {
	if ((preg_match(BS_PREG_DEF_FONT_STRING, $string) && $angle != 0) || BS_FONT_NAME == BS_GRAPH_FONT_NAME) {
		$ttf = BS_FONTPATH.'/'.BS_FONT_NAME.'.ttf';
		imagettftext($image, $fontsize, $angle, $x, $y, $color, $ttf, $string);
	}
	elseif ($angle == 0) {
		$ttf = BS_FONTPATH.'/'.BS_GRAPH_FONT_NAME.'.ttf';
		imagettftext($image, $fontsize, $angle, $x, $y, $color, $ttf, $string);
	}
	else {
		$ttf = BS_FONTPATH.'/'.BS_GRAPH_FONT_NAME.'.ttf';
...
function imageTextSize($fontsize, $angle, $string) {
	if (preg_match(BS_PREG_DEF_FONT_STRING, $string) && $angle != 0) {
		$ttf = BS_FONTPATH.'/'.BS_FONT_NAME.'.ttf';
	}
	else {
		$ttf = BS_FONTPATH.'/'.BS_GRAPH_FONT_NAME.'.ttf';
...
function dashedLine($image, $x1, $y1, $x2, $y2, $color) {
	...
	bs_imageline($image, $x1, $y1, $x2, $y2, IMG_COLOR_STYLED);
}
...
function getBase1024Interval($interval, $minY, $maxY) {
	...
	if ($sideMaxData['pow'] != $intervalData['pow']) {
		...
		if ($intervalData['pow'] < 0) {
			...
			$interval = sprintf('%.6f', round(bcmul($interval, 1.024), BS_UNITS_ROUNDOFF_UPPER_LIMIT));
		}
	}
