 ...
 * @param bool   $debug turns On or Off trace calls when making connections. Suggested debug mode Off during Bsm setup
 *
 * @return bool
 */
		...
		switch ($DB['TYPE']) {
			...
			case BS_DB_POSTGRESQL:
				...
				if (!$DB['DB']) {
					...
					$schemaSet = DBexecute('SET search_path = '.bs_dbstr($DB['SCHEMA'] ? $DB['SCHEMA'] : 'public'), true);
			...
			case BS_DB_ORACLE:
				...
				if ($DB['DB']) {
					DBexecute('ALTER SESSION SET NLS_NUMERIC_CHARACTERS='.bs_dbstr('. '));
				}
				else {
					$ociError = oci_error();
			...
			case BS_DB_DB2:
				...
				if (!$DB['DB']) {
					...
					if (isset($DB['SCHEMA']) && $DB['SCHEMA'] != '') {
						DBexecute('SET CURRENT SCHEMA='.bs_dbstr($DB['SCHEMA']));
					}
				}
...
function DBaddLimit($query, $limit = 0, $offset = 0) {
	...
	if ((isset($limit) && ($limit < 0 || !bs_ctype_digit($limit))) || $offset < 0 || !bs_ctype_digit($offset)) {
...
function get_dbid($table, $field) {
	...
	if ($DB['TYPE'] == BS_DB_POSTGRESQL && $DB['TRANSACTIONS'] && !$DB['TRANSACTION_NO_FAILED_SQLS']) {
		return 0;
	}
	...
	do {
		$dbSelect = DBselect('SELECT i.nextid FROM ids i WHERE i.table_name='.bs_dbstr($table).' AND i.field_name='.bs_dbstr($field));
		if (!$dbSelect) {
			return false;
		}
		...
		if (!$row) {
			...
			if (bccomp($ret1, $min) < 0 || !bccomp($ret1, $max) < 0) {
				DBexecute('DELETE FROM ids WHERE table_name='.bs_dbstr($table).' AND field_name='.bs_dbstr($field));
				continue;
			}

			$sql = 'UPDATE ids SET nextid=nextid+1 WHERE table_name='.bs_dbstr($table).' AND field_name='.bs_dbstr($field);
			DBexecute($sql);

			$row = DBfetch(DBselect('SELECT i.nextid FROM ids i WHERE i.table_name='.bs_dbstr($table).' AND i.field_name='.bs_dbstr($field)));
...
function bs_db_distinct($sql_parts) {
...
function bs_db_search($table, $options, &$sql_parts) {
	...
	foreach ($options['search'] as $field => $patterns) {
		if (!isset($tableSchema['fields'][$field]) || bs_empty($patterns)) {
			continue;
		}
		...
		foreach ((array) $patterns as $pattern) {
			...
			if (!$options['searchWildcardsEnabled']) {
				$fieldSearch[] =
					...
					$exclude.' LIKE '.
					bs_dbstr($start.mb_strtoupper($pattern).'%').
					" ESCAPE '!'";
			}
			else {
				$pattern = str_replace("*", "%", $pattern);
				$fieldSearch[] =
					' UPPER('.$tableShort.'.'.$field.') '.
					$exclude.' LIKE '.
					bs_dbstr(mb_strtoupper($pattern)).
					" ESCAPE '!'";
			}
		}
...
function dbConditionInt($fieldName, array $values, $notIn = false, $sort = true) {
	...
	if ($betweens) {
		...
		foreach ($betweens as $between) {
			$between = $operatorNot.$fieldName.' BETWEEN '.bs_dbstr($between[0]).' AND '.bs_dbstr(end($between));
	...
	if ($dataSize == 1) {
		...
		$condition .= ($condition ? $operatorAnd : '').$fieldName.$operator.bs_dbstr($data[0]);
	}
	else {
		$operatorNot = $notIn ? ' NOT' : '';
...
function dbConditionString($fieldName, array $values, $notIn = false) {
	switch (count($values)) {
		...
		case 1:
			return $notIn
				? $fieldName.'!='.bs_dbstr(reset($values))
				: $fieldName.'='.bs_dbstr(reset($values));
	}
	...
	foreach ($items as $values) {
		...
		$condition .= implode(',', bs_dbstr($values));
	}
 ...
 * shared between Bsm front-end and Bsm Server.
 *
 * @return bool
 */
 ...
 * @return bool|string
 */
function bs_dbcast_2bigint($field) {
	global $DB;
