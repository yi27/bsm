...
function user_type2str($userType = null) {
	$userTypes = [
		USER_TYPE_BSM_USER => _('Bsm User'),
		USER_TYPE_BSM_ADMIN => _('Bsm Admin'),
		USER_TYPE_SUPER_ADMIN => _('Bsm Super Admin')
	];
...
function change_group_status($userGroupIds, $usersStatus) {
	...
	if ($grant) {
		return DBexecute(
			'UPDATE usrgrp'.
			' SET users_status='.bs_dbstr($usersStatus).
...
function change_group_gui_access($userGroupIds, $guiAccess) {
	...
	if ($grant) {
		return DBexecute(
			'UPDATE usrgrp SET gui_access='.bs_dbstr($guiAccess).' WHERE '.dbConditionInt('usrgrpid', $userGroupIds)
		);
	}
	else {
...
function change_group_debug_mode($userGroupIds, $debugMode) {
	...
	return DBexecute(
		'UPDATE usrgrp SET debug_mode='.bs_dbstr($debugMode).' WHERE '.dbConditionInt('usrgrpid', $userGroupIds)
	);
}
...
function getUserFullname($userData) {
	if (!bs_empty($userData['surname'])) {
		if (!bs_empty($userData['name'])) {
		...
		$fullname = bs_empty($userData['name']) ? '' : $userData['name'];
	}

	return bs_empty($fullname) ? $userData['alias'] : $userData['alias'].' ('.$fullname.')';
}
