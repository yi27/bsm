...
function getRegexp($regexpId) {
	return DBfetch(DBselect('SELECT re.* FROM regexps re WHERE regexpid='.bs_dbstr($regexpId)));
}
...
function getRegexpExpressions($regexpId) {
	...
	$dbExpressions = DBselect(
		...
		' FROM expressions e'.
		' WHERE regexpid='.bs_dbstr($regexpId)
	);
...
function addRegexp(array $regexp, array $expressions) {
	try {
		...
		$sql = 'SELECT re.regexpid'.
				' FROM regexps re'.
				' WHERE re.name='.bs_dbstr($regexp['name']);
...
/**
 * Rewrite Bsm regexp expressions.
...
function rewriteRegexpExpressions($regexpId, array $expressions) {
	...
	if ($dbExpressions) {
		$dbExpressionIds = bs_objectValues($dbExpressions, 'expressionid');
