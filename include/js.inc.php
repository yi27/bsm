 ...
 * @return string
 */
function bs_jsvalue($value, $asObject = false, $addQuotes = true) {
	if (!is_array($value)) {
		if (is_object($value)) {
	...
	foreach ($value as $id => $v) {
		...
		$value[$id] = (isset($is_object) ? '"'.str_replace('\'', '\\\'', $id).'":' : '').bs_jsvalue($v, $asObject, $addQuotes);
	}
...
function insert_show_color_picker_javascript() {
	...
	$table[] = (new CDiv($row))->addClass(BS_STYLE_COLOR_PICKER);
	...
	insert_js('var color_picker = null,'."\n".
		'curr_lbl = null,'."\n".
		'curr_txt = null,'."\n".
		'color_table = '.bs_jsvalue(unpack_object($tmp))."\n");
	bs_add_post_js('create_color_picker();');
}
...
function bs_add_post_js($script) {
	global $BS_PAGE_POST_JS;

	if ($BS_PAGE_POST_JS === null) {
		$BS_PAGE_POST_JS = [];
	}

	if (!in_array($script, $BS_PAGE_POST_JS)) {
		$BS_PAGE_POST_JS[] = $script;
	}
}
