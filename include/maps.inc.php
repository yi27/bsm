...
function getActionMapBySysmap($sysmap, array $options = []) {
	$sysmap['selements'] = bs_toHash($sysmap['selements'], 'selementid');
	$sysmap['links'] = bs_toHash($sysmap['links'], 'linkid');
...
function myDrawLine($image, $x1, $y1, $x2, $y2, $color, $drawtype) {
	if ($drawtype == MAP_LINK_DRAWTYPE_BOLD_LINE) {
		bs_imagealine($image, $x1, $y1, $x2, $y2, $color, LINE_TYPE_BOLD);
	}
	...
	elseif ($drawtype == MAP_LINK_DRAWTYPE_DOT && function_exists('imagesetstyle')) {
		...
		bs_imagealine($image, $x1, $y1, $x2, $y2, $color);
	}
}
	...
	switch ($db_element['elementtype']) {
		...
		case SYSMAP_ELEMENT_TYPE_MAP:
			$db_mapselements = DBselect(
				...
				' WHERE se.sysmapid='.bs_dbstr($db_element['elementid'])
			);
...
function getSelementsInfo($sysmap, array $options = []) {
	...
	$allHosts = bs_toHash($allHosts, 'hostid');
	...
	$allTriggers = bs_toHash($allTriggers, 'triggerid');
	...
	if (!empty($elems['sysmaps']) && $mlabel) {
		$subSysmaps = API::Map()->get([
			'sysmapids' => bs_objectValues($elems['sysmaps'], 'elementid'),
			'nopermissions' => true,
			'output' => ['name']
		]);
		$subSysmaps = bs_toHash($subSysmaps, 'sysmapid');
	...
	if (!empty($elems['hostgroups']) && $hglabel) {
		$hostgroups = API::HostGroup()->get([
			'groupids' => bs_objectValues($elems['hostgroups'], 'elementid'),
			'nopermissions' => true,
			'output' => ['name']
		]);
		$hostgroups = bs_toHash($hostgroups, 'groupid');
	...
	foreach ($map['links'] as $link) {
		...
		if (!empty($linktriggers)) {
			...
			foreach ($linktriggers as $link_trigger) {
				...
				$triggers[$id] = bs_array_merge($link_trigger, get_trigger_by_triggerid($link_trigger['triggerid']));
...
function drawMapLabels(&$im, $map, $mapInfo, $resolveMacros, $graphtheme) {
	...
	foreach ($selements as $selementId => $selement) {
		...
		foreach (['problem', 'unack', 'maintenance', 'ok', 'status'] as $caption) {
			if (!isset($elementInfo['info'][$caption]) || bs_empty($elementInfo['info'][$caption]['msg'])) {
				continue;
			}
	...
	if (!empty($elementsHostIds)) {
		...
		$mapHosts = bs_toHash($mapHosts, 'hostid');
	}
