...
function get_service_children($serviceid, $soft = 0) {
	...
	$result = DBselect(
		...
		' WHERE sl.serviceupid='.bs_dbstr($serviceid).
			($soft ? '' : ' AND sl.soft=0')
	);
...
function createServiceMonitoringTree(array $services, array $slaData, $period, &$tree, array $parentService = [], array $service = [], array $dependency = []) {
	...
	// create a not from the given service
		...
		$caption = new CLink($service['name'],
			'bsm.php?action=report.services'.'&serviceid='.$service['serviceid'].'&period='.$periods[$period]
		);
		...
		if ($service['showsla'] && $slaValues['sla'] !== null) {
			...
			$sla = (new CDiv(
				new CLink([
					(new CSpan([new CSpan('80%'), new CSpan('100%')]))->addClass(BS_STYLE_PROGRESS_BAR_LABEL),
					$width_green > 0
						? (new CSpan('&nbsp;'))
							->addClass(BS_STYLE_PROGRESS_BAR_BG)
							->addClass(BS_STYLE_GREEN_BG)
							->setAttribute('style', 'width: '.$width_green.'px;')
						: null,
					$width_red > 0
						? (new CSpan('&nbsp;'))
							->addClass(BS_STYLE_PROGRESS_BAR_BG)
							->addClass(BS_STYLE_RED_BG)
							->setAttribute('style', 'width: '.$width_red.'px;')
						: null
				], 'srv_status.php?serviceid='.$service['serviceid'].'&showgraph=1'.url_param('path'))
			))
				->addClass(BS_STYLE_PROGRESS_BAR_CONTAINER)
				->setAttribute('title', _s('Only the last 20%% of the indicator is displayed.'));

			$sla2 = (new CSpan(sprintf('%.4f', $sla_bad)))
				->addClass($service['goodsla'] > $sla_good ? BS_STYLE_RED : BS_STYLE_GREEN);
...
function checkServiceTime(array $serviceTime) {
	...
	if (!isset($serviceTime['type']) || !in_array($serviceTime['type'], $serviceTypes)) {
		throw new APIException(BS_API_ERROR_PARAMETERS, _('Incorrect service time type.'));
	}
	...
	if ($serviceTime['type'] == SERVICE_TIME_TYPE_ONETIME_DOWNTIME) {
		if (!isset($serviceTime['ts_from']) || !validateUnixTime($serviceTime['ts_from'])) {
			throw new APIException(BS_API_ERROR_PARAMETERS, _('Incorrect service start time.'));
		}
		if (!isset($serviceTime['ts_to']) || !validateUnixTime($serviceTime['ts_to'])) {
			throw new APIException(BS_API_ERROR_PARAMETERS, _('Incorrect service end time.'));
		}
	}
	// recurring downtime validation
	else {
		if (!isset($serviceTime['ts_from']) || !bs_is_int($serviceTime['ts_from']) || $serviceTime['ts_from'] < 0 || $serviceTime['ts_from'] > SEC_PER_WEEK) {
			throw new APIException(BS_API_ERROR_PARAMETERS, _('Incorrect service start time.'));
		}
		if (!isset($serviceTime['ts_to']) || !bs_is_int($serviceTime['ts_to']) || $serviceTime['ts_to'] < 0 || $serviceTime['ts_to'] > SEC_PER_WEEK) {
			throw new APIException(BS_API_ERROR_PARAMETERS, _('Incorrect service end time.'));
		}
	}

	if ($serviceTime['ts_from'] >= $serviceTime['ts_to']) {
		throw new APIException(BS_API_ERROR_PARAMETERS, _('Service start time must be less than end time.'));
	}
}
