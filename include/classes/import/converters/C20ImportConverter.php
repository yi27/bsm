...
class C20ImportConverter extends CConverter {
	...
	public function convert($data) {
		$data['bsm_export']['version'] = '3.0';

		if (array_key_exists('hosts', $data['bsm_export'])) {
			$data['bsm_export']['hosts'] = $this->convertHosts($data['bsm_export']['hosts']);
		}
		if (array_key_exists('templates', $data['bsm_export'])) {
			$data['bsm_export']['templates'] = $this->convertTemplates($data['bsm_export']['templates']);
		}
		if (array_key_exists('graphs', $data['bsm_export'])) {
			$data['bsm_export']['graphs'] = $this->convertGraphs($data['bsm_export']['graphs']);
		}
		if (array_key_exists('triggers', $data['bsm_export'])) {
			$data['bsm_export']['triggers'] = $this->convertTriggers($data['bsm_export']['triggers']);
		}
		if (array_key_exists('screens', $data['bsm_export'])) {
			$data['bsm_export']['screens'] = $this->convertScreens($data['bsm_export']['screens']);
		}
		if (array_key_exists('maps', $data['bsm_export'])) {
			$data['bsm_export']['maps'] = $this->convertMaps($data['bsm_export']['maps']);
		}
	...
	protected function convertScreenItems(array $screen_items) {
		foreach ($screen_items as &$screen_item) {
			...
			if (bs_is_int($screen_item['resourcetype'])) {
	...
	protected function convertGraphs(array $graphs) {
		foreach ($graphs as &$graph) {
			...
			if (bs_is_int($graph['ymin_type_1'])) {
			...
			if (bs_is_int($graph['ymax_type_1'])) {
