...
/**
 * A class for interacting with the Bsm server.
 *
 * Class CBsmServer
 */
class CBsmServer {
	...
	/**
	 * Returns true if the Bsm server is running and false otherwise.
	 *
	 * @return bool
	 */
	...
	protected function request(array $params) {
		...
		if (fwrite($this->socket, CJs::encodeJson($params)) === false) {
			$this->error = _s('Cannot send command, check connection with Bsm server "%1$s".', $this->host);
		...
		while (!feof($this->socket)) {
			...
			if ((time() - $now) >= $this->timeout) {
				$this->error = _s('Connection timeout of %1$s seconds exceeded when connecting to Bsm server "%2$s".', $this->timeout, $this->host);

				return false;
			}
			elseif ($this->totalBytesLimit && ($i * $readBytesLimit) >= $this->totalBytesLimit) {
				$this->error = _s('Size of the response received from Bsm server "%1$s" exceeds the allowed size of %2$s bytes. This value can be increased in the BS_SOCKET_BYTES_LIMIT constant in include/defines.inc.php.', $this->host, $this->totalBytesLimit);
			...
			if (($out = fread($this->socket, $readBytesLimit)) !== false) {
				...
				$this->error = _s('Cannot read the response, check connection with the Bsm server "%1$s".', $this->host);
		...
		if (!strlen($response)) {
			$this->error = _s('Empty response received from Bsm server "%1$s".', $this->host);
		...
		if (!$response || !$this->validateResponse($response)) {
			$this->error = _s('Incorrect response received from Bsm server "%1$s".', $this->host);
	...
	/**
	 * Opens a socket to the Bsm server. Returns the socket resource if the connection has been established or
	 * false otherwise.
	 *
			...
			if (!$socket = @fsockopen($this->host, $this->port, $errorCode, $errorMsg, $this->timeout)) {
				switch ($errorMsg) {
					case 'Connection refused':
						$dErrorMsg = _s("Connection to Bsm server \"%s\" refused. Possible reasons:\n1. Incorrect server IP/DNS in the \"bsm.conf.php\";\n2. Security environment (for example, SELinux) is blocking the connection;\n3. Bsm server daemon not running;\n4. Firewall is blocking TCP connection.\n", $this->host);
						break;

					case 'No route to host':
						$dErrorMsg = _s("Bsm server \"%s\" can not be reached. Possible reasons:\n1. Incorrect server IP/DNS in the \"bsm.conf.php\";\n2. Incorrect network configuration.\n", $this->host);
						break;

					case 'Connection timed out':
						$dErrorMsg = _s("Connection to Bsm server \"%s\" timed out. Possible reasons:\n1. Incorrect server IP/DNS in the \"bsm.conf.php\";\n2. Firewall is blocking TCP connection.\n", $this->host);
						break;

					default:
						$dErrorMsg = _s("Connection to BSM server \"%s\" failed. Possible reasons:\n1. Incorrect server IP/DNS in the \"bsm.conf.php\";\n2. Incorrect DNS server configuration.\n", $this->host);
				}
	...
	/**
	 * Returns true if the response received from the Bsm server is valid.
	 *
