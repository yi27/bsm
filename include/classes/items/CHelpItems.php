	...
	protected function getItems() {
		return [
			ITEM_TYPE_BSM => [
				...
				[
					'key' => 'agent.version',
					'description' => _('Version of Bsm agent. Returns string')
				],
				[
					'key' => 'kernel.maxfiles',
			...
			ITEM_TYPE_AGGREGATE => [
				[
					...
					'description' => _('Aggregate checks do not require any agent running on a host being monitored. Bsm server collects aggregate information by doing direct database queries. See Bsm Manual.')
				]
			],
			ITEM_TYPE_SIMPLE => [
				[
			...
			ITEM_TYPE_INTERNAL => [
				[
					'key' => 'bsm[boottime]',
					'description' => _('Startup time of Bsm server, Unix timestamp.')
				],
				[
					'key' => 'bsm[history]',
				...
				[
					'key' => 'bsm[items]',
					'description' => _('Number of items in Bsm database.')
				],
				[
					'key' => 'bsm[items_unsupported]',
					'description' => _('Number of unsupported items in Bsm database.')
				],
				[
					'key' => 'bsm[java,,<param>]',
					'description' => _('Returns information associated with Bsm Java gateway. Valid params are: ping, version.')
				],
				[
					'key' => 'bsm[process,<type>,<num>,<state>]',
					'description' => _('Time a particular Bsm process or a group of processes (identified by <type> and <num>) spent in <state> in percentage.')
				],
				[
					'key' => 'bsm[proxy,<name>,<param>]',
					'description' => _('Time of proxy last access. Name - proxy name. Param - lastaccess. Unix timestamp.')
				],
				[
					'key' => 'bsm[proxy_history]',
					'description' => _('Number of items in proxy history that are not yet sent to the server')
				],
				[
					'key' => 'bsm[queue,<from>,<to>]',
					'description' => _('Number of items in the queue which are delayed by from to to seconds, inclusive.')
				],
				[
					'key' => 'bsm[rcache,<cache>,<mode>]',
					'description' => _('Configuration cache statistics. Cache - buffer (modes: pfree, total, used, free).')
				],
				[
					'key' => 'bsm[requiredperformance]',
					'description' => _('Required performance of the Bsm server, in new values per second expected.')
				],
				[
					'key' => 'bsm[trends]',
