	...
	public function __construct($name, $value, $insert_color_picker = true) {
		parent::__construct([
			(new CColorCell('lbl_'.$name, $value))
				->setTitle('#'.$value)
				->onClick('javascript: show_color_picker("'.bs_formatDomId($name).'")'),
			(new CTextBox($name, $value))
				->setWidth(BS_TEXTAREA_COLOR_WIDTH)
				->setAttribute('maxlength', 6)
				->onChange('set_color_by_name("'.bs_formatDomId($name).'", this.value)')
		]);

		$this->addClass(BS_STYLE_INPUT_COLOR_PICKER);
