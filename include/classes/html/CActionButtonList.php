...
class CActionButtonList extends CObject {
	...
	public function getSelectedCountElement() {
		if (!$this->selectedCountElement) {
			$this->selectedCountElement = (new CSpan('0 '._('selected')))
				...
				->addClass(BS_STYLE_SELECTED_ITEM_COUNT);
		}
	...
	public function toString($destroy = true) {
		bs_add_post_js('chkbxRange.pageGoName = '.CJs::encodeJson($this->checkboxesName).';');
		bs_add_post_js('chkbxRange.prefix = '.CJs::encodeJson($this->cookieNamePrefix).';');
		...
		$this->items[] = (new CDiv([$this->getSelectedCountElement(), $items]))
			...
			->addClass(BS_STYLE_ACTION_BUTTONS);
