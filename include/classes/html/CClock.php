...
class CClock extends CDiv {
	...
	private function makeClockHands() {
		return [
			...
			(new CTag('rect', true))
				...
				->addClass(BS_STYLE_CLOCK_HAND_SEC),
			(new CTag('circle', true))
				->setAttribute('cx', '50')
				->setAttribute('cy', '50')
				->setAttribute('r', '3.5')
	...
	private function build() {
		...
		if ($this->error !== null) {
			$clock->addClass(BS_STYLE_DISABLED);
		}

		$this->addItem([
			(new CDiv($this->error))
				->addClass(BS_STYLE_TIME_ZONE)
				->addClass($this->error !== null ? BS_STYLE_RED : BS_STYLE_GREY),
			$clock,
			(new CDiv($this->footer))
				->addClass(BS_STYLE_LOCAL_CLOCK)
		...
		if (!defined('BS_CLOCK') && $this->error === null) {
			define('BS_CLOCK', 1);
...
jQuery(function($) {
	...
	$.fn.bs_clock = function(options) {
		...
		if ($this->error === null) {
			bs_add_post_js('jQuery("#'.$this->getId().'").bs_clock('.CJs::encodeJson($options).');');
		}
	}
