...
class CRadioButtonList extends CList {
	...
	public function __construct($name, $value) {
		...
		$this->values = [];
		$this->modern = false;
		$this->setId(bs_formatDomId($name));
	}
	...
	public function addValue($name, $value, $id = null, $on_change = null) {
		$this->values[] = [
			'name' => $name,
			'value' => $value,
			'id' => ($id === null ? null : bs_formatDomId($id)),
			'on_change' => $on_change
		];
	...
	public function toString($destroy = true) {
		if ($this->modern) {
			...
			$this->addClass($this->orientation == self::ORIENTATION_HORIZONTAL
				? BS_STYLE_LIST_HOR_CHECK_RADIO
				: BS_STYLE_LIST_CHECK_RADIO
			);
		}

		foreach ($this->values as $key => $value) {
			if ($value['id'] === null) {
				$value['id'] = bs_formatDomId($this->name).'_'.$key;
			}
