...
class CFormList extends CList {
	...
	public function addRow($term, $description = null, $id = null, $class = null) {
		...
		if ($description === null) {
			$this->addItem([
				(new CDiv(SPACE))->addClass(BS_STYLE_TABLE_FORMS_TD_LEFT),
				(new CDiv($label))->addClass(BS_STYLE_TABLE_FORMS_TD_RIGHT)],
				$class, $id);
		}
		else {
			$this->addItem([
				(new CDiv($label))->addClass(BS_STYLE_TABLE_FORMS_TD_LEFT),
				(new CDiv($description))->addClass(BS_STYLE_TABLE_FORMS_TD_RIGHT)],
				$class, $id);
		}
	...
	public function addInfo($text) {
		$this->addItem(
			[
				(new CDiv(_('Info')))->addClass(BS_STYLE_TABLE_FORMS_TD_LEFT),
				(new CDiv(
					(new CDiv($text))->addClass(BS_STYLE_TABLE_FORMS_SEPARATOR)
