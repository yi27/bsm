...
class CFilter extends CTag {
	...
	public function addColumn($column) {
		$this->columns[] = (new CDiv($column))->addClass(BS_STYLE_CELL);
		return $this;
	}
	...
	private function getHeader() {
		...
		if ($this->opened) {
			$span->addClass(BS_STYLE_ARROW_UP);
			$button = (new CSimpleButton(
				[_('Filter'), $span]
			))
				->addClass(BS_STYLE_FILTER_TRIGGER)
				->addClass(BS_STYLE_FILTER_ACTIVE)
				->setId('filter-mode');
		}
		else {
			$span->addClass(BS_STYLE_ARROW_DOWN);
			$button = (new CSimpleButton(
				[_('Filter'), $span]
			))
				->addClass(BS_STYLE_FILTER_TRIGGER)
		...
		$switch = (new CDiv())
			->addClass(BS_STYLE_FILTER_BTN_CONTAINER)
			->addItem($button);
	...
	private function getTable() {
		...
		$row = (new CDiv())->addClass(BS_STYLE_ROW);
		foreach ($this->columns as $column) {
			$row->addItem($column);
		}

		return (new CDiv())
			->addClass(BS_STYLE_TABLE)
			->addClass(BS_STYLE_FILTER_FORMS)
			->addItem($row);
	}
