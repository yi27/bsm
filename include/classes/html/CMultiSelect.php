...
class CMultiSelect extends CTag {
	...
	public function __construct(array $options = []) {
		...
		$this->setId(bs_formatDomId($options['name']));
		...
		if (array_key_exists('data', $options)) {
			$params['data'] = bs_cleanHashes($options['data']);
		}
		...
		bs_add_post_js('jQuery("#'.$this->getAttribute('id').'").multiSelect('.CJs::encodeJson($params).');');
	}
