...
class CVisibilityBox extends CCheckBox {
	...
	public function __construct($name = 'visibilitybox', $object_name = null, $replace_to = null) {
		...
		foreach ($this->object_name as $obj_name) {
			...
			$action .= 'visibility_status_changeds(this.checked, '.bs_jsvalue($obj_name).', '.bs_jsvalue($this->replace_to).');';
		}
	...
	public function toString($destroy = true) {
		if (!isset($this->attributes['checked'])) {
			foreach ($this->object_name as $obj_name) {
				...
				bs_add_post_js('visibility_status_changeds(false, '.bs_jsvalue($obj_name).', '.bs_jsvalue($this->replace_to).');');
			}
		}
