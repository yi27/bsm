...
class CPageFilter {
	...
	public function __construct(array $options = []) {
		...
		if (isset($options['config']['allow_all'])) {
			$this->config['DDFirst'] = BS_DROPDOWN_FIRST_ALL;
		}
		elseif (isset($options['config']['deny_all'])) {
			$this->config['DDFirst'] = BS_DROPDOWN_FIRST_NONE;
		}
		else {
	...
	private function _updateByGraph(array &$options) {
		...
		if ($graph = reset($graphs)) {
			$groups = bs_toHash($graph['groups'], 'groupid');
			$hosts = bs_toHash($graph['hosts'], 'hostid');
			$templates = bs_toHash($graph['templates'], 'templateid');
	...
	private function _initGroups($groupId, array $options, $hostId) {
		...
		$options = bs_array_merge($defaultOptions, $options);
		...
		if ((!isset($this->data['groups'][$groupId]) && $groupId > 0) || $groupId === null) {
			...
			// Otherwise for 'Dropdown first entry' option ALL or NONE.
				...
				if ($this->config['DDFirst'] == BS_DROPDOWN_FIRST_NONE && $hostId) {
					$groupids = [];
					...
					if ($hosts) {
						$host = reset($hosts);
						$groupids = bs_objectValues($host['groups'], 'groupid');
					}
					else {
						...
						$groupids = bs_objectValues($template['groups'], 'groupid');
					}
		...
		$firstIsAllAndHaveGroups = (($this->config['DDFirst'] == BS_DROPDOWN_FIRST_ALL) && $this->data['groups']);
	...
	private function _initHosts($hostId, array $options) {
		...
		if (!$this->groupsSelected) {
			...
			$hosts = API::Host()->get(bs_array_merge($defaultOptions, $options));
		...
		$this->isSelected['hostsSelected'] = (($this->config['DDFirst'] == BS_DROPDOWN_FIRST_ALL && !empty($this->data['hosts'])) || $hostId > 0);
		$this->isSelected['hostsAll'] = ($this->config['DDFirst'] == BS_DROPDOWN_FIRST_ALL && !empty($this->data['hosts']) && $hostId == 0);
	...
	private function _initGraphs($graphid, array $options) {
		...
		if (!$this->hostsSelected) {
			...
			$options = bs_array_merge($def_ptions, $options);
	...
	private function _initDiscoveries($druleid, array $options) {
		...
		$options = bs_array_merge($def_options, $options);
		...
		if ((!isset($this->data['drules'][$druleid]) && $druleid > 0) || is_null($druleid)) {
			if ($this->config['DDFirst'] == BS_DROPDOWN_FIRST_NONE) {
				$druleid = 0;
			}
		...
		$this->isSelected['drulesSelected'] = ($this->config['DDFirst'] == BS_DROPDOWN_FIRST_ALL && !empty($this->data['drules'])) || $druleid > 0;
		$this->isSelected['drulesAll'] = $this->config['DDFirst'] == BS_DROPDOWN_FIRST_ALL && !empty($this->data['drules']) && $druleid == 0;
	...
	public function getHostsCB() {
		...
		foreach ($this->hosts as $id => $host) {
			$items[$id] = $host['name'];
			$classes[$id] = ($host['status'] == HOST_STATUS_NOT_MONITORED) ? BS_STYLE_RED : null;
		}
	...
	private function _getCB($name, $selectedId, $items, array $options = []) {
		...
		if (!$this->config['popupDD']) {
			if (isset($this->config['DDFirstLabels'][$options['objectName']])) {
				...
				$firstLabel = ($this->config['DDFirst'] == BS_DROPDOWN_FIRST_NONE) ? _('not selected') : _('all');
			}
