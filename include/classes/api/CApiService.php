...
class CApiService {
	...
	protected function createSelectQueryParts($tableName, $tableAlias, array $options) {
		...
		$options = bs_array_merge($this->globalGetOptions, $options);
	...
	protected function createSelectQueryFromParts(array $sqlParts) {
		...
		return 'SELECT '.bs_db_distinct($sqlParts).' '.$sqlSelect.
				' FROM '.$sqlFrom.
				$sqlWhere.
				$sqlGroup.
				$sqlOrder;
	...
	protected function applyQueryFilterOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		...
		if (isset($options[$pkOption])) {
			bs_value2array($options[$pkOption]);
		...
		if (is_array($options['search'])) {
			bs_db_search($tableId, $options, $sqlParts);
		}
	...
	protected function applyQuerySortOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		if ($this->sortColumns && !bs_empty($options['sortfield'])) {
			...
			foreach ($options['sortfield'] as $i => $sortfield) {
				...
				if (!str_in_array($sortfield, $this->sortColumns)) {
					throw new APIException(BS_API_ERROR_INTERNAL, _s('Sorting by field "%1$s" not allowed.', $sortfield));
				}
				...
				if (is_array($options['sortorder'])) {
					if (!empty($options['sortorder'][$i])) {
						$sortorder = ($options['sortorder'][$i] == BS_SORT_DOWN) ? ' '.BS_SORT_DOWN : '';
					}
				}
				else {
					$sortorder = ($options['sortorder'] == BS_SORT_DOWN) ? ' '.BS_SORT_DOWN : '';
				}
	 ...
	 * @param string $fieldId
	 * @param array  $sqlParts
	 * @param string $sortorder		sort direction, BS_SORT_UP or BS_SORT_DOWN
	 *
	 * @return array
	 */
	...
	protected function extendObjects($tableName, array $objects, array $fields) {
		if ($objects) {
			$dbObjects = API::getApiService()->select($tableName, [
				'output' => $fields,
				$this->pkOption($tableName) => bs_objectValues($objects, $this->pk($tableName)),
				'preservekeys' => true
			]);
	...
	protected function checkObjectIds(array $objects, $idField, $messageRequired, $messageEmpty, $messageInvalid) {
		...
		foreach ($objects as $object) {
			if (!isset($object[$idField])) {
				self::exception(BS_API_ERROR_PARAMETERS, _params($messageRequired, [$idField]));
			}
	 ...
	 * @param string $error
	 */
	protected static function exception($code = BS_API_ERROR_INTERNAL, $error = '') {
	...
	protected function checkValidator($value, CValidator $validator) {
		if (!$validator->validate($value)) {
			self::exception(BS_API_ERROR_INTERNAL, $validator->getError());
		}
	}
