...
class CLocalApiClient extends CApiClient {
	...
	public function callMethod($requestApi, $requestMethod, $params, $auth) {
		...
		if (!$this->isValidApi($api)) {
			$response->errorCode = BS_API_ERROR_PARAMETERS;
		...
		try {
			...
			$response->errorCode = ($e instanceof APIException) ? $e->getCode() : BS_API_ERROR_INTERNAL;
	...
	protected function authenticate($auth) {
		if (bs_empty($auth)) {
			throw new APIException(BS_API_ERROR_NO_AUTH, _('Not authorised.'));
		}
