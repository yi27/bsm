...
abstract class CMapElement extends CApiService {
	...
	protected function checkSelementInput(&$selements, $method) {
		...
		if ($update || $delete) {
			...
			$dbSelements = $this->fetchSelementsByIds(bs_objectValues($selements, 'selementid'));
		...
		foreach ($selements as &$selement) {
			if (!check_db_fields($selementDbFields, $selement)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Wrong fields for element.'));
			}
			...
			if (isset($selement['iconid_off']) && $selement['iconid_off'] == 0) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('No icon for map element "%s".', $selement['label']));
			}

			if ($this->checkCircleSelementsLink($dbSelement['sysmapid'], $dbSelement['elementid'], $dbSelement['elementtype'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Circular link cannot be created for map element "%s".', $dbSelement['label']));
			}
		}
		unset($selement);
	...
	protected function checkLinkInput($links, $method) {
		...
		if ($update || $delete) {
			...
			$dbLinks = API::getApiService()->select('sysmap_element_url', [
				'filter' => ['selementid' => bs_objectValues($links, 'linkid')],
				'output' => ['linkid'],
				'preservekeys' => true
			]);
		}
		...
		foreach ($links as $link) {
			if (!check_db_fields($linkDbFields, $link)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Wrong fields for map link.'));
			}

			if (isset($link['color']) && !$colorValidator->validate($link['color'])) {
				self::exception(BS_API_ERROR_PARAMETERS, $colorValidator->getError());
			}
	...
	public function checkCircleSelementsLink($sysmapId, $elementId, $elementType) {
		...
		$dbElements = DBselect(
			...
			' WHERE se.sysmapid='.bs_dbstr($elementId).
	...
	protected function createSelements(array $selements) {
		$selements = bs_toArray($selements);
	...
	protected function updateSelements(array $selements) {
		...
		foreach ($selements as $selement) {
			...
			$diffUrls = bs_array_diff($selement['urls'], $dbSelements[$selement['selementid']]['urls'], 'name');
			...
			// delete url
			$urlsToDelete = array_merge($urlsToDelete, bs_objectValues($diffUrls['second'], 'sysmapelementurlid'));
		}
	...
	protected function deleteSelements(array $selements) {
		$selements = bs_toArray($selements);
		$selementIds = bs_objectValues($selements, 'selementid');
	...
	protected function updateLinks($links) {
		...
		return ['linkids' => bs_objectValues($links, 'linkid')];
	}
	...
	protected function deleteLinks($links) {
		bs_value2array($links);
		$linkIds = bs_objectValues($links, 'linkid');
	...
	protected function createLinkTriggers($linkTriggers) {
		$linkTriggers = bs_toArray($linkTriggers);
	...
	protected function validateCreateLinkTriggers(array $linkTriggers) {
		...
		foreach ($linkTriggers as $linkTrigger) {
			if (!check_db_fields($linkTriggerDbFields, $linkTrigger)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Wrong fields for linktrigger.'));
			}
	...
	protected function updateLinkTriggers($linkTriggers) {
		...
		$linkTriggerIds = bs_objectValues($linkTriggers, 'linktriggerid');
	...
	protected function validateUpdateLinkTriggers(array $linkTriggers) {
		...
		foreach ($linkTriggers as $linkTrigger) {
			if (!check_db_fields($linkTriggerDbFields, $linkTrigger)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Wrong fields for linktrigger update.'));
			}
	...
	protected function validateDeleteLinkTriggers(array $linkTriggers) {
		...
		foreach ($linkTriggers as $linkTrigger) {
			if (!check_db_fields($linktriggerDbFields, $linkTrigger)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Wrong fields for linktrigger delete.'));
			}
		}
	}
}
