...
class CUserGroup extends CApiService {
	...
	public function get($options = []) {
		...
		if (!is_null($options['status'])) {
			$sqlParts['where'][] = 'g.users_status='.bs_dbstr($options['status']);
		}
		...
		if (is_array($options['search'])) {
			bs_db_search('usrgrp g', $options, $sqlParts);
		}
	...
	public function create($usrgrps) {
		if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only Super Admins can delete user groups.'));
		}

		$usrgrps = bs_toArray($usrgrps);
		$insert = [];
		...
		foreach ($usrgrps as $gnum => $usrgrp) {
			...
			if (!check_db_fields($usrgrpDbFields, $usrgrp)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect parameters for user group.'));
			}
			...
			if ($userGroupExists) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('User group "%1$s" already exists.', $usrgrp['name']));
			}
			$insert[$gnum] = $usrgrp;
		}
		...
		foreach ($usrgrps as $gnum => $usrgrp) {
			...
			if (!empty($massAdd)) {
				...
				if (!$this->massAdd($massAdd))
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot add users.'));
			}
		}
	...
	public function update($usrgrps) {
		...
		foreach ($usrgrps as $usrgrp) {
			...
			if (!$this->massUpdate($usrgrp)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot update group.'));
			}
		}

		return ['usrgrpids'=> bs_objectValues($usrgrps, 'usrgrpid')];
	}
	...
	public function massAdd($data) {
		...
		$usrgrpids = array_keys(array_flip(bs_toArray($data['usrgrpids'])));
		$userids = (isset($data['userids']) && !is_null($data['userids'])) ? bs_toArray($data['userids']) : null;
		$rights = (isset($data['rights']) && !is_null($data['rights'])) ? bs_toArray($data['rights']) : null;
		...
		if (!is_null($userids)) {
			...
			foreach ($usrgrps as $usrgrp) {
				if ((($usrgrp['gui_access'] == GROUP_GUI_ACCESS_DISABLED)
					...
					self::exception(BS_API_ERROR_PARAMETERS, _('User cannot change status of himself'));
				}
			}
		...
		if (!is_null($rights)) {
			...
			$sql = 'SELECT groupid,id'.
				...
				' WHERE '.dbConditionInt('groupid', $usrgrpids).
					' AND '.dbConditionInt('id', bs_objectValues($rights, 'id'));
	...
	public function massUpdate($data) {
		...
		$usrgrpids = bs_toArray($data['usrgrpids']);

		if (count($usrgrpids) == 0) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Missing parameter: usrgrpids.'));
		}
		...
		if (isset($data['name'])) {
			...
			if (count($usrgrpids) > 1) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Only one user group name can be changed at a time.'));
			}
			else {
				...
				if ($groupExists && (bccomp($groupExists['usrgrpid'], $usrgrpids[0]) != 0) ) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('User group "%s" already exists.', $data['name']));
				}
			}
		}
		...
		if (isset($data['userids'])) {
			$userids = bs_toArray($data['userids']);
			...
			if (uint_in_array(self::$userData['userid'], $userids)) {
				foreach ($usrgrps as $usrgrp) {
					if (($usrgrp['gui_access'] == GROUP_GUI_ACCESS_DISABLED)
						|| ($usrgrp['users_status'] == GROUP_STATUS_DISABLED)) {
						self::exception(BS_API_ERROR_PARAMETERS, _('User cannot add himself to a disabled group or a group with disabled GUI access.'));
					}
				}
			}
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectRights'] !== null && $options['selectRights'] != API_OUTPUT_COUNT) {
			...
			$db_rights = bs_toHash($db_rights, 'rightid');
