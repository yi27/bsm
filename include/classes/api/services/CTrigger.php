...
class CTrigger extends CTriggerGeneral {
	...
	public function get(array $options = []) {
		...
		if ($userType != USER_TYPE_SUPER_ADMIN && !$options['nopermissions']) {
			...
			$sqlParts['where'][] = 'NOT EXISTS ('.
				...
				' GROUP BY i.hostid'.
				' HAVING MAX(permission)<'.bs_dbstr($permission).
		...
		if (!is_null($options['functions'])) {
			bs_value2array($options['functions']);
		...
		if (!is_null($options['lastChangeSince'])) {
			$sqlParts['where']['lastchangesince'] = 't.lastchange>'.bs_dbstr($options['lastChangeSince']);
		}

		// lastChangeTill
		if (!is_null($options['lastChangeTill'])) {
			$sqlParts['where']['lastchangetill'] = 't.lastchange<'.bs_dbstr($options['lastChangeTill']);
		}
		...
		if (is_array($options['search'])) {
			bs_db_search('triggers t', $options, $sqlParts);
		}
		...
		if (is_array($options['filter'])) {
			...
			if (isset($options['filter']['host']) && !is_null($options['filter']['host'])) {
				bs_value2array($options['filter']['host']);
			...
			if (isset($options['filter']['hostid']) && !is_null($options['filter']['hostid'])) {
				bs_value2array($options['filter']['hostid']);
		...
		if (!is_null($options['group'])) {
			...
			$sqlParts['where']['group'] = ' g.name='.bs_dbstr($options['group']);
		}
		...
		if (!is_null($options['host'])) {
			...
			$sqlParts['where']['fi'] = 'f.itemid=i.itemid';
			$sqlParts['where']['hi'] = 'h.hostid=i.hostid';
			$sqlParts['where']['host'] = ' h.host='.bs_dbstr($options['host']);
		}
		...
		if (!is_null($options['min_severity'])) {
			$sqlParts['where'][] = 't.priority>='.bs_dbstr($options['min_severity']);
		}
		...
		$result = bs_toHash($this->customFetch($this->createSelectQueryFromParts($sqlParts), $options), 'triggerid');
	...
	public function checkInput(array &$triggers, $method) {
		...
		if ($update) {
			...
			$dbTriggers = $this->get([
				'output' => API_OUTPUT_EXTEND,
				'triggerids' => bs_objectValues($triggers, 'triggerid'),
				'editable' => true,
				'preservekeys' => true
			]);
		...
		foreach ($triggers as $tnum => &$trigger) {
			...
			if (!check_db_fields($triggerDbFields, $trigger)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect fields for trigger.'));
			}
			...
			if (isset($trigger['expression']) && $expressionChanged) {
				...
				if (!$expressionData->parse($trigger['expression'])) {
					self::exception(BS_API_ERROR_PARAMETERS, $expressionData->error);
				}
				...
				foreach ($expressionHosts as $host) {
					if (!isset($hosts[$host])) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect trigger expression. Host "%s" does not exist or you have no access to this host.', $host));
					}
					...
					if ($hostsStatusFlags == 0x3) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect trigger expression. Trigger expression elements should not belong to a template and a host simultaneously.'));
					}
				}
				...
				foreach ($expressionData->expressions as $exprPart) {
					$sql = 'SELECT i.itemid,i.value_type'.
							...
							' WHERE i.key_='.bs_dbstr($exprPart['item']).
								' AND '.dbConditionInt('i.flags', [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_CREATED]).
	...
	public function create(array $triggers) {
		...
		return ['triggerids' => bs_objectValues($triggers, 'triggerid')];
	}
	...
	public function delete(array $triggerIds, $nopermissions = false) {
		...
		foreach ($delTriggers as $trigger) {
			info(_s('Deleted: Trigger "%1$s" on "%2$s".', $trigger['description'],
					implode(', ', bs_objectValues($trigger['hosts'], 'name'))));
	...
	protected function validateDelete(array $triggerIds, $nopermissions) {
		...
		if (!$nopermissions) {
			...
			foreach ($dbTriggers as $dbTrigger) {
				if ($dbTrigger['flags'] == BS_FLAG_DISCOVERY_CREATED) {
	...
	public function addDependencies(array $triggersData) {
		$triggersData = bs_toArray($triggersData);

		$triggerIds = array_unique(bs_objectValues($triggersData, 'triggerid'));
	...
	public function deleteDependencies(array $triggers) {
		...
		try {
			...
			self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete dependency'));
		}
	...
	protected function updateReal(array $triggers) {
		...
		foreach ($triggers as &$trigger) {
			...
			$dbTrigger = $dbTriggers[$trigger['triggerid']];
			$hosts = bs_objectValues($dbTrigger['hosts'], 'name');
			...
			if ($expressionChanged) {
				...
				$newTemplates = bs_toHash($expressionData->getHosts());
				$proceed = true;
				...
				if ($proceed) {
					$sql = 'SELECT t.triggerid'.
							' FROM triggers t'.
							' WHERE t.templateid='.bs_dbstr($trigger['triggerid']);
					...
					while ($cTrig = DBfetch($cTrigCursor)) {
						...
						$templateNames = DBfetchArrayAssoc(DBselect('SELECT h.name'.
							...
							' f.triggerid='.bs_dbstr($cTrig['triggerid'])), 'name');
				...
				if (!isset($trigger['dependencies'])) {
					$trigger['dependencies'] = bs_objectValues($dbTrigger['dependencies'], 'triggerid');
				}
			}
	...
	public function syncTemplates(array $data) {
		$data['templateids'] = bs_toArray($data['templateids']);
		$data['hostids'] = bs_toArray($data['hostids']);
	...
	public function syncTemplateDependencies(array $data) {
		$templateIds = bs_toArray($data['templateids']);
		...
		foreach ($triggers as $trigger) {
			...
			if (!$triggerTemplates) {
				...
				if ($triggerDependencyTemplates) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot add dependency from a host to a template.'));
				}
			}

			// the trigger can't depend on itself
			if (in_array($trigger['triggerid'], $trigger['dependencies'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot create dependency on trigger itself.'));
			}
			...
			do {
				...
				foreach ($upTriggersIds as $id) {
					if (bccomp($id, $trigger['triggerid']) == 0) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Cannot create circular dependencies.'));
					}
					$downTriggerIds[] = $id;
				}
			...
			$depTemplateIds = bs_toHash(bs_objectValues($triggerDependencyTemplates, 'templateid'));

			// run the check only if a templated trigger has dependencies on other templates
			$triggerTemplateIds = bs_toHash(bs_objectValues($triggerTemplates, 'hostid'));
			$tdiff = array_diff($depTemplateIds, $triggerTemplateIds);
			if (!empty($triggerTemplateIds) && !empty($depTemplateIds) && !empty($tdiff)) {
				$affectedTemplateIds = bs_array_merge($triggerTemplateIds, $depTemplateIds);
	...
	protected function checkDependencyParents(array $triggers) {
		...
		if ($parentDepTriggers) {
			$parentDepTriggers = bs_toHash($parentDepTriggers, 'triggerid');
			foreach ($triggers as $trigger) {
				foreach ($trigger['dependencies'] as $depTriggerId) {
	...
	protected function checkDependencyDuplicates(array $triggers) {
		...
		if ($duplicateTriggerId === null) {
			...
			foreach ($triggers as $trigger) {
				$dbUpTriggers = DBselect(
					...
					' AND td.triggerid_down='.bs_dbstr($trigger['triggerid'])
				, 1);
		...
		if ($duplicateTriggerId) {
			$dplTrigger = DBfetch(DBselect(
				...
				' FROM triggers t'.
				' WHERE t.triggerid='.bs_dbstr($duplicateTriggerId)
			));
			self::exception(BS_API_ERROR_PARAMETERS,
	...
	protected function validateItems(array $trigger) {
		...
		if ($firstTemplate) {
			$compareLinks = array_merge(
				bs_objectValues($firstTemplate['hosts'], 'hostid'),
				bs_objectValues($firstTemplate['templates'], 'templateid')
			);

			foreach ($templatesData as $data) {
				$linkedTo = array_merge(
					bs_objectValues($data['hosts'], 'hostid'),
					bs_objectValues($data['templates'], 'templateid')
				);
	...
	protected function applyPostSqlFiltering(array $triggers, array $options) {
		...
		if ($options['skipDependent'] !== null) {
			...
			$resultTriggerIds = bs_objectValues($triggers, 'triggerid');
