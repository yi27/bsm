...
class CIconMap extends CApiService {
	...
	public function get(array $options = []) {
		...
		if (!is_null($options['iconmapids'])) {
			bs_value2array($options['iconmapids']);
		...
		if (is_array($options['search'])) {
			bs_db_search('icon_map im', $options, $sqlParts);
		}
	...
	public function create(array $iconMaps) {
		if (USER_TYPE_SUPER_ADMIN != self::$userData['type']) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only Super Admins can create icon maps.'));
		}

		$iconMaps = bs_toArray($iconMaps);
		$iconMapRequiredFields = ['name' => null];

		$duplicates = [];
		foreach ($iconMaps as $iconMap) {
			if (!check_db_fields($iconMapRequiredFields, $iconMap)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect parameter is used for icon map "%s".', $iconMap['name']));
			}
			if (bs_empty($iconMap['name'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Icon map name cannot be empty.'));
			}
			if (isset($duplicates[$iconMap['name']])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Cannot create icon maps with identical name "%s".', $iconMap['name']));
			}
			else {
		...
		foreach ($dbIconMaps as $dbIconMap) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Icon map "%s" already exists.', $dbIconMap['name']));
		}
	...
	public function update(array $iconMaps) {
		if (USER_TYPE_SUPER_ADMIN != self::$userData['type']) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only Super Admins can update icon maps.'));
		}
		...
		foreach ($iconMaps as $iconMap) {
			if (!check_db_fields(['iconmapid' => null], $iconMap)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect parameters for icon map update method "%s".', $iconMap['name']));
			}
		...
		foreach ($iconMaps as $iconMap) {
			if (!isset($iconMapsUpd[$iconMap['iconmapid']])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Icon map with iconmapid "%s" does not exist.', $iconMap['iconmapid']));
			}
			...
			if (isset($iconMap['name'])) {
				...
				if (($iconMapExists = reset($iconMapExists)) && (bccomp($iconMapExists['iconmapid'], $iconMap['iconmapid']) != 0)) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Icon map "%s" already exists.', $iconMap['name']));
				}
			}
		...
		foreach ($iconMaps as $iconMap) {
			...
			foreach ($iconMap['mappings'] as $mapping) {
				if (!isset($mapping['expression'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Required field "expression" is missing in icon mapping.'));
				}
				elseif (!isset($mapping['inventory_link'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Required field "inventory_link" is missing in icon mapping.'));
				}
				elseif (!isset($mapping['iconid'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Required field "iconid" is missing in icon mapping.'));
				}
