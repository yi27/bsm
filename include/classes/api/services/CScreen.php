...
class CScreen extends CApiService {
	...
	public function get($options = []) {
		...
		if (!is_null($options['screenitemids'])) {
			bs_value2array($options['screenitemids']);
		...
		if (is_array($options['search'])) {
			bs_db_search('screens s', $options, $sql_parts);
		}
	...
	protected function validateScreenSize(array $screen) {
		foreach (['vsize', 'hsize'] as $field_name) {
			...
			if (!bs_is_int($screen[$field_name])) {
	...
	protected function validateCreate(array $screens) {
		...
		$db_screens = $this->get([
			'output' => ['name'],
			'filter' => ['name' => bs_objectValues($screens, 'name')],
			'nopermissions' => true,
			'limit' => 1
		]);

		if ($db_screens) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Screen "%1$s" already exists.', $db_screens[0]['name']));
		}
		...
		foreach ($screens as $screen) {
			...
			if (array_key_exists('userid', $screen)) {
				if ($screen['userid'] === '' || $screen['userid'] === null || $screen['userid'] === false) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Screen owner cannot be empty.'));
				}
				elseif ($screen['userid'] != $user_data['userid'] && $user_data['type'] != USER_TYPE_SUPER_ADMIN
						&& $user_data['type'] != USER_TYPE_BSM_ADMIN) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Only administrators can set screen owner.'));
				}
			}
	...
	protected function validateUpdate(array $screens, array $db_screens) {
		...
		$screens = $this->extendFromObjects(bs_toHash($screens, 'screenid'), $db_screens, ['name']);
			...
			if (array_key_exists('name', $screen)) {
				...
				if (array_key_exists('name', $screen)) {
					...
					elseif ($screen['name'] === '' || $screen['name'] === null || $screen['name'] === false) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Screen name cannot be empty.'));
					}
		...
		if ($check_names) {
			...
			$db_screen_names = bs_toHash($db_screen_names, 'name');
	...
	public function update(array $screens) {
		...
		$db_screens = $this->get([
			...
			'screenids' => bs_objectValues($screens, 'screenid'),
			'editable' => true,
			'preservekeys' => true
		]);
		...
		return ['screenids' => bs_objectValues($screens, 'screenid')];
	}
	...
	protected function updateReal(array $screens, array $db_screens) {
		...
		foreach ($screens as $screen) {
			...
			// Screen user shares.
			if (array_key_exists('users', $screen)) {
				$user_shares_diff = bs_array_diff($screen['users'], $db_screen['users'], 'userid');
				...
				$shared_userids_to_delete = array_merge($shared_userids_to_delete,
					bs_objectValues($user_shares_diff['second'], 'screenuserid')
				);
			}

			// Screen user group shares.
			if (array_key_exists('userGroups', $screen)) {
				$user_group_shares_diff = bs_array_diff($screen['userGroups'], $db_screen['userGroups'], 'usrgrpid');
				...
				$shared_user_groupids_to_delete = array_merge($shared_user_groupids_to_delete,
					bs_objectValues($user_group_shares_diff['second'], 'screenusrgrpid')
				);
			}
