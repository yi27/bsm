...
class CApplication extends CApiService {
	...
	public function get($options = []) {
		...
		if (is_array($options['search'])) {
			bs_db_search('applications a', $options, $sqlParts);
		}
	...
	public function checkInput(&$applications, $method) {
		...
		if ($update || $delete) {
			...
			$dbApplications = $this->get([
				...
				'applicationids' => bs_objectValues($applications, 'applicationid'),
				'editable' => true,
				'preservekeys' => true
			]);
		}
			...
			$dbHosts = API::Host()->get([
				...
				'hostids' => bs_objectValues($applications, 'hostid'),
		...
		foreach ($applications as &$application) {
			...
			if ($delete || $update) {
				if ($dbApplications[$application['applicationid']]['templateids']) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot update templated applications.'));
				}

				if ($dbApplications[$application['applicationid']]['flags'] == BS_FLAG_DISCOVERY_CREATED) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Cannot update discovered application "%1$s".',
			...
			if ($update || $create) {
				...
				foreach ($applicationsExists as $applicationExists) {
					if (!$update || (bccomp($applicationExists['applicationid'], $application['applicationid']) != 0)) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('Application "%1$s" already exists.', $application['name']));
					}
				}
			}
		}
	...
	public function create(array $applications) {
		$applications = bs_toArray($applications);
		$this->checkInput($applications, __FUNCTION__);

		foreach ($applications as &$application) {
			$application['flags'] = BS_FLAG_DISCOVERY_NORMAL;
		}
		unset($application);
		...
		return ['applicationids' => bs_objectValues($applications, 'applicationid')];
	}
	...
	public function delete(array $applicationids, $nopermissions = false) {
		...
		if (!$nopermissions) {
			foreach ($applicationids as $applicationid) {
				...
				if ($delApplications[$applicationid]['templateids']) {
					self::exception(BS_API_ERROR_PERMISSIONS, _('Cannot delete templated application.'));
				}

				if ($delApplications[$applicationid]['flags'] == BS_FLAG_DISCOVERY_CREATED) {
					self::exception(BS_API_ERROR_PERMISSIONS, _s('Cannot delete discovered application "%1$s".',
		...
		foreach (bs_array_merge($delApplications, $childApplications) as $delApplication) {
	...
	public function massAdd($data) {
		...
		$applications = bs_toArray($data['applications']);
		$applicationIds = bs_objectValues($applications, 'applicationid');
		$items = bs_toArray($data['items']);
		...
		foreach ($applications as $application) {
			if ($dbApplicationHost['hostid'] != $allowedApplications[$application['applicationid']]['hostid']) {
				self::exception(BS_API_ERROR_PERMISSIONS, _('Cannot process applications from different hosts or templates.'));
			}
		}
		...
		foreach ($itemIds as $itemId) {
			$dbChilds = DBselect('SELECT i.itemid,i.hostid FROM items i WHERE i.templateid='.bs_dbstr($itemId));
			...
			while ($child = DBfetch($dbChilds)) {
				$dbApplications = DBselect(
					...
					' WHERE a1.name=a2.name'.
						' AND a1.hostid='.bs_dbstr($child['hostid']).
				...
				if (!$result = $this->massAdd(['items' => $child, 'applications' => $childApplications])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot add items.'));
				}
			}
		}
		...
		if ($options['selectDiscoveryRule'] !== null && $options['selectDiscoveryRule'] != API_OUTPUT_COUNT) {
			...
			$dbRules = DBselect(
				...
				' WHERE '.dbConditionInt('ad.applicationid', array_keys($result)).
					...
					' AND a.flags='.BS_FLAG_DISCOVERY_CREATED
			);
