...
class CHostPrototype extends CHostBase {
	...
	public function get(array $options) {
		$options = bs_array_merge($this->getOptions, $options);
		$options['filter']['flags'] = BS_FLAG_DISCOVERY_PROTOTYPE;
	...
	protected function validateCreate(array $hostPrototypes) {
		...
		$this->checkDiscoveryRulePermissions(bs_objectValues($hostPrototypes, 'ruleid'));
		...
		$discoveryRules = API::getApiService()->select('items', [
			'output' => ['hostid'],
			'itemids' => bs_objectValues($hostPrototypes, 'ruleid')
		]);
		$this->checkValidator(bs_objectValues($discoveryRules, 'hostid'), new CHostNormalValidator([
		...
		return [
			'validators' => [
				'host' => new CLldMacroStringValidator([
					'regex' => '/^('.BS_PREG_INTERNAL_NAMES.'|\{#'.BS_PREG_MACRO_NAME_LLD.'\})+$/',
	...
	public function create(array $hostPrototypes) {
		$hostPrototypes = bs_toArray($hostPrototypes);

		foreach ($hostPrototypes as &$hostPrototype) {
			// if the visible name is not set, use the technical name instead
			if (!isset($hostPrototype['name']) || bs_empty(trim($hostPrototype['name']))) {
				$hostPrototype['name'] = $hostPrototype['host'];
			}

			if (isset($hostPrototype['templates'])) {
				$hostPrototype['templates'] = bs_toArray($hostPrototype['templates']);
			}
		}
		unset($hostPrototype);
		...
		return ['hostids' => bs_objectValues($hostPrototypes, 'hostid')];
	}
	...
	protected function createReal(array $hostPrototypes) {
		foreach ($hostPrototypes as &$hostPrototype) {
			$hostPrototype['flags'] = BS_FLAG_DISCOVERY_PROTOTYPE;
		}
		unset($hostPrototype);
		...
		foreach ($hostPrototypes as $hostPrototype) {
			if (isset($hostPrototype['templates']) && $hostPrototype['templates']) {
				$this->link(bs_objectValues($hostPrototype['templates'], 'templateid'), [$hostPrototype['hostid']]);
			}
		}
	...
	protected function validateUpdate(array $hostPrototypes, array $dbHostPrototypes) {
		...
		$this->checkHostPrototypePermissions(bs_objectValues($hostPrototypes, 'hostid'));

		$hostPrototypes = $this->extendFromObjects(bs_toHash($hostPrototypes, 'hostid'), $dbHostPrototypes, [
			'host', 'name'
		]);
	...
	public function update(array $hostPrototypes) {
		...
		$dbHostPrototypes = $this->get([
			...
			'hostids' => bs_objectValues($hostPrototypes, 'hostid'),
			'editable' => true,
			'preservekeys' => true
		]);
	...
	protected function updateReal(array $hostPrototypes) {
		...
		foreach ($hostPrototypes as $key => $hostPrototype) {
			...
			// templates
			if (isset($hostPrototype['templates'])) {
				$existingTemplateIds = bs_objectValues($exHostPrototype['templates'], 'templateid');
				$newTemplateIds = bs_objectValues($hostPrototype['templates'], 'templateid');
			...
			if (isset($hostPrototype['inventory']) ) {
				$inventory = bs_array_mintersect(['inventory_mode'], $hostPrototype['inventory']);
	...
	protected function inherit(array $hostPrototypes, array $hostids = null) {
		...
		if (!bs_empty($insertHostPrototypes)) {
			$insertHostPrototypes = $this->createReal($insertHostPrototypes);
		}

		if (!bs_empty($updateHostPrototypes)) {
	...
	protected function prepareInheritedObjects(array $hostPrototypes, array $hostIds = null) {
		...
		$chdHosts = API::Host()->get([
			...
			'templateids' => bs_objectValues($discoveryRules, 'hostid'),
			'hostids' => $hostIds,
			'nopermissions' => true,
		...
		$childHostPrototypes = API::HostPrototype()->get([
			...
			'selectGroupPrototypes' => API_OUTPUT_EXTEND,
			'selectDiscoveryRule' => ['itemid'],
			'discoveryids' => bs_objectValues($childDiscoveryRules, 'itemid'),
		]);
		...
		foreach ($chdHosts as $host) {
			...
			foreach ($parentHostPrototypes as $parentHostPrototype) {
				...
				if ($exHostPrototypes) {
					$exHostPrototypesHosts = bs_toHash($exHostPrototypes, 'host');
					$exHostPrototypesTemplateIds = bs_toHash($exHostPrototypes, 'templateid');
					...
					if (isset($exHostPrototypesTemplateIds[$parentHostPrototype['hostid']])) {
						...
						if (isset($exHostPrototypesHosts[$parentHostPrototype['host']])
							...
							$discoveryRule = DBfetch(DBselect('SELECT i.name FROM items i WHERE i.itemid='.bs_dbstr($exHostPrototype['discoveryRule']['itemid'])));
							self::exception(BS_API_ERROR_PARAMETERS, _s('Host prototype "%1$s" already exists on "%2$s".', $parentHostPrototype['host'], $discoveryRule['name']));
						}
					}
					...
					if (isset($exHostPrototypesHosts[$parentHostPrototype['host']])) {
						...
						if ($exHostPrototype['templateid'] > 0 && !idcmp($exHostPrototype['templateid'], $parentHostPrototype['hostid'])) {
							$discoveryRule = DBfetch(DBselect('SELECT i.name FROM items i WHERE i.itemid='.bs_dbstr($exHostPrototype['discoveryRule']['itemid'])));
							self::exception(BS_API_ERROR_PARAMETERS, _s('Host prototype "%1$s" already exists on "%2$s", inherited from another template.', $parentHostPrototype['host'], $discoveryRule['name']));
						}
					}
				}
				...
				if ($exHostPrototype) {
					...
					$exGroupPrototypesByTemplateId = bs_toHash($exHostPrototype['groupPrototypes'], 'templateid');
					$exGroupPrototypesByName = bs_toHash($exHostPrototype['groupPrototypes'], 'name');
					$exGroupPrototypesByGroupId = bs_toHash($exHostPrototype['groupLinks'], 'groupid');
					...
					foreach ($newHostPrototype['groupPrototypes'] as &$groupPrototype) {
						...
						elseif (isset($groupPrototype['name']) && !bs_empty($groupPrototype['name'])
	...
	public function syncTemplates(array $data) {
		...
		$hostPrototypes = $this->get([
			'discoveryids' => bs_objectValues($discoveryRules, 'itemid'),
			'preservekeys' => true,
	...
	public function delete(array $hostPrototypeIds, $nopermissions = false) {
		...
		if ($discoveredHosts) {
			API::Host()->delete(bs_objectValues($discoveredHosts, 'hostid'), true);
		}
	...
	protected function checkNotInherited(array $hostPrototypeIds) {
		...
		if ($hostPrototype = DBfetch($query)) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Cannot delete templated host prototype.'));
		}
	}
	...
	protected function checkExistingHostPrototypes(array $hostPrototypes, $field, $error) {
		...
		foreach ($valuesByDiscoveryRuleId as $discoveryRuleId => $values) {
			$sqlWhere[] = '(hd.parent_itemid='.bs_dbstr($discoveryRuleId).
		...
		if ($sqlWhere) {
			...
			while ($row = DBfetch($query)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s($error, $row[$field], $row['discovery_name']));
			}
		}
	}
	...
	protected function applyQueryFilterOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		...
		$sqlParts['where'][] = 'ph.flags='.BS_FLAG_DISCOVERY_NORMAL;
	...
	protected function deleteGroupPrototypes(array $groupPrototypeIds) {
		...
		if ($groupPrototypeChildren) {
			$this->deleteGroupPrototypes(bs_objectValues($groupPrototypeChildren, 'group_prototypeid'));
		}
		...
		if ($hostGroups) {
			API::HostGroup()->delete(bs_objectValues($hostGroups, 'groupid'), true);
		}
