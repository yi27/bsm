...
class CMaintenance extends CApiService {
	...
	public function get(array $options = []) {
		...
		if (is_array($options['search'])) {
			bs_db_search('maintenances m', $options, $sqlParts);
		}
	...
	public function create(array $maintenances) {
		$maintenances = bs_toArray($maintenances);
		if (self::$userData['type'] == USER_TYPE_BSM_USER) {
		...
		if (empty($hostids) && empty($groupids)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('At least one host or group should be selected.'));
		}
		...
		foreach ($maintenances as $maintenance) {
			...
			if (!check_db_fields($dbFields, $maintenance)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect parameters for maintenance.'));
			}
		}
		...
		$dbMaintenances = $this->get([
			'output' => ['name'],
			'filter' => ['name' => bs_objectValues($maintenances, 'name')],
			'nopermissions' => true,
			'limit' => 1
		]);
		...
		if ($dbMaintenances) {
			...
			self::exception(BS_API_ERROR_PARAMETERS, _s('Maintenance "%1$s" already exists.', $dbMaintenance['name']));
		}

		foreach ($maintenances as $mnum => $maintenance) {
			// validate maintenance active since
			if (!validateUnixTime($maintenance['active_since'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('"%s" must be between 1970.01.01 and 2038.01.18.', _('Active since')));
			}

			// validate maintenance active till
			if (!validateUnixTime($maintenance['active_till'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('"%s" must be between 1970.01.01 and 2038.01.18.', _('Active till')));
			}

			// validate maintenance active interval
			if ($maintenance['active_since'] > $maintenance['active_till']) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Maintenance "Active since" value cannot be bigger than "Active till".'));
			}

			// validate timeperiods
			if (!array_key_exists('timeperiods', $maintenance) || !is_array($maintenance['timeperiods'])
					|| !$maintenance['timeperiods']) {
				self::exception(BS_API_ERROR_PARAMETERS, _('At least one maintenance period must be created.'));
			}
	...
	public function update(array $maintenances) {
		$maintenances = bs_toArray($maintenances);
		$maintenanceids = bs_objectValues($maintenances, 'maintenanceid');
		...
		foreach ($maintenances as $maintenance) {
			...
			if (isset($maintenance['name']) && !bs_empty($maintenance['name'])
					&& $updMaintenances[$maintenance['maintenanceid']]['name'] !== $maintenance['name']) {
				if (isset($maintenanceNamesChanged[$maintenance['name']])) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Maintenance "%1$s" already exists.',
						$maintenance['name']
					));
				}
				else {
		...
		foreach ($maintenances as $maintenance) {
			...
			$hostDiff = bs_array_diff(
				bs_toObject($maintenance['hostids'], 'hostid'),
			...
			$groupDiff = bs_array_diff(
				bs_toObject($maintenance['groupids'], 'groupid'),
	...
	public function delete(array $maintenanceids) {
		...
		if (!empty($hosts)) {
			DB::update('hosts', [
				...
				'where' => ['hostid' => bs_objectValues($hosts, 'hostid')]
			]);
		}
	...
	protected function replaceTimePeriods(array $oldMaintenance, array $maintenance) {
		...
		$oldTimePeriods = bs_toHash($oldMaintenance['timeperiods'], 'timeperiodid');
