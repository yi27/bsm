...
class CEvent extends CApiService {
	...
	public function get($options = []) {
		...
		if ($userType != USER_TYPE_SUPER_ADMIN && !$options['nopermissions']) {
			...
			if ($options['object'] == EVENT_OBJECT_TRIGGER) {
				...
				if ($options['objectids'] !== null) {
					...
					$options['objectids'] = bs_objectValues($triggers, 'triggerid');
				}
				// all triggers
				else {
			...
			elseif ($options['object'] == EVENT_OBJECT_ITEM || $options['object'] == EVENT_OBJECT_LLDRULE) {
				...
				if ($options['objectids'] !== null) {
					if ($options['object'] == EVENT_OBJECT_ITEM) {
						...
						$options['objectids'] = bs_objectValues($items, 'itemid');
					}
		...
		if (!is_null($options['eventids'])) {
			bs_value2array($options['eventids']);
		...
		if ($options['objectids'] !== null
			...
			bs_value2array($options['objectids']);
		...
		if (!is_null($options['object'])) {
			$sqlParts['where']['o'] = 'e.object='.bs_dbstr($options['object']);
		}

		// source
		if (!is_null($options['source'])) {
			$sqlParts['where'][] = 'e.source='.bs_dbstr($options['source']);
		}
		...
		if (!is_null($options['time_from'])) {
			$sqlParts['where'][] = 'e.clock>='.bs_dbstr($options['time_from']);
		}

		// time_till
		if (!is_null($options['time_till'])) {
			$sqlParts['where'][] = 'e.clock<='.bs_dbstr($options['time_till']);
		}

		// eventid_from
		if (!is_null($options['eventid_from'])) {
			$sqlParts['where'][] = 'e.eventid>='.bs_dbstr($options['eventid_from']);
		}

		// eventid_till
		if (!is_null($options['eventid_till'])) {
			$sqlParts['where'][] = 'e.eventid<='.bs_dbstr($options['eventid_till']);
		}

		// value
		if (!is_null($options['value'])) {
			bs_value2array($options['value']);
			$sqlParts['where'][] = dbConditionInt('e.value', $options['value']);
		}

		// search
		if (is_array($options['search'])) {
			bs_db_search('events e', $options, $sqlParts);
		}
	...
	protected function validateGet(array $options) {
		...
		if (!$sourceValidator->validate($options['source'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect source value.'));
		}
		...
		if (!$objectValidator->validate($options['object'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect object value.'));
		}
		...
		if (!$sourceObjectValidator->validate(['source' => $options['source'], 'object' => $options['object']])) {
			self::exception(BS_API_ERROR_PARAMETERS, $sourceObjectValidator->getError());
		}
	}
	...
	public function acknowledge(array $data) {
		$data['eventids'] = bs_toArray($data['eventids']);

		$this->validateAcknowledge($data);

		$eventIds = bs_toHash($data['eventids']);
			...
			if ($options['object'] == EVENT_OBJECT_TRIGGER) {
				$query = DBselect(
					'SELECT e.eventid,i.hostid'.
						...
						' AND f.itemid=i.itemid'.
						' AND e.object='.bs_dbstr($options['object']).
						' AND e.source='.bs_dbstr($options['source'])
				);
			}
	...
	protected function checkCanBeAcknowledged(array $eventIds) {
		...
		foreach ($eventIds as $eventId) {
			if (!isset($allowedEvents[$eventId])) {
				...
				if ($event) {
					self::exception(BS_API_ERROR_PERMISSIONS, _('Only trigger events can be acknowledged.'));
				}
