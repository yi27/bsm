...
class CMediatype extends CApiService {
	 ...
	 * @param boolean $options['type'] filter by Mediatype type [ USER_TYPE_BSM_USER: 1, USER_TYPE_BSM_ADMIN: 2, USER_TYPE_SUPER_ADMIN: 3 ]
	...
	public function get($options = []) {
		...
		elseif (is_null($options['editable']) && self::$userData['type'] == USER_TYPE_BSM_ADMIN) {
		}
		...
		if (is_array($options['search'])) {
			bs_db_search('media_type mt', $options, $sqlParts);
		}
	...
	protected function validateCreate(array $mediatypes) {
		if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only Super Admins can create media types.'));
		}
	...
	protected function validateUpdate(array $mediatypes) {
		if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only Super Admins can edit media types.'));
		}
		...
		$mediatypeids = bs_objectValues($mediatypes, 'mediatypeid');
		...
		if ($check_names) {
			...
			$db_mediatype_names = bs_toHash($db_mediatype_names, 'description');
		...
		$mediatypes = $this->extendFromObjects(bs_toHash($mediatypes, 'mediatypeid'), $db_mediatypes, ['description']);
	...
	public function create($mediatypes) {
		$mediatypes = bs_toArray($mediatypes);
	...
	public function delete(array $mediatypeids) {
		if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only Super Admins can delete media types.'));
		}
		...
		if (!empty($actions)) {
			...
			self::exception(BS_API_ERROR_PARAMETERS, _s('Media types used by action "%s".', $action['name']));
		}
