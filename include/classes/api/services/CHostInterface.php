...
class CHostInterface extends CApiService {
	...
	public function get(array $options = []) {
		...
		if (is_array($options['search'])) {
			bs_db_search('interface hi', $options, $sqlParts);
		}
	...
	public function checkInput(array &$interfaces, $method) {
		...
		if ($update) {
			...
			$dbInterfaces = $this->get([
				'output' => API_OUTPUT_EXTEND,
				'interfaceids' => bs_objectValues($interfaces, 'interfaceid'),
				'editable' => true,
				'preservekeys' => true
			]);
		}
		...
		$dbHosts = API::Host()->get([
			'output' => ['host'],
			'hostids' => bs_objectValues($interfaces, 'hostid'),
			'editable' => true,
			'preservekeys' => true
		]);
		...
		$dbProxies = API::Proxy()->get([
			'output' => ['host'],
			'proxyids' => bs_objectValues($interfaces, 'hostid'),
			'editable' => true,
			'preservekeys' => true
		]);
		...
		foreach ($interfaces as &$interface) {
			...
			if ($update) {
				...
				if (isset($interface['hostid']) && bccomp($dbInterface['hostid'], $interface['hostid']) != 0) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Cannot switch host for interface.'));
				}
				...
				$interface = bs_array_merge($dbInterface, $interface);
			}
			else {
				...
				elseif (!isset($interface['type'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect arguments passed to method.'));
				}
			}

			if (bs_empty($interface['ip']) && bs_empty($interface['dns'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('IP and DNS cannot be empty for host interface.'));
			}

			if ($interface['useip'] == INTERFACE_USE_IP && bs_empty($interface['ip'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Interface with DNS "%1$s" cannot have empty IP address.', $interface['dns']));
			}

			if ($interface['useip'] == INTERFACE_USE_DNS && bs_empty($interface['dns'])) {
				...
				elseif ($dbProxies && !empty($dbProxies[$interface['hostid']]['host'])) {
					...
					self::exception(BS_API_ERROR_PARAMETERS, _s('Interface with IP "%1$s" cannot have empty DNS name.', $interface['ip']));
				}
			}
		...
		$this->checkValidator(bs_objectValues($interfaces, 'hostid'), new CHostNormalValidator([
	...
	public function create(array $interfaces) {
		$interfaces = bs_toArray($interfaces);
	...
	public function update(array $interfaces) {
		...
		return ['interfaceids' => bs_objectValues($interfaces, 'interfaceid')];
	}
	...
	protected function validateMassRemove(array $data) {
		...
		foreach ($data['interfaces'] as $interface) {
			...
			if ($interfacesToRemove) {
				$this->checkMainInterfacesOnDelete(bs_objectValues($interfacesToRemove, 'interfaceid'));
			}
		}
	}
	...
	public function massRemove(array $data) {
		$data['interfaces'] = bs_toArray($data['interfaces']);
	...
	public function replaceHostInterfaces(array $host) {
		if (isset($host['interfaces']) && !is_null($host['interfaces'])) {
			$host['interfaces'] = bs_toArray($host['interfaces']);
			...
			if ($interfacesToDelete) {
				$this->delete(bs_objectValues($interfacesToDelete, 'interfaceid'));
			}
		}
	}
	...
	protected function checkDns(array $interface) {
		...
		if (!preg_match('/^'.BS_PREG_DNS_FORMAT.'$/', $interface['dns'])
	...
	protected function checkIp(array $interface) {
		...
		if (preg_match('/^'.BS_PREG_MACRO_NAME_FORMAT.'$/', $interface['ip'])
		...
		if (!$ipValidator->validate($interface['ip'])) {
			self::exception(BS_API_ERROR_PARAMETERS, $ipValidator->getError());
		}
	}
	...
	protected function checkPort(array $interface) {
		if (!isset($interface['port']) || bs_empty($interface['port'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Port cannot be empty for host interface.'));
		}
		elseif (!validatePortNumberOrMacro($interface['port'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect interface port "%s" provided.', $interface['port']));
		}
	}
	...
	protected function checkBulk(array $interface) {
		if ($interface['type'] !== null && (($interface['type'] != INTERFACE_TYPE_SNMP && isset($interface['bulk'])
				...
				|| ($interface['type'] == INTERFACE_TYPE_SNMP && isset($interface['bulk'])
					&& (bs_empty($interface['bulk'])
						|| ($interface['bulk'] != SNMP_BULK_DISABLED && $interface['bulk'] != SNMP_BULK_ENABLED))))) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect bulk value for interface.'));
		}
	}
	...
	private function checkMainInterfaces(array $interfaces) {
		...
		foreach ($interfaceTypes as $interfaceHostId => $interfaceType) {
			foreach ($interfaceType as $type => $counters) {
				...
				if ($counters['main'] > 1) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Host cannot have more than one default interface of the same type.'));
				}
			}
		}
	}
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectItems'] !== null) {
			if ($options['selectItems'] != API_OUTPUT_COUNT) {
				...
				$items = bs_toHash($items, 'interfaceid');
