...
class CScript extends CApiService {
	...
	public function get(array $options) {
		...
		if (is_array($options['search'])) {
			bs_db_search('scripts s', $options, $sqlParts);
		}
	...
	public function update(array $scripts) {
		...
		return ['scriptids' => bs_objectValues($scripts, 'scriptid')];
	}
	...
	public function execute(array $data) {
		...
		// execute script
		$bsmServer = new CBsmServer($BS_SERVER, $BS_SERVER_PORT, BS_SCRIPT_TIMEOUT, BS_SOCKET_BYTES_LIMIT);
		$result = $bsmServer->executeScript($scriptId, $hostId);
		if ($result !== false) {
			...
			self::exception(BS_API_ERROR_INTERNAL, $bsmServer->getError());
		}
	}
	...
	protected function validateCreate(array $scripts) {
		...
		foreach ($scripts as $script) {
			if (!check_db_fields($dbFields, $script)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Wrong fields for script.'));
			}

			if ($script['name'] === '') {
				self::exception(BS_API_ERROR_PARAMETERS, _('Script name cannot be empty.'));
			}
			if ($script['command'] === '') {
				self::exception(BS_API_ERROR_PARAMETERS, _('Script command cannot be empty.'));
			}
		}
	}
	...
	protected function validateUpdate(array $scripts) {
		...
		$scripts = bs_toHash($scripts, 'scriptid');
		...
		foreach ($scripts as $script) {
			if (!isset($dbScripts[$script['scriptid']])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Script with scriptid "%1$s" does not exist.', $script['scriptid']));
			}
	...
	protected function validateDelete(array $scriptIds) {
		...
		if (empty($scriptIds)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete scripts. Empty input parameter "scriptids".'));
		}
		...
		foreach ($scriptIds as $scriptId) {
			...
			self::exception(BS_API_ERROR_PERMISSIONS, _s('Cannot delete scripts. Script with scriptid "%1$s" does not exist.', $scriptId));
		}
	...
	protected function validateMenuPath(array $scripts, $method) {
		...
		foreach ($scripts as $script) {
			...
			// menu1/menu2/{empty}
			if (bs_empty($name)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Empty name for script "%1$s".', $script['name']));
			}
			...
			foreach ($dbScripts as $dbScript) {
				...
				if ($path == $dbScriptPath) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Script "%1$s" already exists.', $script['name']));
				}
			}
		}
	}
	...
	protected function unsetExecutionType(array $scripts) {
		foreach ($scripts as $key => $script) {
			if (isset($script['type']) && $script['type'] == BS_SCRIPT_TYPE_IPMI) {
