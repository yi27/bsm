	...
	public function get($options = []) {
		...
		if (is_array($options['filter'])) {
			if (!array_key_exists('flags', $options['filter'])) {
				$options['filter']['flags'] = [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_CREATED];
			}
	...
	protected function inherit($graph, $hostids = null) {
		...
		foreach ($chdHosts as $chdHost) {
			...
			if ($tmpGraph['ymin_itemid'] > 0) {
				...
				if (!$yminItemid) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Graph "%1$s" cannot inherit. No required items on "%2$s" (Ymin value item).', $tmpGraph['name'], $chdHost['host']));
				}
			...
			$chdGraphs = $this->get([
				'filter' => ['templateid' => $tmpGraph['graphid'], 'flags' => [BS_FLAG_DISCOVERY_PROTOTYPE, BS_FLAG_DISCOVERY_NORMAL]],
				'output' => API_OUTPUT_EXTEND,
				'selectGraphItems' => API_OUTPUT_EXTEND,
				'preservekeys' => true,
	...
	public function delete(array $graphids, $nopermissions = false) {
		...
		if (!$nopermissions) {
			foreach ($graphids as $graphid) {
				...
				if ($delGraph['flags'] == BS_FLAG_DISCOVERY_CREATED) {
					self::exception(BS_API_ERROR_PARAMETERS, _s(
