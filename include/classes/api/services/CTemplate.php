...
class CTemplate extends CHostGeneral {
	...
	public function get($options = []) {
		...
		if (!is_null($options['parentTemplateids'])) {
			bs_value2array($options['parentTemplateids']);
	...
	public function create(array $templates) {
		$templates = bs_toArray($templates);
		...
		foreach ($templates as $key => $template) {
			$templates[$key]['groups'] = bs_toArray($template['groups']);
		}

		foreach ($templates as $template) {
			// if visible name is not given or empty it should be set to host name
			if ((!isset($template['name']) || bs_empty(trim($template['name']))) && isset($template['host'])) {
			...
			foreach ($template['groups'] as $group) {
				...
				$result = DBexecute(
					...
					' VALUES ('.bs_dbstr($hostGroupId).','.bs_dbstr($templateId).','.bs_dbstr($group['groupid']).')'
				);

				if (!$result) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot add group.'));
				}
			}
			...
			if (!$result) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot create template.'));
			}
		}
	...
	protected function validateCreate(array $templates) {
		...
		foreach ($templates as $template) {
			...
			if (!isset($template['groups']) || !$template['groups']) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('No groups for template "%1$s".', $template['host']));
			}

			$template['groups'] = bs_toArray($template['groups']);
		...
		foreach ($templates as $template) {
			...
			if (!check_db_fields($templateDbFields, $template)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Field "host" is mandatory.'));
			}

			if (!preg_match('/^'.BS_PREG_HOST_FORMAT.'$/', $template['host'])) {
			...
			if (isset($template['host'])) {
				...
				if ($templateExists) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Template "%1$s" already exists.', $template['host']));
				}
				...
				if ($hostExists) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Host "%1$s" already exists.', $template['host']));
				}
			}
	...
	public function update(array $templates) {
		...
		foreach ($templates as $template) {
			...
			if (!$this->massUpdate($template)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Failed to update template.'));
			}
		}

		return ['templateids' => bs_objectValues($templates, 'templateid')];
	}
	...
	protected function validateUpdate(array $templates) {
		$dbTemplates = $this->get([
			'output' => ['templateid'],
			'templateids' => bs_objectValues($templates, 'templateid'),
			'editable' => true,
			'preservekeys' => true
		]);
	...
	public function massAdd(array $data) {
		$templates = isset($data['templates']) ? bs_toArray($data['templates']) : [];
		$templateIds = bs_objectValues($templates, 'templateid');
	...
	public function massUpdate(array $data) {
		...
		$templates = bs_toArray($data['templates']);
		$templateIds = bs_objectValues($templates, 'templateid');

		$fieldsToUpdate = [];

		if (isset($data['host'])) {
			$fieldsToUpdate[] = 'host='.bs_dbstr($data['host']);
		}

		if (isset($data['name'])) {
			// if visible name is empty replace it with host name
			if (bs_empty(trim($data['name'])) && isset($data['host'])) {
				$fieldsToUpdate[] = 'name='.bs_dbstr($data['host']);
			}
			// we cannot have empty visible name
			elseif (bs_empty(trim($data['name'])) && !isset($data['host'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot have empty visible template name.'));
			}
			else {
				$fieldsToUpdate[] = 'name='.bs_dbstr($data['name']);
			}
		}

		if (isset($data['description'])) {
			$fieldsToUpdate[] = 'description='.bs_dbstr($data['description']);
		}
		...
		$data['templates_clear'] = isset($data['templates_clear']) ? bs_toArray($data['templates_clear']) : [];
		$templateIdsClear = bs_objectValues($data['templates_clear'], 'templateid');
		...
		if (isset($data['hosts']) && $data['hosts'] !== null) {
			...
			$templateHostIds = bs_objectValues($templateHosts, 'hostid');
			$newHostIds = bs_objectValues($data['hosts'], 'hostid');
		...
		if (isset($data['templates_link']) && $data['templates_link'] !== null) {
			...
			$templateTemplateIds = bs_objectValues($templateTemplates, 'templateid');
			$newTemplateIds = bs_objectValues($data['templates_link'], 'templateid');
		...
		if (isset($data['groups']) && $data['groups'] !== null && is_array($data['groups'])) {
			$updateGroups = bs_toArray($data['groups']);

			$templateGroups = API::HostGroup()->get([
				'output' => ['groupid'],
				'templateids' => $templateIds
			]);
			$templateGroupIds = bs_objectValues($templateGroups, 'groupid');
	...
	protected function validateMassUpdate(array $data) {
		...
		if (isset($data['groups']) && !$data['groups']) {
			self::exception(BS_API_ERROR_PARAMETERS, _('No groups for template.'));
		}

		// check name
		if (isset($data['name'])) {
			if (count($templates) > 1) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot mass update visible template name.'));
			}
		...
		if (isset($data['host'])) {
			if (count($templates) > 1) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot mass update template name.'));
			}
		...
		if (isset($data['host']) && !preg_match('/^'.BS_PREG_HOST_FORMAT.'$/', $data['host'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _s(
				'Incorrect characters used for template name "%1$s".',
				$data['host']
			));
	...
	public function massRemove(array $data) {
		...
		if (isset($data['hostids'])) {
			...
			API::Template()->unlink($templateids, bs_toArray($data['hostids']));
		}
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectTemplates'] !== null) {
			if ($options['selectTemplates'] != API_OUTPUT_COUNT) {
				...
				$templates = bs_toHash($templates, 'templateid');
		...
		if ($options['selectHosts'] !== null) {
			if ($options['selectHosts'] != API_OUTPUT_COUNT) {
				...
				$hosts = bs_toHash($hosts, 'templateid');
				foreach ($result as $templateid => $template) {
		...
		if ($options['selectScreens'] !== null) {
			if ($options['selectScreens'] != API_OUTPUT_COUNT) {
				...
				$screens = bs_toHash($screens, 'templateid');
				foreach ($result as $templateid => $template) {
