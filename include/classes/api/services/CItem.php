...
class CItem extends CItemGeneral {
	...
	public function get($options = []) {
		...
		$sqlParts = [
			...
			'from'		=> ['items' => 'items i'],
			'where'		=> ['webtype' => 'i.type<>'.ITEM_TYPE_HTTPTEST, 'flags' => 'i.flags IN ('.BS_FLAG_DISCOVERY_NORMAL.','.BS_FLAG_DISCOVERY_CREATED.')'],
			'group'		=> [],
			'order'		=> [],
			'limit'		=> null
		];
		...
		if (is_array($options['filter'])) {
			...
			if (array_key_exists('flags', $options['filter']) &&
					(is_null($options['filter']['flags']) || !bs_empty($options['filter']['flags']))) {
		...
		if (!is_null($options['group'])) {
			...
			$sqlParts['where'][] = ' g.name='.bs_dbstr($options['group']);
		}
		...
		if (!is_null($options['host'])) {
			$sqlParts['from']['hosts'] = 'hosts h';
			$sqlParts['where']['hi'] = 'h.hostid=i.hostid';
			$sqlParts['where'][] = ' h.host='.bs_dbstr($options['host']);
		}
		...
		if (!is_null($options['application'])) {
			...
			$sqlParts['where'][] = ' a.name='.bs_dbstr($options['application']);
		}
	...
	public function create($items) {
		...
		foreach ($items as &$item) {
			$item['flags'] = BS_FLAG_DISCOVERY_NORMAL;
		...
		if (!$nopermissions) {
			foreach ($itemIds as $itemId) {
				...
				if ($delItem['templateid'] != 0) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete templated item.'));
				}

				if ($delItem['flags'] == BS_FLAG_DISCOVERY_CREATED) {
	...
	public function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectDiscoveryRule'] !== null && $options['selectDiscoveryRule'] != API_OUTPUT_COUNT) {
			...
			$dbRules = DBselect(
				'SELECT id1.itemid,id2.parent_itemid'.
					...
					' AND i.flags='.BS_FLAG_DISCOVERY_CREATED
			);
		...
		if ($requestedOutput) {
			$history = Manager::History()->getLast($result, 2, BS_HISTORY_PERIOD);
