...
class CService extends CApiService {
	...
	public function validateUpdate(array $services) {
		...
		$this->checkServicePermissions(bs_objectValues($services, 'serviceid'));
	...
	public function update(array $services) {
		...
		if ($deleteParentsForServiceIds) {
			$this->deleteParentDependencies(bs_objectValues($services, 'serviceid'));
		}
		...
		return ['serviceids' => bs_objectValues($services, 'serviceid')];
	}
	...
	protected function validateAddDependencies(array $dependencies) {
		...
		$serviceIds = array_merge(
			bs_objectValues($dependencies, 'serviceid'),
			bs_objectValues($dependencies, 'dependsOnServiceid')
		);
	...
	public function addDependencies(array $dependencies) {
		$dependencies = bs_toArray($dependencies);
		...
		return ['serviceids' => bs_objectValues($dependencies, 'serviceid')];
	}
	...
	public function deleteDependencies($serviceIds) {
		$serviceIds = bs_toArray($serviceIds);
	...
	public function validateAddTimes(array $serviceTimes) {
		...
		$this->checkServicePermissions(array_unique(bs_objectValues($serviceTimes, 'serviceid')));
	}
	...
	public function addTimes(array $serviceTimes) {
		$serviceTimes = bs_toArray($serviceTimes);
		$this->validateAddTimes($serviceTimes);

		DB::insert('services_times', $serviceTimes);

		return ['serviceids' => bs_objectValues($serviceTimes, 'serviceid')];
	}
	...
	public function getSla(array $options) {
		$serviceIds = (isset($options['serviceids'])) ? bs_toArray($options['serviceids']) : null;
		$intervals = (isset($options['intervals'])) ? bs_toArray($options['intervals']) : [];
			...
			if ($usedSeviceIds) {
				...
				if ($intervals) {
					...
					foreach ($intervals as $interval) {
						$intervalConditions[] = 'sa.clock BETWEEN '.bs_dbstr($interval['from']).' AND '.bs_dbstr($interval['to']);
					}
					$query = DBselect(
						'SELECT *'.
	...
	protected function escalateProblems(array $services, array $serviceProblems, array $slaData) {
		...
		foreach ($serviceProblems as $serviceId => $problemTriggers) {
			...
			$slaData[$serviceId]['problems'] = bs_array_merge($slaData[$serviceId]['problems'], $problemTriggers);
			...
			foreach ($service['parentDependencies'] as $dependency) {
				...
				if (isset($services[$parentServiceId])) {
					...
					if ($this->isStatusEnabled($parentService) && $parentService['status']) {
						...
						$parentProblems[$parentServiceId] = bs_array_merge($parentProblems[$parentServiceId], $problemTriggers);
					}
				}
			}
		}
		...
		$query = DBSelect(
			...
			' FROM (SELECT MAX(sa3.servicealarmid) AS servicealarmid'.
					' FROM (SELECT sa2.serviceid,MAX(sa2.clock) AS clock'.
							...
							' WHERE sa2.clock<'.bs_dbstr($beforeTime).
	...
	protected function checkName(array $service) {
		if (!isset($service['name']) || bs_empty($service['name'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Empty name.'));
		}
	}
	...
	protected function checkAlgorithm(array $service) {
		if (!isset($service['algorithm']) || !serviceAlgorithm($service['algorithm'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect algorithm for service "%1$s".', $service['name']));
		}
	}
	...
	protected function checkShowSla(array $service) {
		...
		if (!isset($service['showsla']) || !isset($showSlaValues[$service['showsla']])) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect calculate SLA value for service "%1$s".', $service['name']));
		}
	}
	...
	protected function checkGoodSla(array $service) {
		if ((!empty($service['showsla']) && empty($service['goodsla']))
			...
			self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect acceptable SLA for service "%1$s".', $service['name']));
		}
	}
	...
	protected function checkSortOrder(array $service) {
		if (!isset($service['sortorder']) || !bs_is_int($service['sortorder'])
			|| $service['sortorder'] < 0 || $service['sortorder'] > 999) {

			self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect sorder order for service "%1$s".', $service['name']));
		}
	}
	...
	protected function checkTriggerId(array $service) {
		if (!empty($service['triggerid']) && !bs_is_int($service['triggerid'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect trigger ID for service "%1$s".', $service['name']));
		}
	}
	...
	protected function checkParentId(array $service) {
		if (!empty($service['parentid']) && !bs_is_int($service['parentid'])) {
			if (isset($service['name'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect parent for service "%1$s".', $service['name']));
			}
			else {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect parent service.'));
			}
		}

		if (isset($service['serviceid']) && idcmp($service['serviceid'], $service['parentid'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Service cannot be parent and child at the same time.'));
		}
	}
	...
	protected function checkStatus(array $service) {
		if (!empty($service['status']) && !bs_is_int($service['status'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect status for service "%1$s".', $service['name']));
		}
	}
	...
	protected function checkDependency(array $dependency) {
		if (idcmp($dependency['serviceid'], $dependency['dependsOnServiceid'])) {
			...
			$service = reset($service);
			self::exception(BS_API_ERROR_PARAMETERS, _s('Service "%1$s" cannot be dependent on itself.', $service['name']));
		}
	...
	protected function checkThatParentsDontHaveTriggers(array $dependencies) {
		$parentServiceIds = array_unique(bs_objectValues($dependencies, 'serviceid'));
		if ($parentServiceIds) {
			$query = DBselect(
	...
	protected function dfCircularitySearch($id, $depId, &$arr) {
		if ($id == $depId) {
			// cycle found
			self::exception(BS_API_ERROR_PARAMETERS, _('Services form a circular dependency.'));
		}
		if (isset($arr[$depId])) {
	...
	protected function applyQueryFilterOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		if (CWebUser::getType() != USER_TYPE_SUPER_ADMIN) {
			...
			if ($options['filter']['triggerid']) {
				...
				$options['filter']['triggerid'] = bs_objectValues($accessibleTriggers, 'triggerid');
			}
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectDependencies'] !== null && $options['selectDependencies'] != API_OUTPUT_COUNT) {
			...
			$dependencies = bs_toHash($dependencies, 'linkid');
