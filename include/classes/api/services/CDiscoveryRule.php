...
class CDiscoveryRule extends CItemGeneral {
	...
	public function get($options = []) {
		...
		$sqlParts = [
			...
			'from'		=> ['items' => 'items i'],
			'where'		=> ['i.flags='.BS_FLAG_DISCOVERY_RULE],
			'group'		=> [],
			'order'		=> [],
			'limit'		=> null
		];
	...
	public function delete(array $ruleids, $nopermissions = false) {
		...
		if (!empty($iprototypeids)) {
			if (!API::Itemprototype()->delete($iprototypeids, true)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete discovery rule'));
			}
		}
		...
		if ($hostPrototypeIds) {
			if (!API::HostPrototype()->delete($hostPrototypeIds, true)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete host prototype.'));
			}
		}
	...
	public function copy(array $data) {
		...
		if (!isset($data['discoveryids']) || !$data['discoveryids']) {
			self::exception(BS_API_ERROR_PARAMETERS, _('No discovery rule IDs given.'));
		}
		if (!isset($data['hostids']) || !$data['hostids']) {
			self::exception(BS_API_ERROR_PARAMETERS, _('No host IDs given.'));
		}
	...
	protected function copyTriggerPrototypes(array $srcDiscovery, array $dstDiscovery, array $srcHost, array $dstHost) {
		...
		if (!$result) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Cannot clone trigger prototypes.'));
		}
	...
	protected function updateReal($items) {
		...
		$ruleIds = bs_objectValues($items, 'itemid');
		$exRules = $this->get([
			'itemids' => $ruleIds,
			'output' => ['key_', 'name'],
			'selectHosts' => ['name'],
	...
	protected function checkInput(array &$items, $update = false, array $dbItems = []) {
		...
		foreach ($items as &$item) {
			$item['flags'] = BS_FLAG_DISCOVERY_RULE;
		...
		$validateItems = $items;
		if ($update) {
			$validateItems = $this->extendFromObjects(bs_toHash($validateItems, 'itemid'), $dbItems, ['name']);
		}
		...
		return [
			'validators' => [
				'macro' => new CStringValidator([
					'regex' => '/^'.BS_PREG_EXPRESSION_LLD_MACROS.'$/',
	...
	protected function inherit(array $items, array $hostids = null) {
		...
		foreach ($newItems as $newItem) {
			if (isset($newItem['itemid'])) {
				...
				$newItem['flags'] = BS_FLAG_DISCOVERY_RULE;
				$insertItems[] = $newItem;
			}
		}
		...
		if ($prototypes) {
			foreach ($prototypes as $key => $prototype) {
				...
				$prototype['applications'] = get_same_applications_for_host(
					bs_objectValues($prototype['applications'], 'applicationid'),
					$dstHost['hostid']
				);
			...
			if (!$rs) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot clone item prototypes.'));
			}
		}
	...
	protected function copyGraphPrototypes(array $srcDiscovery, array $dstDiscovery) {
		...
		if (!$rs) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Cannot clone graph prototypes.'));
		}
	...
	protected function copyHostPrototypes(array $srcDiscovery, array $dstDiscovery) {
		...
		if ($prototypes) {
			...
			if (!$rs) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot clone host prototypes.'));
			}
		}
		return $rs;
	}
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if (!is_null($options['selectItems'])) {
			if ($options['selectItems'] != API_OUTPUT_COUNT) {
				...
				$items = bs_toHash($items, 'parent_itemid');
		...
		if (!is_null($options['selectTriggers'])) {
			if ($options['selectTriggers'] != API_OUTPUT_COUNT) {
				...
				$triggers = bs_toHash($triggers, 'parent_itemid');
				foreach ($result as $itemid => $item) {
		...
		if (!is_null($options['selectGraphs'])) {
			if ($options['selectGraphs'] != API_OUTPUT_COUNT) {
				...
				$graphs = bs_toHash($graphs, 'parent_itemid');
				foreach ($result as $itemid => $item) {
		...
		if ($options['selectHostPrototypes'] !== null) {
			if ($options['selectHostPrototypes'] != API_OUTPUT_COUNT) {
				...
				$hostPrototypes = bs_toHash($hostPrototypes, 'parent_itemid');
			...
			if ($formulaRequested || $evalFormulaRequested || $conditionsRequested) {
				...
				foreach ($filters as &$filter) {
					...
					// in other cases - generate the formula automatically
						...
						$conditions = bs_toHash($filter['conditions'], 'item_conditionid');
