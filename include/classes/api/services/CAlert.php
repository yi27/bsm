...
class CAlert extends CApiService {
	...
	public function get($options = []) {
		...
		if ($options['eventsource'] == EVENT_SOURCE_TRIGGERS
			...
			$sqlParts['where'][] = 'EXISTS ('.
				...
				' WHERE a.actionid=aa.actionid'.
					' AND aa.eventsource='.bs_dbstr($options['eventsource']).
			')';
		}
		else {
			$sqlParts['where'][] = 'EXISTS ('.
				...
				' FROM events e'.
				' WHERE a.eventid=e.eventid'.
					' AND e.source='.bs_dbstr($options['eventsource']).
					' AND e.object='.bs_dbstr($options['eventobject']).
			')';
		}
		...
		if (!is_null($options['alertids'])) {
			bs_value2array($options['alertids']);
		...
		if (is_array($options['search'])) {
			bs_db_search('alerts a', $options, $sqlParts);
		}

		// time_from
		if (!is_null($options['time_from'])) {
			$sqlParts['where'][] = 'a.clock>'.bs_dbstr($options['time_from']);
		}

		// time_till
		if (!is_null($options['time_till'])) {
			$sqlParts['where'][] = 'a.clock<'.bs_dbstr($options['time_till']);
		}
	...
	protected function validateGet(array $options) {
		...
		if (!$sourceValidator->validate($options['eventsource'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect eventsource value.'));
		}
		...
		if (!$objectValidator->validate($options['eventobject'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect eventobject value.'));
		}
