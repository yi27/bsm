...
class CItemPrototype extends CItemGeneral {
	...
	public function get($options = []) {
		...
		$sqlParts = [
			...
			'from'		=> ['items' => 'items i'],
			'where'		=> ['i.flags='.BS_FLAG_DISCOVERY_PROTOTYPE],
			'group'		=> [],
			'order'		=> [],
			'limit'		=> null
		];
...
// search
		if (is_array($options['search'])) {
			bs_db_search('items i', $options, $sqlParts);
		}
...
//----------
	...
	protected function checkInput(array &$items, $update = false) {
		...
		foreach ($items as &$item) {
			$item['flags'] = BS_FLAG_DISCOVERY_PROTOTYPE;
	...
	public function create($items) {
		$items = bs_toArray($items);
		$this->checkInput($items);
		$this->createReal($items);
		$this->inherit($items);

		return ['itemids' => bs_objectValues($items, 'itemid')];
	}
		...
		foreach ($items as $item) {
			...
			if (array_key_exists('applicationPrototypes', $item) && is_array($item['applicationPrototypes'])
				...
				$db_application_prototypes = DBfetchArray(DBselect(
					...
					' WHERE ap.itemid='.bs_dbstr($item['ruleid']).
						' AND '.dbConditionString('ap.name', bs_objectValues($item['applicationPrototypes'], 'name'))
				));

				$names = bs_objectValues($db_application_prototypes, 'name');
				...
				$ids = array_merge($new_ids, bs_objectValues($db_application_prototypes, 'application_prototypeid'));
			...
			if (array_key_exists('applicationPrototypes', $item) && is_array($item['applicationPrototypes'])) {
				...
				$db_item_application_prototypes = DBfetchArray(DBselect(
					...
					' WHERE ap.application_prototypeid=iap.application_prototypeid'.
						' AND ap.itemid='.bs_dbstr($discovery_ruleid).
						' AND iap.itemid='.bs_dbstr($item['itemid'])
				));
				...
				$application_prototypes = bs_toHash($item['applicationPrototypes'], 'name');
				 ...
				 */
				$db_item_application_prototypes = bs_toHash($db_item_application_prototypes, 'name');
		...
		if (!$nopermissions) {
			foreach ($prototypeids as $prototypeid) {
				...
				if ($delItemPrototypes[$prototypeid]['templateid'] != 0) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete templated items'));
				}
			}
		}
...
// TRIGGER PROTOTYPES
		...
		if ($db_item_application_prototypes) {
			DB::delete('item_application_prototype', [
				'item_application_prototypeid' => bs_objectValues($db_item_application_prototypes,
					'item_application_prototypeid'
				)
			]);

			$this->deleteApplicationPrototypes(bs_objectValues($db_item_application_prototypes,
...
// TODO: remove info from API
	...
	protected function deleteApplicationPrototypes(array $application_prototypeids) {
		...
		if ($db_application_prototypes) {
			...
			DB::delete('application_prototype', [
				'application_prototypeid' => bs_objectValues($db_application_prototypes, 'application_prototypeid')
			]);
			...
			if ($discovered_applications) {
				$discovered_applicationids = bs_objectValues($discovered_applications, 'applicationid');
				...
				$applications_to_delete = array_diff($discovered_applicationids,
					bs_objectValues($discovered_applications_to_delete, 'applicationid')
				);
	...
	public function syncTemplates($data) {
		...
		foreach ($items as $inum => $item) {
			$items[$inum]['applications'] = bs_objectValues($item['applications'], 'applicationid');
		}
	...
	protected function inherit(array $items, array $hostids = null) {
		...
		$dbResult = DBselect(
			...
			' WHERE i.templateid=id.parent_itemid'.
				' AND '.dbConditionInt('id.itemid', bs_objectValues($items, 'itemid'))
		);
		...
		foreach ($newItems as $newItem) {
			if (isset($newItem['itemid'])) {
				...
				$newItem['flags'] = BS_FLAG_DISCOVERY_PROTOTYPE;
				$insertItems[] = $newItem;
			}
		}
	...
	public function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectApplicationPrototypes'] !== null
			...
			$application_prototypes = bs_toHash($application_prototypes, 'application_prototypeid');
		...
		if (!is_null($options['selectTriggers'])) {
			if ($options['selectTriggers'] != API_OUTPUT_COUNT) {
				...
				$triggers = bs_toHash($triggers, 'itemid');
		...
		if (!is_null($options['selectGraphs'])) {
			if ($options['selectGraphs'] != API_OUTPUT_COUNT) {
				...
				$graphs = bs_toHash($graphs, 'itemid');
