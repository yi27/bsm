...
class CTemplateScreen extends CScreen {
	...
	public function get($options = []) {
		...
		if (is_array($options['search'])) {
			bs_db_search('screens s', $options, $sqlParts);
		}
		...
		if (!is_null($options['selectScreenItems']) && !is_null($options['hostids'])) {
			...
			if (!empty($graphids)) {
				...
				$dbGraphs = API::Graph()->get([
					...
					'selectHosts' => ['hostid'],
					'hostids' => $options['hostids'],
					'filter' => ['name' => bs_objectValues($tplGraphs, 'name')],
					'nopermissions' => true,
					'preservekeys' => true
				]);
				$realGraphs = [];
			...
			if (!empty($itemids)) {
				...
				$dbItems = API::Item()->get([
					'output' => ['itemid', 'key_', 'hostid'],
					'hostids' => $options['hostids'],
					'filter' => ['key_' => bs_objectValues($tplItems, 'key_')],
					'nopermissions' => true,
					'preservekeys' => true
				]);
		...
		// hashing
		$options['hostids'] = bs_toHash($options['hostids']);
		if (is_null($options['countOutput'])
		...
		elseif (!is_null($options['noInheritance'])) {
			$result = bs_toHash($result, 'screenid');
		}
	...
	public function copy(array $data) {
		$screenIds = $data['screenIds'] = bs_toArray($data['screenIds']);
		$templateIds = $data['templateIds'] = bs_toArray($data['templateIds']);
		...
		foreach ($templateIds as $templateId) {
			...
			$dbItems = DBselect(
				...
				' WHERE dest.key_=src.key_'.
					' AND dest.hostid='.bs_dbstr($templateId).
			...
			$dbGraphs = DBselect(
				...
				' WHERE dest.name=src.name'.
					...
					' AND desti.hostid='.bs_dbstr($templateId).
						...
						case SCREEN_RESOURCE_LLD_GRAPH:
							if ($resourceId && !isset($resourceGraphsMap[$resourceId])) {
								$graph = DBfetch(DBselect(
									'SELECT g.name FROM graphs g WHERE g.graphid='.bs_dbstr($resourceId)
								));
								$template = DBfetch(DBselect(
									'SELECT h.name FROM hosts h WHERE h.hostid='.bs_dbstr($templateId)
								));
						...
						default:
							if ($resourceId && !isset($resourceItemsMap[$resourceId])) {
								$item = DBfetch(DBselect(
									'SELECT i.name FROM items i WHERE i.itemid='.bs_dbstr($resourceId)
								));
								$template = DBfetch(DBselect(
									'SELECT h.name FROM hosts h WHERE h.hostid='.bs_dbstr($templateId)
								));
	...
	protected function validateCreate(array $screens) {
		...
		$templateIds = bs_objectValues($screens, 'templateid');

		$dbScreens = $this->get([
			'filter' => [
				'name' => bs_objectValues($screens, 'name'),
		...
		foreach ($screens as $screen) {
			...
			if (isset($screen['name'])) {
				...
				if ($dbScreenExist && bccomp($dbScreenExist['screenid'], $screen['screenid']) != 0) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Template screen "%1$s" already exists.', $screen['name']));
				}
			}
		}
	}
	...
	protected function validateCopy(array $data) {
		...
		$dbExistingScreens = $this->get([
			'filter' => [
				'name' => bs_objectValues($dbScreens, 'name'),
				'templateid' => $templateIds
			],
			'output' => ['name', 'templateid'],
			'preservekeys' => true
		...
		foreach ($dbExistingScreens as $dbExistingScreen) {
			$dbTemplate = DBfetch(DBselect(
				'SELECT h.name'.
				' FROM hosts h'.
				' WHERE h.hostid='.bs_dbstr($dbExistingScreen['templateid'])
			));
