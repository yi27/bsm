...
abstract class CGraphGeneral extends CApiService {
	...
	public function update(array $graphs) {
		$graphs = bs_toArray($graphs);
		$graphIds = bs_objectValues($graphs, 'graphid');
			...
			if (isset($graph['gitems'])) {
				foreach ($graph['gitems'] as $item) {
					if (isset($item['gitemid']) && !$item['gitemid']) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Missing "gitemid" field for item.'));
					}
	...
	protected function updateReal(array $graph, array $dbGraph) {
		$dbGitems = bs_toHash($dbGraph['gitems'], 'gitemid');
		$dbGitemIds = bs_toHash(bs_objectValues($dbGitems, 'gitemid'));
	...
	protected function checkAxisItems(array $graph, $tpl = false) {
		...
		if (!empty($axisItems)) {
			...
			if ($cntExist != count($axisItems)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect item for axis value.'));
			}
		}
		...
		if (($graph['graphtype'] == GRAPH_TYPE_NORMAL || $graph['graphtype'] == GRAPH_TYPE_STACKED)
			...
			self::exception(BS_API_ERROR_PARAMETERS, _('Y axis MAX value must be greater than Y axis MIN value.'));
		}
	}
	...
	protected function validateItemsCreate(array $graphs) {
		...
		foreach ($graphs as $graph) {
			...
			if (!check_db_fields($fields, $graph)) {
				self::exception(BS_API_ERROR_PARAMETERS, _($this->getErrorMsg(self::ERROR_MISSING_GRAPH_NAME)));
			}
			...
			if (isset($graph['gitems'])) {
				...
				foreach ($graph['gitems'] as $gitem) {
					...
					if (!check_db_fields($fields, $gitem)) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Missing "itemid" field for item.'));
					}
			...
			if (isset($graph['ymin_type']) && $graph['ymin_type'] == GRAPH_YAXIS_TYPE_ITEM_VALUE) {
				if (!isset($graph['ymin_itemid']) || bs_empty($graph['ymin_itemid'])) {
					self::exception(BS_API_ERROR_PARAMETERS,
			...
			if (isset($graph['ymax_type']) && $graph['ymax_type'] == GRAPH_YAXIS_TYPE_ITEM_VALUE) {
				if (!isset($graph['ymax_itemid']) || bs_empty($graph['ymax_itemid'])) {
					self::exception(BS_API_ERROR_PARAMETERS,
			...
			if (isset($graph['gitems'])) {
				...
				if (HOST_STATUS_TEMPLATE == $host['status']) {
					...
					if ($itemIds) {
						...
						$itemHosts = array_unique(bs_objectValues($itemHosts, 'hostid'));
