...
abstract class CHostBase extends CApiService {
	...
	protected function link(array $templateIds, array $targetIds) {
		...
		$templateIdDuplicates = bs_arrayFindDuplicates($templateIds);
		if (!bs_empty($templateIdDuplicates)) {
			$duplicatesFound = [];
		...
		if (count($visited) < count($all)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Circular template linkage is not allowed.'));
		}
	...
	protected function unlink($templateids, $targetids = null) {
		...
		if (!empty($hosts)) {
			...
			$hosts = implode(', ', bs_objectValues($hosts, 'host'));
			$templates = implode(', ', bs_objectValues($templates, 'host'));
	...
	protected function checkCircularAndDoubleLinkage($graph, $current, &$path, &$visited) {
		if (isset($path[$current])) {
			if ($path[$current] == 1) {
				...
				self::exception(BS_API_ERROR_PARAMETERS, _('Template cannot be linked to another template more than once even through other templates.'));
			}
		}
		$path[$current] = 1;
		$visited[$current] = 1;
