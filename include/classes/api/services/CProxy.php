...
class CProxy extends CApiService {
		...
		foreach ($proxies as $key => $proxy) {
			if (!empty($proxy['hosts'])) {
				$hostUpdate[] = [
					...
					'where' => ['hostid' => bs_objectValues($proxy['hosts'], 'hostid')]
				];
			}
			...
			if ($proxy['status'] == HOST_STATUS_PROXY_PASSIVE) {
				...
				if (!API::HostInterface()->create($proxy['interface'])) {
					self::exception(BS_API_ERROR_INTERNAL, _('Proxy interface creation failed.'));
				}
			}
		}
	...
	public function update(array $proxies) {
		...
		$proxyids = bs_objectValues($proxies, 'proxyid');
		...
		foreach ($proxies as $proxy) {
			...
			elseif (array_key_exists('interface', $proxy) && is_array($proxy['interface'])) {
				...
				if (!$result) {
					self::exception(BS_API_ERROR_INTERNAL, _('Proxy interface update failed.'));
				}
			}
		}
	...
	protected function checkUsedInDiscoveryRule(array $proxyIds) {
		...
		if ($dRule) {
			$proxy = DBfetch(DBselect('SELECT h.host FROM hosts h WHERE h.hostid='.bs_dbstr($dRule['proxy_hostid'])));
	...
	protected function checkUsedForMonitoring(array $proxyIds) {
		...
		if ($host) {
			$proxy = DBfetch(DBselect('SELECT h.host FROM hosts h WHERE h.hostid='.bs_dbstr($host['proxy_hostid'])));
	...
	protected function validateEncryption(array $proxies) {
		foreach ($proxies as $proxy) {
			...
			if ((array_key_exists('tls_connect', $proxy) && $proxy['tls_connect'] == HOST_ENCRYPTION_PSK
				...
				if (!array_key_exists('tls_psk_identity', $proxy) || bs_empty($proxy['tls_psk_identity'])) {
					self::exception(BS_API_ERROR_PARAMETERS,
						_s('Incorrect value for field "%1$s": %2$s.', 'tls_psk_identity', _('cannot be empty'))
					);
				}

				if (!array_key_exists('tls_psk', $proxy) || bs_empty($proxy['tls_psk'])) {
					self::exception(BS_API_ERROR_PARAMETERS,
	...
	protected function validateCreate(array $proxies) {
		...
		foreach ($proxies as $proxy) {
			if (!check_db_fields($proxy_db_fields, $proxy)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Wrong fields for proxy "%1$s".', $proxy['host']));
			}

			if (!preg_match('/^'.BS_PREG_HOST_FORMAT.'$/', $proxy['host'])) {
				self::exception(BS_API_ERROR_PARAMETERS,
		...
		if ($proxy_exists) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Proxy "%s" already exists.', $proxy_exists[0]['host']));
		}
		...
		foreach ($proxies as $proxy) {
			if (!array_key_exists('status', $proxy)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('No status for proxy.'));
			}
			...
			if ($proxy['status'] == HOST_STATUS_PROXY_PASSIVE
				...
				self::exception(BS_API_ERROR_PARAMETERS, _s('No interface provided for proxy "%s".', $proxy['host']));
			}

			if (array_key_exists('hosts', $proxy) && $proxy['hosts']) {
				$hostids = array_merge($hostids, bs_objectValues($proxy['hosts'], 'hostid'));
			}
		}
	...
	protected function validateUpdate(array $proxies, array $db_proxies) {
				...
				if (array_key_exists('host', $proxy) && $proxy['host'] !== $db_proxies[$proxy['proxyid']]['host']) {
					foreach ($proxies_exists as $proxy_exists) {
						if (bccomp($proxy_exists['proxyid'], $proxy['proxyid']) != 0) {
							self::exception(BS_API_ERROR_PARAMETERS, _s('Proxy "%s" already exists.', $proxy['host']));
						}
					}
				}
			}
		...
		$proxies = $this->extendFromObjects(bs_toHash($proxies, 'proxyid'), $db_proxies, [
