...
class CHost extends CHostGeneral {
	...
	public function get($options = []) {
		...
		$sqlParts = [
			...
			'from'		=> ['hosts' => 'hosts h'],
			'where'		=> ['flags' => 'h.flags IN ('.BS_FLAG_DISCOVERY_NORMAL.','.BS_FLAG_DISCOVERY_CREATED.')'],
			'group'		=> [],
			'order'		=> [],
			'limit'		=> null
		];
		...
		if (!is_null($options['proxyids'])) {
			bs_value2array($options['proxyids']);
		...
		if (!is_null($options['interfaceids'])) {
			bs_value2array($options['interfaceids']);
		...
		if (!is_null($options['itemids'])) {
			bs_value2array($options['itemids']);
		...
		if (!is_null($options['httptestids'])) {
			bs_value2array($options['httptestids']);
		...
		if (!is_null($options['applicationids'])) {
			bs_value2array($options['applicationids']);
		...
		if (!is_null($options['dserviceids'])) {
			bs_value2array($options['dserviceids']);
		...
		if (!is_null($options['with_graphs'])) {
			$sqlParts['where'][] = 'EXISTS ('.
					...
					' WHERE i.hostid=h.hostid'.
						...
						' AND g.flags IN ('.BS_FLAG_DISCOVERY_NORMAL.','.BS_FLAG_DISCOVERY_CREATED.')'.
					')';
		}
		...
		if (is_array($options['search'])) {
			bs_db_search('hosts h', $options, $sqlParts);

			if (bs_db_search('interface hi', $options, $sqlParts)) {
				$sqlParts['from']['interface'] = 'interface hi';
				$sqlParts['where']['hi'] = 'h.hostid=hi.hostid';
			}
		}
		...
		if ($options['searchInventory'] !== null) {
			...
			bs_db_search('host_inventory hii',
				[
	...
	public function update($hosts) {
		...
		$inventories = bs_toHash($inventories, 'hostid');
		...
		foreach ($hosts as $host) {
			...
			if (!$result) {
				self::exception(BS_API_ERROR_INTERNAL, _('Host update failed.'));
			}
		}
	...
	public function massAdd(array $data) {
		$hosts = isset($data['hosts']) ? bs_toArray($data['hosts']) : [];
		$hostIds = bs_objectValues($hosts, 'hostid');

		// check permissions
		if (!$this->isWritable($hostIds)) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('You do not have permission to perform this operation.'));
		}
		...
		if (!empty($data['interfaces'])) {
			API::HostInterface()->massAdd([
				'hosts' => $data['hosts'],
				'interfaces' => bs_toArray($data['interfaces'])
			]);
		}
	...
	public function massUpdate($data) {
		$hosts = bs_toArray($data['hosts']);
		$inputHostIds = bs_objectValues($hosts, 'hostid');
		...
		if ((array_key_exists('tls_connect', $data) || array_key_exists('tls_accept', $data)
			...
			self::exception(BS_API_ERROR_PERMISSIONS, _(
		...
		if (isset($data['groups']) && empty($data['groups'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('No groups for hosts.'));
		}
		...
		if (isset($data['name'])) {
			if (count($hosts) > 1) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot mass update visible host name.'));
			}
		}

		if (isset($data['host'])) {
			if (!preg_match('/^'.BS_PREG_HOST_FORMAT.'$/', $data['host'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect characters used for host name "%s".', $data['host']));
			}

			if (count($hosts) > 1) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot mass update host name.'));
			}
			...
			if ($sameHostnameHost && (bccomp($sameHostnameHost['hostid'], $curHost['hostid']) != 0)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Host "%1$s" already exists.', $data['host']));
			}
			...
			if ($sameHostnameTemplate) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Template "%1$s" already exists.', $data['host']));
			}
		}
		...
		if (array_key_exists('templates_clear', $data)) {
			$updateTemplatesClear = bs_toArray($data['templates_clear']);
		}
		...
		if (isset($data['inventory']) && $data['inventory']) {
			if (isset($data['inventory_mode']) && $data['inventory_mode'] == HOST_INVENTORY_DISABLED) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot set inventory fields for disabled inventory.'));
			}
		...
		if (isset($updateTemplatesClear)) {
			$templateIdsClear = bs_objectValues($updateTemplatesClear, 'templateid');
		...
		if (isset($updateTemplates)) {
			...
			$newTemplateids = bs_objectValues($updateTemplates, 'templateid');
			...
			if ($templatesToDel) {
				...
				if (!$result) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot unlink template'));
				}
			}
		}
		...
		if (isset($updateTemplates)) {
			...
			if (!$result) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot link template'));
			}
		}
		...
		if (isset($updateInventory)) {
			...
			if ($updateInventory['inventory_mode'] == HOST_INVENTORY_DISABLED) {
				...
				if (!DBexecute($sql)) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete inventory.'));
				}
			}
		...
		if (isset($updateGroups)) {
			$updateGroups = bs_toArray($updateGroups);

			$hostGroups = API::HostGroup()->get([
				'output' => ['groupid'],
				'hostids' => $hostids
			]);
			$hostGroupIds = bs_objectValues($hostGroups, 'groupid');
			$newGroupIds = bs_objectValues($updateGroups, 'groupid');

			$groupsToAdd = array_diff($newGroupIds, $hostGroupIds);
			if ($groupsToAdd) {
				$this->massAdd([
					'hosts' => $hosts,
					'groups' => bs_toObject($groupsToAdd, 'groupid')
				]);
			}
	...
	public function massRemove(array $data) {
		$hostids = bs_toArray($data['hostids']);
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectInventory'] !== null) {
			...
			$result = $relationMap->mapOne($result, bs_toHash($inventory, 'hostid'), 'inventory');
		}
		...
		if ($options['selectInterfaces'] !== null) {
			if ($options['selectInterfaces'] != API_OUTPUT_COUNT) {
				...
				order_result($interfaces, 'interfaceid', BS_SORT_UP);
				...
				$interfaces = bs_toHash($interfaces, 'hostid');
		...
		if ($options['selectScreens'] !== null) {
			if ($options['selectScreens'] != API_OUTPUT_COUNT) {
				...
				$screens = bs_toHash($screens, 'hostid');
	...
	protected function validateEncryption(array $hosts) {
		foreach ($hosts as $host) {
			...
			if (array_key_exists('tls_connect', $host) && !in_array($host['tls_connect'], $available_connect_types)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect value for field "%1$s": %2$s.', 'tls_connect',
			...
			if (array_key_exists('tls_accept', $host) && !in_array($host['tls_accept'], $available_accept_types)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect value for field "%1$s": %2$s.', 'tls_accept',
			...
			if ((array_key_exists('tls_connect', $host) && $host['tls_connect'] == HOST_ENCRYPTION_PSK)
					|| (array_key_exists('tls_accept', $host)
						&& ($host['tls_accept'] & HOST_ENCRYPTION_PSK) == HOST_ENCRYPTION_PSK)) {
				if (!array_key_exists('tls_psk_identity', $host) || bs_empty($host['tls_psk_identity'])) {
					self::exception(BS_API_ERROR_PARAMETERS,
						_s('Incorrect value for field "%1$s": %2$s.', 'tls_psk_identity', _('cannot be empty'))
					);
				}

				if (!array_key_exists('tls_psk', $host) || bs_empty($host['tls_psk'])) {
					self::exception(BS_API_ERROR_PARAMETERS,
				...
				if (!preg_match('/^([0-9a-f]{2})+$/i', $host['tls_psk'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _(
	...
	protected function validateCreate(array $hosts) {
		...
		foreach ($hosts as &$host) {
			...
			// Validate "host" field.
			if (!preg_match('/^'.BS_PREG_HOST_FORMAT.'$/', $host['host'])) {
				self::exception(BS_API_ERROR_PARAMETERS,
			...
			if (!array_key_exists('groups', $host) || !is_array($host['groups']) || !$host['groups']) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('No groups for host "%1$s".', $host['host']));
			}

			$groupids = array_merge($groupids, bs_objectValues($host['groups'], 'groupid'));
		}
		unset($host);
		...
		$inventory_fields = bs_objectValues(getHostInventories(), 'db_field');
		...
		foreach ($hosts as $host) {
			if (!array_key_exists('interfaces', $host) || !is_array($host['interfaces']) || !$host['interfaces']) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('No interfaces for host "%s".', $host['host']));
			}
			...
			if (array_key_exists('inventory', $host) && $host['inventory']) {
				...
				foreach ($fields as $field) {
					if (!in_array($field, $inventory_fields)) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect inventory field "%s".', $field));
					}
				}
			}
	...
	protected function validateUpdate(array $hosts, array $db_hosts) {
		...
		foreach ($hosts as $host) {
			...
			// Validate "groups" field.
			if (array_key_exists('groups', $host) && (!is_array($host['groups']) || !$host['groups'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('No groups for host "%1$s".',
		...
		foreach ($hosts as &$host) {
			...
			$hosts_full[] = bs_array_merge($db_host, $host);
		}
		unset($host);
