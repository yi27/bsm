...
class CTrend extends CApiService {
	...
	public function get($options = []) {
		...
		$options = bs_array_merge($default_options, $options);
		...
		if ($options['time_from'] !== null) {
			$sql_where['clock_from'] = 't.clock>='.bs_dbstr($options['time_from']);
		}

		if ($options['time_till'] !== null) {
			$sql_where['clock_till'] = 't.clock<='.bs_dbstr($options['time_till']);
		}

		if ($options['countOutput'] === null) {
			$sql_limit = ($options['limit'] && bs_ctype_digit($options['limit'])) ? $options['limit'] : null;
