...
class CDRule extends CApiService {
	...
	public function get(array $options = []) {
		...
		if (CWebUser::getType() < USER_TYPE_BSM_ADMIN) {
			return [];
		}

// druleids
		if (!is_null($options['druleids'])) {
			bs_value2array($options['druleids']);
			$sqlParts['where']['druleid'] = dbConditionInt('dr.druleid', $options['druleids']);
		}

// dhostids
		if (!is_null($options['dhostids'])) {
			bs_value2array($options['dhostids']);
...
// search
		if (!is_null($options['search'])) {
			bs_db_search('drules dr', $options, $sqlParts);
		}
...
// removing keys (hash -> array)
	...
	public function checkInput(array &$dRules) {
		$dRules = bs_toArray($dRules);

		if (empty($dRules)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Empty input.'));
		}
		...
		$proxies = [];
		$ipRangeValidator = new CIPRangeValidator(['ipRangeLimit' => BS_DISCOVERER_IPRANGE_LIMIT]);
		foreach ($dRules as $dRule) {
			if (!isset($dRule['iprange'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('IP range cannot be empty.'));
			}

			if (!$ipRangeValidator->validate($dRule['iprange'])) {
				self::exception(BS_API_ERROR_PARAMETERS, $ipRangeValidator->getError());
			}

			if (isset($dRule['delay']) && $dRule['delay'] < 0) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect delay.'));
			}

			if (isset($dRule['status']) && (($dRule['status'] != DRULE_STATUS_DISABLED) && ($dRule['status'] != DRULE_STATUS_ACTIVE))) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect status.'));
			}

			if (empty($dRule['dchecks'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot save discovery rule without checks.'));
			}
		...
		if (!empty($proxies)) {
			...
			foreach ($proxies as $proxy) {
				if (!isset($proxiesDB[$proxy])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect proxyid.'));
				}
			}
		}
	}
	...
	protected function validateDChecks(array &$dChecks) {
		...
		foreach ($dChecks as $dcnum => $dCheck) {
			if (isset($dCheck['uniq']) && ($dCheck['uniq'] == 1)) {
				if (!in_array($dCheck['type'], [SVC_AGENT, SVC_SNMPv1, SVC_SNMPv2c, SVC_SNMPv3])) {
					self::exception(BS_API_ERROR_PARAMETERS,
						_('Only Bsm agent, SNMPv1, SNMPv2 and SNMPv3 checks can be made unique.')
					);
				}
			...
			if (isset($dCheck['ports']) && !validate_port_list($dCheck['ports'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect port range.'));
			}

			switch ($dCheck['type']) {
				case SVC_AGENT:
					if (!isset($dCheck['key_'])) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect key.'));
					}
				...
				case SVC_SNMPv1:
				case SVC_SNMPv2c:
					if (!isset($dCheck['snmp_community']) || bs_empty($dCheck['snmp_community'])) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect SNMP community.'));
					}
				case SVC_SNMPv3:
					if (!isset($dCheck['key_']) || bs_empty($dCheck['key_'])) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect SNMP OID.'));
					}
					break;
			}
		...
		if ($uniq > 1) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Only one check can be unique.'));
		}
	}
	...
	protected function validateRequiredFields($dRules, $on) {
		if ($on == 'update') {
			foreach ($dRules as $dRule) {
				if (!isset($dRule['druleid']) || bs_empty($dRule['druleid'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Field "druleid" is required.'));
				}
			}
		}
		else {
			foreach ($dRules as $dRule) {
				if (!isset($dRule['name']) || bs_empty($dRule['name'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Field "name" is required.'));
				}
			}
		}
	}
	...
	protected function validateDuplicateChecks(array $dChecks) {
		...
		while ($current = array_pop($dChecks)) {
			foreach ($dChecks as $dCheck) {
				...
				if ($equal) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Checks should be unique.'));
				}
			}
		}
	}
	...
	public function create(array $dRules) {
		...
		$dbDRules = API::getApiService()->select($this->tableName(), [
			'output' => ['name'],
			'filter' => ['name' => bs_objectValues($dRules, 'name')],
			'limit' => 1
		]);

		if ($dbDRules) {
			$dbDRule = reset($dbDRules);
			self::exception(BS_API_ERROR_PARAMETERS, _s('Discovery rule "%1$s" already exists.', $dbDRule['name']));
		}
	...
	public function update(array $dRules) {
		...
		$dRuleIds = bs_objectValues($dRules, 'druleid');
		...
		foreach ($dRules as $dRule) {
			...
			if ($dRulesDb[$dRule['druleid']]['name'] !== $dRule['name']) {
				if (isset($dRuleNamesChanged[$dRule['name']])) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Discovery rule "%1$s" already exists.',
						$dRule['name']
					));
				}
				else {
		...
		foreach ($dRules as $dRule) {
			...
			$delDCheckIds = array_diff(
				bs_objectValues($dbChecks, 'dcheckid'),
				bs_objectValues($oldChecks, 'dcheckid')
			);
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if (!is_null($options['selectDChecks'])) {
			if ($options['selectDChecks'] != API_OUTPUT_COUNT) {
				...
				$dchecks = bs_toHash($dchecks, 'druleid');
		...
		if (!is_null($options['selectDHosts'])) {
			if ($options['selectDHosts'] != API_OUTPUT_COUNT) {
				...
				$dhosts = bs_toHash($dhosts, 'druleid');
				foreach ($result as $druleid => $drule) {
