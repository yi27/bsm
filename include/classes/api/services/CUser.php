...
class CUser extends CApiService {
	 ...
	 * @param bool   $options['type']			filter by User type [USER_TYPE_BSM_USER: 1, USER_TYPE_BSM_ADMIN: 2, USER_TYPE_SUPER_ADMIN: 3]
	...
	public function get($options = []) {
		...
		if ($options['mediaids'] !== null) {
			bs_value2array($options['mediaids']);
		...
		if (is_array($options['filter'])) {
			if (isset($options['filter']['passwd'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('It is not possible to filter by user password.'));
			}
		...
		if (is_array($options['search'])) {
			if (isset($options['search']['passwd'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('It is not possible to search by user password.'));
			}

			bs_db_search('users u', $options, $sqlParts);
		}
		...
		if ($options['getAccess'] !== null) {
			...
			while ($userAccess = DBfetch($access)) {
				$result[$userAccess['userid']] = bs_array_merge($result[$userAccess['userid']], $userAccess);
			}
		}
	...
	protected function checkInput(&$users, $method) {
		...
		if ($update) {
			...
			$dbUsers = $this->get([
				...
				'userids' => bs_objectValues($users, 'userid'),
				'editable' => true,
				'preservekeys' => true
			]);
		}
		...
		foreach ($users as &$user) {
			if (!check_db_fields($userDBfields, $user)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Wrong fields for user "%s".', $user['alias']));
			}

			// permissions
			if ($create) {
				if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
					self::exception(BS_API_ERROR_PARAMETERS, _('You do not have permissions to create users.'));
				}
			...
			elseif ($update) {
				if (!isset($dbUsers[$user['userid']])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('You do not have permissions to update user or user does not exist.'));
				}

				if (bccomp(self::$userData['userid'], $user['userid']) != 0 && self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
					self::exception(BS_API_ERROR_PARAMETERS, _('You do not have permissions to update other users.'));
				}
			...
			if (isset($user['alias'])) {
				...
				if ($dbUser['alias'] === BS_GUEST_USER && $user['alias'] !== BS_GUEST_USER) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot rename guest user.'));
				}
				...
				if (!isset($alias[$user['alias']])) {
					...
					if ($create || bccomp($user['userid'], $alias[$user['alias']]) != 0) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('Duplicate user alias "%s".', $user['alias']));
					}
				}
			}
			...
			if (isset($user['usrgrps'])) {
				if (empty($user['usrgrps'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('User "%s" cannot be without user group.', $dbUser['alias']));
				}
				...
				if (!$create && bccomp(self::$userData['userid'], $user['userid']) == 0) {
					$usrgrps = API::UserGroup()->get([
						'usrgrpids' => bs_objectValues($user['usrgrps'], 'usrgrpid'),
						'output' => API_OUTPUT_EXTEND,
						'preservekeys' => true,
						'nopermissions' => true
					]);
					foreach ($usrgrps as $groupid => $group) {
						if ($group['gui_access'] == GROUP_GUI_ACCESS_DISABLED) {
							self::exception(BS_API_ERROR_PARAMETERS, _s('User may not modify GUI access for himself by becoming a member of user group "%s".', $group['name']));
						}

						if ($group['users_status'] == GROUP_STATUS_DISABLED) {
							self::exception(BS_API_ERROR_PARAMETERS, _s('User may not modify system status for himself by becoming a member of user group "%s".', $group['name']));
						}
					}
				}
			}
			...
			if (isset($user['type']) && (USER_TYPE_SUPER_ADMIN != self::$userData['type'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('You are not allowed to alter privileges for user "%s".', $dbUser['alias']));
			}
			...
			if (array_key_exists('passwd', $user)) {
				if (is_null($user['passwd'])) {
					...
					if ($dbUser['alias'] == BS_GUEST_USER && !bs_empty($user['passwd'])) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Not allowed to set password for user "guest".'));
					}
			...
			if (isset($user['alias'])) {
				...
				if ($exUser = reset($userExist)) {
					if ($create || (bccomp($exUser['userid'], $user['userid']) != 0)) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('User with alias "%s" already exists.', $user['alias']));
					}
				}
			}
		}
	...
	public function create($users) {
		...
		foreach ($users as $unum => $user) {
			...
			$usrgrps = bs_objectValues($user['usrgrps'], 'usrgrpid');
			foreach ($usrgrps as $groupid) {
				$usersGroupdId = get_dbid('users_groups', 'id');
				$sql = 'INSERT INTO users_groups (id,usrgrpid,userid) VALUES ('.bs_dbstr($usersGroupdId).','.bs_dbstr($groupid).','.bs_dbstr($userid).')';

				if (!DBexecute($sql)) {
					self::exception(BS_API_ERROR_PARAMETERS, 'DBerror');
				}
			}
			...
			foreach ($user['user_medias'] as $mediaData) {
				...
				$sql = 'INSERT INTO media (mediaid,userid,mediatypeid,sendto,active,severity,period)'.
					' VALUES ('.bs_dbstr($mediaid).','.bs_dbstr($userid).','.bs_dbstr($mediaData['mediatypeid']).','.
					bs_dbstr($mediaData['sendto']).','.bs_dbstr($mediaData['active']).','.bs_dbstr($mediaData['severity']).','.
					bs_dbstr($mediaData['period']).')';
				if (!DBexecute($sql)) {
					self::exception(BS_API_ERROR_PARAMETERS, 'DBerror');
				}
			}
		...
		foreach ($users as $user) {
			...
			if (isset($user['usrgrps']) && !is_null($user['usrgrps'])) {
				...
				foreach ($usrgrps as $groupid => $group) {
					...
					$usersGroupdId = get_dbid('users_groups', 'id');
					$sql = 'INSERT INTO users_groups (id,usrgrpid,userid) VALUES ('.bs_dbstr($usersGroupdId).','.bs_dbstr($groupid).','.bs_dbstr($user['userid']).')';
	...
	public function delete(array $userids) {
		...
		$del_operations = DBFetchArray(DBselect(
			...
			' FROM operations o'.
			' WHERE '.dbConditionInt('o.operationid', bs_objectValues($db_operations, 'operationid')).
		...
		DB::delete('operations', ['operationid' => bs_objectValues($del_operations, 'operationid')]);
		...
		$actionids = bs_objectValues($del_operations, 'actionid');
		if ($actionids) {
	...
	protected function validateAddMedia(array $data) {
		if (self::$userData['type'] < USER_TYPE_BSM_ADMIN) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Only Bsm Admins can add user media.'));
		}

		if (!isset($data['users']) || !isset($data['medias'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Invalid method parameters.'));
		}

		$users = bs_toArray($data['users']);
		$media = bs_toArray($data['medias']);

		if (!$this->isWritable(bs_objectValues($users, 'userid'))) {
		...
		foreach ($media as $mediaItem) {
			if (!$timePeriodValidator->validate($mediaItem['period'])) {
				self::exception(BS_API_ERROR_PARAMETERS, $timePeriodValidator->getError());
			}
		}
	}
	...
	protected function addMediaReal(array $data) {
		...
		foreach ($users as $user) {
			foreach ($media as $mediaItem) {
				...
				$sql = 'INSERT INTO media (mediaid,userid,mediatypeid,sendto,active,severity,period)'.
						' VALUES ('.bs_dbstr($mediaId).','.bs_dbstr($user['userid']).','.bs_dbstr($mediaItem['mediatypeid']).','.
									bs_dbstr($mediaItem['sendto']).','.bs_dbstr($mediaItem['active']).','.bs_dbstr($mediaItem['severity']).','.
									bs_dbstr($mediaItem['period']).')';

				if (!DBexecute($sql)) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot insert user media.'));
				}
	...
	public function updateMedia(array $data) {
		...
		$userIds = array_keys(array_flip((bs_objectValues($users, 'userid'))));
		...
		if ($mediaToUpdate) {
			foreach ($mediaToUpdate as $media) {
				$result = DBexecute(
					'UPDATE media'.
					' SET mediatypeid='.bs_dbstr($media['mediatypeid']).','.
						' sendto='.bs_dbstr($media['sendto']).','.
						' active='.bs_dbstr($media['active']).','.
						' severity='.bs_dbstr($media['severity']).','.
						' period='.bs_dbstr($media['period']).
					' WHERE mediaid='.bs_dbstr($media['mediaid'])
				);

				if (!$result) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Cannot update user media.'));
				}
			}
		}
	...
	protected function validateUpdateMedia(array $data) {
		if (self::$userData['type'] < USER_TYPE_BSM_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only Bsm Admins can change user media.'));
		}
		...
		if (!$this->isWritable(bs_objectValues($users, 'userid'))) {
			self::exception(BS_API_ERROR_PARAMETERS, _('No permissions to referred object or it does not exist!'));
		}
	...
	protected function validateDeleteMedia(array $mediaIds) {
		if (self::$userData['type'] < USER_TYPE_BSM_ADMIN) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Only Bsm Admins can remove user media.'));
		}
	...
	public function deleteMediaReal($mediaIds) {
		if (!DBexecute('DELETE FROM media WHERE '.dbConditionInt('mediaid', $mediaIds))) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete user media.'));
		}
	}
	...
	protected function ldapLogin(array $user) {
		...
		if (!function_exists('ldap_connect')) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Probably php-ldap module is missing.'));
		}
		...
		if ($ldapValidator->validate($user)) {
			...
			self::exception(BS_API_ERROR_PARAMETERS, _('Login name or password is incorrect.'));
		}
	}
	...
	private function dbLogin($user) {
		$login = DBfetch(DBselect(
			'SELECT NULL'.
			' FROM users u'.
			' WHERE u.alias='.bs_dbstr($user['user']).
				' AND u.passwd='.bs_dbstr(md5($user['password']))
		));
	...
	public function logout() {
		...
		$session = DBfetch(DBselect(
			'SELECT s.userid'.
			' FROM sessions s'.
			' WHERE s.sessionid='.bs_dbstr($sessionId).
				' AND s.status='.BS_SESSION_ACTIVE
		));

		if (!$session) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Cannot logout.'));
		}

		DBexecute('DELETE FROM sessions WHERE status='.BS_SESSION_PASSIVE.' AND userid='.bs_dbstr($session['userid']));
		DBexecute('UPDATE sessions SET status='.BS_SESSION_PASSIVE.' WHERE sessionid='.bs_dbstr($sessionId));
	...
	public function login($user) {
		...
		if ($userInfo['attempt_failed'] >= BS_LOGIN_ATTEMPTS) {
			if ((time() - $userInfo['attempt_clock']) < BS_LOGIN_BLOCK) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Account is blocked for %s seconds', (BS_LOGIN_BLOCK - (time() - $userInfo['attempt_clock']))));
			}

			DBexecute('UPDATE users SET attempt_clock='.time().' WHERE alias='.bs_dbstr($name));
		}

		// check system permissions
		if (!check_perm2system($userInfo['userid'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _('No permissions for system access.'));
		}

		$dbAccess = DBfetch(DBselect(
			'SELECT MAX(g.gui_access) AS gui_access'.
			' FROM usrgrp g,users_groups ug'.
			' WHERE ug.userid='.bs_dbstr($userInfo['userid']).
				' AND g.usrgrpid=ug.usrgrpid'
		));

		if (bs_empty($dbAccess['gui_access'])) {
		...
		switch ($guiAccess) {
			case GROUP_GUI_ACCESS_INTERNAL:
				$authType = ($authType == BS_AUTH_HTTP) ? BS_AUTH_HTTP : BS_AUTH_INTERNAL;
				break;
		...
		try {
			...
			DBexecute(
				'UPDATE users'.
				' SET attempt_failed='.bs_dbstr($userInfo['attempt_failed']).','.
					' attempt_clock='.time().','.
					' attempt_ip='.bs_dbstr($ip).
				' WHERE userid='.bs_dbstr($userInfo['userid'])
			);

			add_audit_details(AUDIT_ACTION_LOGIN, AUDIT_RESOURCE_USER, $userInfo['userid'], '',
				_s('Login failed "%s".', $name), $userInfo['userid']
			);
			self::exception(BS_API_ERROR_PARAMETERS, $e->getMessage());
		}

		// start session
		$sessionid = md5(time().$password.$name.rand(0, 10000000));
		DBexecute('INSERT INTO sessions (sessionid,userid,lastaccess,status)'.
			' VALUES ('.bs_dbstr($sessionid).','.bs_dbstr($userInfo['userid']).','.time().','.BS_SESSION_ACTIVE.')'
		);
		...
		if ($userInfo['attempt_failed']) {
			DBexecute('UPDATE users SET attempt_failed=0 WHERE userid='.bs_dbstr($userInfo['userid']));
		}
	...
	public function checkAuthentication(array $sessionid) {
		...
		if (!$userInfo) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Session terminated, re-login, please.'));
		}
		...
		if ($time != $userInfo['lastaccess']) {
			...
			DBexecute(
				...
				' SET lastaccess='.time().
				' WHERE userid='.bs_dbstr($userInfo['userid']).
					' AND sessionid='.bs_dbstr($sessionid)
			);
		}
	...
	private function _getUserData($userid) {
		$userData = DBfetch(DBselect(
			...
			' FROM users u'.
			' WHERE u.userid='.bs_dbstr($userid)
		));

		$userData['debug_mode'] = (bool) DBfetch(DBselect(
			'SELECT ug.userid'.
			' FROM usrgrp g,users_groups ug'.
			' WHERE ug.userid='.bs_dbstr($userid).
				' AND g.usrgrpid=ug.usrgrpid'.
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		$userIds = bs_objectValues($result, 'userid');
	...
	protected function checkDeleteCurrentUser(array $userIds) {
		if (in_array(self::$userData['userid'], $userIds)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('User is not allowed to delete himself.'));
		}
	}
	...
	protected function checkDeleteInternal(array $userIds) {
		...
		if (in_array($guest['userid'], $userIds)) {
			self::exception(BS_API_ERROR_PARAMETERS,
				_s('Cannot delete Bsm internal user "%1$s", try disabling that user.', BS_GUEST_USER)
			);
		}
	}
	...
	protected function disableActionsWithoutOperations(array $actionids) {
		...
		$actions_without_operations = bs_objectValues($actions, 'actionid');
