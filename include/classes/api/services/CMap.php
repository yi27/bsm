...
class CMap extends CMapElement {
	...
	public function get(array $options = []) {
		...
		if ($user_data['type'] != USER_TYPE_SUPER_ADMIN && $user_data['type'] != USER_TYPE_BSM_ADMIN
		...
		if (!is_null($options['search'])) {
			bs_db_search('sysmaps s', $options, $sql_parts);
		}
	...
	protected function validateCreate(array $maps) {
		...
		foreach ($maps as $map) {
			if (!check_db_fields($map_db_fields, $map)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect fields for sysmap.'));
			}
		}
		...
		$db_maps = $this->get([
			'output' => ['name'],
			'filter' => ['name' => bs_objectValues($maps, 'name')],
			'nopermissions' => true,
			'limit' => 1
		]);

		if ($db_maps) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Map "%1$s" already exists.', $db_maps[0]['name']));
		}
		...
		foreach ($maps as $map) {
			...
			if (array_key_exists('userid', $map)) {
				if ($map['userid'] === '' || $map['userid'] === null || $map['userid'] === false) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Map owner cannot be empty.'));
				}
				elseif ($map['userid'] != $user_data['userid'] && $user_data['type'] != USER_TYPE_SUPER_ADMIN
						&& $user_data['type'] != USER_TYPE_BSM_ADMIN) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Only administrators can set map owner.'));
				}
			}
			...
			foreach ($map_labels as $label_name => $label_data) {
				...
				if ($map[$label_name] == MAP_LABEL_TYPE_CUSTOM) {
					...
					if (!array_key_exists($label_data['string'], $map) || bs_empty($map[$label_data['string']])) {
						self::exception(BS_API_ERROR_PARAMETERS, _s(
			...
			if (array_key_exists('urls', $map) && $map['urls']) {
				$url_names = bs_toHash($map['urls'], 'name');
			...
			if (array_key_exists('links', $map) && $map['links']) {
				$selementids = bs_objectValues($map['selements'], 'selementid');
	...
	protected function validateUpdate(array $maps, array $db_maps) {
		...
		foreach ($maps as $map) {
			...
			if (array_key_exists('name', $map)) {
				...
				elseif ($map['name'] === '' || $map['name'] === null || $map['name'] === false) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Map name cannot be empty.'));
				}
		...
		if ($check_names) {
			...
			$db_map_names = $this->get([
				...
				'filter' => ['name' => bs_objectValues($check_names, 'name')],
				'nopermissions' => true
			]);
			$db_map_names = bs_toHash($db_map_names, 'name');
		...
		foreach ($maps as $map) {
			...
			foreach ($map_labels as $label_name => $labelData) {
				...
				if ($map[$label_name] == MAP_LABEL_TYPE_CUSTOM) {
					...
					if (!array_key_exists($labelData['string'], $map) || bs_empty($map[$labelData['string']])) {
						self::exception(BS_API_ERROR_PARAMETERS, _s(
							'Custom label for map "%2$s" elements of type "%1$s" may not be empty.',
							$labelData['typeName'],
							$map['name']
			...
			if (array_key_exists('urls', $map) && !empty($map['urls'])) {
				$urlNames = bs_toHash($map['urls'], 'name');
	...
	public function update(array $maps) {
		$maps = bs_toArray($maps);
		$sysmapids = bs_objectValues($maps, 'sysmapid');
		...
		foreach ($maps as $map) {
			...
			// Map user shares.
			if (array_key_exists('users', $map)) {
				$user_shares_diff = bs_array_diff($map['users'], $db_map['users'], 'userid');
				...
				$shared_userids_to_delete = array_merge($shared_userids_to_delete,
					bs_objectValues($user_shares_diff['second'], 'sysmapuserid')
				);
			}

			// Map user group shares.
			if (array_key_exists('userGroups', $map)) {
				$user_group_shares_diff = bs_array_diff($map['userGroups'], $db_map['userGroups'],
					'usrgrpid'
				);
				...
				$shared_user_groupids_to_delete = array_merge($shared_user_groupids_to_delete,
					bs_objectValues($user_group_shares_diff['second'], 'sysmapusrgrpid')
				);
			}

			// Urls.
			if (array_key_exists('urls', $map)) {
				$url_diff = bs_array_diff($map['urls'], $db_map['urls'], 'name');
				...
				$url_ids_to_delete = array_merge($url_ids_to_delete,
					bs_objectValues($url_diff['second'], 'sysmapurlid')
				);
			}

			// Map elements.
			if (array_key_exists('selements', $map)) {
				$selement_diff = bs_array_diff($map['selements'], $db_map['selements'], 'selementid');
			...
			if (array_key_exists('links', $map)) {
				$link_diff = bs_array_diff($map['links'], $db_map['links'], 'linkid');
		...
		while ($db_link_trigger = DBfetch($link_trigger_resource)) {
			bs_subarray_push($db_links, $db_link_trigger['linkid'], $db_link_trigger);
		}
		...
		foreach ($update_linkids['linkids'] as $key => $linkid) {
			...
			$db_link_triggers_diff = bs_array_diff($links_to_update[$key]['linktriggers'],
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectIconMap'] !== null && $options['selectIconMap'] != API_OUTPUT_COUNT) {
			...
			$iconMaps = API::IconMap()->get([
				...
				'iconmapids' => bs_objectValues($iconMaps, 'iconmapid'),
				'preservekeys' => true
			]);
		...
		if ($options['selectUsers'] !== null && $options['selectUsers'] != API_OUTPUT_COUNT) {
			...
			$related_userids = bs_objectValues($related_users, 'userid');
		...
		if ($options['selectUserGroups'] !== null && $options['selectUserGroups'] != API_OUTPUT_COUNT) {
			...
			$related_groupids = bs_objectValues($related_groups, 'usrgrpid');
