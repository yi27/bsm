...
class CHistory extends CApiService {
	...
	public function get($options = []) {
		...
		if (!is_null($options['time_from'])) {
			$sqlParts['where']['clock_from'] = 'h.clock>='.bs_dbstr($options['time_from']);
		}

		// time_till
		if (!is_null($options['time_till'])) {
			$sqlParts['where']['clock_till'] = 'h.clock<='.bs_dbstr($options['time_till']);
		}
		...
		if (is_array($options['search'])) {
			bs_db_search($sqlParts['from']['history'], $options, $sqlParts);
		}
