...
class CValueMap extends CApiService {
	...
	public function create($valuemaps) {
		$valuemaps = bs_toArray($valuemaps);
	...
	public function update($valuemaps) {
		...
		return ['valuemapids' => bs_objectValues($valuemaps, 'valuemapid')];
	}
	...
	public function delete(array $valuemapids) {
		...
		if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only super admins can delete value maps.'));
		}
	...
	protected function validateCreate(array $valuemaps) {
		// Check permissions.
		if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only super admins can create value maps.'));
		}
		...
		foreach ($valuemaps as $valuemap) {
			...
			elseif ($valuemap['name'] === '' || $valuemap['name'] === null || $valuemap['name'] === false) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Value map name cannot be empty.'));
			}
		}
		...
		$db_valuemaps = API::getApiService()->select('valuemaps', [
			'output' => ['name'],
			'filter' => ['name' => bs_objectValues($valuemaps, 'name')],
			'limit' => 1
		]);
	...
	protected function validateUpdate(array $valuemaps) {
		// Check permissions.
		if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, _('Only super admins can update value maps.'));
		}
		...
		$db_valuemaps = API::getApiService()->select('valuemaps', [
			'output' => ['valuemapid', 'name'],
			'valuemapids' => bs_objectValues($valuemaps, 'valuemapid'),
			'preservekeys' => true
		]);
		...
		if ($check_names) {
			...
			$db_valuemap_names = bs_toHash($db_valuemap_names, 'name');
		...
		$valuemaps = $this->extendFromObjects(bs_toHash($valuemaps, 'valuemapid'), $db_valuemaps, ['name']);
