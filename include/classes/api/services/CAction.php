...
class CAction extends CApiService {
	...
	public function get($options = []) {
		...
		$options = bs_array_merge($defOptions, $options);
		...
		if ($userType != USER_TYPE_SUPER_ADMIN && !$options['nopermissions']) {
			...
			$sqlParts['where'][] = 'NOT EXISTS ('.
					...
					' FROM conditions cc'.
						' LEFT JOIN rights r'.
							' ON r.id='.bs_dbcast_2bigint('cc.value').
					...
					' HAVING MIN(r.permission) IS NULL'.
						...
						' OR MAX(r.permission)<'.bs_dbstr($permission).
					')';
					...
					' FROM conditions cc,hosts_groups hgg'.
						' LEFT JOIN rights r'.
							' ON r.id=hgg.groupid'.
								' AND '.dbConditionInt('r.groupid', $userGroups).
					' WHERE a.actionid=cc.actionid'.
						' AND '.bs_dbcast_2bigint('cc.value').'=hgg.hostid'.
					...
					' FROM conditions cc,functions f,items i,hosts_groups hgg'.
						' LEFT JOIN rights r'.
							' ON r.id=hgg.groupid'.
								' AND '.dbConditionInt('r.groupid', $userGroups).
					' WHERE a.actionid=cc.actionid'.
						' AND '.bs_dbcast_2bigint('cc.value').'=f.triggerid'.
		...
		if (!is_null($options['actionids'])) {
			bs_value2array($options['actionids']);
		...
		if (!is_null($options['mediatypeids'])) {
			bs_value2array($options['mediatypeids']);
		...
		if (!is_null($options['usrgrpids'])) {
			bs_value2array($options['usrgrpids']);
		...
		if (!is_null($options['userids'])) {
			bs_value2array($options['userids']);
		...
		if (!is_null($options['scriptids'])) {
			bs_value2array($options['scriptids']);
			...
			$sqlParts['where'][] = '('.dbConditionInt('oc.scriptid', $options['scriptids']).
				' AND oc.type='.BS_SCRIPT_TYPE_GLOBAL_SCRIPT.')';
		...
		if (is_array($options['search'])) {
			bs_db_search('actions a', $options, $sqlParts);
		}
	...
	public function create($actions) {
		...
		$actions = bs_toHash($actions, 'actionid');
	...
	public function update($actions) {
		...
		foreach ($actions as $actionId => $action) {
			...
			if (isset($action['operations'])) {
				...
				$operationsDb = bs_toHash($operationsDb, 'operationid');
	...
	protected function addConditions($conditions) {
		foreach ($conditions as $condition) {
			...
			if (!check_db_fields($connectionDbFields, $condition)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect parameters for condition.'));
			}
		}
	...
	protected function addOperations($operations) {
		foreach ($operations as $operation) {
			...
			if (!check_db_fields($operationDbFields, $operation)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect parameter for operations.'));
			}
		}

		$operations = DB::save('operations', $operations);
		$operations = bs_toHash($operations, 'operationid');
	...
	protected function updateOperations($operations, $actionsDb) {
		...
		foreach ($operations as $operation) {
			$operationsDb = bs_toHash($actionsDb[$operation['actionid']]['operations'], 'operationid');
			...
			switch ($operation['operationtype']) {
				case OPERATION_TYPE_MESSAGE:
					if (!isset($operation['opmessage_grp'])) {
						...
						bs_array_push($operation['opmessage_grp'], ['operationid' => $operation['operationid']]);
					}
					...
					if (!isset($operation['opmessage_usr'])) {
						...
						bs_array_push($operation['opmessage_usr'], ['operationid' => $operation['operationid']]);
					}
					...
					if ($typeChanged) {
						...
						$diff = bs_array_diff($operation['opmessage_grp'], $operationDb['opmessage_grp'], 'usrgrpid');
						...
						$diff = bs_array_diff($operation['opmessage_usr'], $operationDb['opmessage_usr'], 'userid');
				...
				case OPERATION_TYPE_COMMAND:
					if (!isset($operation['opcommand_grp'])) {
						...
						bs_array_push($operation['opcommand_grp'], ['operationid' => $operation['operationid']]);
					}
					...
					if (!isset($operation['opcommand_hst'])) {
						...
						bs_array_push($operation['opcommand_hst'], ['operationid' => $operation['operationid']]);
					}
					...
					if ($typeChanged) {
						...
						if ($operation['opcommand']['type'] == BS_SCRIPT_TYPE_GLOBAL_SCRIPT) {
							$operation['opcommand']['command'] = '';
						}
						else {
							$operation['opcommand']['scriptid'] = null;
						}
						if ($operation['opcommand']['type'] != BS_SCRIPT_TYPE_CUSTOM_SCRIPT) {
							$operation['opcommand']['execute_on'] = BS_SCRIPT_EXECUTE_ON_AGENT;
						}
						if ($operation['opcommand']['type'] != BS_SCRIPT_TYPE_SSH
								&& $operation['opcommand']['type'] != BS_SCRIPT_TYPE_TELNET) {
						...
						$diff = bs_array_diff($operation['opcommand_grp'], $operationDb['opcommand_grp'], 'groupid');
						...
						$diff = bs_array_diff($operation['opcommand_hst'], $operationDb['opcommand_hst'], 'hostid');
						$opCommandHstsToInsert = array_merge($opCommandHstsToInsert, $diff['first']);
						$opCommandHostIds = bs_objectValues($diff['second'], 'opcommand_hstid');
						if ($opCommandHostIds) {
				...
				case OPERATION_TYPE_GROUP_REMOVE:
					if (!isset($operation['opgroup'])) {
						...
						bs_array_push($operation['opgroup'], ['operationid' => $operation['operationid']]);
					}
					...
					$diff = bs_array_diff($operation['opgroup'], $operationDb['opgroup'], 'groupid');
				...
				case OPERATION_TYPE_TEMPLATE_REMOVE:
					if (!isset($operation['optemplate'])) {
						...
						bs_array_push($operation['optemplate'], ['operationid' => $operation['operationid']]);
					}
					...
					$diff = bs_array_diff($operation['optemplate'], $operationDb['optemplate'], 'templateid');
			...
			if (!isset($operation['opconditions'])) {
				...
				bs_array_push($operation['opconditions'], ['operationid' => $operation['operationid']]);
			}

			self::validateOperationConditions($operation['opconditions']);

			$diff = bs_array_diff($operation['opconditions'], $operationDb['opconditions'], 'opconditionid');
			$opConditionsToInsert = array_merge($opConditionsToInsert, $diff['first']);

			$opConditionsIdsToDelete = bs_objectValues($diff['second'], 'opconditionid');
		...
		foreach ($operations as $operation) {
			...
			if (isset($operation['esc_period'])) {
				if (isset($operation['esc_period']) && $operation['esc_period'] != 0 && $operation['esc_period'] < SEC_PER_MIN) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation step duration.'));
				}
			}
			...
			switch ($operation['operationtype']) {
				case OPERATION_TYPE_MESSAGE:
					$userIds = isset($operation['opmessage_usr']) ? bs_objectValues($operation['opmessage_usr'], 'userid') : [];
					$userGroupIds = isset($operation['opmessage_grp']) ? bs_objectValues($operation['opmessage_grp'], 'usrgrpid') : [];

					if (empty($userIds) && empty($userGroupIds)) {
						self::exception(BS_API_ERROR_PARAMETERS, _('No recipients for action operation message.'));
					}
				...
				case OPERATION_TYPE_COMMAND:
					if (!isset($operation['opcommand']['type'])) {
						self::exception(BS_API_ERROR_PARAMETERS, _('No command type specified for action operation.'));
					}

					if ((!isset($operation['opcommand']['command']) || bs_empty(trim($operation['opcommand']['command'])))
							&& $operation['opcommand']['type'] != BS_SCRIPT_TYPE_GLOBAL_SCRIPT) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('No command specified for action operation.'));
					}
					...
					switch ($operation['opcommand']['type']) {
						...
						case BS_SCRIPT_TYPE_CUSTOM_SCRIPT:
							if (!isset($operation['opcommand']['execute_on'])) {
								self::exception(BS_API_ERROR_PARAMETERS, _s('No execution target specified for action operation command "%s".', $operation['opcommand']['command']));
							}
							break;
						case BS_SCRIPT_TYPE_SSH:
							if (!isset($operation['opcommand']['authtype']) || bs_empty($operation['opcommand']['authtype'])) {
								self::exception(BS_API_ERROR_PARAMETERS, _s('No authentication type specified for action operation command "%s".', $operation['opcommand']['command']));
							}

							if (!isset($operation['opcommand']['username']) || bs_empty($operation['opcommand']['username'])) {
								self::exception(BS_API_ERROR_PARAMETERS, _s('No authentication user name specified for action operation command "%s".', $operation['opcommand']['command']));
							}

							if ($operation['opcommand']['authtype'] == ITEM_AUTHTYPE_PUBLICKEY) {
								if (!isset($operation['opcommand']['publickey']) || bs_empty($operation['opcommand']['publickey'])) {
									self::exception(BS_API_ERROR_PARAMETERS, _s('No public key file specified for action operation command "%s".', $operation['opcommand']['command']));
								}
								if (!isset($operation['opcommand']['privatekey']) || bs_empty($operation['opcommand']['privatekey'])) {
									self::exception(BS_API_ERROR_PARAMETERS, _s('No private key file specified for action operation command "%s".', $operation['opcommand']['command']));
								}
							}
							break;
						case BS_SCRIPT_TYPE_TELNET:
							if (!isset($operation['opcommand']['username']) || bs_empty($operation['opcommand']['username'])) {
								self::exception(BS_API_ERROR_PARAMETERS, _s('No authentication user name specified for action operation command "%s".', $operation['opcommand']['command']));
							}
							break;
						case BS_SCRIPT_TYPE_GLOBAL_SCRIPT:
							if (!isset($operation['opcommand']['scriptid']) || bs_empty($operation['opcommand']['scriptid'])) {
								self::exception(BS_API_ERROR_PARAMETERS, _('No script specified for action operation command.'));
							}
							...
							if (!isset($scripts[$operation['opcommand']['scriptid']])) {
								self::exception(BS_API_ERROR_PARAMETERS, _('Specified script does not exist or you do not have rights on it for action operation command.'));
							}
							break;
						default:
							self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation command type.'));
					}

					if (isset($operation['opcommand']['port']) && !bs_empty($operation['opcommand']['port'])) {
						if (bs_ctype_digit($operation['opcommand']['port'])) {
							if ($operation['opcommand']['port'] > 65535 || $operation['opcommand']['port'] < 1) {
								self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect action operation port "%s".', $operation['opcommand']['port']));
							}
						}
						else {
					...
					$groupIds = [];
					if (isset($operation['opcommand_grp'])) {
						$groupIds = bs_objectValues($operation['opcommand_grp'], 'groupid');
					}
					...
					if (empty($groupIds) && empty($hostIds) && $withoutCurrent) {
						if ($operation['opcommand']['type'] == BS_SCRIPT_TYPE_GLOBAL_SCRIPT) {
							self::exception(BS_API_ERROR_PARAMETERS, _s('You did not specify targets for action operation global script "%s".', $scripts[$operation['opcommand']['scriptid']]['name']));
						}
						else {
							self::exception(BS_API_ERROR_PARAMETERS, _s('You did not specify targets for action operation command "%s".', $operation['opcommand']['command']));
						}
					}
				...
				case OPERATION_TYPE_GROUP_ADD:
				case OPERATION_TYPE_GROUP_REMOVE:
					$groupIds = isset($operation['opgroup']) ? bs_objectValues($operation['opgroup'], 'groupid') : [];
					if (empty($groupIds)) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Operation has no group to operate.'));
					}
					$hostGroupIdsAll = array_merge($hostGroupIdsAll, $groupIds);
					break;
				case OPERATION_TYPE_TEMPLATE_ADD:
				case OPERATION_TYPE_TEMPLATE_REMOVE:
					$templateIds = isset($operation['optemplate']) ? bs_objectValues($operation['optemplate'], 'templateid') : [];
					if (empty($templateIds)) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Operation has no template to operate.'));
					}
				...
				case OPERATION_TYPE_HOST_INVENTORY:
					...
					if ($operation['opinventory']['inventory_mode'] != HOST_INVENTORY_MANUAL
						...
						self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect inventory mode in action operation.'));
					}
					break;

				default:
					self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation type.'));
			}
		}

		if (!API::HostGroup()->isWritable($hostGroupIdsAll)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation host group. Host group does not exist or you have no access to this host group.'));
		}
		if (!API::Host()->isWritable($hostIdsAll)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation host. Host does not exist or you have no access to this host.'));
		}
		if (!API::User()->isReadable($userIdsAll)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation user. User does not exist or you have no access to this user.'));
		}
		if (!API::UserGroup()->isReadable($userGroupIdsAll)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation user group. User group does not exist or you have no access to this user group.'));
		}
	...
	public static function validateOperationConditions($conditions) {
		$conditions = bs_toArray($conditions);
		$ackStatuses = [
			...
			switch ($condition['conditiontype']) {
				case CONDITION_TYPE_EVENT_ACKNOWLEDGED:
					if (!isset($ackStatuses[$condition['value']])) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation condition acknowledge type.'));
					}
					break;

				default:
					self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operation condition type.'));
					break;
			}
		}
			...
			if ($formulaRequested || $evalFormulaRequested || $conditionsRequested) {
				...
				foreach ($filters as &$filter) {
					...
					// in other cases - generate the formula automatically
						...
						// sort conditions
						$sortFields = [
							['field' => 'conditiontype', 'order' => BS_SORT_DOWN],
							['field' => 'operator', 'order' => BS_SORT_DOWN],
							['field' => 'value', 'order' => BS_SORT_DOWN]
						];
	...
	protected function validateCreate($actions) {
		...
		foreach ($actions as $action) {
			...
			if (isset($action['esc_period']) && $action['esc_period'] < SEC_PER_MIN
				...
				self::exception(BS_API_ERROR_PARAMETERS, _s(
					'Action "%1$s" has incorrect value for "esc_period" (minimum %2$s seconds).',
					$action['name'], SEC_PER_MIN
				));
			}
			if (isset($duplicates[$action['name']])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Action "%1$s" already exists.', $action['name']));
			}
			else {
		...
		if ($dbActionsWithSameName) {
			...
			self::exception(BS_API_ERROR_PARAMETERS, _s('Action "%1$s" already exists.', $dbActionWithSameName['name']));
		}
		...
		foreach ($actions as $actionId => $action) {
			...
			if (isset($action['name'])) {
				...
				if (($actionExists = reset($actionExists))
					...
					self::exception(BS_API_ERROR_PARAMETERS, _s('Action "%1$s" already exists.', $actionName));
				}
			}
			else {
			...
			if (isset($action['operations']) && empty($action['operations'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Action "%1$s" no operations defined.', $actionName));
			}
			elseif (isset($action['operations'])) {
				...
				foreach ($action['operations'] as $operation) {
					...
					elseif (isset($operationsDb[$operation['operationid']])) {
						...
						self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect action operationid.'));
					}
				}
			}
		}
