...
abstract class CItemGeneral extends CApiService {
	...
	protected function checkInput(array &$items, $update = false) {
		if ($update) {
			...
			$dbItems = $this->get([
				...
				'itemids' => bs_objectValues($items, 'itemid'),
				'editable' => true,
				'preservekeys' => true
			]);
			...
			$dbHosts = API::Host()->get([
				...
				'hostids' => bs_objectValues($dbItems, 'hostid'),
			...
			$dbHosts = API::Host()->get([
				'output' => ['hostid', 'status', 'name'],
				'hostids' => bs_objectValues($items, 'hostid'),
				'templated_hosts' => true,
				'editable' => true,
				'selectApplications' => ['applicationid', 'flags'],
				'preservekeys' => true
		...
		$interfaces = API::HostInterface()->get([
			...
			'hostids' => bs_objectValues($dbHosts, 'hostid'),
			'nopermissions' => true,
			'preservekeys' => true
		]);
		...
		foreach ($items as $inum => &$item) {
			...
			if (!check_db_fields($itemDbFields, $item)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect arguments passed to function.'));
			}
			...
			if ($fullItem['type'] == ITEM_TYPE_BSM_ACTIVE) {
				$item['delay_flex'] = '';
			}
			...
			if ($itemInterfaceType !== false && $host['status'] != HOST_STATUS_TEMPLATE) {
				if (!$fullItem['interfaceid']) {
					self::exception(BS_API_ERROR_PARAMETERS, _('No interface found.'));
				}
				elseif (!isset($interfaces[$fullItem['interfaceid']]) || bccomp($interfaces[$fullItem['interfaceid']]['hostid'], $fullItem['hostid']) != 0) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Item uses host interface from non-parent host.'));
				}
				elseif ($itemInterfaceType !== INTERFACE_TYPE_ANY && $interfaces[$fullItem['interfaceid']]['type'] != $itemInterfaceType) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Item uses incorrect interface type.'));
				}
			}
			...
			if ($fullItem['type'] == ITEM_TYPE_DB_MONITOR) {
				if (!isset($fullItem['flags']) || $fullItem['flags'] != BS_FLAG_DISCOVERY_RULE) {
					if (strcmp($fullItem['key_'], BS_DEFAULT_KEY_DB_MONITOR) == 0) {
						self::exception(BS_API_ERROR_PARAMETERS,
							_('Check the key, please. Default example was passed.')
						);
					}
				}
				elseif ($fullItem['flags'] == BS_FLAG_DISCOVERY_RULE) {
					if (strcmp($fullItem['key_'], BS_DEFAULT_KEY_DB_MONITOR_DISCOVERY) == 0) {
						self::exception(BS_API_ERROR_PARAMETERS,
							_('Check the key, please. Default example was passed.')
						);
					}
			...
			elseif (($fullItem['type'] == ITEM_TYPE_SSH && strcmp($fullItem['key_'], BS_DEFAULT_KEY_SSH) == 0)
					|| ($fullItem['type'] == ITEM_TYPE_TELNET && strcmp($fullItem['key_'], BS_DEFAULT_KEY_TELNET) == 0)
					|| ($fullItem['type'] == ITEM_TYPE_JMX && strcmp($fullItem['key_'], BS_DEFAULT_KEY_JMX) == 0)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Check the key, please. Default example was passed.'));
			}
			...
			if ($fullItem['type'] != ITEM_TYPE_TRAPPER && $fullItem['type'] != ITEM_TYPE_SNMPTRAP) {
				...
				if ($item_delay_flex_parser->isValid()) {
					...
					if ($delay_flex_validator->validate($item_delay_flex_parser->getIntervals())) {
						...
						self::exception(BS_API_ERROR_PARAMETERS, $delay_flex_validator->getError());
					}
				}
				else {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Invalid interval "%1$s": %2$s.',
						$fullItem['delay_flex'],
			...
			if ($fullItem['type'] == ITEM_TYPE_SSH || $fullItem['type'] == ITEM_TYPE_TELNET) {
				if (bs_empty($fullItem['username'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('No authentication user name specified.'));
				}

				if ($fullItem['type'] == ITEM_TYPE_SSH && $fullItem['authtype'] == ITEM_AUTHTYPE_PUBLICKEY) {
					if (bs_empty($fullItem['publickey'])) {
						self::exception(BS_API_ERROR_PARAMETERS, _('No public key file specified.'));
					}
					if (bs_empty($fullItem['privatekey'])) {
						self::exception(BS_API_ERROR_PARAMETERS, _('No private key file specified.'));
					}
				}
			}
			...
			if ($fullItem['type'] == ITEM_TYPE_SNMPTRAP
				...
				self::exception(BS_API_ERROR_PARAMETERS, _('SNMP trap key is invalid.'));
			}

			// snmp oid
			if ((in_array($fullItem['type'], [ITEM_TYPE_SNMPV1, ITEM_TYPE_SNMPV2C, ITEM_TYPE_SNMPV3]))
					&& bs_empty($fullItem['snmp_oid'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('No SNMP OID specified.'));
			}

			// snmp community
			if (in_array($fullItem['type'], [ITEM_TYPE_SNMPV1, ITEM_TYPE_SNMPV2C])
					&& bs_empty($fullItem['snmp_community'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('No SNMP community specified.'));
			}

			// snmp port
			if (isset($fullItem['port']) && !bs_empty($fullItem['port']) && !validatePortNumberOrMacro($fullItem['port'])) {
				self::exception(BS_API_ERROR_PARAMETERS,
			...
			if (isset($fullItem['snmpv3_securitylevel']) && $fullItem['snmpv3_securitylevel'] != ITEM_SNMPV3_SECURITYLEVEL_NOAUTHNOPRIV) {
				...
				if (str_in_array($fullItem['snmpv3_securitylevel'], [ITEM_SNMPV3_SECURITYLEVEL_AUTHNOPRIV, ITEM_SNMPV3_SECURITYLEVEL_AUTHPRIV])) {
					if (isset($fullItem['snmpv3_authprotocol']) && (bs_empty($fullItem['snmpv3_authprotocol'])
							|| !str_in_array($fullItem['snmpv3_authprotocol'],
								[ITEM_AUTHPROTOCOL_MD5, ITEM_AUTHPROTOCOL_SHA]))) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect authentication protocol for item "%1$s".', $fullItem['name']));
					}
				}

				// snmpv3 privprotocol
				if ($fullItem['snmpv3_securitylevel'] == ITEM_SNMPV3_SECURITYLEVEL_AUTHPRIV) {
					if (isset($fullItem['snmpv3_privprotocol']) && (bs_empty($fullItem['snmpv3_privprotocol'])
							|| !str_in_array($fullItem['snmpv3_privprotocol'],
								[ITEM_PRIVPROTOCOL_DES, ITEM_PRIVPROTOCOL_AES]))) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect privacy protocol for item "%1$s".', $fullItem['name']));
					}
				}
			}
			...
			if (isset($item['applications']) && $item['applications']) {
				 ...
				 */
				if (array_key_exists('flags', $fullItem) && $fullItem['flags'] == BS_FLAG_DISCOVERY_PROTOTYPE) {
					foreach ($host['applications'] as $num => $application) {
						if ($application['flags'] != BS_FLAG_DISCOVERY_NORMAL) {
	...
	protected function errorInheritFlags($flag, $key, $host) {
		switch ($flag) {
			case BS_FLAG_DISCOVERY_NORMAL:
				self::exception(BS_API_ERROR_PARAMETERS, _s('Item with key "%1$s" already exists on "%2$s" as an item.', $key, $host));
				break;
			case BS_FLAG_DISCOVERY_RULE:
				self::exception(BS_API_ERROR_PARAMETERS, _s('Item with key "%1$s" already exists on "%2$s" as a discovery rule.', $key, $host));
				break;
			case BS_FLAG_DISCOVERY_PROTOTYPE:
				self::exception(BS_API_ERROR_PARAMETERS, _s('Item with key "%1$s" already exists on "%2$s" as an item prototype.', $key, $host));
				break;
			case BS_FLAG_DISCOVERY_CREATED:
				self::exception(BS_API_ERROR_PARAMETERS, _s('Item with key "%1$s" already exists on "%2$s" as an item created from item prototype.', $key, $host));
				break;
			default:
				self::exception(BS_API_ERROR_PARAMETERS, _s('Item with key "%1$s" already exists on "%2$s" as unknown item element.', $key, $host));
		}
	}
	...
	protected function prepareInheritedItems(array $itemsToInherit, array $hostIds = null) {
		...
		$chdHosts = API::Host()->get([
			...
			'templateids' => bs_objectValues($itemsToInherit, 'hostid'),
			'hostids' => $hostIds,
			'preservekeys' => true,
			'nopermissions' => true,
			'templated_hosts' => true
		...
		foreach ($chdHosts as $hostId => $host) {
			$templateids = bs_toHash($host['parentTemplates'], 'templateid');
			...
			$exItemsKeys = bs_toHash($exItems, 'key_');
			$exItemsTpl = bs_toHash($exItems, 'templateid');
			...
			if ($itemids_with_application_prototypes) {
				...
				$discovery_rules = bs_toHash($discovery_rules, 'itemid');
			}
			...
			foreach ($parentItems as $parentItem) {
				...
				if ($parentItem['flags'] == BS_FLAG_DISCOVERY_PROTOTYPE
					...
					$db_application_prototypes = DBfetchArray(DBselect(
						...
						' WHERE ap.itemid='.bs_dbstr($discovery_ruleid).
							' AND '.dbConditionString('ap.name',
								bs_objectValues($parentItem['applicationPrototypes'], 'name')
							)
					));

					$db_application_prototypes = bs_toHash($db_application_prototypes, 'name');
	...
	protected function checkExistingItems(array $items) {
		...
		foreach ($itemKeysByHostId as $hostId => $keys) {
			$sqlWhere[] = '(i.hostid='.bs_dbstr($hostId).' AND '.dbConditionString('i.key_', $keys).')';
		}
