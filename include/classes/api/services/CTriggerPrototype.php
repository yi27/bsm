...
class CTriggerPrototype extends CTriggerGeneral {
	 ...
	 *
	 * @see https://www.bsm.com/documentation/3.0/manual/api/reference/triggerprototype/get
	 *
	 * @param array $options
	 *
	 * @return array|int
	...
	public function get(array $options = []) {
		...
		$sqlParts = [
			...
			'from'		=> ['t' => 'triggers t'],
			'where'		=> ['t.flags='.BS_FLAG_DISCOVERY_PROTOTYPE],
			'group'		=> [],
			'order'		=> [],
			'limit'		=> null
		];
		...
		if ($options['discoveryids'] !== null) {
			bs_value2array($options['discoveryids']);
	 ...
	 *
	 * @see https://www.bsm.com/documentation/3.0/manual/api/reference/triggerprototype/create
	 *
	...
	public function create(array $triggerPrototypes) {
		$triggerPrototypes = bs_toArray($triggerPrototypes);
		...
		return ['triggerids' => bs_objectValues($triggerPrototypes, 'triggerid')];
	}
	...
	protected function validateCreate(array $triggerPrototypes) {
		...
		foreach ($triggerPrototypes as $triggerPrototype) {
			if (!check_db_fields($triggerDbFields, $triggerPrototype)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Wrong fields for trigger.'));
			}
	 ...
	 *
	 * @see https://www.bsm.com/documentation/3.0/manual/api/reference/triggerprototype/update
	 *
	 * @param array $triggerPrototypes
	 *
	 * @return array
	...
	public function update(array $triggerPrototypes) {
		$triggerPrototypes = bs_toArray($triggerPrototypes);
		$triggerPrototypeIds = bs_objectValues($triggerPrototypes, 'triggerid');
		...
		foreach ($triggerPrototypes as $triggerPrototype) {
			$triggerPrototype['flags'] = BS_FLAG_DISCOVERY_PROTOTYPE;
			$this->inherit($triggerPrototype);
		}
	 ...
	 *
	 * @see https://www.bsm.com/documentation/3.0/manual/api/reference/triggerprototype/delete
	 *
	...
	public function delete(array $triggerPrototypeIds, $nopermissions = false) {
		...
		if (!$nopermissions) {
			...
			foreach ($triggerPrototypeIds as $triggerPrototypeId) {
				...
				if ($dbTriggerPrototype['templateid'] != 0) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Cannot delete templated trigger "%1$s:%2$s".',
		...
		foreach ($dbTriggerPrototypes as $dbTriggerPrototype) {
			info(_s('Deleted: Trigger prototype "%1$s" on "%2$s".', $dbTriggerPrototype['description'],
					implode(', ', bs_objectValues($dbTriggerPrototype['hosts'], 'name'))));
	...
	protected function createReal(array &$triggerPrototypes) {
		...
		foreach ($triggerPrototypes as $key => $triggerPrototype) {
			$triggerPrototypes[$key]['flags'] = BS_FLAG_DISCOVERY_PROTOTYPE;
		}
	...
	protected function updateReal(array $triggerPrototypes) {
		...
		$dbTriggerPrototypes = $this->get([
			'triggerids' => bs_objectValues($triggerPrototypes, 'triggerid'),
			'output' => API_OUTPUT_EXTEND,
			'selectHosts' => ['name'],
		...
		foreach ($triggerPrototypes as &$triggerPrototype) {
			...
			$dbTriggerPrototype = $dbTriggerPrototypes[$triggerPrototype['triggerid']];
			$hosts = bs_objectValues($dbTriggerPrototype['hosts'], 'name');
	...
	protected function validateAddDependencies(array $triggerPrototypes) {
		...
		$triggers = API::Trigger()->get([
			...
			'triggerids' => $depTriggerIds,
			'filter' => [
				'flags' => [BS_FLAG_DISCOVERY_NORMAL]
			]
		]);
	...
	protected function checkDependencies(array $triggerPrototypes) {
		$triggerPrototypes = bs_toHash($triggerPrototypes, 'triggerid');
		...
		foreach ($triggerPrototypes as $triggerPrototype) {
			...
			$triggerid_down = $triggerPrototype['triggerid'];
			$triggerids_up = bs_objectValues($triggerPrototype['dependencies'], 'triggerid');
		...
		foreach ($triggerPrototypes as $triggerPrototype) {
			...
			$depTriggerIds = bs_objectValues($triggerPrototype['dependencies'], 'triggerid');
				...
				foreach ($downTriggerIds as $id) {
					if (isset($triggerPrototypes[$id]) && isset($triggerPrototypes[$id]['dependencies'])) {
						$upTriggersIds = array_merge($upTriggersIds,
							bs_objectValues($triggerPrototypes[$id]['dependencies'], 'triggerid')
						);
					}
				}
		...
		if ($duplicateTriggerId === null) {
			...
			foreach ($triggerPrototypes as $triggerPrototype) {
				$dbUpTriggers = DBselect(
					...
					' AND td.triggerid_down='.bs_dbstr($triggerPrototype['triggerid'])
				, 1);
	...
	protected function validateUpdate(array $triggerPrototypes, array $dbTriggerPrototypes) {
		...
		foreach ($triggerPrototypes as $triggerPrototype) {
			...
			if (array_key_exists('templateid', $triggerPrototype)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Cannot set "templateid" for trigger prototype "%1$s".',
					$triggerPrototype['description']
				));
			}
	...
	protected function checkExpression(array $triggerPrototype) {
		...
		if (!$triggerExpression->parse($triggerPrototype['expression'])) {
			self::exception(BS_API_ERROR_PARAMETERS, $triggerExpression->error);
		}
		...
		if ($triggerExpression->expressions) {
			...
			if ($has_host && $has_template) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect trigger prototype expression.'.
			...
			foreach ($expressions as $host => $expression) {
				if ($expression['hostid'] === null) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Incorrect trigger prototype expression.'.
