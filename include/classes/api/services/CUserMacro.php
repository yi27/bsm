...
class CUserMacro extends CApiService {
	...
	public function get($options = []) {
		...
		if (is_array($options['search'])) {
			bs_db_search('hostmacro hm', $options, $sqlParts);
			bs_db_search('globalmacro gm', $options, $sqlPartsGlobal);
		}

		// filter
		if (is_array($options['filter'])) {
			if (isset($options['filter']['macro'])) {
				bs_value2array($options['filter']['macro']);
	...
	public function createGlobal(array $globalmacros) {
		$globalmacros = bs_toArray($globalmacros);
	...
	public function updateGlobal(array $globalmacros) {
		...
		$this->checkIfGlobalMacrosExist(bs_objectValues($globalmacros, 'globalmacroid'));
		...
		return ['globalmacroids' => bs_objectValues($globalmacros, 'globalmacroid')];
	}
	...
	protected function validateCreate(array $hostmacros) {
		...
		$this->checkHostPermissions(array_unique(bs_objectValues($hostmacros, 'hostid')));
	...
	public function create(array $hostmacros) {
		$hostmacros = bs_toArray($hostmacros);
	...
	protected function validateUpdate(array $hostmacros) {
		...
		$this->checkIfHostMacrosExistIn(bs_objectValues($hostmacros, 'hostmacroid'), $db_hostmacros);
		...
		$affected_hostids = array_merge(bs_objectValues($db_hostmacros, 'hostid'),
			bs_objectValues($hostmacros, 'hostid')
		);
	...
	public function update($hostmacros) {
		...
		return ['hostmacroids' => bs_objectValues($hostmacros, 'hostmacroid')];
	}
	...
	protected function validateDelete(array $hostmacroids) {
		...
		// Check permissions for all affected hosts.
		$this->checkHostPermissions(array_unique(bs_objectValues($db_hostmacros, 'hostid')));
	...
	public function replaceMacros(array $macros) {
		...
		foreach ($macros as $hostid => $hostmacros) {
			$db_hostmacros = bs_toHash($db_hosts[$hostid]['macros'], 'hostmacroid');

			/*
			 * Look for db macros which hostmacroids are not in list of new macros. If there are any,
			 * they should be deleted.
			 */
			$hostmacroids = bs_toHash($hostmacros, 'hostmacroid');
	...
	protected function checkHostId(array $hostmacro) {
		if (!array_key_exists('hostid', $hostmacro) || bs_empty($hostmacro['hostid'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('No host given for macro "%1$s".', $hostmacro['macro']));
		}

		if (!is_numeric($hostmacro['hostid'])) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Invalid hostid for macro "%1$s".', $hostmacro['macro']));
		}
	}
	...
	protected function checkDuplicateMacros(array $macros) {
		...
		foreach ($macros as $macro) {
			...
			if (array_key_exists($hostid, $existing_macros) && array_key_exists($macro_name, $existing_macros[$hostid])
				...
				self::exception(BS_API_ERROR_PARAMETERS, _s('Macro "%1$s" is not unique.', $macro['macro']));
			}
	...
	protected function checkIfHostMacrosDontRepeat(array $hostmacros) {
		...
		$db_hostmacros = API::getApiService()->select($this->tableName(), [
			'output' => ['hostmacroid', 'hostid', 'macro'],
			'filter' => ['hostid' => array_unique(bs_objectValues($hostmacros, 'hostid'))],
	...
	protected function checkIfHostMacrosExistIn(array $hostmacroids, array $db_hostmacros) {
		$db_hostmacros = bs_toHash($db_hostmacros, 'hostmacroid');
	...
	protected function checkGlobalMacrosPermissions($error) {
		if (self::$userData['type'] != USER_TYPE_SUPER_ADMIN) {
			self::exception(BS_API_ERROR_PERMISSIONS, $error);
		}
	}
