...
class CHttpTest extends CApiService {
	...
	public function get($options = []) {
		...
		if (is_array($options['search'])) {
			bs_db_search('httptest ht', $options, $sqlParts);
		}
	...
	public function create($httpTests) {
		$httpTests = bs_toArray($httpTests);

		if (!$httpTests) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Empty input parameters.'));
		}
		...
		return ['httptestids' => bs_objectValues($httpTests, 'httptestid')];
	}
	...
	public function update($httpTests) {
		...
		$httpTests = bs_toHash($httpTests, 'httptestid');
	...
	public function delete(array $httpTestIds, $nopermissions = false) {
		...
		if (!$nopermissions) {
			foreach ($httpTestIds as $httpTestId) {
				if (!empty($delHttpTests[$httpTestId]['templateid'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Cannot delete templated web scenario "%1$s".', $delHttpTests[$httpTestId]['name']));
				}
		...
		$delHttpTests = bs_array_merge($delHttpTests, $delHttpTestChilds);
	...
	protected function validateCreate(array $httpTests) {
		foreach ($httpTests as $httpTest) {
			...
			if (!empty($missingKeys)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Web scenario missing parameters: %1$s', implode(', ', $missingKeys)));
			}
		}

		$hostIds = bs_objectValues($httpTests, 'hostid');
		if (!API::Host()->isWritable($hostIds)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('No permissions to referred object or it does not exist!'));
		}

		foreach ($httpTests as $httpTest) {
			if (bs_empty($httpTest['name'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Web scenario name cannot be empty.'));
			}

			$this->checkSslParameters($httpTest);

			if (empty($httpTest['steps'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Web scenario must have at least one step.'));
			}
	...
	protected function checkApplicationHost(array $httpTests) {
		...
		$applicationids = bs_objectValues($httpTests, 'applicationid');
		...
		if ($applicationids) {
			...
			foreach ($applicationids as $applicationid) {
				...
				elseif ($applications[$applicationid]['flags'] == BS_FLAG_DISCOVERY_CREATED) {
	...
	protected function checkSteps(array $httpTest, array $dbHttpTest = []) {
		...
		foreach ($httpTest['steps'] as $step) {
			if ((isset($step['httpstepid']) && array_key_exists('name', $step) && bs_empty($step['name']))
					|| (!isset($step['httpstepid']) && (!array_key_exists('name', $step) || bs_empty($step['name'])))) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Web scenario step name cannot be empty.'));
			}

			if ((isset($step['httpstepid']) && array_key_exists('url', $step) && bs_empty($step['url']))
					|| (!isset($step['httpstepid']) && (!array_key_exists('url', $step) || bs_empty($step['url'])))) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Web scenario step URL cannot be empty.'));
			}

			if (isset($step['no']) && $step['no'] <= 0) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Web scenario step number cannot be less than 1.'));
			}
	...
	protected function checkDuplicateSteps(array $httpTest, array $dbHttpTest = []) {
		if ($duplicate = CArrayHelper::findDuplicate($httpTest['steps'], 'name')) {
			self::exception(BS_API_ERROR_PARAMETERS, _s('Web scenario step "%1$s" already exists.', $duplicate['name']));
		}
	}
	...
	protected  function checkStatusCode($statusCodeRange) {
		...
		if ($statusCodeRange === '' || $user_macro_parser->parse($statusCodeRange) == CParser::PARSE_SUCCESS) {
			...
			foreach ($ranges as $range) {
				$range = explode('-', $range);
				if (count($range) > 2) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Invalid response code "%1$s".', $statusCodeRange));
				}
