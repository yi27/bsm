...
abstract class CHostGeneral extends CHostBase {
	...
	public function massAdd(array $data) {
		...
		if (!empty($data['templates_link'])) {
			...
			$this->link(bs_objectValues(bs_toArray($data['templates_link']), 'templateid'), $allHostIds);
		}

		// create macros
		if (!empty($data['macros'])) {
			$data['macros'] = bs_toArray($data['macros']);
	...
	public function massRemove(array $data) {
		...
		if (!empty($data['templateids_link'])) {
			$this->unlink(bs_toArray($data['templateids_link']), $allHostIds);
		}

		if (isset($data['templateids_clear'])) {
			$this->unlink(bs_toArray($data['templateids_clear']), $allHostIds, true);
		}
		...
		if (isset($data['macros'])) {
			...
			$hostMacroIds = bs_objectValues($hostMacros, 'hostmacroid');
	...
	protected function unlink($templateids, $targetids = null, $clear = false) {
		$flags = ($clear)
			? [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_RULE]
			: [BS_FLAG_DISCOVERY_NORMAL, BS_FLAG_DISCOVERY_RULE, BS_FLAG_DISCOVERY_PROTOTYPE];
		...
		$sql = 'SELECT DISTINCT t.description'.
			...
			')'.
			' AND t.flags='.BS_FLAG_DISCOVERY_NORMAL;
		...
		$triggers = [
			BS_FLAG_DISCOVERY_NORMAL => [],
			BS_FLAG_DISCOVERY_PROTOTYPE => []
		];
		$triggerids = [];
		...
		if (!empty($triggers[BS_FLAG_DISCOVERY_NORMAL])) {
			$triggers[BS_FLAG_DISCOVERY_NORMAL] =
				CMacrosResolverHelper::resolveTriggerExpressions($triggers[BS_FLAG_DISCOVERY_NORMAL]);

			if ($clear) {
				$result = API::Trigger()->delete(array_keys($triggers[BS_FLAG_DISCOVERY_NORMAL]), true);
				if (!$result) self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear triggers'));
			}
			else{
				DB::update('triggers', [
					'values' => ['templateid' => 0],
					'where' => ['triggerid' => array_keys($triggers[BS_FLAG_DISCOVERY_NORMAL])]
				]);

				foreach ($triggers[BS_FLAG_DISCOVERY_NORMAL] as $trigger) {
					info(_s('Unlinked: Trigger "%1$s" on "%2$s".', $trigger['description'], $trigger['host']));
				}
			}
		}

		if (!empty($triggers[BS_FLAG_DISCOVERY_PROTOTYPE])) {
			$triggers[BS_FLAG_DISCOVERY_PROTOTYPE] =
				CMacrosResolverHelper::resolveTriggerExpressions($triggers[BS_FLAG_DISCOVERY_PROTOTYPE]);

			if ($clear) {
				$result = API::TriggerPrototype()->delete(array_keys($triggers[BS_FLAG_DISCOVERY_PROTOTYPE]), true);
				if (!$result) self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear triggers'));
			}
			else{
				DB::update('triggers', [
					'values' => ['templateid' => 0],
					'where' => ['triggerid' => array_keys($triggers[BS_FLAG_DISCOVERY_PROTOTYPE])]
				]);

				foreach ($triggers[BS_FLAG_DISCOVERY_PROTOTYPE] as $trigger) {
		...
		if (!empty($items[BS_FLAG_DISCOVERY_RULE])) {
			if ($clear) {
				$result = API::DiscoveryRule()->delete(array_keys($items[BS_FLAG_DISCOVERY_RULE]), true);
				if (!$result) self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear discovery rules'));
			}
			else{
				DB::update('items', [
					'values' => ['templateid' => 0],
					'where' => ['itemid' => array_keys($items[BS_FLAG_DISCOVERY_RULE])]
				]);

				foreach ($items[BS_FLAG_DISCOVERY_RULE] as $discoveryRule) {
					info(_s('Unlinked: Discovery rule "%1$s" on "%2$s".', $discoveryRule['name'], $discoveryRule['host']));
				}
			}
		}

		if (!empty($items[BS_FLAG_DISCOVERY_NORMAL])) {
			if ($clear) {
				$result = API::Item()->delete(array_keys($items[BS_FLAG_DISCOVERY_NORMAL]), true);
				if (!$result) self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear items'));
			}
			else{
				DB::update('items', [
					'values' => ['templateid' => 0],
					'where' => ['itemid' => array_keys($items[BS_FLAG_DISCOVERY_NORMAL])]
				]);

				foreach ($items[BS_FLAG_DISCOVERY_NORMAL] as $item) {
					info(_s('Unlinked: Item "%1$s" on "%2$s".', $item['name'], $item['host']));
				}
			}
		}

		if (!empty($items[BS_FLAG_DISCOVERY_PROTOTYPE])) {
			$item_prototypeids = array_keys($items[BS_FLAG_DISCOVERY_PROTOTYPE]);
			...
			if ($clear) {
				...
				if (!$result) {
					self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear item prototypes'));
				}
			}
			else {
				DB::update('items', [
				...
				foreach ($items[BS_FLAG_DISCOVERY_PROTOTYPE] as $item) {
				...
				if ($application_prototypes) {
					$application_prototypeids = bs_objectValues($application_prototypes, 'application_prototypeid');
		...
		if (!$clear && isset($items[BS_FLAG_DISCOVERY_RULE])) {
			$discoveryRuleIds = array_keys($items[BS_FLAG_DISCOVERY_RULE]);
		...
		if (!empty($graphs[BS_FLAG_DISCOVERY_PROTOTYPE])) {
			if ($clear) {
				$result = API::GraphPrototype()->delete(array_keys($graphs[BS_FLAG_DISCOVERY_PROTOTYPE]), true);
				if (!$result) self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear graph prototypes'));
			}
			else{
				DB::update('graphs', [
					'values' => ['templateid' => 0],
					'where' => ['graphid' => array_keys($graphs[BS_FLAG_DISCOVERY_PROTOTYPE])]
				]);

				foreach ($graphs[BS_FLAG_DISCOVERY_PROTOTYPE] as $graph) {
					info(_s('Unlinked: Graph prototype "%1$s" on "%2$s".', $graph['name'], $graph['host']));
				}
			}
		}

		if (!empty($graphs[BS_FLAG_DISCOVERY_NORMAL])) {
			if ($clear) {
				$result = API::Graph()->delete(array_keys($graphs[BS_FLAG_DISCOVERY_NORMAL]), true);
				if (!$result) self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear graphs.'));
			}
			else{
				DB::update('graphs', [
					'values' => ['templateid' => 0],
					'where' => ['graphid' => array_keys($graphs[BS_FLAG_DISCOVERY_NORMAL])]
				]);

				foreach ($graphs[BS_FLAG_DISCOVERY_NORMAL] as $graph) {
		...
		if (!empty($httpTests)) {
			if ($clear) {
				...
				if (!$result) {
					self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear Web scenarios.'));
				}
			}
			else {
				DB::update('httptest', [
		...
		if ($applicationTemplates) {
			...
			DB::delete('application_template', [
				'application_templateid' => bs_objectValues($applicationTemplates, 'application_templateid')
			]);

			if ($clear) {
				// Delete inherited applications that are no longer linked to any templates and items.
				$applicationids = bs_objectValues($applicationTemplates, 'applicationid');
				...
				$result = API::Application()->delete(bs_objectValues($applications, 'applicationid'), true);
				if (!$result) {
					self::exception(BS_API_ERROR_INTERNAL, _('Cannot unlink and clear applications.'));
				}
			}
			else {
		...
		if ($targetids) {
			$discovered_applications = API::Application()->get([
				...
				'hostids' => $targetids,
				'filter' => ['flags' => BS_FLAG_DISCOVERY_CREATED],
				'preservekeys' => true
			]);
	...
	protected function addRelatedObjects(array $options, array $result) {
		...
		if ($options['selectParentTemplates'] !== null) {
			if ($options['selectParentTemplates'] != API_OUTPUT_COUNT) {
				...
				$templates = bs_toHash($templates, 'hostid');
		...
		if ($options['selectTriggers'] !== null) {
			if ($options['selectTriggers'] != API_OUTPUT_COUNT) {
				...
				$triggers = bs_toHash($triggers, 'hostid');
		...
		if ($options['selectGraphs'] !== null) {
			if ($options['selectGraphs'] != API_OUTPUT_COUNT) {
				...
				$graphs = bs_toHash($graphs, 'hostid');
				foreach ($result as $hostid => $host) {
		...
		if ($options['selectHttpTests'] !== null) {
			if ($options['selectHttpTests'] != API_OUTPUT_COUNT) {
				...
				$httpTests = bs_toHash($httpTests, 'hostid');
				foreach ($result as $hostId => $host) {
		...
		if ($options['selectApplications'] !== null) {
			if ($options['selectApplications'] != API_OUTPUT_COUNT) {
				...
				$applications = bs_toHash($applications, 'hostid');
				foreach ($result as $hostid => $host) {
