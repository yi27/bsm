...
class CGraphPrototype extends CGraphGeneral {
	...
	public function get($options = []) {
		...
		$sqlParts = [
			...
			'where'		=> ['g.flags='.BS_FLAG_DISCOVERY_PROTOTYPE],
			'group'		=> [],
			'order'		=> [],
			'limit'		=> null
		];
		...
		if (is_array($options['search'])) {
			bs_db_search('graphs g', $options, $sqlParts);
		}
	...
	protected function inherit($graph, $hostids = null) {
		$graphTemplates = API::Template()->get([
			'itemids' => bs_objectValues($graph['gitems'], 'itemid'),
			'output' => ['templateid'],
			'nopermissions' => true
		]);
		...
		foreach ($chdHosts as $chdHost) {
			...
			if (!$tmpGraph['gitems']) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Graph "%1$s" cannot inherit. No required items on "%2$s".', $tmpGraph['name'], $chdHost['host']));
			}

			if ($tmpGraph['ymax_itemid'] > 0) {
				$ymaxItemid = getSameGraphItemsForHost([['itemid' => $tmpGraph['ymax_itemid']]], $chdHost['hostid']);
				if (!$ymaxItemid) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Graph "%1$s" cannot inherit. No required items on "%2$s" (Ymax value item).', $tmpGraph['name'], $chdHost['host']));
				}
			...
			$chdGraphs = $this->get([
				'filter' => [
					...
					'flags' => [BS_FLAG_DISCOVERY_PROTOTYPE, BS_FLAG_DISCOVERY_NORMAL]
				],
				'output' => API_OUTPUT_EXTEND,
				'selectGraphItems' => API_OUTPUT_EXTEND,
				'preservekeys' => true,
			...
			if ($chdGraph = reset($chdGraphs)) {
				...
				elseif ($chdGraph['flags'] != $tmpGraph['flags']) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Graph with same name but other type exist.'));
				}
			...
			// check if graph with same name and items exists
				...
				if ($chdGraph = reset($chdGraph)) {
					if ($chdGraph['templateid'] != 0) {
						self::exception(BS_API_ERROR_PARAMETERS, _s('Graph "%1$s" already exists on "%2$s" (inherited from another template).', $tmpGraph['name'], $chdHost['host']));
					}
					elseif ($chdGraph['flags'] != $tmpGraph['flags']) {
						self::exception(BS_API_ERROR_PARAMETERS, _('Graph with same name but other type exist.'));
					}
					...
					foreach([API::Item(), API::ItemPrototype()] as $api) {
						$chdGraphItemItems += $api->get([
							...
							'itemids' => bs_objectValues($chdGraph['gitems'], 'itemid'),
							'preservekeys' => true
						]);
					}
					...
					if (count($chdGraph['gitems']) == count($tmpGraph['gitems'])) {
						foreach ($tmpGraph['gitems'] as $gitem) {
							...
							self::exception(BS_API_ERROR_PARAMETERS, _s('Graph "%1$s" already exists on "%2$s" (items are not identical).', $tmpGraph['name'], $chdHost['host']));
						}
	...
	public function delete(array $graphids, $nopermissions = false) {
		...
		if (!$nopermissions) {
			foreach ($graphids as $graphid) {
				...
				if ($delGraphs[$graphid]['templateid'] != 0) {
					self::exception(BS_API_ERROR_PERMISSIONS, _('Cannot delete templated graphs.'));
				}
			}
		}
		...
		if (!empty($createdGraphs)) {
			...
			if (!$result) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Cannot delete graphs created by low level discovery.'));
			}
		}
			...
			if (isset($graph['gitems'])) {
				foreach ($graph['gitems'] as &$gitem) {
					if (isset($gitem['gitemid']) && !isset($gitem['itemid'])) {
						$dbGitems = bs_toHash($dbGraphs[$graph['graphid']]['gitems'], 'gitemid');
		...
		foreach ($allowedItems as $item) {
			if (!in_array($item['value_type'], $allowedValueTypes)) {
				foreach ($dbGraphs as $dbGraph) {
					$itemIdsInGraphItems = bs_objectValues($dbGraph['gitems'], 'itemid');
	...
	protected function checkDiscoveryRuleCount(array $graphs, array $items) {
		foreach ($graphs as $graph) {
			...
			if (isset($graph['gitems'])) {
				...
				foreach ($graph['gitems'] as $gitem) {
					if ($items[$gitem['itemid']]['flags'] == BS_FLAG_DISCOVERY_PROTOTYPE) {
