...
class CImage extends CApiService {
	...
	public function get($options = []) {
		...
		if (!is_null($options['editable']) && self::$userData['type'] < USER_TYPE_BSM_ADMIN) {
			return $result;
		}

		// imageids
		if (!is_null($options['imageids'])) {
			bs_value2array($options['imageids']);
			$sqlParts['where']['imageid'] = dbConditionInt('i.imageid', $options['imageids']);
		}

		// sysmapids
		if (!is_null($options['sysmapids'])) {
			bs_value2array($options['sysmapids']);
		...
		if (is_array($options['search'])) {
			bs_db_search('images i', $options, $sqlParts);
		}
		...
		if (!is_null($options['select_image'])) {
			...
			while ($img = DBfetch($dbImg)) {
				...
				$img['image'] = bs_unescape_image($img['image']);
	...
	public function create($images) {
		...
		foreach ($images as $image) {
			...
			$values = [
				'imageid' => $imageid,
				'name' => bs_dbstr($image['name']),
				'imagetype' => bs_dbstr($image['imagetype'])
			];
			...
			switch ($DB['TYPE']) {
				case BS_DB_ORACLE:
					...
					if (!$stmt) {
						$e = oci_error($DB['DB']);
						self::exception(BS_API_ERROR_PARAMETERS, _s('Parse SQL error [%1$s] in [%2$s].', $e['message'], $e['sqltext']));
					}

					oci_bind_by_name($stmt, ':imgdata', $lob, -1, OCI_B_BLOB);
					if (!oci_execute($stmt, OCI_DEFAULT)) {
						$e = oci_error($stmt);
						self::exception(BS_API_ERROR_PARAMETERS, _s('Execute SQL error [%1$s] in [%2$s].', $e['message'], $e['sqltext']));
					}
					if (!$lob->save($image['image'])) {
						$e = oci_error($stmt);
						self::exception(BS_API_ERROR_PARAMETERS, _s('Image load error [%1$s] in [%2$s].', $e['message'], $e['sqltext']));
					}
					$lob->free();
				...
				case BS_DB_DB2:
					...
					if (!$stmt) {
						self::exception(BS_API_ERROR_PARAMETERS, db2_conn_errormsg($DB['DB']));
					}
				...
				break;
				case BS_DB_SQLITE3:
					$values['image'] = bs_dbstr(bin2hex($image['image']));
					$sql = 'INSERT INTO images ('.implode(', ', array_keys($values)).') VALUES ('.implode(', ', $values).')';
					if (!DBexecute($sql)) {
				...
				break;
				case BS_DB_MYSQL:
						$values['image'] = bs_dbstr($image['image']);
						$sql = 'INSERT INTO images ('.implode(', ', array_keys($values)).') VALUES ('.implode(', ', $values).')';
						if (!DBexecute($sql)) {
							self::exception(BS_API_ERROR_PARAMETERS, 'DBerror');
						}
				break;
				case BS_DB_POSTGRESQL:
	...
	public function update($images) {
		...
		foreach ($images as $image) {
			...
			if (isset($image['name'])) {
				$values['name'] = bs_dbstr($image['name']);
			}
			...
			if (isset($image['image'])) {
				...
				switch ($DB['TYPE']) {
					...
					case BS_DB_ORACLE:
						$sql = 'SELECT i.image FROM images i WHERE i.imageid='.bs_dbstr($image['imageid']).' FOR UPDATE';

						if (!$stmt = oci_parse($DB['DB'], $sql)) {
							$e = oci_error($DB['DB']);
							self::exception(BS_API_ERROR_PARAMETERS, 'SQL error ['.$e['message'].'] in ['.$e['sqltext'].']');
						}
					...
					case BS_DB_DB2:
						$stmt = db2_prepare($DB['DB'], 'UPDATE images SET image=? WHERE imageid='.bs_dbstr($image['imageid']));
			...
			if ($values) {
				...
				$sql = 'UPDATE images SET '.implode(', ', $sqlUpd).' WHERE imageid='.bs_dbstr($image['imageid']);
				$result = DBexecute($sql);

				if (!$result) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Could not save image!'));
				}
			}
		}

		return ['imageids' => bs_objectValues($images, 'imageid')];
	}
	...
	public function delete(array $imageids) {
		if (empty($imageids)) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Empty parameters'));
		}
	...
	protected function validateCreate(array $images) {
		...
		foreach ($images as $image) {
			...
			if (!check_db_fields($imageDbFields, $image)) {
				self::exception(BS_API_ERROR_PARAMETERS, _s('Wrong fields for image "%1$s".', $image['name']));
			}
		}
		...
		$dbImages = API::getApiService()->select($this->tableName(), [
			'output' => ['name'],
			'filter' => ['name' => bs_objectValues($images, 'name')],
			'limit' => 1
		]);

		if ($dbImages) {
			$dbImage = reset($dbImages);
			self::exception(BS_API_ERROR_PARAMETERS, _s('Image "%1$s" already exists.', $dbImage['name']));
		}
	}
	...
	protected function validateUpdate(array $images) {
		...
		foreach ($images as $image) {
			if (!check_db_fields(['imageid'], $image)) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect input parameters.'));
			}
		}

		$dbImages = API::getApiService()->select($this->tableName(), [
			'filter' => ['imageid' => bs_objectValues($images, 'imageid')],
			'output' => ['imageid', 'name'],
			'preservekeys' => true
		]);
		...
		foreach ($images as $image) {
			...
			if (isset($image['name']) && !bs_empty($image['name'])
					&& $dbImages[$image['imageid']]['name'] !== $image['name']) {
				if (isset($changedImageNames[$image['name']])) {
					self::exception(BS_API_ERROR_PARAMETERS, _s('Image "%1$s" already exists.', $image['name']));
				}
				else {
	...
	protected function checkImage($image) {
		// check size
		if (strlen($image) > BS_MAX_IMAGE_SIZE) {
			self::exception(BS_API_ERROR_PARAMETERS, _('Image size must be less than 1MB.'));
		}

		// check file format
		if (@imageCreateFromString($image) === false) {
			self::exception(BS_API_ERROR_PARAMETERS, _('File format is unsupported.'));
		}
	}
