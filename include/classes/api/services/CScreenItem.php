...
class CScreenItem extends CApiService {
	...
	public function __construct() {
		...
		$this->getOptions = bs_array_merge($this->getOptions, [
			'screenitemids'	=> null,
			'screenids'		=> null,
			'editable'		=> null,
			'sortfield'		=> '',
	...
	public function create(array $screenItems) {
		$screenItems = bs_toArray($screenItems);
	...
	protected function validateCreate(array $screenItems) {
		...
		$screenIds = array_keys(array_flip(bs_objectValues($screenItems, 'screenid')));
		...
		if (count($dbScreens) < count($screenIds)) {
			...
			if ($dbTemplateScreens) {
				$dbScreens = bs_array_merge($dbScreens, $dbTemplateScreens);
			}
		}
	...
	public function update(array $screenItems) {
		...
		$screenItems = bs_toHash($screenItems, 'screenitemid');
	...
	public function updateByPosition(array $screenItems) {
		...
		$dbScreenItems = $this->get([
			...
			'screenids' => bs_objectValues($screenItems, 'screenid'),
			'editable' => true,
			'preservekeys' => true
		]);
	 ...
	 * $dbScreenItems, a BS_API_ERROR_PERMISSIONS exception will be thrown.
	 *
	...
	protected function checkInput(array $screenItems, array $dbScreenItems = []) {
		...
		foreach ($screenItems as $screenItem) {
			...
			if (!$this->isValidResourceType($screenItem['resourcetype'])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect resource type provided for screen item.'));
			}
			...
			switch ($screenItem['resourcetype']) {
				...
				case SCREEN_RESOURCE_GRAPH:
					if (!$screenItem['resourceid']) {
						self::exception(BS_API_ERROR_PARAMETERS, _('No graph ID provided for screen element.'));
					}
				...
				case SCREEN_RESOURCE_PLAIN_TEXT:
					if (!$screenItem['resourceid']) {
						self::exception(BS_API_ERROR_PARAMETERS, _('No item ID provided for screen element.'));
					}
				...
				case SCREEN_RESOURCE_CLOCK:
					if (isset($screenItem['style'])) {
						...
						elseif ($screenItem['resourceid']) {
							self::exception(BS_API_ERROR_PARAMETERS, _('Cannot set resource ID for screen element.'));
						}
					}
					break;
				...
				case SCREEN_RESOURCE_MAP:
					if (!$screenItem['resourceid']) {
						self::exception(BS_API_ERROR_PARAMETERS, _('No map ID provided for screen element.'));
					}
				...
				case SCREEN_RESOURCE_SCREEN:
					if (!$screenItem['resourceid']) {
						self::exception(BS_API_ERROR_PARAMETERS, _('No screen ID provided for screen element.'));
					}
			...
			if ($screenItem['resourcetype'] == SCREEN_RESOURCE_URL) {
				if (!isset($screenItem['url']) || bs_empty($screenItem['url'])) {
					self::exception(BS_API_ERROR_PARAMETERS, _('No URL provided for screen element.'));
				}
			}
			...
			if (in_array($screenItem['resourcetype'], $lldResources)) {
				...
				if ($error) {
					self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect max columns provided for screen element.'));
				}
			}

			if (isset($validStyles[$screenItem['resourcetype']])
					&& !in_array($screenItem['style'], $validStyles[$screenItem['resourcetype']])) {
				self::exception(BS_API_ERROR_PARAMETERS, _('Incorrect style provided for screen element.'));
			}
		}
		...
		if ($hostGroupsIds) {
			...
			foreach ($hostGroupsIds as $hostGroupsId) {
				if (!isset($dbHostGroups[$hostGroupsId])) {
					self::exception(BS_API_ERROR_PERMISSIONS, _s(
	...
	protected function checkSpans(array $screenItem, array $screen) {
		if (isset($screenItem['rowspan'])) {
			if (!bs_is_int($screenItem['rowspan']) || $screenItem['rowspan'] < 1) {
				self::exception(
		...
		if (isset($screenItem['colspan'])) {
			if (!bs_is_int($screenItem['colspan']) || $screenItem['colspan'] < 1) {
				self::exception(
					BS_API_ERROR_PARAMETERS,
					_s(
	...
	protected function checkDuplicateResourceInCell(array $screenItems, array $dbScreenItems, array $dbScreens) {
		...
		foreach ($screenItems as $screenItem) {
			...
			if (array_key_exists($screenItem['screenid'], $pos)
				...
				self::exception(BS_API_ERROR_PARAMETERS, _s('Screen "%1$s" cell X - %2$s Y - %3$s is already taken.',
	...
	protected function applyQueryFilterOptions($tableName, $tableAlias, array $options, array $sqlParts) {
		...
		if ($options['screenids'] !== null) {
			bs_value2array($options['screenids']);
