...
class CHttpTestManager {
	...
	public function update(array $httpTests) {
		$httpTestIds = bs_objectValues($httpTests, 'httptestid');
		...
		foreach ($httpTests as $httpTest) {
			...
			$dbCheckItems = DBselect(
				...
				' WHERE hi.httptestid='.bs_dbstr($httpTest['httptestid']).
			...
			if (isset($httpTest['steps'])) {
				...
				$dbSteps = bs_toHash($dbHttpTest[$httpTest['httptestid']]['steps'], 'httpstepid');
				...
				if (isset($httpTest['applicationid'])) {
					$dbStepIds = DBfetchColumn(DBselect(
						...
						' WHERE '.dbConditionInt('hi.httpstepid', bs_objectValues($dbHttpTest[$httpTest['httptestid']]['steps'], 'httpstepid')))
						, 'itemid'
					);
					...
					$itemIds = DBfetchColumn(DBselect(
						'SELECT hsi.itemid'.
							...
							' WHERE hs.httpstepid=hsi.httpstepid'.
								' AND hs.httptestid='.bs_dbstr($httpTest['httptestid'])
					), 'itemid');
	...
	public function link($templateId, $hostIds) {
		...
		$dbCursor = DBselect(
			...
			' FROM httptest ht'.
			' WHERE ht.hostid='.bs_dbstr($templateId)
		);
	...
	protected function getChildHostsFromHttpTests(array $httpTests, array $hostIds = []) {
		...
		$dbCursor = DBselect(
			...
			' WHERE '.dbConditionInt('ht.templateid', bs_objectValues($httpTests, 'hostid')).
				$sqlWhere
		);
			...
			foreach ($hostHttpTests as $hostId => $hostHttpTest) {
				...
				if (isset($hostHttpTest['byTemplateId'][$httpTestId])) {
					...
					if (isset($httpTest['name']) && isset($hostHttpTest['byName'][$httpTest['name']])
						...
						$host = DBfetch(DBselect('SELECT h.name FROM hosts h WHERE h.hostid='.bs_dbstr($hostId)));
	...
	protected function createLinkageBetweenHttpTests($parentId, $childId) {
		...
		$dbCursor = DBselect(
			...
			' WHERE hti1.httptestid='.bs_dbstr($parentId).
				' AND hti2.httptestid='.bs_dbstr($childId).
		...
		$dbCursor = DBselect(
			...
			' WHERE hs1.httptestid='.bs_dbstr($parentId).
				' AND hs2.httptestid='.bs_dbstr($childId).
	...
	protected function findChildApplication($parentAppId, $childHostId) {
		$childAppId = DBfetch(DBselect(
			...
			' WHERE a1.applicationid='.bs_dbstr($parentAppId).
				' AND a2.hostid='.bs_dbstr($childHostId))
		);
	...
	protected function compareHttpSteps(array $httpTest, array $exHttpTest) {
		...
		$dbHttpTestSteps = DBfetchArray(DBselect(
			...
			' FROM httpstep hs'.
			' WHERE hs.httptestid='.bs_dbstr($exHttpTest['httptestid'])
		));
	...
	protected function prepareHttpSteps(array $steps, $exHttpTestId) {
		...
		$dbCursor = DBselect(
			...
			' FROM httpstep hs'.
			' WHERE hs.httptestid='.bs_dbstr($exHttpTestId)
		);
		while ($dbHttpStep = DBfetch($dbCursor)) {
	...
	protected function createHttpTestItems(array $httpTest) {
		...
		if (isset($httpTest['templateid']) && $httpTest['templateid']) {
			$parentItems = DBfetchArrayAssoc(DBselect(
				'SELECT i.itemid,i.key_'.
					...
					' AND hti.httptestid='.bs_dbstr($httpTest['templateid'])
			), 'key_');
		}
		...
		foreach ($checkitems as $item) {
			$dbItem = DBfetch(DBselect(
				...
				' FROM items i'.
				' WHERE i.key_='.bs_dbstr($item['key_']).
					' AND i.hostid='.bs_dbstr($httpTest['hostid'])
			));
	...
	protected function createStepsReal($httpTest, $websteps) {
		...
		if (isset($httpTest['templateid']) && $httpTest['templateid']) {
			$parentStepItems = DBfetchArrayAssoc(DBselect(
				...
				' WHERE i.itemid=hsi.itemid'.
					...
					' AND hs.httptestid='.bs_dbstr($httpTest['templateid'])
			), 'key_');
		}
		...
		foreach ($websteps as $snum => $webstep) {
			...
			if (!isset($httpTest['delay']) || !isset($httpTest['status'])) {
				$dbTest = DBfetch(DBselect('SELECT ht.delay,ht.status FROM httptest ht WHERE ht.httptestid='.bs_dbstr($httpTest['httptestid'])));
				$delay = $dbTest['delay'];
	...
	protected function updateStepsReal($httpTest, $websteps) {
		...
		// get all used keys
		$webstepids = bs_objectValues($websteps, 'httpstepid');
		...
		foreach ($websteps as $webstep) {
			...
			$dbStepItems = DBselect(
				...
				' FROM items i,httpstepitem hi'.
				' WHERE hi.httpstepid='.bs_dbstr($webstep['httpstepid']).
					' AND hi.itemid=i.itemid'
			);
	...
	public function getHttpStepItems($httptestid) {
		return DBfetchArray(DBselect(
			...
			' WHERE hi.httpstepid=hs.httpstepid'.
				' AND hs.httptestid='.bs_dbstr($httptestid)
		));
	}
}
