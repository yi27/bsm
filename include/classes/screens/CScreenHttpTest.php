...
class CScreenHttpTest extends CScreenBase {
	...
	public function get() {
		...
		$table = (new CTableInfo())
			->setHeader([
				$this->hostid == 0
					? make_sorting_header(_('Host'), 'hostname', $sort_field, $sort_order, 'bsm.php?action=web.view')
					: null,
				make_sorting_header(_('Name'), 'name', $sort_field, $sort_order, 'bsm.php?action=web.view'),
				_('Number of steps'),
				_('Last check'),
				_('Status')
			]);
		...
		foreach ($httptests as $httptest) {
			if ($this->hostid == 0) {
				...
				if ($httptest['host']['status'] == HOST_STATUS_NOT_MONITORED) {
					$hostname = (new CSpan($hostname))->addClass(BS_STYLE_RED);
				}
			}
			else {
				$hostname = null;
			...
			if (array_key_exists('lastfailedstep', $httptest) && $httptest['lastfailedstep'] !== null) {
				$lastcheck = bs_date2str(DATE_TIME_FORMAT_SECONDS, $httptest['lastcheck']);
				...
				if ($httptest['lastfailedstep'] != 0) {
					...
					$status->addClass(BS_STYLE_RED);
				}
				else {
					$status = (new CSpan(_('OK')))->addClass(BS_STYLE_GREEN);
				}
			}
			// no history data exists
			else {
				$lastcheck = (new CSpan(_('Never')))->addClass(BS_STYLE_RED);
				$status = (new CSpan(_('Unknown')))->addClass(BS_STYLE_GREY);
			}
