...
class CScreenBuilder {
	...
	public function show() {
		...
		$screenTable = (new CTable())
			...
			->addClass(BS_STYLE_SCREEN_TABLE);

		if ($this->mode == SCREEN_MODE_EDIT) {
			$screenTable->addClass(BS_STYLE_DASHED_BORDER);
		}

		// action top row
		if ($this->mode == SCREEN_MODE_EDIT) {
			$newColumns = [(new CCol())->addClass(BS_STYLE_CELL_WIDTH)];

			for ($i = 0, $size = $this->screen['hsize']; $i < $size; $i++) {
				if ($this->screen['hsize'] >= SCREEN_MAX_SIZE) {
					$link = (new CDiv('+'))
						->addClass(BS_STYLE_TREEVIEW_PLUS)
		...
		for ($r = 0; $r < $this->screen['vsize']; $r++) {
			...
			for ($c = 0; $c < $this->screen['hsize']; $c++) {
				...
				if ($screenitem['halign'] == HALIGN_CENTER || $isEditForm) {
					$newColumn->addClass(BS_STYLE_CENTER);
				}
				elseif ($screenitem['halign'] == HALIGN_LEFT) {
					$newColumn->addClass(BS_STYLE_LEFT);
				}
				elseif ($screenitem['halign'] == HALIGN_RIGHT) {
					$newColumn->addClass(BS_STYLE_RIGHT);
				}

				if ($screenitem['valign'] == VALIGN_MIDDLE || $isEditForm) {
					$newColumn->addClass(BS_STYLE_MIDDLE);
				}
				elseif ($screenitem['valign'] == VALIGN_TOP) {
					$newColumn->addClass(BS_STYLE_TOP);
				}
				elseif ($screenitem['valign'] == VALIGN_BOTTOM) {
					$newColumn->addClass(BS_STYLE_BOTTOM);
				}
	...
	public static function insertScreenScrollJs(array $options = []) {
		...
		bs_add_post_js('timeControl.addObject("scrollbar", '.bs_jsvalue($options['timeline']).', '.bs_jsvalue($timeControlData).');');
	}
	...
	public static function insertScreenRefreshTimeJs() {
		bs_add_post_js('timeControl.useTimeRefresh('.CWebUser::$data['refresh'].');');
	}
	...
	public static function insertInitScreenJs($screenid) {
		bs_add_post_js('init_screen("'.$screenid.'", "'.self::makeScreenTableId($screenid).'", "'.$screenid.'");');
	}
	...
	public static function insertScreenCleanJs() {
		bs_add_post_js('window.flickerfreeScreen.cleanAll();');
	}
