...
class CScreenDiscovery extends CScreenBase {
	...
	public function get() {
		...
		if ($druleid > 0) {
			...
			$options['druleids'] = bs_objectValues($drules, 'druleid');
		}
		...
		$dservices = bs_toHash($dservices, 'dserviceid');
			...
			foreach ($drule['dhosts'] as $dhost) {
				...
				foreach ($dhosts[$dhost['dhostid']]['dservices'] as $dservice) {
					...
					if ($dservice['status'] == DSVC_STATUS_DISABLED) {
						$class = BS_STYLE_INACTIVE_BG;
						$time = 'lastdown';
					}
					else {
						$class = BS_STYLE_ACTIVE_BG;
						$time = 'lastup';
					}
			...
			foreach ($discovery_info as $ip => $h_data) {
				...
				foreach ($services as $name => $foo) {
					...
					if (array_key_exists($name, $h_data['services'])) {
						...
						$hint_table->addRow(
							(new CCol(bs_date2age($h_data['services'][$name]['time'])))->addClass($class)
						);
					}
