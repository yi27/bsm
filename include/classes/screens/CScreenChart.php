...
class CScreenChart extends CScreenBase {
	...
	public function get() {
		...
		if ($this->mode == SCREEN_MODE_JS) {
			...
			return 'timeControl.addObject("'.$this->getDataId().'", '.bs_jsvalue($this->timeline).', '.bs_jsvalue($timeControlData).')';
		}
		else {
			if ($this->mode == SCREEN_MODE_SLIDESHOW) {
				insert_js('timeControl.addObject("'.$this->getDataId().'", '.bs_jsvalue($this->timeline).', '.bs_jsvalue($timeControlData).');');
			}
			else {
				bs_add_post_js('timeControl.addObject("'.$this->getDataId().'", '.bs_jsvalue($this->timeline).', '.bs_jsvalue($timeControlData).');');
			}
