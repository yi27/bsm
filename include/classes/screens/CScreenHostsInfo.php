...
class CScreenHostsInfo extends CScreenBase {
	...
	public function get() {
		...
		if ($this->screenitem['resourceid'] != 0) {
			...
			$cond_where = ' AND hg.hostid=h.hostid AND hg.groupid='.bs_dbstr($this->screenitem['resourceid']);
		}
		else {
			$cond_from = '';
			$cond_where = '';
		...
		$avail = (new CCol($avail.'  '._('Available')))->addClass(BS_STYLE_GREEN);
		$notav = (new CCol($notav.'  '._('Not available')))->addClass(BS_STYLE_RED);
		$uncn = (new CCol($uncn.'  '._('Unknown')))->addClass(BS_STYLE_GREY);
