...
class CScreenHistory extends CScreenBase {
	...
	public function get() {
		...
		$stime = bsDateToTime($this->timeline['stime']);
			...
			if (isset($iv_string[$firstItem['value_type']])) {
				...
				if (empty($this->plaintext)) {
					$historyTable = (new CTableInfo())
						->setHeader([
							(new CColHeader(_('Timestamp')))->addClass(BS_STYLE_CELL_WIDTH),
							$isManyItems ? _('Item') : null,
							$useLogItem ? (new CColHeader(_('Local time')))->addClass(BS_STYLE_CELL_WIDTH) : null,
							($useEventLogItem && $useLogItem)
								? (new CColHeader(_('Source')))->addClass(BS_STYLE_CELL_WIDTH)
								: null,
							($useEventLogItem && $useLogItem)
								? (new CColHeader(_('Severity')))->addClass(BS_STYLE_CELL_WIDTH)
								: null,
							($useEventLogItem && $useLogItem)
								? (new CColHeader(_('Event ID')))->addClass(BS_STYLE_CELL_WIDTH)
								: null,
							_('Value')
						]);
				}
				...
				foreach ($historyData as $data) {
					...
					if (empty($this->plaintext)) {
						...
						$row[] = (new CCol(bs_date2str(DATE_TIME_FORMAT_SECONDS, $data['clock'])))
						...
						if ($useLogItem) {
							$row[] = ($data['timestamp'] != 0)
								? (new CCol(bs_date2str(DATE_TIME_FORMAT_SECONDS, $data['timestamp'])))
									->addClass(BS_STYLE_NOWRAP)
									->addClass($color)
								: '';
						...
						$row[] = (new CCol(new CPre(bs_nl2br($data['value']))))->addClass($color);

						$historyTable->addRow($row);
					}
					else {
						$output[] = bs_date2str(DATE_TIME_FORMAT_SECONDS, $data['clock']).' '. $data['clock'].' '.
			...
			// numeric, float
				...
				foreach ($historyData as $data) {
					...
					if (empty($this->plaintext)) {
						...
						$historyTable->addRow([
							(new CCol(bs_date2str(DATE_TIME_FORMAT_SECONDS, $data['clock'])))
								->addClass(BS_STYLE_NOWRAP),
							new CPre(bs_nl2br($value))
						]);
					}
					// plain text
					else {
		...
		if (!$this->plaintext && str_in_array($this->action, [HISTORY_VALUES, HISTORY_GRAPH, HISTORY_BATCH_GRAPH])) {
			...
			if ($this->mode == SCREEN_MODE_JS) {
				...
				bs_add_post_js('timeControl.addObject("'.$this->getDataId().'", '.CJs::encodeJson($this->timeline).', '.CJs::encodeJson($timeControlData).');');
			}
		}
