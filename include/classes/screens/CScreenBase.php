...
class CScreenBase {
	...
	public function insertFlickerfreeJs(array $data = []) {
		...
		bs_add_post_js('window.flickerfreeScreen.add('.bs_jsvalue($jsData).');');
	}
	...
	public static function calculateTime(array $options = []) {
		...
		if (empty($options['period'])) {
			$options['period'] = !empty($options['profileIdx'])
				? CProfile::get($options['profileIdx'].'.period', BS_PERIOD_DEFAULT, $options['profileIdx2'])
				: BS_PERIOD_DEFAULT;
		}
		else {
			if ($options['period'] < BS_MIN_PERIOD) {
				...
				$options['period'] = BS_MIN_PERIOD;
			}
			elseif ($options['period'] > BS_MAX_PERIOD) {
		...
		if (!empty($options['stime'])) {
			$stimeUnix = bsDateToTime($options['stime']);

			if ($stimeUnix > $time || bsAddSecondsToUnixtime($options['period'], $stimeUnix) > $time) {
				$stimeNow = bsAddSecondsToUnixtime(SEC_PER_YEAR, $options['stime']);
				$options['stime'] = date(TIMESTAMP_FORMAT, $time - $options['period']);
				$usertime = date(TIMESTAMP_FORMAT, $time);
				$isNow = 1;
			}
			else {
				$usertime = date(TIMESTAMP_FORMAT, bsAddSecondsToUnixtime($options['period'], $stimeUnix));
				$isNow = 0;
			}
			...
			if (!empty($options['profileIdx'])) {
				...
				if ($isNow) {
					$options['stime'] = date(TIMESTAMP_FORMAT, $time - $options['period']);
					$usertime = date(TIMESTAMP_FORMAT, $time);
					$stimeNow = date(TIMESTAMP_FORMAT, bsAddSecondsToUnixtime(SEC_PER_YEAR, $options['stime']));
					...
					$usertime = date(TIMESTAMP_FORMAT, bsAddSecondsToUnixtime($options['period'], $options['stime']));
				}
			}
		...
		return [
			...
			'stime' => date(TIMESTAMP_FORMAT, bsDateToTime($options['stime'])),
			'stimeNow' => ($stimeNow === null)
				? date(TIMESTAMP_FORMAT, bsAddSecondsToUnixtime(SEC_PER_YEAR, $options['stime']))
				: $stimeNow,
			'starttime' => date(TIMESTAMP_FORMAT, $time - BS_MAX_PERIOD),
			'usertime' => $usertime,
			'isNow' => $isNow
		];
	}
