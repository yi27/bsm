...
class CFrontendSetup {
	...
	public static function getSupportedDatabases() {
		...
		if (bs_is_callable(['mysqli_close', 'mysqli_connect', 'mysqli_connect_error', 'mysqli_error',
				'mysqli_fetch_assoc', 'mysqli_free_result', 'mysqli_query', 'mysqli_real_escape_string'])) {
			$allowed_db[BS_DB_MYSQL] = 'MySQL';
		}

		if (bs_is_callable(['pg_close', 'pg_connect', 'pg_escape_bytea', 'pg_escape_string', 'pg_fetch_assoc',
				'pg_free_result', 'pg_last_error', 'pg_parameter_status', 'pg_query', 'pg_unescape_bytea'])) {
			$allowed_db[BS_DB_POSTGRESQL] = 'PostgreSQL';
		}

		if (bs_is_callable(['oci_bind_by_name', 'oci_close', 'oci_commit', 'oci_connect', 'oci_error', 'oci_execute',
				'oci_fetch_assoc', 'oci_field_type', 'oci_free_statement', 'oci_new_descriptor', 'oci_parse',
				'oci_rollback'])) {
			$allowed_db[BS_DB_ORACLE] = 'Oracle';
		}

		if (bs_is_callable(['db2_autocommit', 'db2_bind_param', 'db2_close', 'db2_commit', 'db2_conn_errormsg',
			...
			$allowed_db[BS_DB_DB2] = 'IBM DB2';
		}

		// Semaphore related functions are checked elsewhere. The 'false' is to prevent autoloading of the SQLite3 class.
		if (class_exists('SQLite3', false) && bs_is_callable(['ftok', 'sem_acquire', 'sem_get', 'sem_release'])) {
			$allowed_db[BS_DB_SQLITE3] = 'SQLite3';
		}
