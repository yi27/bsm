...
class CSetupWizard extends CForm {
	...
	function bodyToString($destroy = true) {
		$setup_left = (new CDiv([
			(new CDiv())->addClass(BS_STYLE_SIGNIN_LOGO), $this->getList()
		]))->addClass(BS_STYLE_SETUP_LEFT);

		$setup_right = (new CDiv($this->getStage()))->addClass(BS_STYLE_SETUP_RIGHT);

		if (CWebUser::$data && CWebUser::getType() == USER_TYPE_SUPER_ADMIN) {
			$cancel_button = (new CSubmit('cancel', _('Cancel')))
				->addClass(BS_STYLE_BTN_ALT)
				->addClass(BS_STYLE_FLOAT_LEFT);
		...
		$setup_footer = (new CDiv([new CDiv([$next_button, $back_button]), $cancel_button]))
			->addClass(BS_STYLE_SETUP_FOOTER);

		$setup_container = (new CDiv([$setup_left, $setup_right, $setup_footer]))->addClass(BS_STYLE_SETUP_CONTAINER);
	...
	function getList() {
		...
		foreach ($this->stage as $id => $data) {
			$list->addItem($data['title'], ($id <= $this->getStep()) ? BS_STYLE_SETUP_LEFT_CURRENT : null);
		}
	...
	function stage0() {
		$setup_title = (new CDiv([new CSpan(_('Welcome to')), 'Bsm 3.0']))->addClass(BS_STYLE_SETUP_TITLE);

		return (new CDiv($setup_title))->addClass(BS_STYLE_SETUP_RIGHT_BODY);
	}

	function stage1() {
		$table = (new CTable())
			->addClass(BS_STYLE_LIST_TABLE)
		...
		return [
			...
			(new CDiv([$message_box, $table]))->addClass(BS_STYLE_SETUP_RIGHT_BODY)
		];
	}
	...
	function stage2() {
		...
		switch ($DB['TYPE']) {
			case BS_DB_SQLITE3:
				$table->addRow(
					_('Database file'),
					(new CTextBox('database', $this->getConfig('DB_DATABASE', 'bsm')))
						->setWidth(BS_TEXTAREA_SMALL_WIDTH)
				);
				break;
			...
			default:
				...
				$table->addRow(_('Database port'), [
					(new CNumericBox('port', $this->getConfig('DB_PORT', '0'), 5, false, false, false))
						...
						->setWidth(BS_TEXTAREA_SMALL_WIDTH),
					(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
					(new CSpan(_('0 - use default port')))->addClass(BS_STYLE_GREY)
				]);
				...
				if ($DB['TYPE'] == BS_DB_DB2 || $DB['TYPE'] == BS_DB_POSTGRESQL) {
					$table->addRow(_('Database schema'),
						(new CTextBox('schema', $this->getConfig('DB_SCHEMA', '')))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
					);
				}

				$table->addRow(_('User'),
					(new CTextBox('user', $this->getConfig('DB_USER', 'bsm')))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
				);
				$table->addRow(_('Password'),
					(new CPassBox('password', $this->getConfig('DB_PASSWORD')))->setWidth(BS_TEXTAREA_SMALL_WIDTH)
				);
				break;
		}
		...
		if ($this->STEP_FAILED) {
			...
			$message_box = makeMessageBox(false, $BS_MESSAGES, _('Cannot connect to the database.'), false, true);
		}
		else {
			$message_box = null;
		}
		...
		return [
			...
			(new CDiv([
				...
				$message_box,
				$table
			]))->addClass(BS_STYLE_SETUP_RIGHT_BODY)
		];
	}
	...
	function stage3() {
		...
		$table->addRow(_('Host'),
			(new CTextBox('bs_server', $this->getConfig('BS_SERVER', 'localhost')))
				->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		);

		$table->addRow(_('Port'),
			(new CNumericBox('bs_server_port', $this->getConfig('BS_SERVER_PORT', '10051'), 5, false, false, false))
				->removeAttribute('style')
				->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		);

		$table->addRow('Name',
			(new CTextBox('bs_server_name', $this->getConfig('BS_SERVER_NAME', '')))
				->setWidth(BS_TEXTAREA_SMALL_WIDTH)
		);

		return [
			new CTag('h1', true, _('Bsm server details')),
			(new CDiv([
				new CTag('p', true, _('Please enter the host name or host IP address and port number of the Bsm server, as well as the name of the installation (optional).')),
				$table
			]))->addClass(BS_STYLE_SETUP_RIGHT_BODY)
		];
	}
	...
	function stage4() {
		...
		$table = new CFormList();
		$table->addRow((new CSpan(_('Database type')))->addClass(BS_STYLE_GREY), $databases[$db_type]);

		switch ($db_type) {
			case BS_DB_SQLITE3:
				$table->addRow((new CSpan(_('Database file')))->addClass(BS_STYLE_GREY), $this->getConfig('DB_DATABASE'));
				break;
			default:
				$db_port = ($this->getConfig('DB_PORT') == 0) ? _('default') : $this->getConfig('DB_PORT');
				$db_password = preg_replace('/./', '*', $this->getConfig('DB_PASSWORD'));

				$table->addRow((new CSpan(_('Database server')))->addClass(BS_STYLE_GREY), $this->getConfig('DB_SERVER'));
				$table->addRow((new CSpan(_('Database port')))->addClass(BS_STYLE_GREY), $db_port);
				$table->addRow((new CSpan(_('Database name')))->addClass(BS_STYLE_GREY), $this->getConfig('DB_DATABASE'));
				$table->addRow((new CSpan(_('Database user')))->addClass(BS_STYLE_GREY), $this->getConfig('DB_USER'));
				$table->addRow((new CSpan(_('Database password')))->addClass(BS_STYLE_GREY), $db_password);
				if ($db_type == BS_DB_DB2 || $db_type == BS_DB_POSTGRESQL) {
					$table->addRow((new CSpan(_('Database schema')))->addClass(BS_STYLE_GREY), $this->getConfig('DB_SCHEMA'));
				}
				break;
		}
		...
		$table->addRow((new CSpan(_('Bsm server')))->addClass(BS_STYLE_GREY), $this->getConfig('BS_SERVER'));
		$table->addRow((new CSpan(_('Bsm server port')))->addClass(BS_STYLE_GREY), $this->getConfig('BS_SERVER_PORT'));
		$table->addRow((new CSpan(_('Bsm server name')))->addClass(BS_STYLE_GREY), $this->getConfig('BS_SERVER_NAME'));
	...
	function stage5() {
		$this->setConfig('BS_CONFIG_FILE_CORRECT', true);
		...
		$config->config = [
			...
			'BS_SERVER' => $this->getConfig('BS_SERVER'),
			'BS_SERVER_PORT' => $this->getConfig('BS_SERVER_PORT'),
			'BS_SERVER_NAME' => $this->getConfig('BS_SERVER_NAME')
		];
		...
		if ($error) {
			...
			$this->setConfig('BS_CONFIG_FILE_CORRECT', false);
			...
			$message_box = null;
			$message = [
				(new CTag('h1', true, _('Congratulations! You have successfully installed Bsm frontend.')))
		...
		return [
			...
			(new CDiv([$message_box, $message]))->addClass(BS_STYLE_SETUP_RIGHT_BODY)
		];
	}
	...
	function checkConnection() {
		...
		$DB['DATABASE'] = $this->getConfig('DB_DATABASE', 'bsm');
		...
		if (!$result = DBconnect($error)) {
			...
			$result = true;
			if (!bs_empty($DB['SCHEMA']) && $DB['TYPE'] == BS_DB_DB2) {
			...
			if (!bs_empty($DB['SCHEMA']) && $DB['TYPE'] == BS_DB_POSTGRESQL) {
				$db_schema = DBselect('SELECT schema_name FROM information_schema.schemata WHERE schema_name = \''.pg_escape_string($DB['SCHEMA']).'\';');
				$result = DBfetch($db_schema);
			}

			if ($result) {
				$result = DBexecute('CREATE TABLE bsm_installation_test (test_row INTEGER)');
				$result &= DBexecute('DROP TABLE bsm_installation_test');
			}
		}
	...
	function eventHandler() {
		...
		elseif ($this->getStep() == 2) {
			...
			$this->setConfig('DB_DATABASE', getRequest('database', $this->getConfig('DB_DATABASE', 'bsm')));
		...
		elseif ($this->getStep() == 3) {
			$this->setConfig('BS_SERVER', getRequest('bs_server', $this->getConfig('BS_SERVER', 'localhost')));
			$this->setConfig('BS_SERVER_PORT', getRequest('bs_server_port', $this->getConfig('BS_SERVER_PORT', '10051')));
			$this->setConfig('BS_SERVER_NAME', getRequest('bs_server_name', $this->getConfig('BS_SERVER_NAME', '')));
		...
		elseif ($this->getStep() == 5) {
			if (hasRequest('save_config')) {
				// make bsm.conf.php downloadable
