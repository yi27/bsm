...
class CTriggerExpression {
	...
	/**
	 * An array of trigger functions like {Bsm server:agent.ping.last(0)}
	 ...
	 * Examples:
	 *   expression:
	 *     {Bsm server:agent.ping.last(0)}=1 and {TRIGGER.VALUE}={$TRIGGER.VALUE}
	 *   results:
	 *     $this->isValid : true
	 *     $this->error : ''
	 *     $this->expressions : array(
	 *       0 => array(
	 *         'expression' => '{Bsm server:agent.ping.last(0)}',
	 *         'pos' => 0,
	 *         'host' => 'Bsm server',
	 *         'item' => 'agent.ping',
	 *         'function' => 'last(0)',
	...
	public function getHosts() {
		...
		return array_unique(bs_objectValues($this->expressions, 'host'));
	}
	...
	private function parseNumber() {
		...
		if (isset($this->expression[$j])
				&& strpos(BS_BYTE_SUFFIXES.BS_TIME_SUFFIXES, $this->expression[$j]) !== false) {
