...
class CMacrosResolver extends CMacrosResolverGeneral {
			...
			if ($usermacros) {
				...
				foreach ($usermacros as $triggerid => &$usermacros_data) {
					if (array_key_exists($triggerid, $db_triggers)) {
						$usermacros_data['hostids'] = bs_objectValues($db_triggers[$triggerid]['hosts'], 'hostid');
					}
				}
					...
					if (array_key_exists($macro, $functions)) {
						...
						if ($options['html']) {
							$style = ($function['status'] == ITEM_STATUS_ACTIVE)
								? ($function['state'] == ITEM_STATE_NORMAL) ? BS_STYLE_GREEN : BS_STYLE_GREY
								: $style = BS_STYLE_RED;

							if ($function['flags'] == BS_FLAG_DISCOVERY_CREATED
									|| $function['type'] == ITEM_TYPE_HTTPTEST) {
								$link = (new CSpan($function['host'].':'.$function['key_']))->addClass($style);
							}
							elseif ($function['flags'] == BS_FLAG_DISCOVERY_PROTOTYPE) {
						...
						$value = $options['html'] ? (new CSpan('*ERROR*'))->addClass(BS_STYLE_RED) : '*ERROR*';
					}
				}
				unset($value);
			}
	...
	private function resolveGraphsFunctionalItemMacros(array $sourceStringList, array $itemsList) {
		...
		foreach ($sourceStringList as $sourceString) {
			...
			preg_match_all('/(?P<macros>{'.
				'(?P<hosts>('.BS_PREG_HOST_FORMAT.'|({('.self::PATTERN_HOST_INTERNAL.')'.self::PATTERN_MACRO_PARAM.'}))):'.
				'(?P<keys>'.BS_PREG_ITEM_KEY_FORMAT.')\.'.
				'(?P<functions>(last|max|min|avg))\('.
				'(?P<parameters>([0-9]+['.BS_TIME_SUFFIXES.']?)?)'.
		...
		foreach ($hostKeyPairs as $host => $keys) {
			$queryParts[] = '(h.host='.bs_dbstr($host).' AND '.dbConditionString('i.key_', array_keys($keys)).')';
		}
	...
	public function resolveMapLabelMacros($label, $replaceHosts = null) {
		$functionsPattern = '(last|max|min|avg)\(([0-9]+['.BS_TIME_SUFFIXES.']?)?\)';

		// Find functional macro pattern.
		$pattern = ($replaceHosts === null)
			? '/{'.BS_PREG_HOST_FORMAT.':.+\.'.$functionsPattern.'}/Uu'
			: '/{('.BS_PREG_HOST_FORMAT.'|{HOSTNAME[0-9]?}|{HOST\.HOST[0-9]?}):.+\.'.$functionsPattern.'}/Uu';
			...
			if ($selement['elementtype'] == SYSMAP_ELEMENT_TYPE_HOST) {
				$res = DBselect(
					...
					' WHERE hi.hostid=h.hostid'.
						' AND hi.main=1 AND hi.hostid='.bs_dbstr($selement['elementid'])
				);
			...
			// Get trigger host list if element is trigger.
				...
				$res = DBselect(
					...
					' WHERE h.hostid=hi.hostid'.
						...
						' AND hi.main=1 AND f.triggerid='.bs_dbstr($selement['elementid']).
