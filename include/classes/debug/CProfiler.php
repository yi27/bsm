...
class CProfiler {
	...
	public function make() {
		...
		foreach ($this->sqlQueryLog as $query) {
			...
			$sql = [
				'SQL ('.$time.'): ',
				(new CSpan($query[1]))
					->addClass(substr($query[1], 0, 6) === 'SELECT' ? BS_STYLE_GREEN : BS_STYLE_BLUE),
				BR()
			];
		...
		return (new CPre())
			->addClass(BS_STYLE_DEBUG_OUTPUT)
			->setAttribute('name', 'bs_debug_info')
	...
	public function formatCallStack(array $callStack = null) {
		...
		foreach ($callStack as $call) {
			...
			if ($call['function'] != 'bs_err_handler') {
