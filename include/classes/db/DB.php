...
class DB {
	...
	protected static function reserveIds($table, $count) {
		...
		$sql = 'SELECT nextid'.
				' FROM ids'.
				' WHERE table_name='.bs_dbstr($table).
					' AND field_name='.bs_dbstr($id_name);

		// SQLite3 does not support this syntax. Since we are in transaction, it can be ignored.
		if ($DB['TYPE'] != BS_DB_SQLITE3) {
			$sql = $sql.' FOR UPDATE';
		}
		...
		if ($res) {
			...
			if (bccomp($maxNextId, BS_DB_MAX_ID) == 1) {
	...
	private static function refreshIds($table, $count) {
		...
		$sql = 'DELETE FROM ids WHERE table_name='.bs_dbstr($table).' AND field_name='.bs_dbstr($id_name);
		...
		$sql = 'INSERT INTO ids (table_name,field_name,nextid)'.
				' VALUES ('.bs_dbstr($table).','.bs_dbstr($id_name).','.$maxNextId.')';
		...
		foreach ($values as $field => $value) {
			...
			if (is_null($values[$field])) {
				...
				elseif (isset($tableSchema['fields'][$field]['default'])) {
					$values[$field] = bs_dbstr($tableSchema['fields'][$field]['default']);
				}
				else {
				...
				switch ($tableSchema['fields'][$field]['type']) {
					...
					case self::FIELD_TYPE_TEXT:
						if ($DB['TYPE'] == BS_DB_DB2 || $DB['TYPE'] == BS_DB_ORACLE) {
							$length = mb_strlen($values[$field]);
	...
	public static function find($tableName, array $criteria = []) {
		...
		foreach ($criteria as $field => $value) {
			...
			$sqlWhere[] = dbConditionString($field, bs_toArray($value));
		}
	...
	public static function replace($tableName, array $oldRecords, array $newRecords) {
		$pk = self::getPk($tableName);
		$oldRecords = bs_toHash($oldRecords, $pk);
