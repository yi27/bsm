...
class CConfigFile {
	...
	const CONFIG_FILE_PATH = '/conf/bsm.conf.php';

	private static $supported_db_types = [
		BS_DB_DB2 => true,
		BS_DB_MYSQL => true,
		BS_DB_ORACLE => true,
		BS_DB_POSTGRESQL => true,
		BS_DB_SQLITE3 => true
	];
	...
	public function load() {
		...
		if (isset($BS_SERVER)) {
			$this->config['BS_SERVER'] = $BS_SERVER;
		}
		if (isset($BS_SERVER_PORT)) {
			$this->config['BS_SERVER_PORT'] = $BS_SERVER_PORT;
		}
		if (isset($BS_SERVER_NAME)) {
			$this->config['BS_SERVER_NAME'] = $BS_SERVER_NAME;
		}
	...
	public function makeGlobal() {
		global $DB, $BS_SERVER, $BS_SERVER_PORT, $BS_SERVER_NAME;

		$DB = $this->config['DB'];
		$BS_SERVER = $this->config['BS_SERVER'];
		$BS_SERVER_PORT = $this->config['BS_SERVER_PORT'];
		$BS_SERVER_NAME = $this->config['BS_SERVER_NAME'];
	}
...
$BS_SERVER      = \''.addcslashes($this->config['BS_SERVER'], "'\\").'\';
$BS_SERVER_PORT = \''.addcslashes($this->config['BS_SERVER_PORT'], "'\\").'\';
$BS_SERVER_NAME = \''.addcslashes($this->config['BS_SERVER_NAME'], "'\\").'\';
...
';
	...
	protected function setDefaults() {
		...
		$this->config['BS_SERVER'] = 'localhost';
		$this->config['BS_SERVER_PORT'] = '10051';
		$this->config['BS_SERVER_NAME'] = '';
	}
