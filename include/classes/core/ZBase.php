...
class ZBase {
	...
	/**
	 * @var array of config data from bsm config file
	 */
	protected $config = [];
	...
	public function run($mode) {
		...
		set_error_handler('bs_err_handler');
	...
	protected function setMaintenanceMode() {
		...
		if (defined('BS_DENY_GUI_ACCESS')) {
			$user_ip = (isset($_SERVER['HTTP_X_FORWARDED_FOR']) && !empty($_SERVER['HTTP_X_FORWARDED_FOR']))
					? $_SERVER['HTTP_X_FORWARDED_FOR']
					: $_SERVER['REMOTE_ADDR'];
			if (!isset($BS_GUI_ACCESS_IP_RANGE) || !in_array($user_ip, $BS_GUI_ACCESS_IP_RANGE)) {
	...
	protected function initLocales() {
		...
		if (function_exists('bindtextdomain')) {
			...
			$locales = bs_locale_variants(CWebUser::$data['lang']);
			$locale_found = false;
			...
			if (!$locale_found && CWebUser::$data['lang'] != 'en_GB' && CWebUser::$data['lang'] != 'en_gb') {
				error('Locale for language "'.CWebUser::$data['lang'].'" is not found on the web server. Tried to set: '.implode(', ', $locales).'. Unable to translate Bsm interface.');
			}
	...
	private function processRequest() {
		...
		// Controller returned redirect to another page
			...
			if (isset($BS_MESSAGES)) {
				CSession::setValue('messages', $BS_MESSAGES);
			}
		...
		// Controller returned fatal error
			...
			redirect('bsm.php?action=system.warning');
		}
	}
}
