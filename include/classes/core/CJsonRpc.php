...
class CJsonRpc {
	...
	public function execute() {
		...
		foreach (bs_toArray($this->_jsonDecoded) as $call) {
	...
	public function processResult($call, CApiClientResponse $response) {
		if ($response->errorCode) {
			$errno = $this->_bs2jsonErrors[$response->errorCode];
	...
	private function initErrors() {
		...
		$this->_bs2jsonErrors = [
			BS_API_ERROR_NO_METHOD => '-32601',
			BS_API_ERROR_PARAMETERS => '-32602',
			BS_API_ERROR_NO_AUTH => '-32602',
			BS_API_ERROR_PERMISSIONS => '-32500',
			BS_API_ERROR_INTERNAL => '-32500'
		];
	}
}
