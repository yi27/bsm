...
class CFavorite {
	...
	public static function add($idx, $favid, $favobj = null) {
		...
		$values = [
			...
			'idx' => bs_dbstr($idx),
			'value_id' => bs_dbstr($favid),
			'type' => PROFILE_TYPE_ID
		];
		if (!is_null($favobj)) {
			$values['source'] = bs_dbstr($favobj);
		}
	...
	public static function remove($idx, $favid = 0, $favobj = null) {
		...
		return DBexecute(
			...
			' WHERE userid='.CWebUser::$data['userid'].
				' AND idx='.bs_dbstr($idx).
				($favid > 0 ? ' AND value_id='.bs_dbstr($favid) : '').
				(is_null($favobj) ? '' : ' AND source='.bs_dbstr($favobj))
		);
	}
