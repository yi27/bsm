...
class CWebUser {
	...
	public static function login($login, $password) {
		try {
			...
			$result &= DBexecute('DELETE FROM sessions WHERE sessionid='.bs_dbstr(get_cookie('bs_sessionid')));
	...
	public static function setSessionCookie($sessionId) {
		...
		bs_setcookie('bs_sessionid', $sessionId,  $autoLogin ? strtotime('+1 month') : 0);
	}

	/**
	 * Retrieves current session ID from bs_sessionid cookie.
	 *
	 * @return string
	 */
	public static function getSessionCookie() {
		return get_cookie('bs_sessionid');
	}
	...
	public static function isGuest() {
		return (self::$data['alias'] == BS_GUEST_USER);
	}
}
