...
class CServiceTree extends CTree {
	...
	protected function makeCol($rowId, $colName) {
		...
		switch ($colName) {
			case 'status':
				if (bs_is_int($this->tree[$rowId][$colName]) && $this->tree[$rowId]['id'] > 0) {
					...
					if (in_array($status, [TRIGGER_SEVERITY_INFORMATION, TRIGGER_SEVERITY_NOT_CLASSIFIED])) {
						return (new CCol(_('OK')))->addClass(BS_STYLE_GREEN);
					}
					else {
			...
			case 'sla':
				return parent::makeCol($rowId, $colName)->addClass(BS_STYLE_CELL_WIDTH);
		}
