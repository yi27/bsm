...
class CPieGraphDraw extends CGraphDraw {
		...
		for ($i = 0; $i < $this->num; $i++) {
			...
			if (!$trendsEnabled || (($item['history'] * SEC_PER_DAY) > (time() - ($from_time + $this->period / 2)))) {
				...
				array_push($sql_arr,
					...
					' FROM history h'.
					' WHERE h.itemid='.bs_dbstr($this->items[$i]['itemid']).
						' AND h.clock>='.bs_dbstr($from_time).
						' AND h.clock<='.bs_dbstr($to_time).
					' GROUP BY h.itemid'
					,
					'SELECT hu.itemid,'.
					...
					' WHERE hu.itemid='.bs_dbstr($this->items[$i]['itemid']).
						' AND hu.clock>='.bs_dbstr($from_time).
						' AND hu.clock<='.bs_dbstr($to_time).
					' GROUP BY hu.itemid'
				);
			}
			else {
				...
				array_push($sql_arr,
					...
					' FROM trends t'.
					' WHERE t.itemid='.bs_dbstr($this->items[$i]['itemid']).
						' AND t.clock>='.bs_dbstr($from_time).
						' AND t.clock<='.bs_dbstr($to_time).
					' GROUP BY t.itemid'
					,
					'SELECT t.itemid,'.
						'AVG(t.value_avg) AS avg,MIN(t.value_min) AS min,'.
	...
	protected function drawLegend() {
		...
		$displayHostName = (count(array_unique(bs_objectValues($this->items, 'hostname'))) > 1);
