...
class CConfigurationExport {
	...
	protected function filterOptions(array $options) {
		if ($options['hosts']) {
			...
			$options['hosts'] = bs_objectValues($hosts, 'hostid');
		}
	...
	protected function gatherTemplates(array $templateIds) {
		...
		foreach ($templates as &$template) {
			...
			$this->data['groups'] += bs_toHash($template['groups'], 'groupid');
	...
	protected function gatherHosts(array $hostIds) {
		...
		foreach ($hosts as &$host) {
			// merge host groups with all groups
			$this->data['groups'] += bs_toHash($host['groups'], 'groupid');
		...
		$dbProxies = DBselect(
			...
			' FROM hosts h'.
			' WHERE '.dbConditionInt('h.hostid', bs_objectValues($hosts, 'proxy_hostid'))
		);
	...
	protected function prepareItems(array $items) {
		...
		foreach ($items as $idx => $item) {
			...
			foreach ($item['applications'] as $application) {
				if ($application['flags'] == BS_FLAG_DISCOVERY_CREATED) {
					unset($items[$idx]);
					continue 2;
				}
			}
	...
	protected function prepareDiscoveryRules(array $items) {
		...
		$prototypes = API::ItemPrototype()->get([
			...
			'discoveryids' => bs_objectValues($items, 'itemid'),
			'inherited' => false,
			'preservekeys' => true
		]);
	...
	protected function prepareGraphs(array $graphs) {
		...
		foreach ($graphs as $gnum => $graph) {
			if ($graph['ymin_itemid'] && isset($graphItems[$graph['ymin_itemid']])) {
				...
				if ($axisItem['flags'] == BS_FLAG_DISCOVERY_CREATED || $axisItem['type'] == ITEM_TYPE_HTTPTEST) {
					unset($graphs[$gnum]);
					continue;
				}
			...
			foreach ($graph['gitems'] as $ginum => $gItem) {
				...
				// unset lld and web graphs
				if ($item['flags'] == BS_FLAG_DISCOVERY_CREATED || $item['type'] == ITEM_TYPE_HTTPTEST) {
					unset($graphs[$gnum]);
					continue 2;
				}
	...
	protected function prepareTriggers(array $triggers) {
		...
		foreach ($triggers as $trigger) {
			$itemids = array_merge($itemids, bs_objectValues($trigger['items'], 'itemid'));
		}
	...
	protected function gatherMaps(array $mapIds) {
		...
		$images = API::Image()->get([
			...
			'sysmapids' => bs_objectValues($sysmaps, 'sysmapid'),
			'select_image' => true,
			'preservekeys' => true
		]);
