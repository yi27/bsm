...
class CTextTriggerConstructor {
	...
	public function getExpressionFromParts($host, $itemKey, array $expressions) {
		...
		$BS_PREG_EXPESSION_FUNC_FORMAT = '^(['.BS_PREG_PRINT.']*) (and|or) (not )?(-)? ?[(]*(([a-zA-Z_.\$]{6,7})(\\((['.BS_PREG_PRINT.']+?){0,1}\\)))(['.BS_PREG_PRINT.']*)$';
		...
		foreach ($expressions as $expression) {
			...
			while (preg_match('/'.$BS_PREG_EXPESSION_FUNC_FORMAT.'/i', $expr, $arr)) {
