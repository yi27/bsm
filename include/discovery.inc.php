...
function check_right_on_discovery() {
	return (CWebUser::getType() >= USER_TYPE_BSM_ADMIN);
}
...
function discovery_status2style($status) {
	switch ($status) {
		case DRULE_STATUS_ACTIVE:
			$status = BS_STYLE_GREEN;
			break;
		case DRULE_STATUS_DISABLED:
			$status = BS_STYLE_RED;
			break;
		default:
			$status = BS_STYLE_GREY;
			break;
	}
...
function get_discovery_rule_by_druleid($druleid) {
	return DBfetch(DBselect('SELECT d.* FROM drules d WHERE d.druleid='.bs_dbstr($druleid)));
}
