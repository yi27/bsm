...
function getUserFormData($userId, array $config, $isProfile = false) {
	...
	if ($userId != 0 && (!hasRequest('form_refresh') || hasRequest('register'))) {
		...
		$userGroup = bs_objectValues($userGroups, 'usrgrpid');
		$data['user_groups']	= bs_toHash($userGroup);

		$data['user_medias'] = [];
		$dbMedia = DBselect('SELECT m.mediaid,m.mediatypeid,m.period,m.sendto,m.severity,m.active'.
				' FROM media m'.
				' WHERE m.userid='.bs_dbstr($userId)
		);
		...
		$data['user_type']		= getRequest('user_type', USER_TYPE_BSM_USER);
	...
	if (!empty($data['user_medias'])) {
		...
		$dbMediaTypes = DBselect(
			'SELECT mt.mediatypeid,mt.description FROM media_type mt WHERE '.
				dbConditionInt('mt.mediatypeid', bs_objectValues($data['user_medias'], 'mediatypeid'))
		);
...
function getPermissionsFormList($rights = [], $user_type = USER_TYPE_BSM_USER, $rightsFormList = null) {
	// group
			...
			$output[] = (new CSpan([
				(new CSpan($element['name']))
					...
					->onClick(CHtml::encode(
						'javascript: create_var("bs_filter", "subfilter_set", "1", false);'.
						'create_var("bs_filter", '.CJs::encodeJson($subfilterName.'['.$id.']').', null, true);'
					)),
				SPACE,
				new CSup($element['count'])
			]))->addClass(BS_STYLE_SUBFILTER_ENABLED);
		}
		// isn't activated
		else {
			// subfilter has 0 items
			if ($element['count'] == 0) {
				$output[] = (new CSpan($element['name']))->addClass(BS_STYLE_GREY);
				$output[] = SPACE;
...
function getItemFilterForm(&$items) {
	...
	bs_subarray_push($fTypeVisibility, -1, 'filter_delay_row');

	$item_types = item_type2str();
	unset($item_types[ITEM_TYPE_HTTPTEST]); // httptest items are only for internal bsm logic

	$cmbType->addItems($item_types);

	foreach ($item_types as $type => $name) {
		if ($type != ITEM_TYPE_TRAPPER && $type != ITEM_TYPE_SNMPTRAP) {
			bs_subarray_push($fTypeVisibility, $type, 'filter_delay_row');
		}

		switch ($type) {
			case ITEM_TYPE_SNMPV1:
			case ITEM_TYPE_SNMPV2C:
				bs_subarray_push($fTypeVisibility, $type, 'filter_snmp_community_row');
				bs_subarray_push($fTypeVisibility, $type, 'filter_snmp_oid_row');
				bs_subarray_push($fTypeVisibility, $type, 'filter_port_row');
				break;

			case ITEM_TYPE_SNMPV3:
				bs_subarray_push($fTypeVisibility, $type, 'filter_snmpv3_securityname_row');
				bs_subarray_push($fTypeVisibility, $type, 'filter_snmp_oid_row');
				bs_subarray_push($fTypeVisibility, $type, 'filter_port_row');
				break;
		}
	...
	bs_add_post_js("var filterTypeSwitcher = new CViewSwitcher('filter_type', 'change', ".bs_jsvalue($fTypeVisibility, true).');');

	// type of information select
	$fVTypeVisibility = [];

	bs_subarray_push($fVTypeVisibility, ITEM_VALUE_TYPE_UINT64, 'filter_data_type_row');

	bs_add_post_js("var filterValueTypeSwitcher = new CViewSwitcher('filter_value_type', 'change', ".bs_jsvalue($fVTypeVisibility, true).');');
	...
	$filterColumn1->addRow(_('Host group'),
		...
		]))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH)
	);
	...
	$filterColumn2->addRow(_('Update interval (in sec)'),
		(new CNumericBox('filter_delay', $filter_delay, 5, false, true))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH),
		'filter_delay_row'
	);
	...
	$filterColumn1->addRow(_('Application'),
		[
			(new CTextBox('filter_application', $filter_application))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH),
			(new CDiv())->addClass(BS_STYLE_FORM_INPUT_MARGIN),
			(new CButton(null, _('Select')))
				->addClass(BS_STYLE_BTN_GREY)
				->onClick(
	...
	$filterColumn2->addRow(_('SNMP community'),
		(new CTextBox('filter_snmp_community', $filter_snmp_community))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH),
	...
	$filterColumn3->addRow(_('History (in days)'),
		(new CNumericBox('filter_history', $filter_history, 8, false, true))
			->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH)
	);
	...
	$filterColumn1->addRow(_('Name like'),
		(new CTextBox('filter_name', $filter_name))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH)
	);
	$filterColumn2->addRow(_('SNMP OID'),
		(new CTextBox('filter_snmp_oid', $filter_snmp_oid))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH),
		'filter_snmp_oid_row'
	);
	...
	$filterColumn1->addRow(_('Key like'),
		(new CTextBox('filter_key', $filter_key))->setWidth(BS_TEXTAREA_FILTER_SMALL_WIDTH)
	);
	$filterColumn2->addRow(_('Port'),
		(new CNumericBox('filter_port', $filter_port, 5, false, true))->setWidth(BS_TEXTAREA_NUMERIC_STANDARD_WIDTH),
		'filter_port_row'
	);
	...
	$table_subfilter = (new CTableInfo())
		->addRow([
			new CTag('h4', true, [
				_('Subfilter'), SPACE, (new CSpan(_('affects only filtered data')))->addClass(BS_STYLE_GREY)
			])
		]);
	...
	foreach ($items as $item) {
		...
		// interval
		if (bs_empty($filter_delay) && $filter_type != ITEM_TYPE_TRAPPER) {
	...
	// output
	if (bs_empty($filter_hostId) && count($item_params['hosts']) > 1) {
	...
	if (bs_empty($filter_history) && count($item_params['history']) > 1) {
	...
	if (bs_empty($filter_trends) && (count($item_params['trends']) > 1)) {
	...
	if (bs_empty($filter_delay) && $filter_type != ITEM_TYPE_TRAPPER && count($item_params['interval']) > 1) {
...
function getItemFormData(array $item = [], array $options = []) {
	...
	if (!hasRequest('form_refresh') && ($item || $data['limited'])) {
		...
		if (!$data['limited'] || !isset($_REQUEST['form_refresh'])) {
			...
			if (($data['type'] == ITEM_TYPE_TRAPPER || $data['type'] == ITEM_TYPE_SNMPTRAP) && $data['delay'] == 0) {
				$data['delay'] = BS_ITEM_DELAY_DEFAULT;
			}
			...
			$data['applications'] = array_unique(bs_array_merge($data['applications'], get_applications_by_itemid($data['itemid'])));
			...
			if ($data['parent_discoveryid'] != 0) {
				...
				$application_prototypes = DBfetchArray(DBselect(
					...
					' WHERE ap.application_prototypeid=iap.application_prototypeid'.
						' AND ap.itemid='.bs_dbstr($data['parent_discoveryid']).
						' AND iap.itemid='.bs_dbstr($data['itemid'])
				));
				...
				$data['application_prototypes'] = array_unique(
					bs_array_merge($data['application_prototypes'], bs_objectValues($application_prototypes, 'name'))
				);
			}
		}
	}
	...
	$data['db_applications'] = DBfetchArray(DBselect(
		...
		' FROM applications a'.
		' WHERE a.hostid='.bs_dbstr($data['hostid']).
			($data['parent_discoveryid'] ? ' AND a.flags='.BS_FLAG_DISCOVERY_NORMAL : '')
	));
	...
	if ($data['parent_discoveryid'] != 0) {
		...
		$data['db_application_prototypes'] = DBfetchArray(DBselect(
			...
			' WHERE ap.itemid='.bs_dbstr($data['parent_discoveryid'])
		));
	...
	if (empty($data['parent_discoveryid'])) {
		...
		$data['alreadyPopulated'] = bs_toHash($data['alreadyPopulated'], 'inventory_link');
	}
...
function getTriggerMassupdateFormData() {
	...
	foreach ($data['dependencies'] as &$dependency) {
		order_result($dependency['hosts'], 'name', BS_SORT_UP);
	}
	unset($dependency);

	order_result($data['dependencies'], 'description', BS_SORT_UP);
	...
	if (!empty($data['triggerid'])) {
		...
		do {
			$db_triggers = DBfetch(DBselect(
				...
				' WHERE t.triggerid='.bs_dbstr($tmp_triggerid)
			));
		...
		if (!$data['limited'] || !isset($_REQUEST['form_refresh'])) {
			...
			$db_triggers = DBselect(
				...
				' WHERE t.triggerid=d.triggerid_up'.
					' AND d.triggerid_down='.bs_dbstr($data['triggerid'])
			);
...
function get_timeperiod_form() {
	...
	if (isset($new_timeperiod['dayofweek'])) {
		$dayofweek = bs_num2bitstr($new_timeperiod['dayofweek'], true);
	}
	...
	if (isset($new_timeperiod['month'])) {
		$month = bs_num2bitstr($new_timeperiod['month'], true);
	}
	...
	elseif ($new_timeperiod['timeperiod_type'] == TIMEPERIOD_TYPE_MONTHLY) {
		...
		if (isset($_REQUEST['add_timeperiod'])) {
			...
			$date = bsDateToTime($new_timeperiod['start_date']
