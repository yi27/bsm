...
function check_type(&$field, $flags, &$var, $type, $caption = null) {
	...
	if ($error) {
		if ($flags & P_SYS) {
			...
			return BS_VALID_ERROR;
		}
		else {
			info($message);

			return BS_VALID_WARNING;
		}
	}
...
function check_field(&$fields, &$field, $checks) {
	...
	if ($opt == O_MAND) {
		if (!isset($_REQUEST[$field])) {
			...
			return ($flags & P_SYS) ? BS_VALID_ERROR : BS_VALID_WARNING;
		}
	}
	elseif ($opt == O_NO) {
		if (!isset($_REQUEST[$field])) {
	...
	elseif ($opt == O_OPT) {
		...
		elseif ($flags & P_ACT) {
			if (!isset($_REQUEST['sid'])
					|| (isset($_COOKIE['bs_sessionid'])
							&& $_REQUEST['sid'] != substr($_COOKIE['bs_sessionid'], 16, 16))) {
...
function invalid_url($msg = null) {
	if (empty($msg)) {
		$msg = _('Bsm has received an incorrect request.');
	}

	// required global parameters for correct including page_header.php
	global $DB, $BS_MESSAGES;
...
function check_fields(&$fields, $show_messages = true) {
	...
	$system_fields = [
		'sid' =>			[T_BS_STR, O_OPT, P_SYS, HEX(),		null],
		'triggers_hash' =>	[T_BS_STR, O_OPT, P_SYS, NOT_EMPTY,	null],
		'print' =>			[T_BS_INT, O_OPT, P_SYS, IN('1'),		null],
		'page' =>			[T_BS_INT, O_OPT, P_SYS, null,		null], // paging
		'ddreset' =>		[T_BS_INT, O_OPT, P_SYS, null,		null]
	];
	$fields = bs_array_merge($system_fields, $fields);
	...
	if ($err&BS_VALID_ERROR) {
		invalid_url();
	}

	if ($show_messages && $err != BS_VALID_OK) {
		show_messages(($err == BS_VALID_OK), null, _('Page received incorrect data'));
	}

	return ($err == BS_VALID_OK);
}
...
function validatePortNumber($port) {
	return validateNumber($port, BS_MIN_PORT_NUMBER, BS_MAX_PORT_NUMBER);
}
